;; sequence(s): A133533
;; terms: 16418 134003 1904835 6716019 30735939 76010259 219219339 789905091 1630362891 4048053411 8203334331 13637193699 21850682619 39264939507 75124110099 115865269131 184159290171 270079040451 369892892379
;; small program: loop2(loop(loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2 + (2 + 2), 1, x), 1, y) + x, 1 + y, 1 + 2, 0, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((((x * x) * x) * x) * x), y, 2, 1, x), 1, y) + x, 1 + y, 1 + 2, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ (v8 x y) 1) (* (* (* (* x x) x) x) x)))))
