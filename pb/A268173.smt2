;; sequence(s): A268173
;; terms: 0 ~1 0 ~1 1 ~1 1 ~1 1 ~2 1 ~2 1 ~2 1 ~2 2 ~2 2 ~2
;; small program: loop(((y div x) + x) div 2, x, 1) div loop(0 - x, x, 2)
;; fast program: (((loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, x div 2) + x, 2, x) - 1) - x) div 2) div loop(0 - x, x mod 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (- (- (v2 x) 1) x) 2) (v5 x)))))
