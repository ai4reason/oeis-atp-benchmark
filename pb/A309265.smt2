;; sequence(s): A309265
;; terms: 6 7 8 9 11 12 13 15 16 17 19 21 23 24 25 27 28 29 31 33
;; small program: ((loop((loop(((loop(x * y, x, 1) mod (2 + x)) + 1) mod 2, 1, x div 2) + 1) + x, x, 1) + 1) + 2) + 2
;; fast program: loop(((loop(loop(((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2, 1, x div 2), x mod 2, x) mod 2) + 1) + x, x, 2 + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (v2 x) (+ 2 x)) 1) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (mod (mod (ite (<= (- (v6 x) 2) 0) (+ 1 x) 1) (+ 1 x)) 2))))
(assert (= g5 1))
