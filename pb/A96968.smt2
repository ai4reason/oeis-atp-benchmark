;; sequence(s): A96968
;; terms: 10000 194481 9150625 68574961 1222830961 3722098081 23372600161 92173567201 258439040161 1325558466241 2609649624481 6407383500961 13788812262241 26975984333281 59128856241841 109250345339521
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + 2, 1, x), 1, y) * x, 2 + y, 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 2, 1, x), 1, y) * x, 2 + y, 2, 1, x)
(set-logic UFNIA)
