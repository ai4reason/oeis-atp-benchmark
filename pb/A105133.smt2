;; sequence(s): A105133
;; terms: 0 1 3 4 6 7 12 13 18 19 21 22 24 28 33 34 36 39 43 46
;; small program: compr((1 + loop2(x * y, y, x, 1, 2 + 2)) mod (1 + (2 * (x + x))), 1 + x) div 2
;; fast program: (compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (v2 x) 2) 1))))
