;; sequence(s): A291263
;; terms: 1 2 11 9468
;; small program: ((((loop2((x + x) + y, x, loop2(2 * (x + y), x, x, 0, 1), 1, 1) div 2) div (1 + (2 + 2))) * 2) + 1) + x
;; fast program: loop(((((((((loop2(x + y, x, 2 * x, 1, 2) + (x * x)) + x) div 2) * 2) + 1) + x) - 1) + x) + x, x - 1, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (* (div (div (w0 x) 2) (+ 1 (+ 2 2))) 2) 1) x))))
...
(assert (forall ((x Int)) (= (f2 x) (+ (+ (- (+ (+ (* (div (+ (+ (w3 x) (* x x)) x) 2) 2) 1) x) 1) x) x))))
