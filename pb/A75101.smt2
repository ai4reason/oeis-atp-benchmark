;; sequence(s): A75101
;; terms: 2 2 8 4 32 32 128 32 512 512 2048 1024 8192 8192 32768 4096 131072 131072 524288 262144
;; small program: loop2((2 - ((x * y) mod 2)) * x, y div 2, x, 1, x) * 2
;; fast program: ((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1) * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) div (1 + x)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (- 2 (mod (* x y) 2)) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (* (+ (* (v1 x) 2) 1) (v7 x)) (+ 1 x)) 2))))
