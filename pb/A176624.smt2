;; sequence(s): A176624
;; terms: 1 5 2 11 6 19 10 27 14 39 20 49 28 57 32 69 42 79 48 91
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) - loop(0 - x, x, 1 + x)
;; fast program: (compr(((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2, x mod 2, 1 + x) + x) mod 2) mod (1 + x), x) + 2) - loop(0 - x, x mod 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (v3 x) 2) (v6 x)))))
