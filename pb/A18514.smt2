;; sequence(s): A18514
;; terms: 1 2 5 10 11 22 25 50 55 110 275 550
;; small program: loop(compr(loop(2 - ((2 + y) * x), 2 + 2, x) mod (1 + x), y), x, 0) + 1
;; fast program: compr((loop(loop(x + y, x, (2 * (2 * (x + x))) * x) * x, 1, 2 + 2) - 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h5 x) (* (* 2 (* 2 (+ x x))) x))))
