;; sequence(s): A99544
;; terms: 1 1 0 1 2 0 1 1 0 2 2 0 1 1 0 1 2 0 1 2
;; small program: (1 + loop(x div (1 + (x mod 2)), x, x)) mod (1 + 2)
;; fast program: ((loop(loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) * 2) + 1) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ 1 (v0 x)) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (* (v1 x) 2) 1) (+ 1 2)))))
