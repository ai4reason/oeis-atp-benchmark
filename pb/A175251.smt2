;; sequence(s): A175251
;; terms: 4 9 12 15 16 18 21 24 25 26 28 32 33 34 36 38 39 40 42 45
;; small program: loop(compr((loop(x * y, x, x) mod (1 + x)) mod 2, x) + 1, 2, x)
;; fast program: loop(compr(loop((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div (1 + 2), x) mod (1 + x), x) + 1, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (ite (<= (- (v6 x) 2) 0) (+ 1 x) 1) 1))))
