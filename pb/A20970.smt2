;; sequence(s): A20970
;; terms: 1 26 453 6610 87221 1079106 12770773 146348690 1637020341 17972357986 194425446293 2078548587570 22007974284661 231179027537666 2412362060669013 25033514235831250 258557417951152181
;; small program: loop((loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((((loop2((2 * (2 * (x + x))) + y, (2 * ((2 * (y + y)) + x)) + y, x, 1, 1) - 1) + loop2(x * y, y, x, 1, 1 + (2 * (2 + 2)))) div 2) - loop2(x * y, y, x, 1, 1 + (2 * (2 + 2))), 1, 2 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* 2 (* 2 (+ x x))) y))))
(assert (forall ((x Int) (y Int)) (= (g4 x y) (+ (* 2 (+ (* 2 (+ y y)) x)) y))))
...
(assert (forall ((x Int)) (= (f3 x) (- (div (+ (- (w4 x) 1) (w5 x)) 2) (w6 x)))))
(assert (= g3 1))
