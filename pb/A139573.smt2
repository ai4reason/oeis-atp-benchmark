;; sequence(s): A139573
;; terms: 1099511627776 12157665459056928801 9094947017729282379150390625 6366805760909027985741435139224001 452592555681759518058893560348969204658401
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * ((x * x) * x), y, 2 + 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * ((x * x) * x), y, 2 + 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g6 x) (- (div x 2) 1))))
(assert (= h6 2))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (* (* x x) x)))))
