;; sequence(s): A36771
;; terms: 1 4 420 201600 264264000 734557824000 3723191087616000 31125877492469760000 399532678960326912000000 7462849882264211635200000000 194563959280510261541299200000000 6847568575944052279580806348800000000 316573366618757452963440048714547200000000
;; small program: ((loop(x * y, (x + x) + x, 1) div loop(2 * ((x + x) + x), x, 1)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 2 + (x + x))
;; fast program: ((loop(x * y, (x + x) + x, 1) div loop2(x * y, y, x, 1, 2 + (2 + 2))) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 2 + (x + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (div (v0 x) (v1 x)) (v2 x)) (w3 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (div (v4 x) (w5 x)) (v6 x)) (w7 x)))))
