;; sequence(s): A99210
;; terms: 100000 4084101 503284375 6240321451 228669389707 919358226007 9138686662951 50787635527751 184267035634793 1422324234276593 3316864672715351 10194147150028951 26571041229338407 61478268295547399 163964318358625093 353206366482671393
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 1 + (2 + 2), 1, x), 1, y) * x, 2 + y, 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (((x * x) * x) * x), y, 2, 1, x), 1, y) * x, 2 + y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ (v8 x y) 1) (* (* (* x x) x) x)))))
...
(assert (forall ((x Int) (y Int)) (= (g5 x y) (+ 2 y))))
(assert (= h5 2))
(assert (= i5 1))
