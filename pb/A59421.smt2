;; sequence(s): A59421
;; terms: 16 136 616 2016 5376 12432 25872 49632 89232 152152 248248 390208 594048 879648 1271328 1798464 2496144 3405864 4576264 6063904
;; small program: (loop((loop(loop(((y * y) - y) + x, y, x), y, 0) * y) + x, 2 + x, 0) * 2) * 2
;; fast program: loop(loop((loop(x * x, 1, 1 + (((2 + x) * x) + x)) div (1 + 2)) * (1 + x), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 1 (+ (* (+ 2 x) x) x)))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (* (div (v5 x) (+ 1 2)) (+ 1 x)))))
(assert (= g4 1))
