;; sequence(s): A75516
;; terms: 1 63 2394 71442 1848987 43615341 964942308 20385709344 416206043253 8280505692459 161494678323342 3101091077181006 58823743379417199 1104995938593100617 20595841868175915096
;; small program: loop(loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, y, 1) + (((x + x) + x) * 2), x, 1) * loop((x + x) + x, x, 1)
;; fast program: loop(loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, y, 1) + (((x + x) + x) * 2), x, 1) * loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (= g9 1))
(assert (forall ((x Int)) (= (f10 x) (+ 2 (+ x x)))))
...
(assert (forall ((x Int)) (= (f8 x) (div (div (v9 x) 2) (+ 1 2)))))
(assert (= g8 1))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (v7 x y) (* (+ x x) 2)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (v6 x y) (* (+ (+ x x) x) 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (v5 x) (w11 x)))))
