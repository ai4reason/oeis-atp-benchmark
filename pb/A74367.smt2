;; sequence(s): A74367
;; terms: 1 5 11 29 41 71 89 131 209 239 341 419 461 551 701 869 929 1121 1259 1331
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x) div 2) div 2) - 1
;; fast program: ((loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, y, 2, 1, x) div 2) div 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (div (div (w3 x) 2) 2) 1))))
