;; sequence(s): A123564
;; terms: 2 3 1 1 2 3 1 1 2 2 3 1 2 2 3 1 2 2 3 1
;; small program: loop(loop(((compr(loop2((((y div 2) * y) mod 2) + x, y div 2, x, 0, x), x) + 1) mod 2) + x, 2, x) - x, 1, x + x) + 1
;; fast program: loop(loop((((loop(((((x - (x div (2 + (2 * (2 * (2 + 2)))))) div (2 + 2)) + x) mod 2) + x, 2, x) + 1) + x) mod 2) + x, 2, x) - x, 1, x + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (mod (* (div y 2) y) 2) x))))
...
(assert (forall ((x Int)) (= (f6 x) (+ (mod (+ (div (- x (div x (+ 2 (* 2 (* 2 (+ 2 2)))))) (+ 2 2)) x) 2) x))))
(assert (= g6 2))
(assert (forall ((x Int)) (= (h6 x) x)))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 g6 (h6 x)))))
(assert (forall ((x Int)) (= (f5 x) (+ (mod (+ (+ (v6 x) 1) x) 2) x))))
(assert (= g5 2))
