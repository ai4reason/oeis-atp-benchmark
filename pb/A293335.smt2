;; sequence(s): A293335
;; terms: 1 1 2 4 8 15 29 58 115 229 458 916 1832 3664 7328 14655 29309 58618 117235 234469
;; small program: ((loop2(y + y, 2 * (x + y), x, 1, 1) - 1) div loop2(x + y, x, x, 1, 2)) + 1
;; fast program: (((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) * loop2(x + y, x, x - 2, 1 + 1, 1)) - 2) div loop2(x + y, x, x, 1, 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (- (w0 x) 1) (w1 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (- (* (v2 x) (w5 x)) 2) (w6 x)) 1))))
