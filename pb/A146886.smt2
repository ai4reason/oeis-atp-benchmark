...
;; small program: loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1)
;; fast program: ((((compr(((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x), x) * 2) + 1) div (if x <= 0 then 1 else 2)) * (if x <= 0 then 1 else 2)) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (* (* (* (+ 1 (mod x 2)) (v4 x)) (v6 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 2)) (ite (<= x 0) 1 2)) 1))))
