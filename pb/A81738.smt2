;; sequence(s): A81738
;; terms: 0 4 13 13 38 38 87 87 87 87 208 208 377 377 377 377 666 666 1027 1027
;; small program: loop(loop((((loop(x * y, x, x) mod (1 + x)) mod 2) * (1 + x)) * (1 + x), 1, y) + x, x, 0)
;; fast program: loop((loop(if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 0, 1, y) * (1 + y)) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (* (mod (mod (v2 x) (+ 1 x)) 2) (+ 1 x)) (+ 1 x)))))
(assert (= g1 1))
