;; sequence(s): A68901
;; terms: 0 1 1 1 4 5 4 5 4 1 2 11 11 13 13 11 9 11 9 9
;; small program: ((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) * x) + x) mod (1 + x)
;; fast program: (((((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), (1 + x) mod 2, x), x) + 1) + 1) * x) + 1) mod (1 + x)) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (* (v0 x) x) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (mod (+ (* (+ (+ (v2 x) 1) 1) x) 1) (+ 1 x)) 1))))
