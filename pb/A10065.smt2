;; sequence(s): A10065
;; terms: 1 2 4 5 7 11 16 17 19 23 28 32 34 38 43 50 55 62 70 74
;; small program: loop(loop2((y mod (2 + 2)) + x, (y div 2) div 2, x, x, x), x, 1)
;; fast program: loop((loop2((y mod (2 + 2)) + x, y div (2 + 2), 1 + (2 + 2), 1, x) - 1) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (mod y (+ 2 2)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (mod y (+ 2 2)) x))))
