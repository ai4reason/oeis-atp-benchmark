;; sequence(s): A153483
;; terms: ~16 49 593 2369 14609 28529 83489 130289 279809 707249 923489 1874129 2825729 3418769 4879649 7890449 12117329 13845809 20151089 25411649
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + 2, 1, x) - (2 * loop(x * x, 2, 2))
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 2, 1, x) - (2 * (2 * (2 * (2 + 2))))
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (- (w0 x) (* 2 v3)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ (v5 x y) 1) (* (* x x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (w4 x) (* 2 (* 2 (* 2 (+ 2 2))))))))
