;; sequence(s): A203683
;; terms: 1 5 1700 601120000 3496121614336000000 5335507266769461885009920000000000 34161019296423817239835748940949012820787200000000000000
;; small program: loop(loop(loop(((loop((x * x) + x, 1, loop2(x * y, y, x, 1, 2 + 2)) div 2) + 1) div 2, 1, y) * x, y, 1) * x, x, 1)
;; fast program: loop(loop((loop2((1 + (y * y)) * x, y + y, x, 1, 2) * loop2(x * y, y + y, x - 1, 1, 2)) * loop2(x * y, y + y, x - 1, 1, 2), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div (+ (div (v3 x) 2) 1) 2))))
(assert (= g2 1))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ 1 (* y y)) x))))
...
(assert (forall ((x Int)) (= (f6 x) (* (* (w7 x) (w8 x)) (w9 x)))))
(assert (= g6 1))
