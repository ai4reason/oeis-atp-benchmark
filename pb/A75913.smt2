;; sequence(s): A75913
;; terms: 1 75 3500 131250 4344375 132890625 3855156250 107765625000 2933008203125 78271552734375 2058270703125000 53524929199218750 1380066321044921875 35349237725830078125 900813505310058593750 22863955398559570312500 578500758117828369140625
;; small program: loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1) * loop((2 * (x + x)) + x, x, 1)
;; fast program: loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1) * loop2(x * y, y, x, 1, 1 + (2 + 2))
(set-logic UFNIA)
