;; sequence(s): A246303
;; terms: 3 4 5 9 10 11 12 16 17 18 22 23 24 28 29 30 35 36 37 41
;; small program: compr(loop2(loop((loop(x * y, y, 1) + x) div (1 + y), 2, x) + y, y - x, x, 1, 2), x)
;; fast program: loop((compr(((((loop((((((x mod (1 + (2 + 2))) + x) mod (1 + 2)) div 2) + 1) + x, x, 1) + 1) div 2) + 1) + x) mod 2, 1 + x) - 1) - x, 1, 1 + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ (div (mod (+ (mod x (+ 1 (+ 2 2))) x) (+ 1 2)) 2) 1) x))))
(assert (forall ((x Int)) (= (g6 x) x)))
(assert (= h6 1))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 (g6 x) h6))))
(assert (forall ((x Int)) (= (f5 x) (mod (+ (+ (div (+ (v6 x) 1) 2) 1) x) 2))))
