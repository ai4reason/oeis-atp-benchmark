;; sequence(s): A156778
;; terms: 0 1 5 15 34 70 123 203 308 450 645 880 1182 1547 1967 2460 3048 3740 4509 5396
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, x) * x) div 2
;; fast program: ((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) - 1) + x) * x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (- (* (v3 x) (ite (<= x 0) 1 2)) 1) x) x) 2))))
