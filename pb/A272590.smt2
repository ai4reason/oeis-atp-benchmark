;; sequence(s): A272590
;; terms: 2 8 24 120 840 9240 120120 2042040 38798760 892371480 25878772920 802241960520 29682952539240 1217001054108840 52331045326680120 2459559130353965640 130356633908760178920 7691041400616850556280 469153525437627883933080 31433286204321068223516360
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, x, 2) * (if x <= 0 then 1 else 2)
;; fast program: loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, x - 1, if x <= 0 then 1 else (2 + 2)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (ite (<= x 0) 1 (+ 2 2)))))
