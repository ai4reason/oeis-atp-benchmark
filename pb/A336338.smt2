;; sequence(s): A336338
;; terms: 2 6 10 14 16 18 20 22 24 28 32 34 36 42 48 50 52 54 56 60
;; small program: compr(loop(((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + 1) + x, x + x, 2) mod 2, 1 + x) * 2
;; fast program: compr(loop((if loop(((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2, (1 + x) mod 2, x) <= 0 then 2 else 1) + x, x + x, 2) mod 2, 1 + x) * 2
(set-logic UFNIA)
