;; sequence(s): A18279
;; terms: 1 2 4 23 46 92
;; small program: 1 + compr(loop(2 * (2 + x), 2 + 2, 2) mod (1 + x), x)
;; fast program: compr(((loop(((x * x) * x) - x, 1, x - 2) * x) mod ((2 * (2 * (2 + (2 + 2)))) - 1)) - 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (mod (* (v3 x) x) (- (* 2 (* 2 (+ 2 (+ 2 2)))) 1)) 1))))
