;; sequence(s): A28046
;; terms: 1 27 481 7203 98665 1285179 16245937 201600531 2472643129 30100201131 364634299393 4403082925059 53056831273993 638440928326683 7675363823623249 92216951605023987 1107504122083376857 13297255155917290635 159624707672057279905
;; small program: loop(loop2((((((y - 1) + y) * (y * y)) + x) + x) + x, y + y, y, 1, 2) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(((loop(((loop2((2 * (x + y)) + x, y + y, x, 1, 2 + 2) div 2) + 1) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (+ (+ (* (+ (- y 1) y) (* y y)) x) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (* 2 (+ x y)) x))))
...
(assert (forall ((x Int)) (= (f4 x) (* (+ (div (w5 x) 2) 1) (v6 x)))))
(assert (= g4 1))
