;; sequence(s): A163086
;; terms: 1 1 2 24 1728 3732480 161243136000 975198486528000000 412860031256494080000000000 110116706384632080236544000000000000000 7401233839469056679744633202278400000000000000000000
;; small program: loop(loop(loop(loop(x * y, x, 1) div loop((x * y) * y, x div 2, 1), 1, y) * x, y, 1) * x, x, 1)
;; fast program: loop(loop(loop((loop2((x * y) * y, y - 1, x div 2, 1, x) * (1 + x)) div loop(x * y, x, 1), 1, y) * x, y, 1) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div (v3 x) (v4 x)))))
(assert (= g2 1))
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (* (* x y) y))))
(assert (forall ((x Int) (y Int)) (= (g8 x y) (- y 1))))
...
(assert (forall ((x Int)) (= (f7 x) (div (* (w8 x) (+ 1 x)) (v9 x)))))
(assert (= g7 1))
