;; sequence(s): A137787
;; terms: ~2 ~4 ~4 26 272 1826 10736 59426 318272 1670786 8656976 44454626 226827872 1151991746 5830280816 29429454626 148249811072 745630312706 3745590106256 18797445635426
;; small program: loop2(((x + x) + x) + y, (2 * (y + y)) + y, x, 0, 2) - loop((x * x) + x, 1, loop(x + x, x, 1))
;; fast program: (loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) - loop((x * x) + x, 1, loop(x + x, x, 1))) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (w3 x) (v6 x)) (w8 x)))))
