;; sequence(s): A131568
;; terms: 0 3 45 630 8925 128898 1891890
;; small program: loop(2 * ((x - (x div y)) + x), x, loop(x + x, x + x, 1) - 1) div 2
;; fast program: ((loop(x * x, 1, loop(x + x, x, 1)) - 1) * loop2(x * y, 1 + y, x, 1, 1 + x)) div loop(x * y, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (- (v2 x) 1) (w4 x)) (v5 x)))))
