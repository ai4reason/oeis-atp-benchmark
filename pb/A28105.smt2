;; sequence(s): A28105
;; terms: 1 33 700 12150 187951 2698923 36799570 483082800 6161352901 76830423813 940920228040 11355049670250 135378462029851 1597705036844703 18694490136746110 217145575509718500
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * ((2 * (x + x)) + x)), y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
