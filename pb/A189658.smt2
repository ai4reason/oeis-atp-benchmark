;; sequence(s): A189658
;; terms: 1 2 4 5 9 10 11 13 14 18 21 24 25 26 28 29 31 32 36 37
;; small program: loop(compr(loop2(((y + y) div (1 + 2)) + x, y div (1 + 2), x, 0, x) mod 2, y), x, 0) + 1
;; fast program: ((compr(((((((((((((((loop2(((y + y) div (1 + 2)) + x, y div (1 + 2), x, x, x) mod 2) + 1) + x) + x) + x) + x) + x) + x) div 2) + x) + x) + 2) - x) + x) mod 2, x) + 1) * 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (+ y y) (+ 1 2)) x))))
(assert (forall ((x Int) (y Int)) (= (g2 x y) (div y (+ 1 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (div (+ y y) (+ 1 2)) x))))
(assert (forall ((x Int) (y Int)) (= (g4 x y) (div y (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (- (+ (+ (+ (div (+ (+ (+ (+ (+ (+ (+ (mod (w4 x) 2) 1) x) x) x) x) x) x) 2) x) x) 2) x) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (v3 x) 1) 2) 2))))
