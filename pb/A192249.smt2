;; sequence(s): A192249
;; terms: 0 5 20 90 300 930 2610 6900 17295 41605 96660 218145 480225 1034765 2188385 4552745 9334760 18892805 37794765 74817520
;; small program: loop(loop(loop(((2 * (x + x)) div y) + x, x, 1) * loop2(x + y, x, x, 0, 1), 1, y) + x, x, 0)
;; fast program: loop(loop((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 * (2 + (2 + 2))))) * loop2(x + y, x, x - 2, 1, 1), 1, y) + x, x, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 2 x) (- (* (* x x) x) x)))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (f5 x) (* (div (v6 x) (* 2 (* 2 (+ 2 (+ 2 2))))) (w7 x)))))
(assert (= g5 1))
