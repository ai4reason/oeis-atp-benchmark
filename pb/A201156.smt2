;; sequence(s): A201156
;; terms: 1 3 10 11 56 57 400 401 402 403 4434 4435 57656 57657 57658 57659 980204 980205 18623896 18623897
;; small program: loop((((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + 1) + x, x, 1)
;; fast program: loop((loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x) + 1, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (* (* (v1 x y) x) y) 1) x))))
