;; sequence(s): A94639
;; terms: 1 2 6 31 227 1991 19415 203456 2248356 25887400 307993016 3763786812 47032778956 598933188956 7751562502556 101741582076581 1351906409905481 18159677984049581 246298405721739581
;; small program: loop(loop2(loop((2 * ((2 * (x * y)) - x)) div (1 + y), y, x), y, 2, 1, y) + x, x, 1)
;; fast program: loop(loop((loop(x * x, 1, loop(2 * ((x - (x div y)) + x), x, 1)) div (1 + x)) div (1 + x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (div (v5 x) (+ 1 x)) (+ 1 x)))))
(assert (= g4 1))
