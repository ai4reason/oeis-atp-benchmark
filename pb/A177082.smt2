;; sequence(s): A177082
;; terms: 6 32 90 208 390 672 1050 1568 2286 3160 4290 5664 7254 9128 11370 14016 16966 20376 24206 28400
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, 2 + x, x) * 2) * (1 + x)
;; fast program: ((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + y) + x, x, 1) * 2) + 1) + x) * (1 + x)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (+ (+ (* (v3 x) 2) 1) x) (+ 1 x)) 2))))
