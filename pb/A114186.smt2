;; sequence(s): A114186
;; terms: 0 1 3 5 9 11 17 19 27 36 46 48 60 62 76 91 107 109 127 129
;; small program: loop(loop((loop((x * y) * y, x, 2) - 1) mod (1 + x), 1, y) + x, x - 1, x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else x, 1, y) + x, x - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) 2) 0) 1 x))))
(assert (= g4 1))
