;; sequence(s): A28747
;; terms: 3 5 6 7 10 11 12 14 20 22 23 24 27 28 29 31
;; small program: compr((1 + loop(x * x, 2, x * x)) mod (1 + loop(x * x, 2, 2)), x)
;; fast program: compr((1 + loop(x * x, 2, x * x)) mod (1 + (2 * (2 * (2 + 2)))), x)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f0 x) (mod (+ 1 (v1 x)) (+ 1 v2)))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ 1 (v4 x)) (+ 1 (* 2 (* 2 (+ 2 2))))))))
