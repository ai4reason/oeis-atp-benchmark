;; sequence(s): A293336
;; terms: 0 1 2 4 7 14 29 57 114 229 458 916 1832 3664 7327 14654 29309 58617 117234 234469
;; small program: loop(((loop2(y + y, (2 * (x + y)) + y, y, 1, 0) + x) div x) + x, x, 1) div 2
;; fast program: ((((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) * loop2(x + y, x, x, 2, 2)) - 1) div loop2(x + y, x, x, 1 + 2, 1)) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (+ (* 2 (+ x y)) y))))
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (+ (w1 x y) x) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (- (* (v2 x) (w5 x)) 1) (w6 x)) 1) 2))))
