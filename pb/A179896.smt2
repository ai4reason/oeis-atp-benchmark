;; sequence(s): A179896
;; terms: 0 18 45 84 108 135 198 273 315 360 459 570 630 693 828 900 975 1053 1134 1305
;; small program: (loop2((loop(compr((loop(x * y, x, x) mod (1 + x)) mod 2, x), 1, y) * x) + 1, y, 2, 1, x) div 2) * (1 + 2)
;; fast program: ((loop2((loop(compr((if (loop((1 + ((x div 2) mod 2)) * (1 + x), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x), x) + 1, 1, y) * x) - 1, y, 2, 1, x) + 1) div 2) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (* (+ 1 (mod (div x 2) 2)) (+ 1 x)))))
(assert (forall ((x Int)) (= (g7 x) x)))
(assert (= h7 1))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (forall ((x Int)) (= (v7 x) (u7 (g7 x) h7))))
(assert (forall ((x Int)) (= (f6 x) (mod (ite (<= (mod (v7 x) (+ 1 x)) 0) 1 0) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (+ (w4 x) 1) 2) (+ 1 2)))))
