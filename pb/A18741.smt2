;; sequence(s): A18741
;; terms: 1 2 3 6 9 18 53 106 159 318 477 954
;; small program: loop(compr(loop((loop(x * x, 2, 2) + y) * (2 + x), 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop((1 + (x + x)) * ((x * x) - x), 1, 2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ v3 y) (+ 2 x)))))
(assert (= g2 2))
(assert (= h2 1))
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 1 (+ x x)) (- (* x x) x)))))
(assert (= g6 1))
(assert (= h6 (* 2 (+ 2 2))))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (mod (+ 2 v6) (+ 1 x)))))
