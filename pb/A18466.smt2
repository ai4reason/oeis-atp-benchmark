;; sequence(s): A18466
;; terms: 1 3 9 17 27 51 153 459
;; small program: loop(compr(loop(1 + (2 * (2 + ((x * x) + y))), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + (2 * (2 * (2 + 2)))) * loop((x * x) * x, 1, 1 + 2)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (* 2 (+ 2 (+ (* x x) y)))))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 (* 2 (* 2 (+ 2 2)))) v4) (+ 1 (+ x x))))))
