;; sequence(s): A337823
;; terms: 1 2 2 4 5 9 9 13 13 17 21 21 27 28 29 33 37 41 41 47
;; small program: loop((compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x), y) - (x div 2)) + 1, x, 1)
;; fast program: loop((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) - (x div 2)) + 1, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- (v1 x y) (div x 2)) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (- (v5 x y) (div x 2)) 1))))
