;; sequence(s): A7993
;; terms: 3 12 40 102 219 419 738 1221 1923 2910 4260 6064 8427 11469 15326 20151 26115 33408 42240 52842
;; small program: loop(loop(loop(loop(x + y, 1 + y, x), 1 + y, x), y, x), x, 1) + 2
;; fast program: ((((((((loop((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 + (2 + 2)))) * x, 1, 2 + x) div (2 + (2 * (2 + 2)))) - 1) - (((x * x) + x) div 2)) - 2) - x) - x) - x) - x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (- (- (- (- (- (div (v4 x) (+ 2 (* 2 (+ 2 2)))) 1) (div (+ (* x x) x) 2)) 2) x) x) x) x) x))))
