;; sequence(s): A57812
;; terms: 2 5 6 11 12 17 18 23 24 25 26 27 28 31 32 33 34 35 36 41
;; small program: compr(loop(loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then (1 mod x) else 0, 1, y) + x, x, 0) mod 2, 1 + x) + 1
;; fast program: compr((loop(loop((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, (x - 1) div 2, 1) + (if x <= 0 then 0 else 1)) mod 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (+ 1 (v3 x)) (+ 1 x)) 0) (mod 1 x) 0))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f6 x) (mod (+ (+ (ite (<= (- (mod (+ 1 (v7 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) (ite (<= x 0) 1 2)))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (v5 x) (ite (<= x 0) 0 1)) 2))))
