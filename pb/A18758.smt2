;; sequence(s): A18758
;; terms: 1 2 3 4 6 8 12 24 41 82 123 164 246 328 492 984
;; small program: loop(compr(loop((2 + loop(x + y, x, y)) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((2 + ((2 * ((2 * (x * x)) + x)) + x)) * x, 1, 2 + (2 + 2)) mod (1 + x), x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 (v3 x y)) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 2 (+ (* 2 (+ (* 2 (* x x)) x)) x)) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
