;; sequence(s): A55583
;; terms: 1 10 58 256 955 3178 9740 28064 77093 203930 523262 1309520 3209871 7731642 18348240 42989520 99612345 228586890 520090690 1174401760
;; small program: loop((loop((loop(loop(loop(x + y, y, x), 1 + y, x) + x, y, 1) + x) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop((loop((loop((((x * x) * x) - x) div (2 + (2 + 2)), 1, 2 + y) + x) + x, y, 1) + x) + x, y, 1) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (div (- (* (* x x) x) x) (+ 2 (+ 2 2))))))
(assert (= g8 1))
(assert (forall ((x Int) (y Int)) (= (h8 x y) (+ 2 y))))
