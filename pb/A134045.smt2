;; sequence(s): A134045
;; terms: 1 3 7 18 61 258 1177 5358 23821 103338 439297 1838598 7605781 31191618 127100617 515462238 2083142941 8396683098 33779525137 135697396278
;; small program: (((loop(((2 + (2 + (2 + x))) * x) + x, 1, loop(x + x, x, 1)) - 1) div 2) - 1) - loop(1 + ((x + x) + x), x, 1)
;; fast program: ((loop(((2 + (2 + (2 + x))) * x) + x, 1, loop(x + x, x, 1)) - loop2(x * y, y, x, 1 + 2, 1 + 2)) div 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (- (div (- (v0 x) 1) 2) 1) (v2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (div (- (v3 x) (w5 x)) 2) 1))))
