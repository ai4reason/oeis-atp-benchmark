;; sequence(s): A3278
;; terms: 1 2 4 5 10 11 13 14 28 29 31 32 37 38 40 41 82 83 85 86
;; small program: compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, (x - 2) + x), 1 + x)
;; fast program: (loop((((((loop(((((loop((((((loop(((((loop(x * x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) + 1
(set-logic UFNIA)
