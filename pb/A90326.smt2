;; sequence(s): A90326
;; terms: 1 4 15 54 185 608 1939 6058 18669 57012 173063 523262 1577953 4750216 14283387 42915666 128878037 386896220 1161212911 3484687270
;; small program: loop(((loop((x + x) + x, y, 1) - y) + x) + x, x, 1)
;; fast program: ((loop2(x * y, y, x div 2, loop(1 + (2 * (2 + 2)), x mod 2, 1 + 2), 1 + (2 * (2 + 2))) - loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2)))) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (w2 x) (v4 x)) 2) x))))
