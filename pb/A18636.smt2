;; sequence(s): A18636
;; terms: 1 2 5 7 10 11 14 22 35 55 70 77 110 154 385 770
;; small program: loop(compr((2 + loop(2 * (x * y), 2 + 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + ((1 + 2) * loop(x * x, 2, 2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (+ 2 (* (+ 1 2) v5)) (+ 1 x)))))
