;; sequence(s): A7606
;; terms: 1 4 5 6 11 12 13 14 15 22 23 24 25 26 27 28 37 38 39 40
;; small program: loop(compr(loop(2 + (y div x), x + x, 1) mod 2, y), x, 0) + 1
;; fast program: (1 + loop((x * x) - x, 1, loop((y div (x * x)) + x, x, 1))) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ 1 (v3 x)) x))))
