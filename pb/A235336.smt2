;; sequence(s): A235336
;; terms: 0 7 11 13 14 19 21 22 25 26 28 31 35 37 38 41 42 44 47 49
;; small program: loop(compr(loop(loop2(x - y, y div 2, x, x, x) + x, 2, x) mod 2, y), x, 0)
;; fast program: compr(((((loop2(x - y, y div 2, x, x, x) + x) * (1 + 2)) + 1) div (2 + 2)) mod 2, x) + 0
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (div (+ (* (+ (w5 x) x) (+ 1 2)) 1) (+ 2 2)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (v4 x) 0))))
