;; sequence(s): A108568
;; terms: 2 3 5 9 13 17 21 25 33 39 45 53 57 61 69 79 85 91 99 103
;; small program: ((loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + x, 1 + y, 2, 0, x) - 1) - x) - x
;; fast program: ((loop2(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + x, 1 + y, 2, 0, x) - 1) - x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (- (- (w0 x) 1) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (w3 x) 1) x) x))))
