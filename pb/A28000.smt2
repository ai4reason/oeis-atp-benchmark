;; sequence(s): A28000
;; terms: 1 28 513 7808 107309 1384836 17143081 206182696 2429008197 28183193324 323282753729 3676063130064 41519535153565 466480044231892 5219284450672857 58204869911960312 647392469287421813
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x)), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x + x, x - 1, 2), 1, y) + (2 * (2 * (x + x)))) + x, x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
