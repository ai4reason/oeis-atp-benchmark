;; sequence(s): A166232
;; terms: 1 1 3 20 272 7424 405504 44302336 9680453632 4230542786560 3697657604210688 6463791365183504384 22598414411798807576576 158015104883301198495481856 2209780998563745292895322636288
;; small program: (loop2(2 * ((x - y) + x), x, x - 1, 2, 1) div 2) * loop2(x * y, y + y, x - 1, 1, 1)
;; fast program: loop2(2 * ((x - y) + x), x, x - 2, (if (x - 1) <= 0 then 0 else 2) + 1, 1) * loop2(x * y, y + y, x - 2, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (w0 x) 2) (w1 x)))))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* 2 (+ (- x y) x)))))
(assert (forall ((x Int)) (= (g2 x) x)))
(assert (forall ((x Int)) (= (h2 x) (- x 2))))
(assert (forall ((x Int)) (= (i2 x) (+ (ite (<= (- x 1) 0) 0 2) 1))))
(assert (= j2 1))
