;; sequence(s): A261971
;; terms: 8 26 98 218 602 866 1538 1946 2906 4706 5402 7778 9602 10586 12698 16226 20186 21602 26138 29402
;; small program: (loop2(compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x, y, 2, 1 + 2, x) + 1) * 2
;; fast program: (loop2(compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x, y, 2, 1 + 2, x) + 1) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (w3 x) 1) 2))))
