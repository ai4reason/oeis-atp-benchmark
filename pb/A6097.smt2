;; sequence(s): A6097
;; terms: 1 31 651 11811 200787 3309747 53743987 866251507 13910980083 222984027123 3571013994483 57162391576563 914807651274739 14638597687734259 234230965858250739 3747802679431278579 59965700687947706355 959458073589354016755
;; small program: loop((loop(loop(loop(1 + (loop(x * x, 2, 2) * x), y, 1) + (2 * (x + x)), y, 1) + (2 * (2 * (x + x))), y, 1) + x) + x, x, 1)
;; fast program: loop(loop(loop(loop(((loop(((x - 1) + x) * ((x * x) - x), 1, loop(x + x, x, 2)) div (1 + 2)) + 1) div 2, 1, y) + x, x, 1) * loop(x + x, x, 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (* (+ (- x 1) x) (- (* x x) x)))))
(assert (= g9 1))
...
(assert (forall ((x Int)) (= (f8 x) (div (+ (div (v9 x) (+ 1 2)) 1) 2))))
(assert (= g8 1))
