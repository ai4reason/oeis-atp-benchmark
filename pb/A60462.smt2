;; sequence(s): A60462
;; terms: 1 3 5 7 8 9 11 13 14 15 17 19 20 21 23 24 25 26 27 29
;; small program: compr(loop(x * y, x, 2) mod (1 + x), 1 + x)
;; fast program: compr((loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x) - 1) - (x mod 2), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (- (v3 x) 1) (mod x 2)))))
