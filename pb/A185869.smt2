;; sequence(s): A185869
;; terms: 2 7 9 16 18 20 29 31 33 35 46 48 50 52 54 67 69 71 73 75
;; small program: ((loop(loop((if x <= 0 then 1 else (2 + y)) - x, x, x), 1, 1 + (x + x)) + x) + x) + x
;; fast program: (loop2(loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + (2 + 2), x), 1, y) * x, y, 2, 1, x) + (1 + x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (- (ite (<= x 0) 1 (+ 2 y)) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (w2 x) (+ 1 x)) x))))
