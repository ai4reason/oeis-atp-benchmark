;; sequence(s): A35303
;; terms: 1 2 3 4 5 6 8 9 10 12 15 16 18 20 24 25 30 36 40 45
;; small program: loop(compr(loop((1 + (x * y)) * (x * x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(x * x, 1, loop(((x * x) * x) - x, 1, 2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 (* x y)) (* x x)))))
(assert (= g2 2))
(assert (= h2 2))
