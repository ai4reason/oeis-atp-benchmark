;; sequence(s): A14148
;; terms: 2 7 17 34 62 103 161 238 338 467 627 824 1062 1343 1671 2052 2492 2993 3561 4200
;; small program: loop(loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) + x, y, 2) + x, x, 2)
;; fast program: loop(loop((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) + x) * 2) - 1) - x, 1, y) + x, x, 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (+ (* (v2 x y) 2) 1) x))))
