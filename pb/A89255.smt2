;; sequence(s): A89255
;; terms: 5 9 11 17 21 23 29 33 39 47 51 53 57 59 71 77 81 89 93 99
;; small program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod (2 + 2)), x) - 1, 2 + x) div 2) + 1) + 2
;; fast program: (((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 2)) mod (1 + 2)) + 1) mod 2, x) * 2) + 2) div 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (mod (- (mod (+ (v4 x) x) 2) (mod x 2)) (+ 1 2)) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (* (v3 x) 2) 2) 2) 2))))
