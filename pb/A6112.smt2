;; sequence(s): A6112
;; terms: 1 156 20306 2558556 320327931 40053706056 5007031143556 625886840206056 78236053707784181 9779511680526143556 1222439084242108174806 152804888634672088643556 19100611156944225555440431 2387576396558283557830831056
;; small program: loop(loop(loop((loop(1 + (loop(x * x, 1, 1 + (2 + 2)) * x), y, 1) + (2 * (x + x))) + x, x, 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x, x, 1)
;; fast program: (loop(loop((loop2(loop2(1 + (x * y), y, y, 1, 1 + (2 + 2)) * x, 1 + y, 2, 1, x) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x, x, 1) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (v2 x) (w5 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ 1 (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (w9 x y) (u9 (h9 x y) i9 j9))))
(assert (forall ((x Int) (y Int)) (= (f8 x y) (* (w9 x y) x))))
...
(assert (forall ((x Int)) (= (f7 x) (* (div (w8 x) (+ 1 2)) (w10 x)))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (v6 x) 2) 1))))
