;; sequence(s): A274428
;; terms: 3 6 9 10 14 15 19 20 21 26 27 28 33 34 35 36 42 43 44 45
;; small program: compr(loop((((x + y) + y) div ((x * x) div 2)) + x, x + x, 2) mod 2, 1 + x) + 1
;; fast program: (compr(loop((((((loop(loop((((y - x) + y) div (x * x)) + x, x div 2, 1) + x, 2, x) + 1) + x) mod 2) * 2) - 1) mod (1 + x), (1 + x) mod 2, x) mod 2, 1 + x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (div (+ (+ x y) y) (div (* x x) 2)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div (+ (- y x) y) (* x x)) x))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (* (mod (+ (+ (v4 x) 1) x) 2) 2) 1) (+ 1 x)))))
