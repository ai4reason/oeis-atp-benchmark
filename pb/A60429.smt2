;; sequence(s): A60429
;; terms: 17 37 101 197 485 677 1157 1445 2117 3365 3845 5477 6725 7397 8837 11237 13925 14885 17957 20165
;; small program: (loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 2, x) * 2) + 1
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (x * x), 0, 2, 1, x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (w3 x) 2) 1))))
