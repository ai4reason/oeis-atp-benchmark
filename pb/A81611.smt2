;; sequence(s): A81611
;; terms: 1 2 2 3 4 4 4 4 4 5 6 6 7 8 8 8 8 8 8 8
;; small program: loop((if (loop(2 * ((x - (x div y)) + x), y, 1) mod (1 + 2)) <= 0 then 0 else 1) + x, x, 1)
;; fast program: loop(loop(if (loop2((x * y) + x, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) <= 0 then 0 else 1, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (ite (<= (mod (v1 x y) (+ 1 2)) 0) 0 1) x))))
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= (mod (w4 x) (+ 1 2)) 0) 0 1))))
(assert (= g3 1))
