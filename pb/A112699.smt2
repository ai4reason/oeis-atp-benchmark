;; sequence(s): A112699
;; terms: 1 6 56 681 9431 140681 2203181 35718806 594312556 10090406306 174113843806 3044524000056 53828703687556 960689055250056 17284175383375056 313147365080640681 5708299647795484431
;; small program: loop((loop(2 * ((1 + (2 + 2)) * ((x - (x div y)) + x)), y, 1) div (1 + y)) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x - 1, x)) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* 2 (* (+ 1 (+ 2 2)) (+ (- x (div x y)) x))))))
