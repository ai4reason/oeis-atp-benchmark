;; sequence(s): A186299
;; terms: 2 6 14 18 26 30 50 54 74 78 86 90 98 114 134 138 146 158 174 186
;; small program: compr((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x)), 1 + x)
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) div 2) * 2) + 1) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (div (v3 x) 2) 2) 1) 2))))
