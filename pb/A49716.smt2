;; sequence(s): A49716
;; terms: 1 2 2 2 4 2 2 4 2 2 4 2 4 6 2 2 4 6 2 4
;; small program: loop((loop((loop(if ((loop(x + x, x, 1) mod (1 + x)) - 1) <= 0 then 1 else 0, 1, x + x) + 1) mod 2, 1, y) * x) + 2, x, 1)
;; fast program: loop((loop((loop(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0, 1, x + x) + 1) mod 2, 1, y) * x) + 2, x, 1)
(set-logic UFNIA)
