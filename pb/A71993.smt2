;; sequence(s): A71993
;; terms: 1 0 1 2 1 0 ~1 0 1 0 1 2 1 2 3 2 1 0 1 2
;; small program: (loop((loop(compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), y), x, 0) - x) - x, 2, x) - x) + 1
;; fast program: ((((compr((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + (2 * (2 + 2)), x), 2, x) + 1) mod 2, x) * (0 - 1)) + 2) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (ite (<= (mod x 2) 0) 2 x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (* (v4 x) (- 0 1)) 2) x) x) x))))
