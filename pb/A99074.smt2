;; sequence(s): A99074
;; terms: 1 4 11 24 43 70 109 158 221 300 391 500 633 780 961 1168 1391 1644 1933 2240
;; small program: loop(loop((x div ((y - 1) + y)) + x, x, y) + x, x, 1) + x
;; fast program: loop(loop(loop2((x div y) + x, y - 1, x, x, x), 1, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (div x (+ (- y 1) y)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (div x y) x))))
