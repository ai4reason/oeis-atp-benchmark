;; sequence(s): A18614
;; terms: 1 2 4 7 8 13 14 26 28 52 56 91 104 182 364 728
;; small program: loop(compr((2 - loop(((x * x) * x) + 1, 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((2 * (2 + (x div 2))) * (2 + x), 2, 2) mod (1 + x), x) + 1) + 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (* 2 (+ 2 (div x 2))) (+ 2 x)))))
(assert (= g4 2))
(assert (= h4 2))
