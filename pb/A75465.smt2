;; sequence(s): A75465
;; terms: 2 2 3 4 6 7 8 10 11 13 15 16 18 20 22 24 26 28 30 32
;; small program: ((((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) + 1) + (x div 2)) div (1 + x)) + 1
;; fast program: ((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) + 1) + (x div 2)) div (1 + x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (+ (+ (* (v0 x) 2) 1) (div x 2)) (+ 1 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (+ (* (v3 x) 2) 1) (div x 2)) (+ 1 x)) 1))))
