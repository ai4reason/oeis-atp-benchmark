...
;; small program: compr(loop(loop(y - x, x, 1), 1 + (2 + 2), x) - 1, x) + 1
;; fast program: compr((if ((x - 2) - (2 * (2 * (2 * (2 + 2))))) <= 0 then 1 else 2) - ((0 - x) mod (1 + 2)), x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (ite (<= (- (- x 2) (* 2 (* 2 (* 2 (+ 2 2))))) 0) 1 2) (mod (- 0 x) (+ 1 2))))))
