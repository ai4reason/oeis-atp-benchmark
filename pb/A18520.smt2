;; sequence(s): A18520
;; terms: 1 3 11 17 33 51 187 561
;; small program: loop(compr(loop((loop(x * x, 2, 2) + x) * x, 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((2 * (x * x)) - x, 1, 1 + (2 * (2 * (2 + 2)))) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
