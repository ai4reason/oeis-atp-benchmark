;; sequence(s): A83236
;; terms: 2 0 3 2 5 6 7 10 9 14 15 16 21 20 23 24 29 30 31 36
;; small program: loop((compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x), y) - x) + 1, x, 2)
;; fast program: ((loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) - x, x - 1, 1) - (x mod 2)) + 1) - (x mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (- (v4 x) (mod x 2)) 1) (mod x 2)))))
