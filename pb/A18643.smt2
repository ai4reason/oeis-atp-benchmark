;; sequence(s): A18643
;; terms: 1 2 17 23 34 46 391 782
;; small program: loop(compr(loop(2 + ((x div y) + (x * x)), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(2 * ((x * x) + ((x * x) - (2 div 2))), 2, 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* 2 (+ (* x x) (- (* x x) (div 2 2)))))))
(assert (= g5 2))
(assert (= h5 2))
