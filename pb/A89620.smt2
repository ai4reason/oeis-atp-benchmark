;; sequence(s): A89620
;; terms: 3 11 32 71 136 229 360 531 752 1029 1362 1765 2238 2787 3422 4149 4972 5893 6926 8071
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 1) + loop((x * x) * x, 1, 1 + x)
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) - (if x <= 0 then 1 else 2)) + 1) + loop((x * x) * x, 1, 1 + x)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (- (* (v4 x) 2) (ite (<= x 0) 1 2)) 1) (v9 x)) 2))))
