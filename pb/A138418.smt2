;; sequence(s): A138418
;; terms: 6 36 300 1176 7260 14196 41616 64980 139656 353220 461280 936396 1412040 1708476 2438736 3943836 6056940 6921060 10073316 12703320
;; small program: (loop2((loop(loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) div 2) + 1
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, y, 2, 1, x), 1, y) * (x - 1), y, 2, 2, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (w5 x) 2))))
