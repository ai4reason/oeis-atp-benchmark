;; sequence(s): A296021
;; terms: 0 1 1 2 3 3 3 4 4 5 6 6 6 7 7 8 8 8 9 9
;; small program: loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y + y) + x, x, 0)
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y + y) + x, x, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (- (mod (+ 1 (v2 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g1 1))
