;; sequence(s): A301707
;; terms: 0 4 15 20 50 60 119 136 153 170 308 336 533 574 615 656 986 1044 1463 1540
;; small program: loop((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * (1 + y)) + x, x, 0) * (1 + x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 0, 1, y) + x, x, 0) * (1 + x)
(set-logic UFNIA)
