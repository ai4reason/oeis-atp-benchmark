;; sequence(s): A309212
;; terms: 1 1 2 2 3 4 6 7 10 13 18 24 32 42 56 75 100 133 177 237
;; small program: loop((1 + x) div (1 + 2), x, loop(2 * (x + x), x, 1))
;; fast program: ((loop((x + x) * x, 1, loop(x + x, x, 1)) div loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (v2 x) (w4 x)) 1) 2))))
