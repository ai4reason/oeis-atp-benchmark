;; sequence(s): A82752
;; terms: 1 2 3 4 6 7 8 12 14 15 16 24 28 30 31 32 48 56 60 62
;; small program: loop(1 + ((loop((x mod y) + y, x, 0) + x) div 2), x, 1)
;; fast program: compr(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), 2, x), x) + 1
(set-logic UFNIA)
