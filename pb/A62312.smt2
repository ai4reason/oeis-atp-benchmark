;; sequence(s): A62312
;; terms: 1 16 36 64 81 100 144 196 225 256 324 400 441 484 576 625 676 729 784 900
;; small program: loop2((compr((loop(x * y, x, x) mod (1 + x)) mod 2, y) * x) + x, y, 2, 1, x)
;; fast program: loop2(loop(compr((if (loop((1 + ((x div 2) mod 2)) * (1 + x), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x), x) + 1, 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
