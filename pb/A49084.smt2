;; sequence(s): A49084
;; terms: 0 1 2 0 3 0 4 0 0 0 5 0 6 0 0 0 7 0 8 0
;; small program: (loop(loop(((loop(x * y, x, x) mod (1 + x)) + 1) mod 2, 1, y) + x, x, 1) * loop(x * y, x, 1)) mod (1 + x)
;; fast program: (loop(loop((((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, x div 2, 1) * loop(x * y, x - 1, 1)) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (* (v0 x) (v3 x)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (* (v4 x) (v7 x)) (+ 1 x)))))
