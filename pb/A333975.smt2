;; sequence(s): A333975
;; terms: 1 2 4 7 8 11 13 14 16 19 21 22 25 26 28 32 35 37 38 41
;; small program: loop(compr(loop(loop((2 + x) mod y, x, 1), 1 + 2, x), y), x, 1)
;; fast program: compr((((((loop2(x - y, y div 2, 2 + (2 + 2), x, x div 2) * 2) - (if x <= 0 then 1 else 2)) * 2) + 1) mod (1 + (2 * (2 + 2)))) - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (mod (+ (* (- (* (w5 x) 2) (ite (<= x 0) 1 2)) 2) 1) (+ 1 (* 2 (+ 2 2)))) 1))))
