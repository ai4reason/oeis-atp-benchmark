;; sequence(s): A184052
;; terms: 1 2 5 13 30 66 137 279 556 1104 2179 4309 8518 16886 33509 66643 132672 264492 527639 1053441
;; small program: loop(loop(((loop2(1 + y, x + x, x, 1, 1) + loop(x + x, x, 1)) - 2) - x, 1, y) + x, x, 1)
;; fast program: loop(loop(((((1 + loop(x + x, x - (x div 2), 1)) * (1 + loop(x + x, x div 2, 1))) - 2) - 2) - x, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (- (+ (w2 x) (v3 x)) 2) x))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (- (- (- (* (+ 1 (v6 x)) (+ 1 (v7 x))) 2) 2) x))))
(assert (= g5 1))
