;; sequence(s): A176693
;; terms: 0 1 2 4 6 8 9 10 12 14 16 18 20 22 24 25 26 28 30 32
;; small program: loop((((if loop((if x <= 0 then 0 else y) - x, x, x) <= 0 then 2 else x) mod 2) + 1) + x, x, 1) - 1
;; fast program: loop((((loop(loop(((loop(loop(((loop(if (loop(loop(y, x - y, x), x, 1 + x) - 1) <= 0 then 1 else 0, 1, x div 2) + x) mod 2) + x, (1 + x) mod 2, x) - x, 1, x div 2) + x) mod 2) + x, (1 + x) mod 2, x) - x, 1, x div 2) + x) mod 2) + x) + 1, x, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (- (ite (<= x 0) 0 y) x))))
(assert (forall ((x Int)) (= (g1 x) x)))
(assert (forall ((x Int)) (= (h1 x) x)))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v1 x) (u1 (g1 x) (h1 x)))))
(assert (forall ((x Int)) (= (f0 x) (+ (+ (mod (ite (<= (v1 x) 0) 2 x) 2) 1) x))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (mod (+ (v5 x) x) 2) x))))
