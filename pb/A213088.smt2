;; sequence(s): A213088
;; terms: 0 1 2 1 2 3 4 3 2 3 4 5 6 5 4 3 4 5 6 7
;; small program: (loop(loop(loop(((y + y) div (x * x)) + x, x + x, 1) mod 2, 1, y) + x, x, 0) * 2) - x
;; fast program: loop((loop(loop(((loop((loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, x div 2) + 1) + x, 2, x) + 1) + x) mod 2, 1, y) + x, x - 1, 0) * 2) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (div (v8 x y) x))))
(assert (forall ((x Int) (y Int)) (= (g7 x y) y)))
(assert (= h7 (+ 1 2)))
(assert (= i7 (+ 2 2)))
(assert (forall ((x Int)) (= (j7 x) (div x 2))))
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ (w7 x) 1) x))))
(assert (= g6 2))
