;; sequence(s): A344346
;; terms: 3 4 11 12 15 16 19 20 27 28 35 36 43 44 47 48 51 52 59 60
;; small program: compr(loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), x + x, x) mod 2, 1 + x) + 1
;; fast program: ((loop(((compr((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) - 1) mod 2, x) * 2) - 1) - x, 1, x div 2) * 2) + 1) + x
(set-logic UFNIA)
