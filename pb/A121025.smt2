;; sequence(s): A121025-A277592
;; terms: 5 15 25 35 45 50 55 65 75 85 95 105 115 125 135 145 150 155 165 175
;; small program: compr(loop(x div loop(1, x mod (2 + (2 * (2 + 2))), 2 + (2 * (2 + 2))), x div (2 + (2 + 2)), x) mod (1 + (2 + 2)), 1 + x)
;; fast program: compr(loop(x div loop(1, x mod (2 + (2 * (2 + 2))), 2 + (2 * (2 + 2))), x div (2 + (2 * (2 + 2))), x) mod (1 + (2 + 2)), 1 + x)
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int)) (= (g1 x) (div x (+ 2 (+ 2 2))))))
(assert (forall ((x Int)) (= (h1 x) x)))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y))))))
(assert (forall ((x Int)) (= (v1 x) (u1 (g1 x) (h1 x)))))
(assert (forall ((x Int)) (= (f0 x) (mod (v1 x) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (v4 x) (+ 1 (+ 2 2))))))
