;; sequence(s): A79704
;; terms: 8 18 50 98 242 338 578 722 1058 1682 1922 2738 3362 3698 4418 5618 6962 7442 8978 10082
;; small program: loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 2, x)
;; fast program: ((((((((loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (x * x), 2, 2, 1, x) + 2) * 2) + 1) + x) div 2) * 2) - 1) - x) div (1 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (- (- (* (div (+ (+ (* (+ (w3 x) 2) 2) 1) x) 2) 2) 1) x) (+ 1 (+ 2 2))))))
