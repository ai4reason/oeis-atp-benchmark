;; sequence(s): A123476
;; terms: 1 4 18 72 450 2700 22050 176400 1587600 15876000 192099600 2305195200 32464832400 454507653600 6817614804000 109081836864000 1970290678356000 35465232210408000 711274934886516000
;; small program: loop((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) + y) * x, x, 1 + x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else x, 1, y) * x, x, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) 2) 0) (+ 1 x) x))))
(assert (= g4 1))
