;; sequence(s): A55042
;; terms: 10 26 40 42 58 74 90 104 106 122 138 154 160 168 170 186 202 218 232 234
;; small program: compr(loop((loop(x div (if (x mod (2 + 2)) <= 0 then (2 + 2) else 1), x div (2 + (2 + 2)), x) + 1) mod (2 * (2 + 2)), 1, (x + x) + x) - 1, 1 + x) * 2
;; fast program: compr(loop((loop(x div loop(1, x mod (2 + 2), 2 + 2), x div (2 * (2 * (2 + (2 + 2)))), x) + 1) mod (2 * (2 + 2)), 1, (x + x) + x) - 1, 1 + x) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (div x (+ 2 (+ 2 2))))))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 (g2 x) (h2 x)))))
(assert (forall ((x Int)) (= (f1 x) (mod (+ (v2 x) 1) (* 2 (+ 2 2))))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (g5 x) (div x (* 2 (* 2 (+ 2 (+ 2 2))))))))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (mod (+ (v5 x) 1) (* 2 (+ 2 2))))))
(assert (= g4 1))
