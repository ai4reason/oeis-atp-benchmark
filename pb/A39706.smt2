;; sequence(s): A39706
;; terms: 2 3 5 7 3 5 1 3 7 5 7 5 1 3 7 5 3 5 3 7
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 * (2 + 2))) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(((2 * (x + x)) div (2 + (x mod 2))) + y, 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div (* 2 (+ x x)) (+ 2 (mod x 2))) y))))
(assert (= g5 2))
(assert (= h5 2))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (mod (+ (v2 x) 1) v5))))
