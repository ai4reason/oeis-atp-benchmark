;; sequence(s): A61066
;; terms: 1 3 6 15 21 36 45 66 105 120 171 210 231 276 351 435 465 561 630 666
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x) div 2) div 2) div 2
;; fast program: (loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, y, 2, 1, x) div (2 * (2 + 2))) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (w3 x) (* 2 (+ 2 2))) 1))))
