...
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (div (+ (+ (* (+ (div (+ (+ (* (div (v0 x) 2) 2) 1) x) (+ 1 2)) x) (+ 1 2)) 1) x) (+ 1 2)) x) x) x))))
(assert (forall ((x Int) (y Int)) (= (f1 x y) y)))
(assert (forall ((x Int)) (= (g1 x) (mod x (+ 2 2)))))
(assert (= h1 2))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v1 x) (u1 (g1 x) h1))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (div (+ (+ (* (+ (div (+ (+ (- (v1 x) 1) x) x) (+ 1 2)) (ite (<= x 0) 1 x)) (+ 1 2)) 1) x) (+ 1 2)) x) x) x))))
