;; sequence(s): A142342
;; terms: 20 30 50 70 110 130 170 190 230 290 310 370 410 430 470 530 590 610 670 710
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, 2, 2, 2, x)
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * (((2 + 2) * 2) + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) (+ (* (+ 2 2) 2) 2)))))
