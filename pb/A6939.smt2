;; sequence(s): A6939
;; terms: 1 2 12 360 75600 174636000 5244319080000 2677277333530800000 25968760179275365452000000 5793445238736255798985527240000000 37481813439427687898244906452608585200000000 7517370874372838151564668004911177464757864076000000000 55784440720968513813368002533861454979548176771615744085560000000000
;; small program: loop(loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, x), x, 1)
;; fast program: loop(loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y - 1, 2) * x, x, 1)
(set-logic UFNIA)
