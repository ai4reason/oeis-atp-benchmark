;; sequence(s): A18332
;; terms: 1 2 4 5 8 10 20 25 40 50 100 200
;; small program: 1 + compr(loop((2 * (2 + 2)) * (1 + x), 2, 2) mod (1 + x), x)
;; fast program: compr((2 * loop(x * x, 1, 2 + (2 * (2 + 2)))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (* 2 (+ 2 2)) (+ 1 x)))))
(assert (= g1 2))
(assert (= h1 2))
