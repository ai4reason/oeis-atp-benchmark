;; sequence(s): A235534
;; terms: 1 3 55 1428 43263 1430715 50067108 1822766520 68328754959 2619631042665 102240109897695 4048514844039120 162250238001816900 6568517413771094628 268225186597703313816 11034966795189838872624 456949965738717944767791
;; small program: loop(loop2((2 + y) * x, 1 + y, x - 1, 1, x + x) div loop(x * y, x, 1), 1, x + x)
;; fast program: loop((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x - 1, 1)) div (if x <= 0 then 1 else x), 1, x + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (div (w4 x) (v5 x)) (ite (<= x 0) 1 x)))))
(assert (= g3 1))
