;; sequence(s): A81115
;; terms: 2 4 10 14 24 30 44 70 80 114 140 154 184 234 290 310 374 420 444 520
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), 2 + y) * x) + x, y, 2, 1, x) div 2) div 2) div (1 + 2)
;; fast program: (loop(loop2(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y, 2, 1, x), 1, 2 + x) div (2 + 2)) div (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (div (div (w0 x) 2) 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (v3 x) (+ 2 2)) (+ 1 2)))))
