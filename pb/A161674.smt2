;; sequence(s): A161674
;; terms: 0 1 4 5 6 7 8 9 12 13 16 17 20 21 22 23 24 25 28 29
;; small program: loop(compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x div 2), y), x, 0)
;; fast program: (loop((compr((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) + 1) mod 2, x) - 1) - x, 1, x div 2) * 2) + x
(set-logic UFNIA)
