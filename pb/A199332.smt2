;; sequence(s): A199332
;; terms: 1 2 3 4 4 4 5 6 7 8 9 9 9 9 9 10 11 12 13 14
;; small program: loop(loop(loop((((2 - loop(loop(y, x - y, x), x, 1 + x)) mod (2 + 2)) + 1) + x, x, 1) mod 2, 1, y) + x, x, 1)
;; fast program: (((compr((((((((loop(loop(y, x - y, x), x, 1 + x) - 1) mod (1 + x)) mod (1 + 2)) - 1) mod (1 + x)) mod (1 + 2)) - 1) mod (1 + x), 1 + x) * 2) + 1) div (1 + 2)) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (+ (mod (- 2 (v3 x)) (+ 2 2)) 1) x))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (- (mod (mod (- (mod (mod (- (v6 x) 1) (+ 1 x)) (+ 1 2)) 1) (+ 1 x)) (+ 1 2)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (div (+ (* (v5 x) 2) 1) (+ 1 2)) x))))
