...
;; small program: compr(loop(if (x mod 2) <= 0 then ((x div 2) div (1 + (2 + 2))) else 1, x div 2, x), x)
;; fast program: compr(loop(loop(if (x mod 2) <= 0 then ((1 + x) div (2 + (2 * (2 + 2)))) else 1, x div 2, x), (1 + x) mod 2, x), x)
(set-logic UFNIA)
