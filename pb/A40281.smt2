;; sequence(s): A40281
;; terms: 17 3 2 3 34 3 2 3 34 3 2 3 34 3 2 3 34 3 2 3
;; small program: ((1 + loop(x * x, 2, loop(0, x mod (2 + 2), 2))) * (if x <= 0 then 1 else 2)) + (x mod 2)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ 1 (v0 x)) (ite (<= x 0) 1 2)) (mod x 2)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (v2 x) (mod x 2)))))
