;; sequence(s): A72055
;; terms: 5 7 11 15 23 27 35 39 47 59 63 75 83 87 95 107 119 123 135 143
;; small program: ((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) * 2) + 1) + 2
;; fast program: (((((compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) * 2) + 2) + 2) - (if x <= 0 then 1 else 2)) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (+ (+ (* (v3 x) 2) 2) 2) (ite (<= x 0) 1 2)) 2) 1))))
