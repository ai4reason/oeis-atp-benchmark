;; sequence(s): A114284
;; terms: 1 ~2 1 ~2 ~2 1 ~2 ~2 ~2 1 ~2 ~2 ~2 ~2 1 ~2 ~2 ~2 ~2 ~2
;; small program: if loop(loop(y - 1, x - y, x), x, x) <= 0 then 1 else (0 - 2)
;; fast program: ((((((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x) + 1) + x, 2, x) + 1) + x) mod 2) * 2) - 1) * (1 + 2)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (- (* (mod (+ (+ (v2 x) 1) x) 2) 2) 1) (+ 1 2)) 2))))
