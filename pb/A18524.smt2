;; sequence(s): A18524
;; terms: 1 2 3 5 6 10 15 19 30 38 57 95 114 190 285 570
;; small program: loop(compr(loop(x * loop(x + y, x, 2), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: compr((2 + loop(((1 + x) * (x * x)) - x, 1, 2 * (2 + 2))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* x (v3 x)))))
(assert (= g2 2))
(assert (= h2 2))
