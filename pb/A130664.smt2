;; sequence(s): A130664
;; terms: 1 2 4 6 9 12 15 18 21 24 28 32 36 40 44 48 52 56 60 64
;; small program: loop((loop(loop(x div y, x, y) + x, x, 1) - 1) + x, x, 1)
;; fast program: loop(loop(loop(if (loop2(x + y, x, x, 1, 2) mod (1 + (x div 2))) <= 0 then 1 else 0, 1, y) + x, y div 2, x), x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (mod (w6 x) (+ 1 (div x 2))) 0) 1 0))))
(assert (= g5 1))
