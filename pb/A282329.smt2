;; sequence(s): A282329
;; terms: 2 ~1 ~6 ~13 ~24 ~37 ~54 ~73 ~96 ~125 ~156 ~193 ~234 ~277 ~324 ~377 ~436 ~497 ~564 ~635
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y) + x, x, x) * (0 - 1)) + 2
;; fast program: loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) * (0 - 1)) + x, x, 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (v0 x) (- 0 1)) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (* (v4 x y) 2) (- 0 1)) x))))
