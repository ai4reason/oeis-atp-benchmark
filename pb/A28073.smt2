;; sequence(s): A28073
;; terms: 1 31 630 10670 163691 2363901 32800480 442568140 5850370581 76144382171 979096198730 12468275928810 157530257684671 1977386553510841 24685911967987380 306761026472700680
;; small program: loop(((loop((loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop((loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (x + x))) + x, y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
