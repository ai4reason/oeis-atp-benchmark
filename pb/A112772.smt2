;; sequence(s): A112772
;; terms: 14 26 38 62 74 86 122 134 146 158 194 206 218 254 278 302 314 326 362 386
;; small program: (compr(loop((loop(x + x, x, 2) - 2) mod (1 + x), 1 - (x mod (1 + 2)), x) - 1, 2 + x) * 2) + 2
;; fast program: ((((compr(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1 + x) * 2) + 1) * (1 + 2)) - 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (- (* (+ (* (v3 x) 2) 1) (+ 1 2)) 2) 2))))
