;; sequence(s): A50991
;; terms: 9 15 21 33 39 51 57 63 69 87 93 111 123 129 141 159 177 183 195 201
;; small program: compr((2 + loop(x + x, x - 1, x)) mod (1 + x), 2 + x) + 1
;; fast program: loop((compr(((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, (x - 1) div 2, 1))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, 1 + x) * 2) + 1, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (mod (- (mod (* (* (+ 1 (mod x 2)) (v4 x)) (v6 x)) (+ 1 (+ x x))) 1) (+ 1 x)) 1))))
