;; sequence(s): A137558
;; terms: 49 69 109 149 229 269 349 389 469 589 629 749 829 869 949 1069 1189 1229 1349 1429
;; small program: ((((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) * (1 + (2 + 2))) + 2) * (2 + 2)) + 1
;; fast program: ((((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1) * (1 + (2 + 2))) + 2) * (2 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (* (+ (v0 x) 1) (+ 1 (+ 2 2))) 2) (+ 2 2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ (* (+ (v2 x) 1) (+ 1 (+ 2 2))) 2) (+ 2 2)) 1))))
