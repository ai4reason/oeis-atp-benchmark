;; sequence(s): A21314
;; terms: 1 24 401 5796 77637 992880 12310537 149304732 1781240813 20982537576 244696801713 2830553706708 32524979660629 371663665151712 4227189205549529 47887970149917324 540656560537946685
;; small program: loop(loop2((2 * (((2 * (x + x)) + x) + y)) - 1, y + y, y, 1, 2) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(((((loop2(1 + (x * y), y, x, 1, 2 + (2 * (2 + 2))) * (1 + (2 + 2))) + 1) div 2) - loop(x + x, x, 1)) div 2, 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (- (div (+ (* (w5 x) (+ 1 (+ 2 2))) 1) 2) (v6 x)) 2))))
(assert (= g4 1))
