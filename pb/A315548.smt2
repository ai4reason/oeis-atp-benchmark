...
(assert (= v0 (u0 g0 h0)))
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (+ (div (+ (+ (* (div (+ (+ (div (+ (* 2 (+ x x)) x) v0) 1) x) (ite (<= x 0) 1 2)) 2) 1) x) (+ 1 2)) x) x) x) x) x))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (div (+ (+ (* (div (+ (+ (div (+ (* 2 (+ x x)) x) (+ 1 (+ 2 (* 2 (+ 2 2))))) 1) x) (ite (<= x 0) 1 2)) 2) 1) x) (+ 1 2)) x) x) x) x) x))))
