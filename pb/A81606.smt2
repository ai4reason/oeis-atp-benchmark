;; sequence(s): A81606
;; terms: 1 3 4 5 7 9 10 11 12 13 14 15 16 17 19 21 22 23 25 27
;; small program: loop(compr(loop2((x * y) + x, y div (1 + 2), x, 1, x) mod 2, y), x, 1)
;; fast program: compr((loop((((loop(((((loop(((loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x div (1 + 2), x) + 1) mod 2) + 1, 2, x) + 1) + x) mod 2) + (x mod 2)) + x, 2, x) + x) mod 2) + x) - 1, 2, x) + 1) - x, x)
(set-logic UFNIA)
