;; sequence(s): A314133
;; terms: 1 5 11 16 21 27 33 39 44 49 55 60 65 71 76 81 87 93 99 104
;; small program: (((loop(compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, 1 + 2, x) mod 2, y), x, 1) + x) + x) + x) + x
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (+ (* (+ (* (+ (+ (div (+ 1 (+ 2 (+ 2 x))) (+ 1 (+ 2 (* 2 (+ 2 2))))) x) x) 2) (ite (<= x 0) 1 x)) 2) 1) (+ 1 2)) x) x))))
