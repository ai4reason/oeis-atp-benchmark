;; sequence(s): A172048
;; terms: 2 14 23 32 38 41 50 53 59 68 74 77 83 86 95 98 104 113 116 122
;; small program: (loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) * (1 + 2)) + 2
;; fast program: ((compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) * (1 + 2)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (v3 x) 2) (+ 1 2)) 2))))
