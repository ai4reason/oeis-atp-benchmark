;; sequence(s): A138964
;; terms: 0 2 28 80 208 1664 28864 137600 1660672 3207680 45816832 245602304 1239224320 1206665216 9653321728 390683230208 2185095872512 554107535360 38286179172352 306289433378816
;; small program: loop(x + x, (x + x) + x, 1) mod loop(2 * ((x + x) + x), x, 1)
;; fast program: loop((x * x) * x, 1, loop(x + x, x - 1, 2)) mod loop2(x * y, y, x div 2, loop(2 + (2 + 2), x mod 2, 1), loop(x * x, 1, 2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (v2 x) (w4 x)))))
