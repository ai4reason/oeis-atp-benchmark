;; sequence(s): A50436
;; terms: 16 21 25 26 28 33 36 38 39 42 48 49 50 52 55 56 57 60 64 68
;; small program: loop(compr(loop(x * y, x, 2) mod (1 + x), 2 + x), 1 + 2, x) + 1
;; fast program: loop(compr((if (loop((1 + ((x div 2) mod 2)) * (1 + x), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x), x) + 1, 1 + 2, 1 + x)
(set-logic UFNIA)
