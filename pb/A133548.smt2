;; sequence(s): A133548
;; terms: 27 152 495 1826 4023 8936 15795 27962 52351 82142 132795 201716 281223 385046 533923 739302 966283 1267046 1624957 2013974
;; small program: loop(loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 1) * (x * x), y, 2, 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) * (x * x), y, 2, 1, x), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (- 2 (v3 x)) (+ 1 x)))))
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ (v2 x y) 1) (* x x)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (* (v7 x y) 2) 1) (* x x)))))
