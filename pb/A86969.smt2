...
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) - x) + 1
;; fast program: (((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) * (if x <= 0 then 2 else 1)) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (* (v2 x) 2) 1) (ite (<= x 0) 2 1)) x))))
