;; sequence(s): A107757
;; terms: 3 9 11 27 29 35 39 81 83 89 93 107 111 117 119 243 245 251 255 269
;; small program: compr(loop(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, 2 + (x + x)) mod (1 + x), x mod 2, x), 1 + x)
;; fast program: compr(((loop2((x * y) + x, y div (1 + 2), 1 + (2 * (2 + 2)), 1, x) mod (1 + 2)) - 1) mod (1 + x), 1 + x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (w4 x) (+ 1 2)) 1) (+ 1 x)))))
