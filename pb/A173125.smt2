;; sequence(s): A173125
;; terms: 1 1 2 3 6 10 20 35 70 127 254 474 948 1807 3614 6995 13990 27370 54740 107883
;; small program: (loop((x - loop2(y - x, x, y, 1, 2)) + x, x, 1) + loop2(x + y, x, x, 1, 0)) div 2
;; fast program: (loop((x - loop2(y - x, x, y, 1, 1)) + x, x - 1, 1) + loop2(x + y, x, x - 1, 1, 1)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (- x (w4 x y)) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v3 x) (w5 x)) 2))))
