;; sequence(s): A161824
;; terms: 0 1 2 3 6 7 8 9 12 13 16 17 18 19 22 23 24 25 26 27
;; small program: (loop((loop(loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), x, 1 + x) + x, x, 1) - 1) - x, 1, x div 2) * 2) + x
;; fast program: ((loop((compr(((loop((((loop(loop((loop(x div loop(1, x mod (2 + 2), 2 + 2), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (2 + 2)) - 2, x) - 1, 1, x div 2) + x) mod 2) + x) - 2, 2, x) + 1) + x) mod 2, 1 + x) - 1) - x, 1, x div 2) * 2) + x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (+ (mod (v8 x) 2) x))))
(assert (forall ((x Int)) (= (g7 x) (- (mod x (+ 2 2)) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (- (+ (mod (+ (v6 x) x) 2) x) 2))))
(assert (= g5 2))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (v3 x) 2) x) 2))))
