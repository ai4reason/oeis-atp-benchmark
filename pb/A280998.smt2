;; sequence(s): A280998
;; terms: 2 4 5 6 8 9 11 12 13 14 16 17 19 21 23 24 25 27 28 29
;; small program: compr((loop(loop(loop((x - 1) mod y, x, 0), 2, x) + x, x, x) mod (1 + x)) mod 2, 1 + x)
;; fast program: compr((((((((loop((loop(loop(2 + (x mod y), x, 1) - 1, 1 + (2 + 2), x) - 1) - x, 2, x) * 2) - x) + 1) + x) div (2 + x)) + 1) + x) mod 2, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (mod (- x 1) y))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (div (+ (+ (- (* (v5 x) 2) x) 1) x) (+ 2 x)) 1) x) 2))))
