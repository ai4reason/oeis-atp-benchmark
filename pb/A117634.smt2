;; sequence(s): A117634
;; terms: 0 2 4 7 10 14 19 24 30 37 44 52 61 71 81 92 104 117 131 145
;; small program: loop(loop((loop(((((y - x) + y) + y) div (x * x)) + x, x, 2) - 1) - x, 2, y) + x, x, x)
;; fast program: loop(loop(loop((loop(((((y - x) + y) + y) div (x * x)) + x, x, 2) - 1) - x, 1, y) + x, x, 0) - x, 2, x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (+ (+ (- y x) y) y) (* x x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (div (+ (+ (- y x) y) y) (* x x)) x))))
