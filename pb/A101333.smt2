;; sequence(s): A101333
;; terms: 0 0 0 0 0 0 0 1 2 4 8 16 32 65 130 260 520 1040 2080 4161
;; small program: loop((loop(((loop(x * y, x, 1) mod (2 + x)) + x) mod 2, 1, y) + x) + x, x, 0)
;; fast program: loop((loop(if loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2, x mod 2, 1 + x) <= 0 then 1 else 0, 1, y) + x) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (v2 x) (+ 2 x)) x) 2))))
(assert (= g1 1))
