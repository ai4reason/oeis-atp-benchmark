;; sequence(s): A82100
;; terms: 5 50 1300 65000 7930000 1348100000 390949000000 141523538000000 75007475140000000 63156294067880000000 60756354893300560000000 83236206203821767200000000 140003298834828212430400000000
;; small program: loop((loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 2, 1, x), 1, y) * x) + x, 1 + x, 1)
;; fast program: loop((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) * x) + x, y, 2, 1, x), 1, y) * x) + x, 1 + x, 1)
(set-logic UFNIA)
