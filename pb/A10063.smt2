;; sequence(s): A10063
;; terms: 1 2 4 6 8 12 14 18 20 24 28 30 32 36 38 42 46 50 56 60
;; small program: loop(2 * (loop2(x - y, y div (1 + 2), x, x, x) + x), x, 1)
;; fast program: loop((loop2(x - y, y div (1 + 2), 2 + (2 + 2), x, x) + x) * 2, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (* 2 (+ (w1 x) x)))))
...
(assert (forall ((x Int)) (= (f2 x) (* (+ (w3 x) x) 2))))
