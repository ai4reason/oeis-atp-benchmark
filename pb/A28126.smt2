;; sequence(s): A28126
;; terms: 1 30 583 9372 135949 1854282 24319171 310765224 3901290217 48373167414 594541806079 7261303210356 88278005955205 1069623213421026 12928092282515707 155970779176282368
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * (x + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x - 1, 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
