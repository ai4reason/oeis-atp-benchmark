;; sequence(s): A7694
;; terms: 1 2 4 6 8 12 16 18 24 32 36 48 54 64 72 96 108 128 144 162
;; small program: loop(compr(loop((x * x) * x, 2, 2 * (2 * (2 + (x mod 2)))) mod (1 + x), y), x, x) + 1
;; fast program: compr(loop(loop((x * x) * x, 2, 2 * (2 + (2 + 2))) mod (1 + x), x mod 2, x) div 2, x) + 1
(set-logic UFNIA)
...
(assert (= g2 2))
(assert (forall ((x Int)) (= (h2 x) (* 2 (* 2 (+ 2 (mod x 2)))))))
