;; sequence(s): A139786
;; terms: 0 1 13 127 1105 1255 24337 263671 725953 42823 40610545 163341463 780593185 5464152295 51309760081 45711664183 2200721587585 12583941205639 3454291215793 430419865184215
;; small program: loop((2 * ((x + x) + x)) + x, x, 1) mod loop(2 * ((x + x) + x), x, 1)
;; fast program: loop2(x * y, y, x div 2, loop(1 + (2 + (2 + 2)), x mod 2, 1), (2 * (2 * (2 * (2 + (2 + 2))))) + 1) mod loop2(x * y, y, x div 2, loop(2 + (2 + 2), x mod 2, 1), loop(x * x, 1, 2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (w2 x) (w4 x)))))
