;; sequence(s): A135158
;; terms: ~1 0 12 90 528 2850 14832 75810 383808 1932930 9705552 48648930 243605088 1219100610 6098716272 30503196450 152544778368 762810181890 3814309582992 19072323542370
;; small program: loop2(((x + x) + x) + y, (2 * (y + y)) + y, x, 0, 2) - loop(x + x, x, 1)
;; fast program: (loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) - loop(x + x, x, 1)) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (w2 x) (v5 x)) (w6 x)))))
