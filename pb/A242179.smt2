...
;; small program: loop2(loop(0 - x, y, x), (y - 1) div 2, x, 1, x)
;; fast program: ((loop(loop2((loop((loop(y, x mod 2, 2) + x) mod (1 + x), 1, y) * x) + x, y div 2, 2 + (2 + 2), 1, x), 1, 1 + x) - 2) mod (1 + 2)) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (mod (- (v2 x) 2) (+ 1 2)) 1))))
