;; sequence(s): A173919
;; terms: 1 2 3 4 5 6 7 10 11 12 13 16 17 18 19 22 23 28 29 30
;; small program: compr(loop((loop(x * y, x, x) - 1) mod ((1 - (x mod 2)) + x), 2, x), 1 + x)
;; fast program: ((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, (1 + x) div 2) * 2) + 1) - (x mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (v2 x) 1) (+ (- 1 (mod x 2)) x)))))
(assert (= g1 2))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (v3 x) 2) 1) (mod x 2)))))
