;; sequence(s): A136417
;; terms: 2 2 3 3 5 6 9 12 17 23 33 46 65 91 129 182 257 363 513 725
;; small program: (loop2(2 * ((x - y) + x), x, x, 1, 0) div loop2((x + x) + y, x, x, 1, 1)) + 1
;; fast program: (loop2(2 * ((x - y) + x), x, x - 1, (if x <= 0 then 0 else 2) + 1, 1) div loop2((x + x) + y, x, x, 1, 0)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (w0 x) (w1 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (w2 x) (w3 x)) 1))))
