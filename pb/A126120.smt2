;; sequence(s): A126120
;; terms: 1 0 1 0 2 0 5 0 14 0 42 0 132 0 429 0 1430 0 4862 0
;; small program: loop((x * y) div (y - (y div 2)), 1 + x, 1 - (x mod 2)) div (1 + x)
;; fast program: (loop(loop(2 * ((x - (x div y)) + x), x div 2, 1) + x, 1 - (x mod 2), x) - x) div (1 + (x div 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (- (v1 x) x) (+ 1 (div x 2))))))
