;; sequence(s): A111215
;; terms: 0 2 3 6 8 9 12 14 17 21 23 24 26 27 33 36 38 42 44 47
;; small program: (compr((loop((loop((x - 2) + x, 2, y) * x) div y, x, 2) - 1) mod (1 + (x + x)), 1 + x) div 2) - 1
;; fast program: ((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 2)) mod (1 + 2)) + 1) mod 2, x) + 1) div 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (div (+ (v3 x) 1) 2) 1))))
