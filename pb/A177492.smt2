;; sequence(s): A177492
;; terms: 36 100 196 225 441 484 676 900 1089 1156 1225 1444 1521 1764 2116 2601 3025 3249 3364 3844
;; small program: loop2((compr(loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, x div 2, 1) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x)
;; fast program: loop2((compr(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, (x - 1) div 2, 1) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (v4 x) (+ 1 x)) 2))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (f6 x) (mod (v7 x) (+ 1 x)))))
