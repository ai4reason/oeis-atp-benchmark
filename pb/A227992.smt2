;; sequence(s): A227992
;; terms: 0 1 2 4 6 8 10 16 18 24 26 32 34 40 42 64 66 72 74 96
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div (2 + 2)) else x, x, x) div 2, y), x, 0)
;; fast program: compr(loop(if (x mod 2) <= 0 then ((x div 2) div 2) else x, x, x) div 2, x) + (1 div 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod x 2) 0) (div x (+ 2 2)) x))))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (mod x 2) 0) (div (div x 2) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (v3 x) (div 1 2)))))
