;; sequence(s): A80344
;; terms: 0 0 0 1 1 1 1 2 3 3 3 3 3 4 5 6 6 6 6 6
;; small program: loop(loop(loop((x + y) div (1 + x), 2 * (x + x), 0) mod 2, 1, y) + x, x, 0)
;; fast program: loop(loop(((loop((loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, x div 2) + 1) + x, 2, x) + 1) + x) mod 2, 1, y) + x, x - 1, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (div (+ x y) (+ 1 x)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (div (v7 x y) x))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (+ (w6 x) 1) x))))
(assert (= g5 2))
