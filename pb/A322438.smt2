...
;; small program: (((((loop((((if ((loop(x + x, x, 1) mod (1 + x)) - 1) <= 0 then 1 else x) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) - 1) * x) + 1) div 2) * x) mod 2
;; fast program: loop((loop((loop(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) - 1, x mod 2, x) mod 2) + 1, x mod 2, x div 2) mod 2) + 1, x mod 2, x) mod 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (+ (mod (+ (ite (<= (- (mod (v1 x) (+ 1 x)) 1) 0) 1 x) x) 2) (mod x 2)))))
(assert (= g0 1))
(assert (forall ((x Int)) (= (f3 x) (div x 2))))
...
(assert (forall ((x Int)) (= (small x) (mod (* (div (+ (* (- (v0 x) 1) x) 1) 2) x) 2))))
...
(assert (forall ((x Int)) (= (f7 x) (+ (mod (+ (ite (<= (- (mod (+ 1 (v8 x)) (+ 1 x)) 2) 0) 1 x) x) 2) (mod x 2)))))
(assert (= g7 1))
