;; sequence(s): A57811
;; terms: 1 3 4 7 8 9 10 13 14 15 16 19 20 21 22 29 30 37 38 39
;; small program: compr(loop(loop(if (loop2(1 + (x + y), x, x, 0, 1) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x - 1, 0) mod 2, 1 + x)
;; fast program: compr(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, (x - 1) div 2, 1) mod (if (x - 1) <= 0 then 1 else 2), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (w3 x) (+ 1 x)) 0) 1 0))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f6 x) (ite (<= (- (mod (+ 1 (v7 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (f4 x) (mod (v5 x) (ite (<= (- x 1) 0) 1 2)))))
