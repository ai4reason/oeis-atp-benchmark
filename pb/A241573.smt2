;; sequence(s): A241573
;; terms: 7 11 35 131 2051 8195 131075 524291 8388611 536870915 2147483651 137438953475 2199023255555 8796093022211 140737488355331 9007199254740995 576460752303423491 2305843009213693955 147573952589676412931 2361183241434822606851
;; small program: ((loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) + 1) + 2
;; fast program: ((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) + 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (w4 x) 2) 1) 2))))
