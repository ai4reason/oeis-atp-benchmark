;; sequence(s): A18602
;; terms: 1 2 3 4 6 12 59 118 177 236 354 708
;; small program: loop(compr(loop(((x * x) + y) + y, 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((((((((((compr(loop(((x * x) + y) + y, 2 + 2, 2) mod (1 + x), x) + 1) + x) - x) + x) - x) + 1) - (x mod 2)) + x) + (x mod 2)) - x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (* x x) y) y))))
(assert (= g4 (+ 2 2)))
(assert (= h4 2))
...
(assert (forall ((x Int)) (= (fast x) (- (- (+ (+ (- (+ (- (+ (- (+ (+ (v3 x) 1) x) x) x) x) 1) (mod x 2)) x) (mod x 2)) x) 1))))
