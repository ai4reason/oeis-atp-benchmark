;; sequence(s): A61725
;; terms: 6 11 27 51 123 171 291 363 531 843 963 1371 1683 1851 2211 2811 3483 3723 4491 5043
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 2, y, 2, 1, x) + 1
;; fast program: (((((loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1, 1, y) * x, y, 2, 1, x) + 1) div 2) * 2) * 2) + 2) div (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (* (div (+ (w3 x) 1) 2) 2) 2) 2) (ite (<= x 0) 1 2)))))
