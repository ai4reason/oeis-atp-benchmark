;; sequence(s): A244738
;; terms: 2 0 0 2 1 0 2 1 0 1 1 2 1 0 2 0 1 1 2 1
;; small program: (((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) * 2) + 2) mod (1 + (2 + 2))) div 2
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(((2 * (x + x)) div (1 + (2 + 2))) + x, 2, 2)) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (mod (+ (* (v0 x) 2) 2) (+ 1 (+ 2 2))) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (div (* 2 (+ x x)) (+ 1 (+ 2 2))) x))))
(assert (= g5 2))
(assert (= h5 2))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (mod (mod (+ (v2 x) 1) v5) (+ 1 2)))))
