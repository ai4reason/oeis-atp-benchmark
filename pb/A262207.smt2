;; sequence(s): A262207
;; terms: 0 1 17 97 1676 21241 214259 5020449 34808102 7233300201 46070142226 7806783217105 165239209697109 1608006723911113 48560388990668468 4867006141797699265 530779430908845468654 18442832496573633213385
;; small program: loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x) + x, y, 1 + x, 1, x) mod loop2(x * y, y, x, 1 + x, 1 + x)
;; fast program: loop2(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y, 1 + x, 1, x) mod loop2(x * y, y, x, 1 + x, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (w0 x) (w3 x)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (w4 x) (w7 x)))))
