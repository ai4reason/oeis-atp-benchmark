;; sequence(s): A21534
;; terms: 1 22 337 4522 57253 705334 8574889 103567234 1246828045 14986093486 179978152081 2160608272186 25932522746677 311221616234278 3734847461630713 44819297962008178 537838346143305949
;; small program: loop2(((loop((loop(1 + (2 * (x + x)), y, 1) + x) + x, y, 1) + x) + x) + x, y - 1, 1 + x, 0, x)
;; fast program: loop(loop((loop((2 * ((x * x) div (1 + 2))) - x, 1, loop(x + x, x, 2)) + 1) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (* 2 (div (* x x) (+ 1 2))) x))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (f4 x) (* (+ (v5 x) 1) (w7 x)))))
(assert (= g4 1))
