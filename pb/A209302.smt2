;; sequence(s): A209302
;; terms: 1 2 3 2 3 4 5 4 3 4 5 6 7 6 5 4 5 6 7 8
;; small program: loop((loop(loop(((y + y) div (x * x)) + x, x + x, 1) mod 2, 1, y) * 2) + x, x, 1) - x
;; fast program: ((loop(loop(loop(((loop(((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2, (x div 2) - 1) + 2) + 1) + x, 2, x) + 1) + x) mod 2, 1, y) + x, x - 1, 1) - x, 2, x) * 2) + 1) - x
(set-logic UFNIA)
...
(assert (= h7 (+ 1 2)))
(assert (= i7 2))
(assert (forall ((x Int)) (= (j7 x) (- (div x 2) 1))))
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ (+ (w7 x) 2) 1) x))))
(assert (= g6 2))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (v3 x) 2) 1) x))))
