;; sequence(s): A57627
;; terms: 0 0 0 1 1 1 1 2 3 3 3 4 4 4 4 5 5 6 6 7
;; small program: loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 0 else 1, 1, y) + x, x, 0)
;; fast program: loop(loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x) <= 0 then 1 else 0, 1, y) + x, x, 0) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (- (v2 x) x) 0) 0 1))))
(assert (= g1 1))
