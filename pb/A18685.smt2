;; sequence(s): A18685
;; terms: 1 3 5 9 15 19 45 57 95 171 285 855
;; small program: loop(compr(loop((1 + (2 + x)) * (1 + y), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + (2 * ((2 - x) * x))) * (1 + x), 1, 2 * (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 (+ 2 x)) (+ 1 y)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* 2 (* (- 2 x) x))) (+ 1 x)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
