;; sequence(s): A338467
;; terms: 1 3 4 7 8 13 12 19 16 25 24 29 32 35 36 41 44 49 48 57
;; small program: (loop(compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x), y) - x, x, 0) + 1) + x
;; fast program: (loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) - x, x - 1, 1) + (x mod 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (v4 x) (mod x 2)) x))))
