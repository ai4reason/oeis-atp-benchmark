;; sequence(s): A138452
;; terms: 28 351 7750 58653 885115 2412306 12066328 23519511 74011861 297399466 443736945 1282837878 2375017660 3160641771 5389555753 11082106126 21090164131 25760073690 45229040703 64049963005
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 1 + 2, 1, x), 1, y) * (x - 1), y, 2, 2, x) div 2
;; fast program: (loop2((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (x * x), y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (w5 x) 2) 1))))
