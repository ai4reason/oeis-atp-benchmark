;; sequence(s): A153235
;; terms: 1 4 6 7 10 11 13 14 16 17 19 21 22 25 26 28 30 31 34 35
;; small program: (compr((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else (2 * (2 + 2))), 1 + x) div 2) div 2
;; fast program: compr((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else (2 * (2 + 2))), 1 + x) div (2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ (+ (ite (<= (- (mod (+ 1 (v1 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) (ite (<= x 0) 1 (* 2 (+ 2 2)))))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) (ite (<= x 0) 1 (* 2 (+ 2 2)))))))
