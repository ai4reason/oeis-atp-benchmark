;; sequence(s): A25950
;; terms: 1 24 387 5306 66969 806148 9425599 108151662 1225448037 13766224472 153719012811 1709234017218 18948174785905 209602732183596 2315012544644823 25540197959076374 281542657422715773
;; small program: loop(((loop((loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + x) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + x) + x) + x, y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) (* (+ (* (+ x x) 2) x) 2)) x))))
