;; sequence(s): A100688
;; terms: 17 80 1214 15308 1948616 20726198 2195382770 22082967872 2165293113020 1990280943581606 19147875284802356 16660504517966902430 1495392851464002242522 14115049597965094337960 1249674274871002654525988 1027312020387041054530226318
;; small program: loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop2(x * y, y, x, 1, 1 + 2), y, 2, 0, x) - 1
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop2(x * y, y, x, 1, 1 + 2), y, 2, 0, x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 1) (w4 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (w5 x) 1))))
