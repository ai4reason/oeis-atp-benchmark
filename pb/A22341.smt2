;; sequence(s): A22341
;; terms: 1 5 9 17 21 33 37 41 65 69 73 81 85 129 133 137 145 149 161 165
;; small program: compr(loop(((loop2((((y div 2) * x) * y) + x, y div 2, x, 1, x) mod 2) + x) mod (1 + x), x mod 2, x), 1 + x)
;; fast program: compr(loop(((loop2((((y div 2) * x) * y) + x, y div 2, 1 + (2 * (2 + 2)), 1, x) mod 2) + x) mod (1 + x), x mod 2, x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (w2 x) 2) x) (+ 1 x)))))
