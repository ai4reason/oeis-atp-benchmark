...
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 2 + x) mod (2 + 2)) - 1
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 2 + x) mod (2 + 2)) div 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (div (mod (v2 x) (+ 2 2)) 1) 1))))
