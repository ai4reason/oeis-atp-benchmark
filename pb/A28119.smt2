;; sequence(s): A28119
;; terms: 1 28 509 7688 105417 1368660 17211253 212371456 2591003393 31398579212 378973353837 4563240432504 54868960089529 659203880450884 7915910524882661 95029322307819632 1140621385518013425
;; small program: loop((loop((loop2(loop((2 + y) * x, 2, x) + (y * y), y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
