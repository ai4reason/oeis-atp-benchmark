;; sequence(s): A28225
;; terms: 1 37 860 16070 263991 3983007 56583850 768771040 10093501781 129000877577 1613435099640 19826334683610 240097655804371 2872253241385747 34007147715912230 399112139110865780
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop(loop(loop((((loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) + loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2))))) div 2) - loop2(x * y, y, x, 1, 1 + (2 * (2 + 2)))) div 2, 1, 2 + x) div 2, 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
