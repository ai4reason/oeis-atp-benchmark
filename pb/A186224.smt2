...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (* (+ y y) y) (+ (+ x 1) y)) y))))
(assert (forall ((x Int)) (= (g0 x) (+ 1 x))))
(assert (= h0 0))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (+ (v0 x) x))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (div (div (+ (+ (div (+ (div (+ (+ (div (+ (div (div (div x 2) 2) 2) x) 2) x) (div x (+ 1 (+ 2 (+ 2 2))))) 2) x) 2) x) x) 2) 2) x) x) 2))))
