;; sequence(s): A153052
;; terms: 2 5 8 10 11 14 15 17 20 22 23 25 26 29 30 32 35 36 38 40
;; small program: loop(compr(loop(x * y, x, x) mod (1 + (2 * (2 + x))), y), x, 2)
;; fast program: (compr(((loop(((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) + x) mod 2, x mod 2, 1 + x) + x) mod 2) mod (1 + x)) - (x mod 2)) + 1, 2, x) + 1) + x) mod 2, x) div 2) - 1
(set-logic UFNIA)
