;; sequence(s): A39712
;; terms: 2 3 5 7 11 13 3 5 9 1 3 9 13 1 5 11 3 5 11 1
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (loop(x * x, 2, 2) - 2)) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(loop(1 + x, 2 + 2, x div 2) + x, 2, 2)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (- v2 2)) 1))))
