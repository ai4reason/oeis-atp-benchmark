;; sequence(s): A6335
;; terms: 1 2 16 192 2816 46592 835584 15876096 315031552 6466437120 136383037440 2941129850880 64614360416256 1442028424527872 32619677465182208 746569714888605696 17262927525017812992 402801642250415636480 9474719710174783733760 224477974671833337692160
;; small program: loop(loop2((2 + y) * x, 2 + y, x, 1, x) div loop((2 + y) * x, x, 1), 1, x + x)
;; fast program: ((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * loop(x * x, 1, loop(x + x, x, 1))) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (w3 x) (v4 x)) (v5 x)) (+ 1 x)))))
