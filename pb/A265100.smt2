;; sequence(s): A265100
;; terms: 5 14 32 41 86 95 113 122 248 257 275 284 329 338 356 365 734 743 761 770
;; small program: (((compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x) mod 2, x + x) div 2) + 1) * (1 + 2)) + 2
;; fast program: ((((((loop(((((loop((((((loop(((((loop((((((loop(((((loop2(x - y, y div 2, x, x, x) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * 2) - 1) * (1 + 2)) div 2) * (1 + 2)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (div (v0 x) 2) 1) (+ 1 2)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (* (- (* (+ (v2 x) x) 2) 1) (+ 1 2)) 2) (+ 1 2)) 2))))
