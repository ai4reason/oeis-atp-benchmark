;; sequence(s): A153081
;; terms: 0 2 3 5 8 9 12 14 15 17 20 23 24 27 29 30 33 35 38 42
;; small program: loop((loop((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 2 + x) div 2) - 2, 1, 2 + x) - 2) - 2, 1, 2 + x)
;; fast program: loop((loop((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 2 + x) - 1) - 2, 1, 2 + x) div 2) - 2, 1, 2 + x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (div (v2 x) 2) 2))))
(assert (= g1 1))
