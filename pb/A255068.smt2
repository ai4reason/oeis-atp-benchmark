;; sequence(s): A255068
;; terms: 2 5 6 10 11 13 14 18 21 22 23 26 27 29 30 34 37 38 42 43
;; small program: compr(1 - (loop(loop((y mod 2) - x, x, x), x, x) mod 2), x) - 1
;; fast program: compr(loop(((((((1 + loop(loop(x div 2, x mod 2, x), x, x)) mod (2 + 2)) - 1) * (if x <= 0 then 1 else 2)) - 1) mod (1 + 2)) - 1, 2, x), x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- 1 (mod (v1 x) 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (- (mod (- (* (- (mod (+ 1 (v5 x)) (+ 2 2)) 1) (ite (<= x 0) 1 2)) 1) (+ 1 2)) 1))))
(assert (= g4 2))
