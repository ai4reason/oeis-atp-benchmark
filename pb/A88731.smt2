;; sequence(s): A88731
;; terms: 1 24 3122 823540 285311670608 302875106592250 827240261886336764174 1978419655660313589123976 20880467999847912034355032910564 2567686153161211134561828214731016126483466
;; small program: (loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop2(x * y, y, x - 1, 1, x), y, 2, 1, x) - 1) - 2
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop2(x * y, y, x - 1, 1, x), y, 2, 1, x) - (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (w4 x) (+ 1 2)))))
