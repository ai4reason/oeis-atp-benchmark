;; sequence(s): A30436
;; terms: 1 1 2 3 6 10 20 34 68 116 232 396 792 1352 2704 4616 9232 15760 31520 53808
;; small program: loop2((x + x) + y, 2 * (x + y), x div 2, 1, x mod 2)
;; fast program: loop2(y, 2 * ((y - x) + y), x div 2, 1, (x mod 2) + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (* 2 (+ (- y x) y)))))
(assert (forall ((x Int)) (= (h1 x) (div x 2))))
(assert (= i1 1))
(assert (forall ((x Int)) (= (j1 x) (+ (mod x 2) 2))))
