;; sequence(s): A31988
;; terms: 0 1 2 4 5 8 13 14 17 26 40 41 44 53 80 121 122 125 134 161
;; small program: (compr(((loop((loop2(x + y, y div (1 + 2), x, 0, x) - x) - x, 2, x) - x) - 2) mod (1 + x), 1 + x) div 2) - 1
;; fast program: compr((loop(if (((loop((loop2(x + y, y div (1 + 2), x, 0, x) - x) - x, 2, x) - x) - 2) mod (1 + x)) <= 0 then 1 else 2, 1, x + x) + 1) mod 2, 1 + x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (- (w6 x) x) x))))
(assert (= g5 2))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (ite (<= (mod (- (- (v5 x) x) 2) (+ 1 x)) 0) 1 2))))
(assert (= g4 1))
