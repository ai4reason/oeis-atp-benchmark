;; sequence(s): A5823
;; terms: 0 2 6 8 18 20 24 26 54 56 60 62 72 74 78 80 162 164 168 170
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x div 2, x), y), x, 0)
;; fast program: (loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * 2
(set-logic UFNIA)
