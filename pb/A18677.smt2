;; sequence(s): A18677
;; terms: 1 2 4 211 422 844
;; small program: loop(loop(loop2((y * y) * (x + y), x, x, 1, 1), 1 - (x mod (2 + 2)), x) + x, x, 1)
;; fast program: loop(loop((2 * ((2 + ((2 + x) * x)) * x)) - 1, 1 - (x mod (2 + 2)), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (* 2 (* (+ 2 (* (+ 2 x) x)) x)) 1))))
