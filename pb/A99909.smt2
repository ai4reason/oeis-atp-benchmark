;; sequence(s): A99909
;; terms: 11 29 68 131 206 305 416 641 869 1113 1478 1721 1976 2441 3071 3539 4023 4688 5111 5691
;; small program: (loop2(loop((compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) * 2) + 1, 1, y) * x, 1 + y, 2, 1, x) div 2) div 2
;; fast program: (loop2(loop((((compr(loop((((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) + x) mod 2) - (x mod 2)) + 1) mod 2, 1, x + x), 1 + x) * 2) + 1) * 2) - 1, 1, y) * x, 1 + y, 2, 1, x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (+ (* (v2 x) 2) 1))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f7 x) (mod (+ (- (mod (+ (ite (<= (- (mod (+ 1 (v8 x)) (+ 1 x)) 2) 0) 1 2) x) 2) (mod x 2)) 1) 2))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (f5 x) (- (* (+ (* (v6 x) 2) 1) 2) 1))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (div (div (w4 x) 2) 2))))
