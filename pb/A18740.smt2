;; sequence(s): A18740
;; terms: 1 2 4 7 8 14 17 28 34 56 68 119 136 238 476 952
;; small program: loop(compr(loop((2 + ((y * y) + x)) * x, 2, 2 + 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((2 + ((y * y) + x)) * x, 2, 2 + 2) mod (1 + x), x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ 2 (+ (* y y) x)) x))))
(assert (= g4 2))
(assert (= h4 (+ 2 2)))
