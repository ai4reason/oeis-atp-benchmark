;; sequence(s): A18683
;; terms: 1 2 3 4 6 12 71 142 213 284 426 852
;; small program: loop(compr(loop(2 * (2 + (x + x)), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((((((1 + (2 + (loop((x * x) + x, 2, 2 + 2) - x))) * (if x <= 0 then 1 else 2)) - x) + 1) - x) - x) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (- (- (+ (- (* (+ 1 (+ 2 (- v4 x))) (ite (<= x 0) 1 2)) x) 1) x) x) (+ 1 x)))))
