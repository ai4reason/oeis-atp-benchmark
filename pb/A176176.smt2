;; sequence(s): A176176
;; terms: 1 2 3 4 5 7 8 11 13 16 17 19 23 28 29 31 32 37 41 43
;; small program: compr(loop(((1 * x) - 1) * x, 1, loop(x + x, x, 1)) mod (1 + x), x) + 1
;; fast program: (compr(loop((x - 1) * x, 1, loop(x + x, x - 1, 2)) mod (1 + x), x) + 1) mod loop(x + x, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (- (* 1 x) 1) x))))
(assert (= g1 1))
