;; sequence(s): A18472
;; terms: 1 2 5 10 47 94 235 470
;; small program: loop(compr((2 + loop((1 + y) * ((x * x) + x), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop((1 + (x + x)) * (x * x), 1, 2 + (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 y) (+ (* x x) x)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (+ x x)) (* x x)))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
