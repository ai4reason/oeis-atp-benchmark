...
;; small program: (loop(loop(loop(((y - 1) div (x * x)) + x, x, 1) mod 2, 1, y) + x, x, 0) * 2) - x
;; fast program: (loop(loop(loop((((((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2, (x div 2) - 1) + 2) + x, 2, x) + 1) + x) div 2) + 1) + x) mod 2, 1, y) + x, x - 1, 1) - x, 2, x) * 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (- y 1) (* x x)) x))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ (+ (div (+ (+ (v6 x) 1) x) 2) 1) x) 2))))
(assert (= g5 1))
