;; sequence(s): A189997
;; terms: 1 5 41 941 45041 5381141 907182041 261527642141 94345513738241 49864774158575141 41906795264466408041 40266416996450293824941 55107620882419848027561041 92623330477259151438437945141 171251267391917835862107238146041
;; small program: loop(loop2(loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, x), y, 2, 1, y) + x, x, 1)
;; fast program: loop2((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) * x) + x, y, 2, 1, x), 1, y) * x) + 1, y - 1, x, 1, x)
(set-logic UFNIA)
