;; sequence(s): A101279
;; terms: 1 1 1 1 2 1 3 1 4 2 5 1 6 3 7 1 8 4 9 2
;; small program: loop2(y, (((x + y) div x) * x) div 2, x, 1, x)
;; fast program: ((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * x) div (1 + x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g0 x y) (div (* (div (+ x y) x) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (v1 x) x) (+ 1 x)) 1))))
