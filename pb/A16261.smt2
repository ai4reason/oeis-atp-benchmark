;; sequence(s): A16261
;; terms: 1 20 291 3730 44681 513240 5730271 62683550 675263061 7188478660 75807419051 793377882570 8251512054241 85374719599280 879483587504631 9026463398652790 92349281698986221 942254646401987100 9591402928728995011 97433737469672066210
;; small program: loop(loop(1 + (2 * ((2 * (x + x)) + x)), y, 1) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((((1 + loop2(x * y, y, x, 1, 2 + (2 * (2 + 2)))) - loop2(1 + (x * y), y, x, 1, 1 + (2 * (2 + 2)))) div (1 + 2)) div (1 + 2), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (div (- (+ 1 (w4 x)) (w5 x)) (+ 1 2)) (+ 1 2)))))
(assert (= g3 1))
