;; sequence(s): A128913
;; terms: 0 2 6 8 15 18 28 32 36 40 55 60 78 84 90 96 119 126 152 160
;; small program: loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) + x, x, 0) * (1 + x)
;; fast program: (loop(loop(if (((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod (1 + x)) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (mod (v3 x) (+ 1 x)) (+ 1 x)))))
