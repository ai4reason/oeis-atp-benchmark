...
;; small program: loop((x div 2) - x, x mod ((2 * loop(x * x, 2, 2)) - 1), 1) * loop(0 - x, x, 1)
;; fast program: loop((x div 2) - x, x mod ((2 * (2 * (2 * (2 + 2)))) - 1), 1) * loop(0 - 1, x mod 2, 1)
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (g0 x) (mod x (- (* 2 v1) 1)))))
(assert (= h0 1))
...
(assert (forall ((x Int)) (= (g3 x) (mod x (- (* 2 (* 2 (* 2 (+ 2 2)))) 1)))))
(assert (= h3 1))
