;; sequence(s): A18779
;; terms: 1 2 3 4 5 6 10 12 15 17 20 30 34 51 60 68 85 102 170 204
;; small program: loop(compr(loop(((x * x) + y) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: compr(loop((loop(((x * x) * x) - x, 1, (2 * (2 + x)) + x) div (2 * (2 + (2 + (2 + 2))))) * x, 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ (* 2 (+ 2 x)) x))))
