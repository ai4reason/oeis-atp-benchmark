;; sequence(s): A2804
;; terms: 1 4 9 19 37 73 143 279 548 1079 2132 4223 8384 16673 33203 66190 132055 263619 526502 1051899
;; small program: (loop(x div 2, x, loop(1 + ((x + x) + x), x, 1)) + loop(x + x, x, 2)) - 2
;; fast program: ((loop2(x * y, y, x div 2, loop(1 + (2 * (2 + 2)), x mod 2, 1 + 2), 1 + (2 * (2 + 2))) div loop(x + x, x, 2)) + loop(x + x, x, 2)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (+ (v0 x) (v2 x)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (div (w3 x) (v5 x)) (v6 x)) 2))))
