;; sequence(s): A24521
;; terms: 1 2 6 11 18 29 42 59 78 101 130 161 198 239 282 329 382 441 502 569
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) mod x) + x, x, x) + 1
;; fast program: (loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if (x - 1) <= 0 then 1 else 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (mod (v1 x y) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (v3 x) (ite (<= (- x 1) 0) 1 2)) x))))
