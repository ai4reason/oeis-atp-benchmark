;; sequence(s): A18703
;; terms: 1 2 3 4 6 8 12 24 37 74 111 148 222 296 444 888
;; small program: loop(compr(loop((2 + ((x * x) div y)) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((1 + (2 * ((2 * (2 + (x + x))) + x))) * (2 + x), 2, 2 + 2) mod (1 + x), x) + 1) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 (div (* x x) y)) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (* 2 (+ (* 2 (+ 2 (+ x x))) x))) (+ 2 x)))))
(assert (= g4 2))
(assert (= h4 (+ 2 2)))
