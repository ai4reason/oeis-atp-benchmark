;; sequence(s): A79632
;; terms: 0 1 2 0 0 1 1 2 0 0 0 1 1 1 2 0 0 0 0 1
;; small program: loop((loop(((1 + (y + y)) div (x * x)) + x, x + x, 1) - 1) - x, x + x, x)
;; fast program: loop(loop((((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, (x - 1) div 2) + 2) + x, 2, x) - x) + 1) mod 2) + x, 2, x) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (div (+ 1 (+ y y)) (* x x)) x))))
...
(assert (= h5 (+ 1 2)))
(assert (= i5 (+ 2 2)))
(assert (forall ((x Int)) (= (j5 x) (div (- x 1) 2))))
...
(assert (forall ((x Int)) (= (f3 x) (+ (mod (+ (- (v4 x) x) 1) 2) x))))
(assert (= g3 2))
