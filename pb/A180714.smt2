;; sequence(s): A180714
;; terms: 0 1 2 1 0 ~1 ~2 ~1 0 1 2 3 4 3 2 1 0 ~1 ~2 ~3
;; small program: (loop(loop(loop((((y - 1) - x) div (x * x)) + x, x, 2) mod 2, 1, y) + x, x, 0) * 2) - x
;; fast program: (loop(loop((loop2(loop(x + y, x, y) div (1 + x), y, 1 + 2, 2 + 2, (x - 1) div 2) + 1) mod 2, 1, y) + x, x, 0) * 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (- (- y 1) x) (* x x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (div (v6 x y) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (w5 x) 1) 2))))
(assert (= g4 1))
