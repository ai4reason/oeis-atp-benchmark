;; sequence(s): A6230
;; terms: 1 13 73 301 1081 3613 11593 36301 111961 342013 1038313 3139501 9467641 28501213 85700233 257493901 773268121 2321377213 6967277353 20908123501
;; small program: (loop2((1 + 2) * (x + y), 2 + (y + y), x, 0, 2) * 2) + 1
;; fast program: (((((((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) - loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2)))) * (1 + 2)) + 1) + 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ (+ (* (- (* (+ 1 2) (w1 x)) (v3 x)) (+ 1 2)) 1) 2) 2) 1))))
