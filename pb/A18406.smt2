;; sequence(s): A18406
;; terms: 1 2 5 7 10 14 25 35 50 70 175 350
;; small program: compr(loop((1 + (x * y)) * (2 + x), 2, 2) mod (1 + x), x) + 1
;; fast program: compr((2 * loop((2 + x) * (x * x), 1, 1 + (2 + 2))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ 1 (* x y)) (+ 2 x)))))
(assert (= g1 2))
(assert (= h1 2))
