;; sequence(s): A18594
;; terms: 1 2 3 5 6 10 15 23 30 46 69 115 138 230 345 690
;; small program: loop(compr(loop(2 + (2 * (2 + ((x * x) + x))), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((loop(1 * (x * x), 1, 2 + 2) - 1) * (1 + x), 2, 2) mod (1 + x), x) + 1) + x) - x) div 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (* (- v5 1) (+ 1 x)))))
(assert (= g4 2))
(assert (= h4 2))
