;; sequence(s): A246331
;; terms: 1 9 25 49 89 113 161 233 345 369 417 489 609 681 825 1041 1369 1393 1441 1513
;; small program: loop(loop(loop2((((y mod 2) * (2 + (x mod 2))) * x) + x, y div 2, x, 1, x), 1, y) + x, x + x, 1)
;; fast program: (loop(((((loop(loop(loop2((((y mod 2) * 2) * x) + x, y div 2, x, 1, x) div (1 + 2), 1, y) + x, x, 1) + x) * 2) - 1) - x) - x, 1, x + x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (* (mod y 2) (+ 2 (mod x 2))) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (* (* (mod y 2) 2) x) x))))
...
(assert (forall ((x Int)) (= (f5 x) (div (w6 x) (+ 1 2)))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (* (+ (v4 x) x) 2) 1) x) x))))
(assert (= g3 1))
