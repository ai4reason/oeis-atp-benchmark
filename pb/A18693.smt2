;; sequence(s): A18693
;; terms: 1 2 3 5 6 10 15 29 30 58 87 145 174 290 435 870
;; small program: loop(compr(loop((x * x) - x, 2, 2 + (2 + 2)) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop(((((1 + (2 * ((2 * (2 * (x + x))) + x))) * (2 + x)) div 2) div (1 + 2)) * x, 1, 2 + (2 + 2)) mod (1 + x), x) + 1) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (div (div (* (+ 1 (* 2 (+ (* 2 (* 2 (+ x x))) x))) (+ 2 x)) 2) (+ 1 2)) x))))
(assert (= g4 1))
(assert (= h4 (+ 2 (+ 2 2))))
