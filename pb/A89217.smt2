;; sequence(s): A89217
;; terms: 5 9 13 21 25 33 45 49 61 69 73 81 85 105 109 129 133 141 153 165
;; small program: (compr(loop((1 + loop(x + x, x, x)) mod (1 + x), (x div 2) mod 2, x), 1 + x) + 1) + 2
;; fast program: ((compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 1 + x) * 2) + 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g1 x) (mod (div x 2) 2))))
