;; sequence(s): A50624
;; terms: 1 2 4 13 42 138 458 1525 5081 16936 56451 188168 627226 2090752 6969172 23230574 77435244 258117480 860391598 2867971991
;; small program: 1 + (loop(2 * ((2 * (x + x)) + x), x, 1) div loop((x + x) + x, 1 + x, 1))
;; fast program: ((loop(x + x, x, 1) * loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), 1 + (2 * (2 * (2 + (2 + 2)))))) div loop2(x * y, y, x div 2, loop(1 + (2 * (2 + 2)), x mod 2, 1 + 2), 1 + (2 * (2 + 2)))) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (div (v0 x) (v1 x))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (v2 x) (w3 x)) (w5 x)) 1))))
