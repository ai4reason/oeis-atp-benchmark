;; sequence(s): A198680
;; terms: 0 15 21 33 39 45 57 63 78 87 93 99 111 117 132 135 150 156 165 171
;; small program: compr(loop2(x + y, y div (1 + 2), x + 1, 0, x) mod (1 + 2), x) * (1 + 2)
;; fast program: ((((loop2(x - y, y div (1 + 2), 2 + 2, 0, x) mod (1 + 2)) + x) + x) + x) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (+ (mod (w2 x) (+ 1 2)) x) x) x) (+ 1 2)))))
