;; sequence(s): A18753
;; terms: 1 3 5 13 15 25 39 65 75 195 325 975
;; small program: loop(compr(loop(((2 * (x + x)) + x) + y, 1 + (2 + 2), 0) mod (1 + x), y), x, x) + 1
;; fast program: loop(compr(loop((1 + (x * x)) * ((x - 1) + x), 1, 2 * (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* x x)) (+ (- x 1) x)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
