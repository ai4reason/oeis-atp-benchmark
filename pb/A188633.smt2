;; sequence(s): A188633
;; terms: 36 60 72 84 100 108 120 132 140 144 156 168 180 196 200 204 216 220 228 240
;; small program: (compr((loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then 1 else 0, 1, loop(loop(x div 2, x mod 2, x), x, x)) mod 2) mod (1 + x), 1 + x) + 1) * (2 + 2)
;; fast program: ((compr(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) mod (1 + x), 1 + x) * 2) + 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 1 (v2 x)) (+ 1 x)) 0) 1 0))))
(assert (= g1 1))
(assert (forall ((x Int)) (= (f4 x) (div x 2))))
...
(assert (forall ((x Int)) (= (f0 x) (mod (mod (v1 x) 2) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f6 x) (+ (mod (+ (ite (<= (- (mod (+ 1 (v7 x)) (+ 1 x)) 2) 0) 1 0) x) 2) (mod x 2)))))
(assert (= g6 1))
(assert (forall ((x Int)) (= (f9 x) (div x 2))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (v6 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v5 x) 2) 2) 2))))
