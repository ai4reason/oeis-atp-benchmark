;; sequence(s): A18668
;; terms: 1 2 7 14 59 118 413 826
;; small program: loop(compr(loop((2 * (((y * x) * x) + x)) + x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((loop(((2 * (2 * (x * x))) - x) * x, 1, 2 + (2 + 2)) - 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* 2 (+ (* (* y x) x) x)) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (- v5 2) (+ 1 x)))))
