;; sequence(s): A262187
;; terms: 1 6 181440 1814811575069725360128000000
;; small program: loop(x * y, loop((x + x) + x, x, 1), 1) div loop(x * y, x, 1)
;; fast program: (loop(x * y, loop((x + x) + x, x, 1) - 1, 1) div loop(x * y, x, 1)) * loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g3 x) (- (v4 x) 1))))
(assert (= h3 1))
...
(assert (forall ((x Int)) (= (fast x) (* (div (v3 x) (v5 x)) (w6 x)))))
