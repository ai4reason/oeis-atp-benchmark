;; sequence(s): A18554
;; terms: 1 2 4 5 10 20 31 62 124 155 310 620
;; small program: loop(compr(loop((2 + x) * ((x * x) - 2), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((loop(((1 + (2 + x)) * (2 + x)) + x, 2, 2) - 2) mod (1 + x), x) + 1) div (1 mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* (+ 2 x) (- (* x x) 2)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (+ (* (+ 1 (+ 2 x)) (+ 2 x)) x))))
(assert (= g4 2))
(assert (= h4 2))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (- v4 2) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v3 x) 1) (mod 1 2)))))
