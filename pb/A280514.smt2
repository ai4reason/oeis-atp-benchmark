;; sequence(s): A280514
;; terms: 1 2 1 3 2 1 5 4 3 2 1 8 7 6 5 4 3 2 1 13
;; small program: loop(loop(loop(((y * y) div (2 + x)) + y, x + 1, 0), y div x, x), x, 1) - x
;; fast program: loop(loop(loop(((y * y) div (1 + x)) + y, 1 + x, 0), y - x, x), 1 + x, 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (div y x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div (* y y) (+ 1 x)) y))))
