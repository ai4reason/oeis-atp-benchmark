...
;; small program: loop(0 - x, loop(x div (2 - (x mod 2)), x, x) div 2, 1)
;; fast program: (loop((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) + 1) mod 2, 1, x - 1) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div x (- 2 (mod x 2))))))
