;; sequence(s): A153184
;; terms: 1 2 4 6 8 9 10 12 14 16 17 18 19 20 22 24 26 28 29 30
;; small program: loop((loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then 1 else 0, 1, (x + x) + x) + 1) + x, x, 1)
;; fast program: compr(((loop(if ((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x)) <= 0 then 1 else 0, 1, (x + x) + x) + 1) div 2) mod (1 + x), x) + 1
(set-logic UFNIA)
