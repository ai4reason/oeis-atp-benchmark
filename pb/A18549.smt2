;; sequence(s): A18549
;; terms: 1 2 5 10 61 122 305 610
;; small program: loop(compr((2 + loop(2 * (((x + y) * x) + x), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((((x * x) * x) - (1 + 2)) * x, 1, 1 + (2 + 2)) mod (1 + x), y), x, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (- (* (* x x) x) (+ 1 2)) x))))
(assert (= g5 1))
(assert (= h5 (+ 1 (+ 2 2))))
