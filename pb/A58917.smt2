;; sequence(s): A58917
;; terms: 1 4 6 10 14 17
;; small program: (loop2((x div y) + x, x, x, 1, x) + x) + x
;; fast program: (((x - loop2(y, 0, x - 2, x, 1)) + x) + x) + (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (- x (w1 x)) x) x) (ite (<= x 0) 1 2)))))
