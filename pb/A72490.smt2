;; sequence(s): A72490
;; terms: 0 0 1 2 2 3 4 5 5 5 6 7 7 8 9 10 10 11 11 12
;; small program: loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0, 1, y) + x, x - 1, 0)
;; fast program: loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x) <= 0 then 1 else 0, 1, y) + x, x - 1, 0)
(set-logic UFNIA)
