;; sequence(s): A285964
;; terms: 1 2 3 5 6 7 8 9 11 12 13 15 16 18 19 20 21 22 23 25
;; small program: (((compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 1 + (x + x)) * (1 + 2)) + 1) div (2 + 2)) - x
;; fast program: ((((compr(((loop((((loop(loop2(x + y, y div 2, 2 * (2 + (2 + 2)), 1, x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * (1 + 2)) div 2) + 1) div 2) - x
(set-logic UFNIA)
