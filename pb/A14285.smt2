;; sequence(s): A14285
;; terms: 2 8 23 51 106 184 303 455 662 952 1293 1737 2270 2872 3577 4425 5428 6526 7799 9219
;; small program: loop(((compr((loop(x + x, x, 2) - 2) mod (1 + x), y) * y) + x) + y, 1 + x, 0)
;; fast program: loop(loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) * (1 + x), 1, y) + x, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ (* (v5 x) 2) 1) (+ 1 x)))))
(assert (= g4 1))
