;; sequence(s): A348774
;; terms: 2 6 12 18 24 32 42 48 60 68 74 84 98 104 110 128 138 150 158 168
;; small program: compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, loop(1 + y, x, x) + x) + 2
;; fast program: loop(compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x), 1, 2 * x) + (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (+ (v3 x) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (v4 x) (ite (<= x 0) 1 2)))))
