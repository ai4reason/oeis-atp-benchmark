;; sequence(s): A4232
;; terms: 3 7 14 23 36 49 66 83 104 129 152 181 210 239 272 309 348 385 428 471
;; small program: (loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 2) + ((2 + x) * x)
;; fast program: ((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) + (if x <= 0 then 1 else ((2 + x) * x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v3 x) 2) 2) (ite (<= x 0) 1 (* (+ 2 x) x))))))
