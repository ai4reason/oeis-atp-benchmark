;; sequence(s): A153219
;; terms: 3 7 8 13 14 18 19 21 23 27 28 30 33 35 38 40 41 42 43 47
;; small program: compr((loop((((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod 2, 1, (x + x) + x) mod 2) mod (1 + x), 1 + x) - 1
;; fast program: compr(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, (x + x) + x) mod (1 + x), 1 + x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (+ (ite (<= (- (mod (+ 1 (v2 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g4 1))
