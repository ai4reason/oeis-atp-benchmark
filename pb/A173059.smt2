;; sequence(s): A173059
;; terms: 0 1 3 6 7 10 12 13 15 18 21 22 25 27 28 31 33 36 40 42
;; small program: loop((loop(((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 2 + x) div 2) - 2) - 2, 1, 2 + x) - 2) - 2, 1, (2 + 1) + x)
;; fast program: (loop((loop((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) - 2) - 2, 1, 2 + x) div 2) - 2, 1, 2 + (2 + x)) - 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (w3 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f1 x) (- (- (div (v2 x) 2) 2) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f7 x) (mod (w8 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f5 x) (- (- (v6 x) 2) 2))))
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (- (div (v5 x) 2) 2))))
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ 2 (+ 2 x)))))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (- (- (v4 x) 2) 2))))
