;; sequence(s): A18652
;; terms: 1 2 3 6 7 14 19 21 38 42 57 114 133 266 399 798
;; small program: loop(compr(loop(((2 + y) * (x * x)) + x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((((compr(loop((2 * (((2 * (2 + 2)) + y) * x)) + x, 2, 2) mod (1 + x), x) + 1) + x) - x) + x) - x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (+ 2 y) (* x x)) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* 2 (* (+ (* 2 (+ 2 2)) y) x)) x))))
(assert (= g4 2))
(assert (= h4 2))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (- (+ (+ (v3 x) 1) x) x) x) x) 1))))
