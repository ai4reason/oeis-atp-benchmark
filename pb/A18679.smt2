;; sequence(s): A18679
;; terms: 1 2 3 6 9 18 47 94 141 282 423 846
;; small program: loop(compr((1 + loop(2 + (2 + (x * x)), 1 + 2, x)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 * (2 * (((x * x) * x) - x))) + x, 1, 2 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (* 2 (* 2 (- (* (* x x) x) x))) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
