;; sequence(s): A191107
;; terms: 1 4 10 13 28 31 37 40 82 85 91 94 109 112 118 121 244 247 253 256
;; small program: compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x), x + x) + 1
;; fast program: ((loop(((((loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (v2 x) x) (+ 1 2)) 2))))
