...
;; small program: loop2(y, y - x, x div 2, x mod 2, 0)
;; fast program: loop((((((1 + ((2 + x) mod (2 + 2))) * x) + 1) mod (1 + 2)) - 1) + x, x mod 2, x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (+ (- (mod (+ (* (+ 1 (mod (+ 2 x) (+ 2 2))) x) 1) (+ 1 2)) 1) x))))
