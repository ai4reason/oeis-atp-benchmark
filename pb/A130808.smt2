;; sequence(s): A130808
;; terms: 17 19 20 21 23
;; small program: (1 + (loop(y, x div 2, x) + x)) + loop(x * x, 2, 2)
;; fast program: (1 + (loop(y, x div 2, x) + x)) + (2 * (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ 1 (+ (v2 x) x)) (* 2 (* 2 (+ 2 2)))))))
