;; sequence(s): A254710
;; terms: 1 2 9 22 119 300 1651 4172 22989 58102 320189 809250 4459651 11271392 62114919 156990232 865149209 2186591850 12049974001 30455295662
;; small program: loop(loop(loop2((2 * (x + x)) - y, x, x - 1, 2, 1) div (1 + (x mod 2)), 1, y) + x, x, 1)
;; fast program: loop(loop(loop2((2 * (x + x)) - y, x, x, 2, 1) div (2 - (x mod 2)), 1, y) + x, x - 1, if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (w2 x) (+ 1 (mod x 2))))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (div (w5 x) (- 2 (mod x 2))))))
(assert (= g4 1))
