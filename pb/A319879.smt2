;; sequence(s): A319879
;; terms: 1 8 13 18 21 26 29 34 37 40 45 48 51 56 59 62 65 70 73 76
;; small program: ((loop(loop((x - (y div (0 - x))) div 2, x, x) + x, 2, x) * 2) + 1) + x
;; fast program: ((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2, (x - 2) - (x div 2)) + 2) + x, 2, x) * 2) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (div (- x (div y (- 0 x))) 2))))
...
(assert (= h3 (+ 1 2)))
(assert (= i3 2))
(assert (forall ((x Int)) (= (j3 x) (- (- x 2) (div x 2)))))
