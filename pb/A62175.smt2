;; sequence(s): A62175
;; terms: 0 0 1 0 1 4 1 0 7 4 1 4 1 4 1 0 1 16 1 4
;; small program: loop(x + x, x + x, 1) mod (1 + x)
;; fast program: loop2(loop(((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + x), 1, y) * x, y, 2, 1, x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (* (+ 1 (mod x 2)) (v3 x)) (+ 1 x)))))
(assert (= g2 1))
