;; sequence(s): A153482
;; terms: 8 73 617 2393 14633 28553 83513 130313 279833 707273 923513 1874153 2825753 3418793 4879673 7890473 12117353 13845833 20151113 25411673
;; small program: loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2 + 2, 1, x) - (2 * (2 + 2))
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 2, 1, x) - (2 * (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (w3 x) (* 2 (+ 2 2))))))
