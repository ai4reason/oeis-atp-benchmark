;; sequence(s): A171508
;; terms: 1 3 5 7 8 10 11 13 14 15 16 18 19 21 22 23 24 26 27 29
;; small program: loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) + x, x - 1, if x <= 0 then 1 else 2) + x
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else 0, 1, y) + x, x - 1, if x <= 0 then 1 else 2) + x
(set-logic UFNIA)
