;; sequence(s): A242313
;; terms: 1 2 4 8 11 16 21 22 31 32 41 42 44 51 61 62 64 71 81 82
;; small program: loop(compr((loop(x div (1 + (x mod 2)), x, x) div 2) mod (1 + (2 + 2)), y), x, 0) + 1
;; fast program: compr((loop(x div (2 - (x mod 2)), x div 2, x) mod (2 + (2 * (2 + 2)))) - 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (div (v2 x) 2) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f3 x) (- (mod (v4 x) (+ 2 (* 2 (+ 2 2)))) 1))))
