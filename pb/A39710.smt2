;; sequence(s): A39710
;; terms: 2 3 5 7 11 1 5 7 11 5 7 1 5 7 11 5 11 1 7 11
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 * (2 + (2 + 2)))) + 1
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(((if ((2 - x) mod (2 + 2)) <= 0 then 1 else 2) + x) + x, 2, 2)) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (* 2 (+ 2 (+ 2 2)))) 1))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (+ (ite (<= (mod (- 2 x) (+ 2 2)) 0) 1 2) x) x))))
(assert (= g5 2))
(assert (= h5 2))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (mod (+ (v2 x) 1) v5) 1))))
