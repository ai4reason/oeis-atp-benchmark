;; sequence(s): A192109
;; terms: 1 2 6 10 14 22 26 30 34 38 46 58 62 74 82 86 94 106 118 122
;; small program: ((compr(loop((loop(x + x, x, 1) - 1) mod (1 + x), x mod 2, x) div 2, x) + 1) * 2) div 2
;; fast program: ((compr(loop(((loop(x * x, 1, loop(x + x, x div 2, 1)) * (1 + (x mod 2))) - 1) mod (1 + x), x mod 2, x) - 2, 1 + x) + 1) div 2) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (div (+ (v3 x) 1) 2) (ite (<= x 0) 1 2)))))
