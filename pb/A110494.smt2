;; sequence(s): A110494
;; terms: 3 5 13 25 61 85 145 181 265 421 481 685 841 925 1105 1405 1741 1861 2245 2521
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) div 2) + 1
;; fast program: ((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (x * x), loop(y, 2, ((2 * (1 + (x + x))) + x) + x), 2, 1, x) div 2) div (1 + (2 + 2))) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (div (w3 x) 2) (+ 1 (+ 2 2))) 1))))
