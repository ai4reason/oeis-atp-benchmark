;; sequence(s): A323739
;; terms: 2 1 1 2 6 30 180 1440 12960 142560 1995840 29937600 538876800 10777536000 226328256000 5205549888000 135344297088000 3924984615552000 117749538466560000 3885734769396480000
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) div 2, x, 2)
;; fast program: loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * x, x - 1, 1) + (if x <= 0 then 1 else 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (v3 x) (ite (<= x 0) 1 0)))))
