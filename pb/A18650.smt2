;; sequence(s): A18650
;; terms: 1 3 5 15 53 159 265 795
;; small program: loop(compr((1 + loop((2 * ((2 * (x * x)) - 2)) + x, 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + (2 * (2 + (x + x)))) * (1 + (2 + x)), 1, 2 * (2 + (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (* 2 (- (* 2 (* x x)) 2)) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* 2 (+ 2 (+ x x)))) (+ 1 (+ 2 x))))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 (+ 2 2)))))
