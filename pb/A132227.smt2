;; sequence(s): A132227
;; terms: 1 4 10 16 28 34 46 52 64 82 88 106 118 124 136 154 172 178 196 208
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) * (1 + 2)) - 2
;; fast program: (((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) * (if x <= 0 then 2 else 1)) - 1) * (1 + 2)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (* (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 1) (+ 1 2)) 2))))
