;; sequence(s): A28157
;; terms: 1 33 697 12033 185353 2657361 36306649 479622561 6184444585 78342273009 979414153081 12123907404609 148962749140297 1819935176767377 22139457629549593 268445978800100577
;; small program: loop((loop(loop(((loop(1 + (x + x), y, 1) + x) + x) + x, x, 1) * loop(x + x, x + x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
;; fast program: loop((loop(((loop2((2 * (x + y)) + x, y + y, x, 1, 2 + 2) div 2) + 1) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (v1 x y) x) (* (* (+ x x) 2) 2)))))
