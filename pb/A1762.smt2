;; sequence(s): A1762
;; terms: 1 1 10 180 4620 152880 6168960 293025600 15990004800 984647664000 67493121696000 5094263446272000 419688934689024000 37465564582397952000 3601861863990534144000 370962724717928318976000 40744403224500159055872000
;; small program: ((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * loop((1 + (2 + y)) * x, x, 1)) div (1 + x)) + 1) div 2
;; fast program: ((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x - 1, if x <= 0 then 1 else 2, 1 + (2 + 2))) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (div (* (div (w0 x) (v1 x)) (v2 x)) (+ 1 x)) 1) 2))))
...
(assert (forall ((x Int)) (= (i5 x) (ite (<= x 0) 1 2))))
(assert (= j5 (+ 1 (+ 2 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (w3 x) (v4 x)) (w5 x)) (+ 1 x)))))
