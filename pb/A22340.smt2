;; sequence(s): A22340
;; terms: 0 2 4 8 10 16 18 20 32 34 36 40 42 64 66 68 72 74 80 82
;; small program: compr(loop2((((y div 2) * y) mod 2) + x, y div 2, x, 0, x div 2), x + x)
;; fast program: (((compr(((loop2((((y div 2) * x) * y) + x, y div 2, 2 + (((2 + 2) * 2) + 2), 1, x) mod 2) + x) mod (1 + x), x) * 2) + 1) div 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (* (div y 2) x) y) x))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (+ (mod (w3 x) 2) x) (+ 1 x)))))
