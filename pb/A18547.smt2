;; sequence(s): A18547
;; terms: 1 2 4 8 16 19 32 38 76 152 304 608
;; small program: loop(compr(loop(2 * (((x + y) * x) + x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((1 + (2 + (2 * (2 * (2 + 2))))) * (1 + x), 1, (2 * (2 * (2 * (2 + 2)))) - 1) mod (1 + x), x) + 1) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (+ 2 (* 2 (* 2 (+ 2 2))))) (+ 1 x)))))
(assert (= g4 1))
