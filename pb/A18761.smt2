;; sequence(s): A18761
;; terms: 1 2 4 13 19 26 38 52 76 247 494 988
;; small program: loop(compr(loop((loop(x + y, 2 + 2, x) + y) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((((compr(loop(1 + (((((x * x) + x) div 2) + x) + x), 1, loop((x * x) + x, 2, 2)) mod (1 + x), x) + 1) + x) - x) + x) - x) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ 1 (+ (+ (div (+ (* x x) x) 2) x) x)))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (* (- (+ (- (+ (+ (v4 x) 1) x) x) x) x) 1))))
