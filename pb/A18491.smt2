;; sequence(s): A18491
;; terms: 1 2 11 22 23 46 253 506
;; small program: loop(compr(((2 * (loop(x * x, 2, 2 + 2) - 2)) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop(((x * x) * x) - x, 1, 2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (- (* 2 (- v2 2)) 2) (+ 1 x)))))
