;; sequence(s): A117152
;; terms: 0 0 1 7 25 75 195 468 1056 2280 4755 9650 19154 37328 71635 135685 254125 471317 866669 1581620
;; small program: loop(loop(((x * x) - x) * loop2(x + y, x, x, 0, 1), 1, y) + x, x, 0) div 2
;; fast program: loop(loop(((x * x) - x) * loop2(x + y, x, x - 2, 1, 1), 1, y) + x, x, 0) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (- (* x x) x) (w2 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (* (- (* x x) x) (w5 x)))))
(assert (= g4 1))
