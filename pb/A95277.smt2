;; sequence(s): A95277
;; terms: 3 6 8 9 12 13 15 18 21 22 23 24 27 28 29 30 33 35 36 38
;; small program: compr(loop(loop(x * y, x - (x div (1 + 2)), 1) mod (1 + (x + x)), x mod 2, x), 1 + x) div 2
;; fast program: compr(loop((if (((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + (x + x)), x mod 2, x), 1 + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (ite (<= (mod (- (mod (* (* 2 (v5 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1) (+ 1 x)) 0) 1 0) (+ 1 (+ x x))))))
