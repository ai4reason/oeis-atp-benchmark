;; sequence(s): A80240
;; terms: 0 1 2 4 5 6 8 9 10 11 12 14 15 16 17 18 19 20 21 22
;; small program: loop(((loop(loop(((if (loop(1 + loop((x * x) + y, 2, 2), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x)) mod 2, 1, x div 2), x mod 2, x) mod 2) + 1) + x, x, 1) - 1
;; fast program: loop(((loop(loop(((if (loop(1 + loop((x * x) * x, 1, 1 + 2), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x)) mod 2, 1, x div 2), x mod 2, x) mod 2) + 1) + x, x, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (mod (ite (<= (mod (v3 x) (+ 1 x)) 0) 1 0) (+ 1 x)) 2))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f7 x) (mod (mod (ite (<= (mod (v8 x) (+ 1 x)) 0) 1 0) (+ 1 x)) 2))))
(assert (= g7 1))
