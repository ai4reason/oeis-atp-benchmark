;; sequence(s): A91311
;; terms: 0 1 4 5 14 15 18 19 46 47 50 51 60 61 64 65 146 147 150 151
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x div 2, x), y), x, 0) - x
;; fast program: (loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) * 2) + x
(set-logic UFNIA)
