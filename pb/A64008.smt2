;; sequence(s): A64008
;; terms: 2 33 555 7777 122221 1444443 18888887 211111109 2555555553 32222222219 344444444441 4111111111107 45555555555551 477777777777773 5222222222222217 58888888888888883 655555555555555549 6777777777777777771 74444444444444444437 788888888888888888881
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) * loop(1 + (2 * ((2 * (x + x)) + x)), x, 1)
;; fast program: (compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + x) + 1) * loop2(1 + (x * y), y, x, 1, 2 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 1) (w6 x)))))
