;; sequence(s): A123939
;; terms: 8 11 11 14 15 16 17 20 22
;; small program: (loop((loop(compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1, 1, 2 + (2 + (2 + x))) - 2) - x, 1, 2 + x) - 2) div 2
;; fast program: (loop((loop(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + x) + 1, 1, 2 + (2 + (2 + x))) - 2) - x, 1, 2 + x) - 2) div 2
(set-logic UFNIA)
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 2 (+ 2 (+ 2 x))))))
