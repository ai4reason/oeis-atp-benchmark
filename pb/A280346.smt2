;; sequence(s): A280346
;; terms: 302231454903657293676544 16423203268260658146231467800709255289 3308722450212110699485634768279851414263248443603515625 827269706064171159838078900184013751038269841857389464208009274449 1692892739326831320764318961708001178036611459414853872137348292520966629744627081
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * ((x * x) * x), y, 1 + 2, 1, x), 1, y) * x, y, 2 + (2 + 2), 1, x)
;; fast program: loop2(loop(loop2(loop(loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 1 + 2, 1, x), 1, y) * x, y, 2, 1, x), 1, y) * (x * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (v3 x y) 1) (* (* x x) x)))))
...
(assert (forall ((x Int)) (= (g12 x) (div x 2))))
(assert (= h12 1))
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (* (+ (v10 x y) 1) (* (* x x) x)))))
