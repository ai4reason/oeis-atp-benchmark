;; sequence(s): A75515
;; terms: 1 45 1260 28350 563031 10333575 179866170 3016747800 49263275061 788796913905 12445575859080 194186867360850 3004103990159091 46168557763591035 705914973500103990 10750288516418083500
;; small program: loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1) * loop((x + x) + x, x, 1)
;; fast program: loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1) * loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
