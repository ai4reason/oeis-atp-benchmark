;; sequence(s): A186266
;; terms: 1 3 18 140 1260 12474 132132 1472328 17065620 204155380 2506399896 31443925968 401783498480 5215458874500 68633685693000 914099013896400 12304253831789700 167193096184907100 2291164651422801000 31637804708163654000
;; small program: ((loop2((x * y) * y, 1 + y, x, 1, 1 + x) div loop(x * y, x, 1)) * (1 + (x + x))) div loop2(x * y, 1 + y, x, 1, 2 + 2)
;; fast program: ((loop(2 * ((x - (x div y)) + x), x, 1) * loop2(x * y, 1 + y, x, 1, 1 + x)) * (1 + (x + x))) div loop2(x * y, 1 + y, x, 1, 2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (* (div (w0 x) (v1 x)) (+ 1 (+ x x))) (w2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (v3 x) (w4 x)) (+ 1 (+ x x))) (w5 x)))))
