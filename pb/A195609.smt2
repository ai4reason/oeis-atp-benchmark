;; sequence(s): A195609
;; terms: 3 4 5 9 15 16 17 23 27 28 29 33 39 43 44 45 51 52 53 57
;; small program: (compr(loop((loop2(x + y, y div 2, x, 1, x) mod 2) + (1 * x), 2, x) mod 2, x) + 2) - 1
;; fast program: compr((loop((((loop(loop2(x + y, y div 2, 2 * (2 + (2 + 2)), 1, x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) mod (if x <= 0 then 1 else 2), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (+ (mod (w2 x) 2) (* 1 x)))))
(assert (= g1 2))
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (v4 x) 1) (ite (<= x 0) 1 2)))))
