;; sequence(s): A6101
;; terms: 1 40 1210 33880 925771 25095280 678468820 18326727760 494894285941 13362799477720 360801469802830 9741692640081640 263026177881648511 7101711092201899360 191746238094034963240 5177148775980218655520 139783020078437440101481
;; small program: loop(((loop(loop(1 + (loop((x * x) + y, 2, 2) * x), y, 1) + loop((x + x) + x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(loop2((((y div 2) + y) * y) + x, (y + y) + y, x, 1, 1 + 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* (+ (div y 2) y) y) x))))
(assert (forall ((x Int) (y Int)) (= (g7 x y) (+ (+ y y) y))))
...
(assert (forall ((x Int)) (= (f6 x) (* (w7 x) (w8 x)))))
(assert (= g6 1))
