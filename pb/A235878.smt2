;; sequence(s): A235878
;; terms: 51 105 231 537 1311 3345 8871 24297 68271 195585 568311 1668057 4930431 14643825 43636551 130319817 389779791 1166980065 3496221591 10479227577
;; small program: ((((2 * loop(x + x, x, 2)) + loop2(x * y, y, x, 1, 1 + 2)) * (1 + 2)) + 2) * (1 + 2)
;; fast program: (((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2))) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) * (1 + 2)) + 2) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ (* (+ (* 2 (v0 x)) (w1 x)) (+ 1 2)) 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (+ (v2 x) (w5 x)) (+ 1 2)) 2) (+ 1 2)))))
