;; sequence(s): A18676
;; terms: 1 2 3 4 5 6 7 8 10 12 14 15 20 21 24 28 30 35 40 42
;; small program: compr(loop(loop((x - y) * x, 2, 2 + (2 + 2)) mod y, x, 1), x)
;; fast program: compr((2 * loop((x * x) + x, 2, 2 + 2)) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int) (y Int)) (= (f1 x y) (mod v2 y))))
