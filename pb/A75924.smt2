;; sequence(s): A75924
;; terms: 1 105 6860 360150 16689351 714717675 29027537770 1135995214200 43285014073301 1617172212901245 59536438207963080 2167526889938878650 78241359077417918851 2805721220626405336815 100098458195602131838790
;; small program: loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1) * loop((2 * ((x + x) + x)) + x, x, 1)
;; fast program: loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1) * loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (div (div (v7 x) 2) (+ 1 2)))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (fast x) (* (v4 x) (w9 x)))))
