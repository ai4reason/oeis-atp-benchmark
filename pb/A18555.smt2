;; sequence(s): A18555
;; terms: 1 3 9 23 27 69 207 621
;; small program: loop(compr((1 + loop((2 + x) * ((x * x) - 2), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(((loop(x * x, 2, 1 + (2 + 2)) - 2) - 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (- (- v5 2) 2) (+ 1 x)))))
