;; sequence(s): A55746
;; terms: 1 1 2 20 2800 16464000 12778698240000 4254956888736153600000 2026001446509988558521630720000000 4690285643617101997210180025102660272128000000000
;; small program: loop(loop(loop(2 * ((x - (x div y)) + x), y, x) div (1 + y), y, x), x, 1)
;; fast program: loop(loop(loop(loop(loop2(x * y, y - 1, x - 1, 2, 1 + (x + x)) div loop(x * y, x, 1), 1, y) * x, x, 1), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (div (v2 x y) (+ 1 y)))))
