;; sequence(s): A18476
;; terms: 1 2 4 7 14 17 28 34 68 119 238 476
;; small program: loop(compr(loop(((2 * (x + y)) + y) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 * (loop((x * x) - x, 1, 2 * (2 * (2 + 2))) - 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* 2 (- v5 2)) (+ 1 x)))))
