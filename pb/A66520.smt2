;; sequence(s): A66520
;; terms: 0 0 1 1 0 0 1 1 1 1 2 2 1 1 1 1 0 0 1 1
;; small program: loop(loop((1 + loop(((y mod 2) + y) * x, x, 1)) mod (1 + x), 2, y) + x, x, 0) - x
;; fast program: loop(loop(if (loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + x, (1 + x) mod 2, x) - x) <= 0 then 1 else (x mod (2 + 2)), 1, y) + x, x, 0) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (mod y 2) y) x))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (+ (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 x) x) 2) x))))
(assert (forall ((x Int)) (= (g5 x) (mod (+ 1 x) 2))))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) x) 0) 1 (mod x (+ 2 2))))))
(assert (= g4 1))
