;; sequence(s): A18590
;; terms: 1 2 11 22 31 62 341 682
;; small program: loop(compr(loop(2 + (2 * (x + x)), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((((2 * (2 + 2)) + x) * x) + (if x <= 0 then 1 else x), 2, 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (* (+ (* 2 (+ 2 2)) x) x) (ite (<= x 0) 1 x)))))
(assert (= g5 2))
(assert (= h5 2))
