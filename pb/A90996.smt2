;; sequence(s): A90996
;; terms: 0 1 1 2 1 1 2 3 1 1 1 1 2 2 3 4 1 1 1 1
;; small program: loop2(loop((if loop((x - 1) mod y, x, 0) <= 0 then x else 0) mod 2, 1, y) + x, y div 2, x, 0, x)
;; fast program: loop2(loop(loop((if loop(loop((x * x) * x, 1, 2 + 2) mod (1 + x), x mod 2, x) <= 0 then 1 else 0) mod (1 + x), x mod 2, x) mod 2, 1, y) + x, y div 2, 1 + (2 * (2 + 2)), 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (ite (<= (v2 x) 0) x 0) 2))))
(assert (= g1 1))
