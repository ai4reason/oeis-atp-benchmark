;; sequence(s): A129074
;; terms: 25 33 49 65 97 113 145 161 193 241 257 305 337 353 385 433 481 497 545 577
;; small program: ((loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 2) * (2 * (2 + 2))) + 1
;; fast program: ((((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) * 2) * 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (v0 x) 2) (* 2 (+ 2 2))) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 2) 2) 2) 1))))
