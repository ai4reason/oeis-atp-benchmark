;; sequence(s): A49653
;; terms: 1 1 1 3 1 1 3 1 1 3 1 3 5 1 1 3 5 1 3 1
;; small program: loop((loop((loop(if ((loop(x + x, x, 1) mod (1 + x)) - 1) <= 0 then 1 else 0, 1, x + x) + 1) mod 2, 1, y) * x) + 2, 1 + x, 0) - 1
;; fast program: loop((loop((loop(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0, 1, x + x) + 1) mod 2, 1, y) * x) + 2, 1 + x, 0) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (- (mod (v3 x) (+ 1 x)) 1) 0) 1 0))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f6 x) (ite (<= (- (mod (+ 1 (v7 x)) (+ 1 x)) 2) 0) 1 0))))
(assert (= g6 1))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* (v5 x y) x) 2))))
