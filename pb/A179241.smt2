;; sequence(s): A179241
;; terms: 1 1 1 2 2 3 4 5 6 8 10 12 15 19 24 29 36 45 56 69
;; small program: (loop(x + x, x, 2) - 1) div loop2(x + y, x, x, 1, 2)
;; fast program: ((2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) - 1) div loop2(x + y, x, x, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (- (v0 x) 1) (w1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (* 2 (v2 x)) 1) (w5 x)))))
