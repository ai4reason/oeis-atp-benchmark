;; sequence(s): A191106
;; terms: 1 3 7 9 19 21 25 27 55 57 61 63 73 75 79 81 163 165 169 171
;; small program: compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, (x div 2) div 2, x) mod 2, x) + 1
;; fast program: ((loop(((((loop((((((loop(((((loop((((((loop(((((loop((x + y) div 2, x, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (div (+ x y) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (v2 x) x) 2) 1))))
