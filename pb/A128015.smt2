;; sequence(s): A128015
;; terms: 1 1 3 3 10 10 35 35 126 126 462 462 1716 1716 6435 6435 24310 24310 92378 92378
;; small program: loop(2 * ((x - (x div (1 + y))) + x), x div 2, 2) div 2
;; fast program: loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1)) * (1 + (x + x)), 1, x div 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (div (w2 x) (v3 x)) (+ 1 (+ x x))))))
(assert (= g1 1))
