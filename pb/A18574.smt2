;; sequence(s): A18574
;; terms: 1 2 3 6 109 218 327 654
;; small program: loop(compr(loop((2 * ((2 + x) * x)) + (x div 2), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(((1 + 2) * (2 + loop((x * x) * x, 1, 2 + (2 + 2)))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* (+ 1 2) (+ 2 v5)) (+ 1 x)))))
