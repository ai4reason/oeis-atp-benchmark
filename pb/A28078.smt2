;; sequence(s): A28078
;; terms: 1 28 511 7762 107065 1397536 17652307 218558494 2673170269 32451457324 392152259863 4725695939146 56851786048513 683251595827192 8206387016186779 98529159613684918 1182723165477092197 14195324589704967940 170362624423844562655
;; small program: loop((loop(loop2(((2 * (y * y)) - y) + x, y + y, x, 1, 2) * loop((x + x) + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (div (div (v6 x) 2) (+ 1 2)) (w8 x)))))
(assert (= g5 1))
(assert (forall ((x Int) (y Int)) (= (h5 x y) y)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int) (y Int)) (= (v5 x y) (u5 g5 (h5 x y)))))
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) x) (* (+ (+ x x) x) 2)))))
