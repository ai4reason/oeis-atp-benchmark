;; sequence(s): A138032
;; terms: 131072 129140163 762939453125 232630513987207 505447028499293771 8650415919381337933 827240261886336764177 5480386857784802185939 141050039560662968926103 7257147736730073114838109 22550116774162743178682911 456487940826035155404146917
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 1 + loop(x * x, 2, 2), 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x * x, 2, ((x * x) * x) * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (= g8 2))
(assert (forall ((x Int)) (= (h8 x) (* (* (* x x) x) x))))
