;; sequence(s): A153486
;; terms: ~448 217 15113 117137 1771049 4826297 24137057 47045369 148035377 594822809 887503169 2565725897 4750103729 6321362537 10779214817 22164360617 42180533129 51520373849 90458381657 128100283409
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + (2 + 2), 1, x) - loop((x * x) * x, 2, 2)
;; fast program: ((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, y, 2 + (2 + 2), 1, x) - 1) - loop((x * x) * x, 2, 2)) + 1
(set-logic UFNIA)
...
(assert (= v8 (u8 g8 h8)))
(assert (forall ((x Int)) (= (fast x) (+ (- (- (w4 x) 1) v8) 1))))
