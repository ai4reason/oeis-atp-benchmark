;; sequence(s): A18709
;; terms: 1 3 13 23 39 69 299 897
;; small program: loop(compr((loop(2 + ((x * y) * y), 2 + 2, x) + x) mod (1 + x), y), x, 0) + 1
;; fast program: ((loop(compr(loop((((1 + x) * loop(x * x, 1, 2 + (2 * (2 + 2)))) - 1) * (1 + x), 1, 2) mod (1 + x), y), x, x) + 1) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 2 (* (* x y) y)))))
(assert (= g2 (+ 2 2)))
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (* (- (* (+ 1 x) v6) 1) (+ 1 x)))))
(assert (= g5 1))
(assert (= h5 2))
