;; sequence(s): A20979
;; terms: 1 29 563 9145 134211 1845249 24249163 308352185 3824765171 46535545969 557596038363 6599196683625 77316285612931 898280118486689 10363489117309163 118857642802045465 1356308170915657491
;; small program: loop((loop2((2 * ((2 * (x + x)) + x)) + ((y * y) * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop((loop2(x * y, y, 1 + x, 1, 2 + (2 * (2 + 2))) - loop((x * x) * x, 1, loop(x + x, x, 2))) div 2, 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- (w1 x y) x) (v2 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (div (- (w5 x) (v6 x)) 2))))
(assert (= g4 1))
(assert (forall ((x Int) (y Int)) (= (h4 x y) y)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int) (y Int)) (= (v4 x y) (u4 g4 (h4 x y)))))
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (v4 x y) x) (* (+ (* (* x 2) 2) x) 2)))))
