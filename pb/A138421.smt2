;; sequence(s): A138421
;; terms: 2 12 100 392 2420 4732 13872 21660 46552 117740 153760 312132 470680 569492 812912 1314612 2018980 2307020 3357772 4234440
;; small program: ((loop2((loop(loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) div (1 + 2)) div 2) + 1
;; fast program: (loop2((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) + 1) div (2 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ 2 (v4 x)) (+ 2 x)))))
...
(assert (forall ((x Int)) (= (small x) (+ (div (div (w0 x) (+ 1 2)) 2) 1))))
...
(assert (forall ((x Int)) (= (g9 x) (mod (+ 1 x) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ (v8 x y) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (- (* (v6 x y) x) 1))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (w5 x) 1) (+ 2 (+ 2 2))))))
