;; sequence(s): A21021
;; terms: 1 33 727 13365 221431 3428733 50631967 721942485 10021257511 136192514733 1819621847407 23973890545605 312209398691191 4026262617877533 51492399583946047 653858524870924725
;; small program: loop(loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(((loop2(x * y, y, x, 1, 2 * (2 + (2 + 2))) + loop2(x * y, y, x, 1, 2 + (2 * (2 + 2)))) div 2) - loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2)))), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (div (+ (w6 x) (w7 x)) 2) (w8 x)))))
(assert (= g5 1))
