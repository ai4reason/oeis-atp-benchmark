;; sequence(s): A245245
;; terms: 1 3 495 4686825 2254848913647 52588547141148893628 58152371703925106867047535565 3012179439602547459232394950891834843500 7255167425905233148164780983569428433097979870294255 808718755397067598640202627155266231883064669446721506930287016061
;; small program: loop(loop2((x * y) + x, 1 + y, x, 1, x + x) div loop(x * y, x, 1), 1, x * x)
;; fast program: loop((loop2(x * y, 1 + y, x - 1, 1, 1 + (x + x)) div loop(x * y, x - 1, 1)) * (if x <= 0 then 1 else (1 + 2)), 1, x * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (div (w4 x) (v5 x)) (ite (<= x 0) 1 (+ 1 2))))))
(assert (= g3 1))
