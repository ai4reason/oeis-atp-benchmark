;; sequence(s): A18509
;; terms: 1 2 3 4 5 6 9 10 12 15 18 20 27 30 36 45 54 60 90 108
;; small program: compr(loop((1 + 2) * ((2 + (2 + y)) * x), 2, 2) mod (1 + x), x) + 1
;; fast program: compr((2 * (2 * loop((2 + (x * x)) * x, 1, 1 + (2 + 2)))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ 1 2) (* (+ 2 (+ 2 y)) x)))))
(assert (= g1 2))
(assert (= h1 2))
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (* 2 (* 2 v3)) (+ 1 x)))))
