;; sequence(s): A3527
;; terms: 1 3 5 15 17 51 85 255 257 771 1285 3855 4369 13107 21845 65535
;; small program: loop(compr((loop(x * x, 2 + 2, 2) + x) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(((loop(x * x, 2 + 2, 2 + 2) - 2) - x) mod (1 + x), x) + 1) div 2) * 2) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (- (- v4 2) x) (+ 1 x)))))
