;; sequence(s): A81601
;; terms: 0 3 9 12 27 30 36 39 81 84 90 93 108 111 117 120 243 246 252 255
;; small program: compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x), x + x)
;; fast program: (loop((((((loop(((((loop((((((loop(((((loop(x * x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v2 x) x) (+ 1 2)))))
