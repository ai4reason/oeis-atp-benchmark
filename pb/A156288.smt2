;; sequence(s): A156288
;; terms: 5 8 10 11 14 15 17 20 23 24 25 26 29 30 31 32 35 37 38 40
;; small program: (compr(loop(loop(x * y, x - (x div (1 + 2)), 1) mod (1 + (x + x)), x mod 2, x), 1 + x) div 2) + 2
;; fast program: (compr(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 2 else 1, x mod 2, x) - 1, 1 + x) div 2) + 2
(set-logic UFNIA)
