;; sequence(s): A7521-A294919
;; terms: 5 13 29 37 53 61 101 109 149 157 173 181 197 229 269 277 293 317 349 373
;; small program: (compr((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x)), 1 + x) * 2) + 1
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) * 2) * 2) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ 1 (v1 x)) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (mod (+ 1 (v5 x)) (+ 1 (+ x x))) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (* (v3 x) 2) 2) 1) 1))))
