;; sequence(s): A50435
;; terms: 9 12 15 16 18 21 24 25 26 28 32 33 34 36 38 39 40 42 45 48
;; small program: loop(compr(loop(x * y, x, 2) mod (1 + x), 2 + x), 2, x) + 1
;; fast program: loop(compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x), 1 + x), 2, x) + 1
(set-logic UFNIA)
