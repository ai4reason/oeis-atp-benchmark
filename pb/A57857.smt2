;; sequence(s): A57857
;; terms: 1 2 4 11 52 485 5766 92167 1658888 36495369 1021870090 30656102411 1103619686412 44144787456013 1854081073152014 85287729364992015 4434961926979584016 257227791764815872017 15433667505888952320018
;; small program: loop(compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x, x, 1) + x
;; fast program: (loop(compr(((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2, y) * x, x - 1, 1) * loop(x + x, x - 1, 1)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (v3 x) (v6 x)) x))))
