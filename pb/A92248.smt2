...
;; small program: (loop(loop(1 + (2 * (x * y)), y, 1) + x, x, 0) div (1 + x)) mod 2
;; fast program: ((loop(loop(loop2(1 - (x * y), 2 + y, x, 1, 2), 1, y) + x, x, 1) - 1) div (1 + x)) mod 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (div (- (v2 x) 1) (+ 1 x)) 2))))
