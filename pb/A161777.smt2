;; sequence(s): A161777
;; terms: 0 6 15 28 36 45 66 91 105 120 153 190 210 231 276 300 325 351 378 435
;; small program: loop2((compr((loop(x * y, x, x) mod (1 + x)) mod 2, y) * x) + 1, y, 2, 1, x) div 2
;; fast program: (loop2((loop(compr((if (loop((1 + ((x div 2) mod 2)) * (1 + x), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x), x) + 1, 1, y) * x) - 1, y, 2, 1, x) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 1 (mod (div x 2) 2)) (+ 1 x)))))
(assert (forall ((x Int)) (= (g6 x) x)))
(assert (= h6 1))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 (g6 x) h6))))
(assert (forall ((x Int)) (= (f5 x) (mod (ite (<= (mod (v6 x) (+ 1 x)) 0) 1 0) (+ 1 x)))))
