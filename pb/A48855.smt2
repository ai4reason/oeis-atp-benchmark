;; sequence(s): A48855
;; terms: 1 1 1 2 8 32 192 1152 9216 82944 829440 8294400 99532800 1194393600 16721510400 250822656000 4013162496000 64210599936000 1155790798848000 20804234379264000
;; small program: loop((loop(((loop(x * y, x, x) mod (1 + x)) + 1) mod 2, 1, y) + y) * x, x - 1, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then ((1 + x) - 1) else (1 + x), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) 2) 0) (- (+ 1 x) 1) (+ 1 x)))))
(assert (= g4 1))
