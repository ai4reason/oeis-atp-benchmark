;; sequence(s): A162886
;; terms: 24 42 54 60 78 84 96 114 132 138 144 150 168 174 180 186 204 216 222 234
;; small program: (((compr(loop(loop2(x * y, y - 2, x div 2, 1, x) mod (1 + (x + x)), x mod 2, x), 1 + x) div 2) * 2) + 2) * (1 + 2)
;; fast program: (compr(loop((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0) mod (1 + x), x mod 2, x), 1 + x) + 1) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (w2 x) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (small x) (* (+ (* (div (v0 x) 2) 2) 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 1) (+ 1 2)))))
