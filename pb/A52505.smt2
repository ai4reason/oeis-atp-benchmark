;; sequence(s): A52505
;; terms: 1 3 300 141120 182952000 505008504000 2547446533632000 21222189199411200000 271682221693022300160000 5064076705822143609600000000 131801391770668241689267200000000 4632178742550388306775251353600000000
;; small program: ((loop(x * y, (x + x) + x, 1) div loop(2 * ((x + x) + x), x, 1)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + (x + x))
;; fast program: ((loop(x * y, (x + x) + x, 1) div loop2(x * y, y, x, 1, 2 + (2 + 2))) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + (x + x))
(set-logic UFNIA)
...
(assert (= i7 1))
(assert (forall ((x Int)) (= (j7 x) (+ 1 (+ x x)))))
