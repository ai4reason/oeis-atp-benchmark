;; sequence(s): A28084
;; terms: 1 29 544 8414 116935 1521023 18936478 228705968 2702760709 31433459057 361216744252 4113147855362 46506852459523 522961531303331 5855208956119066 65331927878971796 726978513100416577
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(((loop2((2 * (x + y)) + x, y + y, x, 1, 2 + 2) div 2) + 1) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (v2 x y) (- (v4 x) x)))))
...
(assert (forall ((x Int)) (= (f6 x) (* (+ (div (w7 x) 2) 1) (w8 x)))))
(assert (= g6 1))
