;; sequence(s): A14687
;; terms: 4 4 8 10 14 16 20 22 30 30 38 40 44 46 54 58 62 66 72 72
;; small program: compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) + loop(0, x mod 2, 2)
;; fast program: ((compr((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, 1 + x) - (x mod 2)) * 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (- (v3 x) (mod x 2)) 2) 2))))
