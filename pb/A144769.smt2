;; sequence(s): A144769
;; terms: 0 1 1 2 3 4 5 6 7 9 10 12 13 14 15 17 19 20 22 23
;; small program: compr(loop2(2 + (x + y), x, x, 0, 2) mod (1 + x), 2 + x) div (1 + 2)
;; fast program: (((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + ((2 + x) * x), 2 + 2, 2)) div (1 + 2)) div 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (div (mod (+ (v2 x) 1) v5) (+ 1 2)) 1))))
