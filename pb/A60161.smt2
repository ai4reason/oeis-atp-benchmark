;; sequence(s): A60161
;; terms: 1 5 9 19 37 73 143 281 553 1091 2157 4273 8479 16849 33521 66755 133045 265337 529455 1056937
;; small program: (loop(x + x, x, 2) - 1) + loop2(x + y, x, x, 0, 2)
;; fast program: ((2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) - 1) + loop2(x + y, x, x - 2, if x <= 0 then 0 else 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (i5 x) (ite (<= x 0) 0 2))))
(assert (= j5 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (* 2 (v2 x)) 1) (w5 x)))))
