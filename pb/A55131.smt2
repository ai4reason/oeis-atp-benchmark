;; sequence(s): A55131
;; terms: 15 39 51 87 111 123 159 183 219 267 291 303 327 339 411 447 471 519 543 579
;; small program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod (2 + 2)), x) - 1, 2 + x) + 1) * (1 + 2)
;; fast program: (((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 2)) mod (1 + 2)) - 1) mod (1 + x), 1 + x) * 2) * (1 + 2)) + 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (- (mod (+ (v4 x) x) 2) (mod x 2)) (+ 1 2)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (* (v3 x) 2) (+ 1 2)) 1) 2))))
