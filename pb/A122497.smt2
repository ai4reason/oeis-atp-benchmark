;; sequence(s): A122497
;; terms: 1 2 1 1 2 2 1 2 1 1 2 1 2 2 1 1 2 2 1 2
;; small program: loop2(loop(1 - x, y, x), (y - 1) div 2, x, 0, x) + 1
;; fast program: loop2((loop((loop(y, x mod 2, 2) + x) mod (1 + x), 1, y) * x) + x, y div 2, 2 + (2 + 2), 1, 1 + x) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (w2 x) (+ 1 2)))))
