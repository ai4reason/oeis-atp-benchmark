;; sequence(s): A86937
;; terms: 0 0 1 0 2 0 0 2 0 2 2 0 2 0 0 0 2 2 0 2
;; small program: loop2((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + y) * x) + 2, y, 2, 1, x) mod (1 + (2 + 2))
;; fast program: (loop(((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (1 + (2 + 2))) - 1, 2, x) + 1) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (w0 x) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f4 x) (- (mod (+ (v5 x) 1) (+ 1 (+ 2 2))) 1))))
(assert (= g4 2))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (mod (+ (v4 x) 1) (+ 1 2)))))
