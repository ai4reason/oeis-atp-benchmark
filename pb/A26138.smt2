;; sequence(s): A26138-A26170-A26174
;; terms: 1 2 4 5 8 10 11 13 14 17 20 22 23 26 28 29 31 32 35 37
;; small program: loop(compr((loop(x div (if (x mod (1 + 2)) <= 0 then (1 + 2) else 1), x, x) mod (1 + 2)) - 1, y), x, 0) + 1
;; fast program: compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x, 0), x) mod (1 + 2)) div 2, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (mod (v4 x) (+ 1 2)) 2))))
