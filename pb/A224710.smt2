;; sequence(s): A224710
;; terms: 0 0 0 0 0 0 1 1 1 2 2 2 3 3 4 5 5 5 6 7
;; small program: loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 0 else 1, 1, y) + x, x - 2, 0)
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 0 else 1, 1, y) + x, x - 2, 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (- (mod (+ 1 (v2 x)) (+ 1 (+ x x))) 2) 0) 0 1))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 0 1))))
(assert (= g4 1))
