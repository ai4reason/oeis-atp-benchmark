;; sequence(s): A121924
;; terms: 0 1 1 3 4 4 7 9 10 10 14 17 19 20 20 25 29 32 34 35
;; small program: 1 - loop(loop(loop(x - (if x <= 0 then 0 else (1 + y)), x, x), 1, y) + x, x, 1)
;; fast program: loop(loop(1 - loop(x - (if x <= 0 then 0 else (1 + y)), x - (x div 2), x), 1, y) + x, x, 0) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (- x (ite (<= x 0) 0 (+ 1 y))))))
