;; sequence(s): A18745
;; terms: 1 2 13 26 37 74 481 962
;; small program: loop(compr(loop(2 + (loop(x + y, x, 2) * x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(((1 + 2) * ((2 * (x * x)) - x)) - x, 1, 1 + (2 * (2 + (2 + 2)))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ 2 (* (v3 x) x)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f6 x) (- (* (+ 1 2) (- (* 2 (* x x)) x)) x))))
(assert (= g6 1))
(assert (= h6 (+ 1 (* 2 (+ 2 (+ 2 2))))))
