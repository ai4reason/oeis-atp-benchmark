;; sequence(s): A28042
;; terms: 1 26 447 6484 86429 1100862 13670539 167326808 2031126537 24536941858 295591283351 3555147513372 42718200467125 513013681404614 6158932166842083 73926566183932576 887254472530712993
;; small program: loop(((loop((loop2(loop((2 + y) * x, 2, x) + (y * y), y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + x) + x) + x, y, 1) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (div (w7 x) 2) (v8 x)))))
(assert (= g6 1))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) (* (+ (+ x x) x) 2)) x))))
