;; sequence(s): A81765
;; terms: 1 7 13 19 31 37 49 55 61 67 85 91 109 121 127 139 157 175 181 193
;; small program: compr((loop(x * x, x mod 2, loop(x + x, x div 2, 1)) - 1) mod (2 + x), 1 + x)
;; fast program: compr(loop(loop(x * x, 1, loop(x + x, x div 2, 1)) mod (loop(x + x, x mod 2, 1) + x), x mod 2, x) - 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (v5 x) (+ (v7 x) x)))))
