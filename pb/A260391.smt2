;; sequence(s): A260391
;; terms: 2 4 6 8 10 14 16 18 20 22 24 26 28 30 32 34 38 40 42 44
;; small program: compr(loop(x div loop(1, x mod (2 * (2 + (2 + 2))), 2 + 2), x, x) mod 2, 1 + x)
;; fast program: (((((compr((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + (2 * (2 + 2)), x), 2, x) + 1) mod 2, x) * (1 + 2)) div 2) div 2) div 2) + 1) * 2
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (div (div (div (* (v3 x) (+ 1 2)) 2) 2) 2) 1) 2))))
