;; sequence(s): A285953
;; terms: 3 5 8 12 14 18 21 23 26 30 33 35 39 41 44 48 50 54 57 59
;; small program: (compr(loop2(loop(0 - x, y, x), y div 2, x, 1, x), x) + 2) + x
;; fast program: ((((((loop2((y - x) div 2, x, 2 + 2, x div 2, 0) + 1) + x) mod 2) + 2) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (mod (+ (+ (w3 x) 1) x) 2) 2) x) x) x))))
