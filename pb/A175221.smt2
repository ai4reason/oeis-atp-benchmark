;; sequence(s): A175221
;; terms: 6 7 9 11 15 17 21 23 27 33 35 41 45 47 51 57 63 65 71 75
;; small program: ((compr((1 + loop(x + x, x, x)) mod (1 + x), x) + loop(1, x, 2)) + 2) + 2
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) + (if x <= 0 then 2 else 1)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (* (v3 x) 2) 2) (ite (<= x 0) 2 1)) 2))))
