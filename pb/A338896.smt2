;; sequence(s): A338896
;; terms: 0 2 0 4 4 0 6 8 6 0 8 12 12 8 0 10 16 18 16 10
;; small program: (loop(loop(loop(loop((if x <= 0 then 0 else (1 + y)) - x, x, x), 1, 1 + (x + x)) - x, 1, y) + x, x, 0) - x) * 2
;; fast program: (loop(loop(loop2(loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + (2 + 2), x), 1, y) * x, y, 2, 1, x), 1, y) + x, x, 0) mod (2 + x)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (mod (v4 x) (+ 2 x)) 2))))
