;; sequence(s): A18448
;; terms: 1 2 3 6 71 142 213 426
;; small program: loop(compr(loop(2 + (2 * (x + x)), 2 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop(((2 + (2 + (2 + ((x div (1 + (2 * (2 + 2)))) + x)))) * (x * x)) - x, 1, 2 + (2 + 2)) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (* (+ 2 (+ 2 (+ 2 (+ (div x (+ 1 (* 2 (+ 2 2)))) x)))) (* x x)) x))))
(assert (= g4 1))
(assert (= h4 (+ 2 (+ 2 2))))
