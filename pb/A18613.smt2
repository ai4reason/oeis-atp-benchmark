;; sequence(s): A18613
;; terms: 1 2 3 6 11 22 33 66 121 242 363 726
;; small program: loop(compr(loop((y - x) * y, 2 + (2 + 2), 1) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((2 * ((x * x) * (2 + (2 * (2 + 2))))) + x, 1, 2 + (2 + 2)) mod (1 + x), x) + 1) * 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ (* 2 (* (* x x) (+ 2 (* 2 (+ 2 2))))) x))))
(assert (= g4 1))
(assert (= h4 (+ 2 (+ 2 2))))
