;; sequence(s): A18525
;; terms: 1 2 4 11 13 22 26 44 52 143 286 572
;; small program: loop(compr(loop(2 * (2 - (2 * (x * x))), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((((compr(loop(((2 * ((2 * (x * x)) - x)) - x) * (1 + (((x * x) + x) div 2)), 1, 2 + 2) mod (1 + x), x) + 1) + x) - x) + x) - x) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (- (* 2 (- (* 2 (* x x)) x)) x) (+ 1 (div (+ (* x x) x) 2))))))
(assert (= g4 1))
(assert (= h4 (+ 2 2)))
...
(assert (forall ((x Int)) (= (fast x) (* (- (+ (- (+ (+ (v3 x) 1) x) x) x) x) 1))))
