;; sequence(s): A25528
;; terms: 0 1 2 3 4 4 5 6 7 7 8 8 9 9 9 10 11 11 12 12
;; small program: loop(loop(if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then 1 else 0, 1, y) + x, x, 0)
;; fast program: loop(loop(if (loop((1 + (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) mod (1 + x), 2 - (x mod 2), x) - 1) <= 0 then 1 else 0, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g6 x) (- 2 (mod x 2)))))
