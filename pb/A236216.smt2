;; sequence(s): A236216
;; terms: 1024 60073 9825698 292300947 26229725548 164088217397 2180082117846 8311148375647 49737659589296 470444892889497 1290073179870298 6098657552288147 19521316862440548 41132799175724797 93731931411554846 268619401777067895 779736155077709296
;; small program: loop(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 2 + (2 * (2 + 2)), 1, x), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop((x * x) * x, 1, (x * x) * x), y, 2, 1, x - 1), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (= g11 1))
(assert (forall ((x Int)) (= (h11 x) (* (* x x) x))))
