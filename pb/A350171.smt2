;; sequence(s): A350171
;; terms: 3 3 6 7 12 13 18 19 24 29 32 37 42 43 48 53 60 61 68 71
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) - (x mod 2)) + 2
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + ((2 + x) * x), 2 + 2, 2)) + (((1 + x) - 2) mod 2)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (v2 x) 1) v5) (mod (- (+ 1 x) 2) 2)))))
