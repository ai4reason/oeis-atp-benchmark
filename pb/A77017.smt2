;; sequence(s): A77017
;; terms: 2 2 6 10 14 22 26 34 38 46 58 62 74 82 86 94 106 118 122 134
;; small program: loop2(compr((1 + loop(x + x, x, x)) mod (1 + x), y) + x, 0, x, 1, x - 1) * 2
;; fast program: compr(loop((loop((loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + (x mod 2), 1, x + x) + 1) mod 2, 1, x div 2) mod 2) + 1, x mod 2, x) div 2, if x <= 0 then 1 else x) * 2
(set-logic UFNIA)
