;; sequence(s): A213199
;; terms: 3 5 6 7 9 10 11 12 13 14 15 17 18 19 21 22 23 25 26 27
;; small program: loop(((loop((if (x mod 2) <= 0 then 2 else x) div 2, x, x - 2) mod 2) + 1) + x, x, 1) + 2
;; fast program: loop(((loop(loop(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) mod 2, 1, x div 2) + 1, x mod 2, x) mod 2) + 1) + x, x, 1) + 2
(set-logic UFNIA)
