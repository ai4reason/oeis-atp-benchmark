;; sequence(s): A18381
;; terms: 1 3 9 11 27 33 99 297
;; small program: loop(compr((1 + loop(loop(x + y, x, 1) * x, 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((1 + x) * (2 + (x * x)), 1, 2 * (2 + 2)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 x) (+ 2 (* x x))))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod v5 (+ 1 (+ x x))))))
