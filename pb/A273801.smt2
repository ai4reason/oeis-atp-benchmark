;; sequence(s): A273801
;; terms: 16 24 32 48 56 72 80 96 120 128 152 168 176 192 216 240 248 272 288 296
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) + 2) * (2 + 2)
;; fast program: (((((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 1 + x) * 2) + 2) * 2) div 2) * 2) * 2
(set-logic UFNIA)
