;; sequence(s): A80100
;; terms: 1 1 2 1 4 2 2 1 8 4 4 2 4 2 2 1 16 8 8 4
;; small program: loop2((loop((loop(1, x, x) + x) mod 2, 1, y) * x) + x, y div 2, x, 1, x)
;; fast program: loop2((loop((loop(2 + (((y * y) div x) + (x div 2)), 2 - x, 1) + x) mod 2, 1, y) * x) + x, y div 2, 2 + (2 + 2), 1, x)
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ 2 (+ (div (* y y) x) (div x 2))))))
