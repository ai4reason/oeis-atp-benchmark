;; sequence(s): A160189
;; terms: 0 0 0 1 2 8 17 36 66 115 202 340 569 944 1550 2531 4122 6704 10879 17640
;; small program: ((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) - loop2(x + y, x, x, 2, 1)) * (1 - 2)
;; fast program: (((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) - loop2(x + y, x, x, 1, 1)) - loop2(x + y, x, x, 1, 0)) * (1 - 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- (+ (v0 x) 1) (w2 x)) (- 1 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (- (v5 x) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (- (+ (v3 x) 1) (w7 x)) (w8 x)) (- 1 2)))))
