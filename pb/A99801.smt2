;; sequence(s): A99801
;; terms: 0 2 3 4 4 5 6 6 7 8 8 9 9 9 10 11 11 11 12 12
;; small program: loop(loop(loop(x * y, x, x) mod (1 + x), x, y + y) + x, x, 0)
;; fast program: ((loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x, 2) - 1) * (if x <= 0 then 1 else 2)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (- (v3 x) 1) (ite (<= x 0) 1 2)) 2))))
