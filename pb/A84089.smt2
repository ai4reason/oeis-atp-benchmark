;; sequence(s): A84089
;; terms: 1 4 7 13 16 19 25 28 31 37 43 49 52 55 61 64 67 73 76 79
;; small program: loop(compr(((loop(x div (1 + (x mod 2)), x, x) div 2) - x) mod (1 + 2), y), x, 0) + 1
;; fast program: (compr(((((((((((loop(loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + 2, x), 2, x) - 1, x mod 2, x) - x) + 1) mod 2) + x) mod (1 + x)) + 1) mod (1 + 2)) - 1) mod (1 + 2)) - 1) mod (1 + x), 1 + x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div x (+ 1 (mod x 2))))))
(assert (forall ((x Int)) (= (g2 x) x)))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 (g2 x) (h2 x)))))
(assert (forall ((x Int)) (= (f1 x) (mod (- (div (v2 x) 2) x) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f6 x) (div (ite (<= (mod x 2) 0) 2 x) 2))))
(assert (= g6 (+ 1 2)))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (- (mod (+ (mod (+ (mod (+ (- (v4 x) x) 1) 2) x) (+ 1 x)) 1) (+ 1 2)) 1) (+ 1 2)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (v3 x) 2) 1))))
