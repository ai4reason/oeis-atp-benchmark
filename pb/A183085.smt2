;; sequence(s): A183085
;; terms: 1152921504606846976 42391158275216203514294433201 867361737988403547205962240695953369140625 508021860739623365322188197652216501772434524836001
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + 2, 1, x), 1, y) * (x * x), y, 2 + 2, 1, x)
;; fast program: loop2(loop(loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop((x * x) * x, 1, (x * x) * x), y, 2, 1, x), 1, y) * x, y, 2, 1, x), 1, y) * (x * x), y, 2, 1, x)
(set-logic UFNIA)
