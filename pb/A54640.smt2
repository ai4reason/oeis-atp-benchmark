;; sequence(s): A54640
;; terms: 1 3 12 72 576 6912 96768 1741824 34836480 836075520 25082265600 802632499200 30500034969600 1281001468723200 56364064623820800 2705475101943398400 146095655504943513600 8765739330296610816000 543475838478389870592000 36956357016530511200256000
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) + 2) * x, x, 1)
;; fast program: loop((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + 2) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 2) x))))
