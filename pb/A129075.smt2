;; sequence(s): A129075
;; terms: 10 13 19 25 37 43 55 61 73 91 97 115 127 133 145 163 181 187 205 217
;; small program: ((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 2) * (1 + 2)) + 1
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) * (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (v0 x) 2) (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) (+ 1 2)) 1))))
