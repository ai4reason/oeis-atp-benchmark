;; sequence(s): A18550
;; terms: 1 2 3 4 6 9 12 17 18 34 36 51 68 102 153 204 306 612
;; small program: compr(loop((2 + (x * y)) * (2 + x), 2, 2) mod (1 + x), x) + 1
;; fast program: compr((2 * loop((x * x) + x, 1, 1 + (2 * (2 * (2 + 2))))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ 2 (* x y)) (+ 2 x)))))
(assert (= g1 2))
(assert (= h1 2))
