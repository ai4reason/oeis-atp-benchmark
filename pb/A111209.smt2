;; sequence(s): A111209
;; terms: 0 1 3 9 21 51 111 237 489 995 2017 4059 8151 16341 32721 65483 131013 262083 524221 1048505
;; small program: ((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) - loop(x + x, x, 2)) * (1 - 2)
;; fast program: ((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1) - loop(x + x, x, 2)) * (1 - 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- (+ (v0 x) 1) (v2 x)) (- 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (+ (v3 x) 1) (v6 x)) (- 1 2)))))
