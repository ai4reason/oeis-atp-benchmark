;; sequence(s): A5599
;; terms: 0 1 2 3 4 5 6 7 6 7 8 9 10 11 12 11 12 13 14 15
;; small program: loop((loop(loop(loop(loop2(x + y, y div 2, x, 0, x) mod 2, 1, (x + x) + x) mod 2, 1, y) + x, x - 1, 0) * 2) - x, 2, x)
;; fast program: loop((loop(loop(loop(loop2(x + y, y div 2, 1 + (2 * (2 + 2)), 0, x) mod 2, 1, (x + x) + x) mod 2, 1, y) + x, x - 1, 0) * 2) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (w4 x) 2))))
(assert (= g3 1))
...
(assert (forall ((x Int) (y Int)) (= (g9 x y) (div y 2))))
(assert (= h9 (+ 1 (* 2 (+ 2 2)))))
(assert (= i9 0))
...
(assert (forall ((x Int)) (= (f8 x) (mod (w9 x) 2))))
(assert (= g8 1))
(assert (forall ((x Int)) (= (h8 x) (+ (+ x x) x))))
