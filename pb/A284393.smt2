;; sequence(s): A284393
;; terms: 1 2 5 6 7 10 13 16 17 18 21 22 23 26 27 28 31 34 37 38
;; small program: (loop(compr(loop(if (x mod 2) <= 0 then ((x div 2) - x) else x, x, x), y), x, 0) + 1) - x
;; fast program: (compr(loop(x div loop(1, x mod (2 + 2), 2 + 2), loop(x * x, 2, 2) - 1, x) mod 2, x) - x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod x 2) 0) (- (div x 2) x) x))))
