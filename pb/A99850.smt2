;; sequence(s): A99850
;; terms: 0 1 3 6 7 8 11 14 19 28 37 38 40 41 43 48 56 63 73 84
;; small program: loop(loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, x) * 2) + 1) mod (1 + x), 1, y) + x, x, 0)
;; fast program: loop(loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) mod (1 + x), 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (* (v2 x) 2) 1) (+ 1 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ (* (v6 x) 2) 1) (+ 1 x)))))
(assert (= g5 1))
