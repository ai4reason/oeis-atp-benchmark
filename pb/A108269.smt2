;; sequence(s): A108269
;; terms: 4 12 16 20 28 36 44 48 52 60 64 68 76 80 84 92 100 108 112 116
;; small program: compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x - 1, x), 1 + x) + 1
;; fast program: (compr((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + (2 * (2 + 2)), x), 2, x) + 1) mod 2, x) * 2) + 2
(set-logic UFNIA)
