;; sequence(s): A94974
;; terms: 1 1 2 4 7 12 21 35 59 99 165 275 459 765 1276 2126 3544 5907 9846 16410
;; small program: loop(x div (1 + 2), x, loop((2 * (x + x)) + x, x, 1))
;; fast program: loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) div loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (w2 x) (w5 x)))))
