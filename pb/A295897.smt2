;; sequence(s): A295897
;; terms: 0 1 3 6 7 12 13 15 24 25 27 30 31 48 49 51 54 55 60 61
;; small program: loop(compr(loop(if ((x mod (1 + 2)) - 1) <= 0 then (x div 2) else 2, x, x), y), x, 0)
;; fast program: (((compr(loop(((loop2((((y div 2) * x) * y) + x, y div 2, 1 + (2 * (2 + 2)), 1, x) mod 2) + x) mod (1 + x), x mod 2, x) - 1, 1 + x) div 2) * (1 + 2)) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (- (mod x (+ 1 2)) 1) 0) (div x 2) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (* (div (v3 x) 2) (+ 1 2)) 2) 2))))
