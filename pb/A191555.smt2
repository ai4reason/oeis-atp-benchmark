;; sequence(s): A191555
;; terms: 1 2 12 720 3628800 144850083840000 272760108249915378892800000000 1264767303092594444142256488682840323816161280000000000000000
;; small program: loop(((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x) * x, x, 1)
;; fast program: loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * (x * x), x - 1, if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (* (v4 x y) 2) 1) (* x x)))))
