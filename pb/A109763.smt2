;; sequence(s): A109763
;; terms: 2 2 3 3 5 5 7 7 11 11 13 13 17 17 19 19 23 23 29 29
;; small program: compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + (x div 2)) + 1
;; fast program: (loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1, 1, x div 2) * 2) div (if (x - 1) <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (v2 x) 2) (ite (<= (- x 1) 0) 1 2)))))
