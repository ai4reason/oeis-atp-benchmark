;; sequence(s): A122263
;; terms: 2 3 8 21 38 63 92 129 178 231 296 369 446 531 628 737 850 975 1108 1245
;; small program: (loop((compr((1 + loop(x + x, x, x)) mod (2 + x), y) - 2) + x, x, 1) * 2) - x
;; fast program: (((((loop((((((((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 1) + x) * 2) - 1) - x) - x) - x) - 1) - x, 1, 1 + x) * 2) + 1) - x) - x) + 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (- (- (- (* (+ (v4 x) x) 2) 1) x) x) x) 1) x))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (+ 1 x))))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (- (+ (- (- (+ (* (v3 x) 2) 1) x) x) 1) x))))
