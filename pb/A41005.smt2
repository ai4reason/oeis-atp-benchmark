;; sequence(s): A41005
;; terms: 1 16 159 1260 8722 55152 326811 1844084 10015566 52754624 270976342 1362986520 6734927460 32775704608 157408497171 747269225028 3511471892470 16351481223840 75525932249922 346305571781224
;; small program: loop(loop((((loop((((loop(loop(2 * (((x div y) + x) + x), x, 2) div (2 + x), 1, y) + x) + x) + x) + x, y, 1) + x) + x) + x) + x, y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop(loop((((loop((((loop(loop2(x * y, 1 + y, x, 1, (1 + 2) + x) div loop(x * y, x, 1), 1, y) + x) + x) + x) + x, y, 1) + x) + x) + x) + x, y, 1) + (2 * (x + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* 2 (+ (+ (div x y) x) x)))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (= h4 2))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) h4))))
(assert (forall ((x Int)) (= (f3 x) (div (v4 x) (+ 2 x)))))
(assert (= g3 1))
...
(assert (= i9 1))
(assert (forall ((x Int)) (= (j9 x) (+ (+ 1 2) x))))
...
(assert (forall ((x Int)) (= (w9 x) (u9 (h9 x) i9 (j9 x)))))
...
(assert (forall ((x Int)) (= (f8 x) (div (w9 x) (v10 x)))))
(assert (= g8 1))
