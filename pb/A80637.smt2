;; sequence(s): A80637
;; terms: 2 3 5 6 7 9 11 12 13 14 15 17 19 21 23 24 25 26 27 28
;; small program: loop(loop(loop(y, (x - 1) div 2, x), x, y) + x, x, 2)
;; fast program: compr((((loop(2 + loop((if (x + x) <= 0 then (x div 2) else 2) - x, x, x), ((2 + x) mod (2 + 2)) - 1, x) - x) + 1) + x) mod 2, 1 + x) div 2
(set-logic UFNIA)
