;; sequence(s): A168606
;; terms: 1 4 20 102 496 2294 10200 44062 186416 776934 3203080 13101422 53279136 215749174 870919160 3507493182 14101520656 56620923014 227128606440 910449955342
;; small program: loop(((loop((loop(2 + (2 * (x + x)), y, 1) + x) + x, y - 1, 1) + x) + x) + x, x, 1)
;; fast program: loop(((loop(((((((1 + (2 + 2)) * loop(x * x, 1, loop(x + x, x, 1))) div (1 + 2)) - loop(x + x, x, 1)) div 2) + 1) - loop(x + x, x, 1), 1, y) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (+ (div (- (div (* (+ 1 (+ 2 2)) (v5 x)) (+ 1 2)) (v7 x)) 2) 1) (v8 x)))))
(assert (= g4 1))
