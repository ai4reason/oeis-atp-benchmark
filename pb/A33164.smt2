;; sequence(s): A33164
;; terms: 4 5 7 8 13 14 16 17 31 32 34 35 40 41 43 44 85 86 88 89
;; small program: (loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x), y), x, 0) + 2) + 2
;; fast program: loop(((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x
(set-logic UFNIA)
