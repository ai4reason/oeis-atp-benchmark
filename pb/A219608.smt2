;; sequence(s): A219608
;; terms: 1 3 7 9 15 19 25 31 33 39 51 57 63 67 73 79 97 103 115 121
;; small program: (loop(compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) div (1 + (x mod 2)), x, x), y), x, 0) * 2) + 1
;; fast program: (compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) div (1 + (x mod 2)), 1 + (x div 2), x) div 2, x) * 2) + 1
(set-logic UFNIA)
(declare-fun f4 () Int)
...
(assert (forall ((x Int)) (= (f6 x) (div (v7 x) (+ 1 (mod x 2))))))
(assert (forall ((x Int)) (= (g6 x) (+ 1 (div x 2)))))
