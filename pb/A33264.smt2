;; sequence(s): A33264
;; terms: 0 1 0 1 1 1 0 1 1 2 1 1 1 1 0 1 1 2 1 2
;; small program: ((loop(loop((x - 1) mod y, x, x) - 1, x, x) + x) div 2) + (x mod 2)
;; fast program: (loop2(((y - x) mod 2) + x, y div 2, 1 + (2 + (2 + 2)), 1, x + 1) - 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (mod (- y x) 2) x))))
