;; sequence(s): A18386
;; terms: 1 2 5 10 31 62 155 310
;; small program: loop(compr((2 - loop(2 * ((x * x) + x), 2, 2)) mod (1 + x), y), x, x) + 1
;; fast program: compr(loop((1 + x) * (((x * x) * x) - 2), 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 x) (- (* (* x x) x) 2)))))
(assert (= g4 1))
(assert (= h4 (+ 2 2)))
