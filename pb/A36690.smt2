;; sequence(s): A36690
;; terms: 6 12 30 56 132 182 306 380 552 870 992 1406 1722 1892 2256 2862 3540 3782 4556 5112
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 1) * (1 + x), y, 2, 0, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (1 + x), y, 2, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 1) (+ 1 x)))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (+ 1 x)))))
