;; sequence(s): A26232
;; terms: 3 6 7 9 12 15 16 18 19 21 24 25 27 30 33 34 36 39 42 43
;; small program: loop(compr(loop2((((y div x) mod (1 + 2)) + 1) * x, y, x, 1, x) mod 2, y), x, 1) + 2
;; fast program: compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x - 1, 1), x) mod (1 + 2)) mod 2, 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (mod (div y x) (+ 1 2)) 1) x))))
