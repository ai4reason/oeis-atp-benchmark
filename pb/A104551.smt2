;; sequence(s): A104551
;; terms: 0 1 1 ~1 ~1 5 5 ~15 ~15 55 55 ~197 ~197 727 727 ~2705 ~2705 10165 10165 ~38455
;; small program: loop(loop(loop2(2 * (x * y), y - 2, x, 1, x) div loop(x * y, x, 1), 1, y) + x, x, 0) div 2
;; fast program: loop(loop((loop2(x * y, (y - 1) - 1, x - 1, 1, x - 2) * loop(x + x, x - 1, x)) div loop(x * y, x, 1), 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g6 x y) (- (- y 1) 1))))
(assert (forall ((x Int)) (= (h6 x) (- x 1))))
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (- x 2))))
...
(assert (forall ((x Int)) (= (f5 x) (div (* (w6 x) (v7 x)) (v8 x)))))
(assert (= g5 1))
