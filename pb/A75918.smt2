;; sequence(s): A75918
;; terms: 1 90 5040 226800 9008496 330674400 11511434880 386143718400 12611398415616 403864019919360 12744269679697920 397694704355020800 12304809943691636736 378212825199337758720 11565710925825703772160
;; small program: loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1) * loop(2 * ((x + x) + x), x, 1)
;; fast program: loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1) * loop2(x * y, y, x, 1, 2 + (2 + 2))
(set-logic UFNIA)
