;; sequence(s): A189817
;; terms: 1 4 5 6 7 8 9 10 13 16 19 22 25 28 31 32 33 34 35 36
;; small program: loop(compr(loop(if (1 - (x mod (1 + 2))) <= 0 then (x div (1 + 2)) else 1, x + x, x), y), x, 0) + 1
;; fast program: loop(compr((loop(if (1 - (x mod (1 + 2))) <= 0 then (x div (1 + 2)) else 1, x, x) + x) mod 2, y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (- 1 (mod x (+ 1 2))) 0) (div x (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (- 1 (mod x (+ 1 2))) 0) (div x (+ 1 2)) 1))))
