;; sequence(s): A329469
;; terms: 1 2 3 4 5 6 8 9 10 12 15 18 20 24 25 30 36 40 45 50
;; small program: loop(compr(loop(loop((2 + (2 + y)) * x, 2, x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((((compr((((1 + loop(1 + (2 + 2), x mod 2, 2)) * (1 + 2)) * loop(x * x, 1, 2 + (2 * (2 + 2)))) mod (1 + x), x) + 1) + x) - x) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ 2 (+ 2 y)) x))))
(assert (= g3 2))
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f4 x) (mod (* (* (+ 1 (v5 x)) (+ 1 2)) v6) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (- (+ (+ (v4 x) 1) x) x) x) x))))
