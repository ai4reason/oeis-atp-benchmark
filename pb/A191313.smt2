;; sequence(s): A191313
;; terms: 0 0 2 5 15 30 71 134 296 551 1188 2211 4720 8815 18722 35105 74307 139842 295223 557366
;; small program: loop(loop((loop(2 * ((x - (x div y)) + x), x div 2, 1 - (x mod 2)) + loop(x + x, x, 1)) div 2, 1, y) + x, x, 0) - x
;; fast program: ((((loop(loop((((loop(loop(2 * ((x - (x div y)) + x), x div 2, 1) + x, 1 - (x mod 2), x) - x) + 1) + loop(x + x, x, 1)) div 2, 1, y) + x, x, 1) * 2) - 1) - x) - x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h2 x) (- 1 (mod x 2)))))
...
(assert (forall ((x Int)) (= (f1 x) (div (+ (v2 x) (v3 x)) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (+ (- (v6 x) x) 1) (v8 x)) 2))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (div (- (- (- (* (v4 x) 2) 1) x) x) 2))))
