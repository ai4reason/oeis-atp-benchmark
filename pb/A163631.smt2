;; sequence(s): A163631
;; terms: 1 10 25 46 71 98 131 166 205 250 299 350 405 462 525 590 659 734 811 892
;; small program: loop((compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y) * 2) + x, x, 1) + x
;; fast program: (((loop(loop(compr((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0) mod (1 + x), x), 1, y) + x, x, 1) + x) * 2) - 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 (+ x x))) 2) 0) 1 0) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (- (* (+ (v3 x) x) 2) 1) x))))
