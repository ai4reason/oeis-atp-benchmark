;; sequence(s): A330410
;; terms: 11 17 29 41 65 77 101 113 137 173 185 221 245 257 281 317 353 365 401 425
;; small program: (((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) * 2) + 1) * (1 + 2)) + 2
;; fast program: (((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * 2) * (1 + 2)) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (* (v0 x) 2) 1) (+ 1 2)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) 2) (+ 1 2)) 1))))
