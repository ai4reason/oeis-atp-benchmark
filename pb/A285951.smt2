;; sequence(s): A285951
;; terms: 2 6 9 11 15 17 20 24 27 29 32 36 38 42 45 47 51 53 56 60
;; small program: (compr(loop2(loop(1 - x, y, x), y div 2, x, 0, x), x) + 2) + x
;; fast program: (((((loop2((y - x) div 2, x, 2 + 2, x div 2, 0) + x) mod 2) + 2) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (mod (+ (w3 x) x) 2) 2) x) x) x))))
