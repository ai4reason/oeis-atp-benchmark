;; sequence(s): A72538
;; terms: 2 3 5 11 13 17 19 23 29
;; small program: loop(compr((1 + loop(x + x, x, x)) mod (2 + x), y) + (2 mod y), x, 2)
;; fast program: compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, (if (x - 2) <= 0 then 1 else 2) + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (v1 x y) (mod 2 y)))))
...
(assert (forall ((x Int)) (= (g3 x) (+ (ite (<= (- x 2) 0) 1 2) x))))
