;; sequence(s): A262744
;; terms: 0 2 4 7 13 20 2 5 10 19 28 41 56 71 88 109 134 159 188 9
;; small program: ((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) mod (1 + loop(x + y, x, x))
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) mod (((1 + x) * (2 + x)) div 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (* (v0 x) 2) x) (+ 1 (v3 x))))))
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (* (v4 x) 2) x) (div (* (+ 1 x) (+ 2 x)) 2)))))
