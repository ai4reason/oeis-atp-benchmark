;; sequence(s): A285959
;; terms: 2 6 10 15 18 22 26 29 34 38 42 47 50 55 59 63 66 70 74 79
;; small program: loop(loop(if (x mod 2) <= 0 then (x div 2) else 2, y + y, x) + x, x + x, 2) + x
;; fast program: (((compr(((loop((((loop(loop2(x + y, y div 2, 2 * (2 + (2 + 2)), 1, x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else (if x <= 0 then 1 else 2)), 1 + x) + 1) - x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (+ (+ (v3 x) 1) x) (ite (<= x 0) 1 (ite (<= x 0) 1 2))))))
