;; sequence(s): A278603
;; terms: 0 1 2 1 2 3 2 1 2 3 4 5 4 3 4 5 6 7 6 5
;; small program: (loop(loop(loop(loop(if ((2 - loop(x + x, x, 2)) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x - 2, 1) mod 2, 1, y) + x, x, 0) * 2) - x
;; fast program: ((loop(loop(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 0, 1, y) + x, x - 1, 1) mod 2, 1, y) + x, x - 1, if x <= 0 then 1 else 2) * 2) - x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= (mod (- 2 (v4 x)) (+ 1 x)) 0) 1 0))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (f8 x) (ite (<= (- (v9 x) 2) 0) 1 0))))
(assert (= g8 1))
