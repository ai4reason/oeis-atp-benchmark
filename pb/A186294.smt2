;; sequence(s): A186294
;; terms: 9 21 37 45 49 57 69 97 117 121 129 141 157 169 177 201 205 217 225 229
;; small program: compr((((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else x) mod 2) - (x mod 2)) + 1) mod 2) mod 2, 1 + x) + 1
;; fast program: (compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (2 * (x + x)))) - 2, 1 + x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (mod (+ 1 (v3 x)) (+ 1 (* 2 (+ x x)))) 2))))
