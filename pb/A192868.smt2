;; sequence(s): A192868
;; terms: 3 5 7 9 11 13 15 19 21 23 25 27 31 33 37 39 43 45 47 51
;; small program: (compr((loop(if ((2 + loop(x * y, x, 2)) mod (1 + x)) <= 0 then 1 else 0, 1, loop(loop(x div 2, x mod 2, x), x, x)) + 1) mod 2, 1 + x) * 2) + 1
;; fast program: (compr((loop(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) mod 2, x mod 2, x) + 1) mod 2, 1 + x) - 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 2 (v2 x)) (+ 1 x)) 0) 1 0))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f6 x) (mod (v7 x) 2))))
