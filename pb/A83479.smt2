;; sequence(s): A83479
;; terms: 0 1 1 1 2 2 3 3 4 5 5 6 7 7 8 9 10 10 11 12
;; small program: loop(loop((((x * x) div y) - x) + y, x, 0), 2, x)
;; fast program: (loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, (x - 1) div 2) + x, 2, x) * x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (div (* x x) y) x) y))))
