;; sequence(s): A102615
;; terms: 1 8 10 14 15 16 20 22 24 25 27 30 32 33 35 36 38 39 40 44
;; small program: loop(compr((loop(x * y, x, x) mod (1 + x)) mod 2, x), 2, x) + 1
;; fast program: loop(compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x) mod (1 + x), x), 2, x) + 1
(set-logic UFNIA)
