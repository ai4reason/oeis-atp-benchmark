;; sequence(s): A76080
;; terms: 1 1 1 1 2 2 2 2 2 2 2 3 3 3 3 3 3 3 3 3
;; small program: compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) div (1 + x)
;; fast program: ((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), (1 + x) mod 2, x), x) + 1) div (1 + x)) mod ((1 + x) + 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (div (+ (v2 x) 1) (+ 1 x)) (+ (+ 1 x) 1)))))
