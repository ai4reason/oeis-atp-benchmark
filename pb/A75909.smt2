;; sequence(s): A75909
;; terms: 1 84 4256 169344 5843712 183794688 5421678592 152720375808 4157366140928 110282217357312 2867778350481408 73424436820180992 1857023919127527424 46511918954689069056 1155904251854380335104
;; small program: loop(((loop((loop((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, y, 1) + x) + x) + x, x, 1) * loop(x + x, x + x, 1)
;; fast program: loop(loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, y, 1) + (((x + x) + x) * 2), x, 1) * loop(x * x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (div (div (v10 x) 2) (+ 1 2)))))
(assert (= g9 1))
