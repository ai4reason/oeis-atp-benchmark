;; sequence(s): A28093
;; terms: 1 29 540 8230 111911 1414839 17015290 197394560 2229204021 24659970049 268405356440 2883764656890 30659268255331 323156832303659 3381832557579990 35179011908571220 364094453676865841
;; small program: loop(((loop((loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * ((2 * (x + x)) + x)), y, 1) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
