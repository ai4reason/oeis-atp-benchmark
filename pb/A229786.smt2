;; sequence(s): A229786
;; terms: 2 3 5 7 11 13 17 19 0 6 8 14 18 20 1 7 13 15 21 2
;; small program: ((compr(loop((loop2(x + y, x, x, 1, 2) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + (x + x), 1 + 2, 2)) div 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(2 + x, ((((2 + 2) + 2) + 2) + 1) + 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (w2 x) 1) (+ 1 x)))))
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (div (mod (+ (v0 x) 1) v3) 1))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (w6 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f7 x) (+ 2 x))))
(assert (= g7 (+ (+ (+ (+ (+ 2 2) 2) 2) 1) 2)))
(assert (= h7 1))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (mod (+ (v4 x) 1) v7))))
