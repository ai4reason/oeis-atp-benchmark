;; sequence(s): A130290
;; terms: 1 1 2 3 5 6 8 9 11 14 15 18 20 21 23 26 29 30 33 35
;; small program: compr((1 + loop(x + x, x, x)) mod (1 + x), loop(y, x, 1)) div 2
;; fast program: compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), if x <= 0 then 1 else x)
(set-logic UFNIA)
