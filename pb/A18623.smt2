;; sequence(s): A18623
;; terms: 1 2 3 4 6 8 12 24 31 62 93 124 186 248 372 744
;; small program: loop(compr(loop(loop(x * y, y, 1 + x), 2 + 2, 0) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop(((loop(2 * ((x * x) - x), 2, 2) * (1 + x)) + x) * (2 + x), 1, 2 + 2) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (* (+ (* v6 (+ 1 x)) x) (+ 2 x)))))
(assert (= g5 1))
(assert (= h5 (+ 2 2)))
