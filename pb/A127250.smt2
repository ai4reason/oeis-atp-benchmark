;; sequence(s): A127250
;; terms: 1 3 5 1 3 1 1 3 5 1 1 3 1 3 5 1 3 1 1 3
;; small program: loop((loop(loop2(x + y, y div 2, x, 0, x) mod 2, 1, y) * x) + 2, x, 2) - 1
;; fast program: (((loop((((loop2(x + y, y div 2, 1 + (2 * (2 + 2)), 1, x) mod 2) + x) - 1) + x, 2, x) + 1) mod (2 + 2)) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (+ (- (+ (mod (w4 x) 2) x) 1) x))))
(assert (= g3 2))
(assert (forall ((x Int)) (= (h3 x) x)))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (- (* (mod (+ (v3 x) 1) (+ 2 2)) 2) 1))))
