;; sequence(s): A343501
;; terms: 4 6 14 16 20 22 24 30 36 38 46 52 54 56 62 64 68 70 78 80
;; small program: compr((1 + loop(loop(y - x, x, x), 2 + x, x)) mod 2, x) * 2
;; fast program: compr(((1 + loop(loop(y - x, x, x), x, x)) mod 2) * x, 1 + x) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (mod (+ 1 (v4 x)) 2) x))))
