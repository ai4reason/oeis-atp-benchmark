;; sequence(s): A186262
;; terms: 1 2 9 56 420 3564 33033 327184 3413124 37119160 417733316 4837527072 57397642640 695394516600 8579210711625 107541060458400 1367139314643300 17599273282621800 229116465142280100 3013124257920348000
;; small program: (loop2((2 + y) * (x * y), 1 + y, x, 1, 1 + x) div loop(x * y, x, 1)) div loop2(x * y, 1 + y, x, 1, 2 + 2)
;; fast program: ((loop2(x * y, 1 + y, x, 1, (1 + 2) + x) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + x)) div loop2(x * y, 1 + y, x, 1, 2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (div (w0 x) (v1 x)) (w2 x)))))
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* x y))))
(assert (forall ((x Int) (y Int)) (= (g3 x y) (+ 1 y))))
(assert (forall ((x Int)) (= (h3 x) x)))
(assert (= i3 1))
(assert (forall ((x Int)) (= (j3 x) (+ (+ 1 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (w3 x) (v4 x)) (w5 x)) (w6 x)))))
