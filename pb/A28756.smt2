;; sequence(s): A28756
;; terms: 2 3 5 7 8 12 18 19 20 22 26 27 28 29 30 32 33 34 37 39
;; small program: compr(((loop((x * x) mod (1 + loop((x * x) + x, 2, 2)), 2 + (2 + 2), x) + 1) + x) mod 2, x)
;; fast program: compr(((loop2((x * x) * y, x, 2, x, x) mod (1 + loop((x * x) + x, 2, 2))) mod (1 + (2 + 2))) mod 2, 1 + x)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* x x) (+ 1 v2)))))
(assert (= g1 (+ 2 (+ 2 2))))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f3 x) (mod (mod (mod (w4 x) (+ 1 v5)) (+ 1 (+ 2 2))) 2))))
