;; sequence(s): A339356
;; terms: 16 32 144 256 688 1120 2352 3584 6496 9408 15456 21504 32928 44352 64416 84480 117744 151008 203632 256256
;; small program: loop(loop(loop(loop(((y * y) + x) + y, y + y, x), y, x), y div 2, x), 2 + x, 0) * 2
;; fast program: (((loop(loop(loop(loop((y * y) + x, y, x), y, x), y div 2, x), 2 + x, 0) * 2) * 2) * 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (* y y) x) y))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* y y) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (* (* (v4 x) 2) 2) 2) 2))))
