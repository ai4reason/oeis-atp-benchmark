;; sequence(s): A191257
;; terms: 1 3 5 7 8 9 11 13 15 17 19 21 23 24 25 27 29 31 33 35
;; small program: loop(compr(loop(loop((if (x mod 2) <= 0 then (2 + (2 + 2)) else x) div 2, 1 + 2, x), 2, x), y), x, 0) + 1
;; fast program: loop((((loop(loop(x div loop(1, x mod (2 * (2 + 2)), 2 * (2 + 2)), 2, x), 1 - (x mod (2 * (2 + 2))), x) + 1) mod 2) + 1) + x, x, 2) - 1
(set-logic UFNIA)
