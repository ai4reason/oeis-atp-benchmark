;; sequence(s): A26179
;; terms: 1 2 5 6 8 11 14 15 17 18 20 23 24 26 29 32 33 35 38 41
;; small program: loop(compr((loop(x div (if (x mod (1 + 2)) <= 0 then (1 + 2) else 1), x, x) mod (1 + 2)) mod 2, y), x, 1)
;; fast program: compr((if ((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x, 0), x) mod (1 + 2)) div 2) <= 0 then x else 0) div 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div x (ite (<= (mod x (+ 1 2)) 0) (+ 1 2) 1)))))
...
(assert (forall ((x Int)) (= (f3 x) (div (ite (<= (div (mod (v4 x) (+ 1 2)) 2) 0) x 0) 2))))
