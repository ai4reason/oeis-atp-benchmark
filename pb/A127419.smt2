;; sequence(s): A127419
;; terms: 1 2 3 4 6 8 11 15 19 24 30 37 45 53 62 72 83 95 108 122
;; small program: 1 + loop(loop(y div (2 + x), x + x, 1) + x, x, 0)
;; fast program: loop2(loop((loop((((y - x) + y) div (x * x)) + x, x - 2, 1) * (if x <= 0 then 1 else 2)) div 2, 1, y) + x, x, x, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (* (v4 x) (ite (<= x 0) 1 2)) 2))))
(assert (= g3 1))
