;; sequence(s): A260456
;; terms: 1 0 1 1 0 1 0 1 0 1 1 0 1 1 0 1 1 0 1 0
;; small program: (loop(compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x, x), y), x, 0) + 1) mod 2
;; fast program: loop(loop((loop(x div (if (x mod (2 + 2)) <= 0 then (2 + 2) else 1), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (2 + 2)) - 2, x) + 1, x, 1) mod 2
(set-logic UFNIA)
