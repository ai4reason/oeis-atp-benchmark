;; sequence(s): A173177
;; terms: 2 5 8 14 17 20 29 32 35 38 47 50 53 62 68 74 77 80 89 95
;; small program: (compr(loop((2 - loop(x + x, x, 2)) mod (1 + x), 1 - (x mod (1 + 2)), x) - 1, 2 + x) div 2) - 1
;; fast program: ((compr(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1 + x) - 1) * (1 + 2)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (- (v3 x) 1) (+ 1 2)) 2))))
