;; sequence(s): A211995
;; terms: 0 1 0 2 6 13 46 33 246 479 398 883 20 6215 9467 16751 42245 115091 140675 492363
;; small program: loop(x div 2, x, loop((2 * ((x + x) + x)) + x, x, 1)) mod loop(x + x, x, 1)
;; fast program: (loop2(x * y, y, x div 2, loop(1 + (2 + (2 + 2)), x mod 2, 1), (2 * (2 * (2 * (2 + (2 + 2))))) + 1) div loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (div (w3 x) (v5 x)) (v8 x)))))
