;; sequence(s): A15253
;; terms: 1 13 221 3485 55965 894621 14317213 229062301 3665049245 58640578205 938250090141 15011998086813 240191982810781 3843071671285405 61489146955314845 983826350426044061 15741221610252678813
;; small program: loop(loop(1 + (loop(x * x, 2, 2) * x), y, 1) - (2 * (x + x)), x, 1)
;; fast program: loop2(loop((1 + loop(x * x, 1, loop(x + x, x, 2))) div (1 + (2 + 2)), 1, y) * x, 1 + y, 2, 1, x) div (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (+ 1 (v5 x)) (+ 1 (+ 2 2))))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (div (w3 x) (+ 1 2)))))
