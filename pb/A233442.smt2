;; sequence(s): A233442
;; terms: 0 4 8 16 32 64 128 256 512 24 717 640 1601 2664 2393 0 3334 5536 3004 576
;; small program: loop(x + x, x, 2) mod loop((x * x) * x, 1, 1 + x)
;; fast program: loop(loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) mod ((x * x) * x), 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (v3 x) (* (* x x) x)))))
(assert (= g2 1))
