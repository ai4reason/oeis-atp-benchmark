;; sequence(s): A246438
;; terms: 0 2 3 5 7 8 9 11 12 14 16 17 19 20 22 24 25 26 28 29
;; small program: loop(compr(loop((loop((2 + x) mod y, x - 1, x) + 1) div 2, x, x), y), x, 0)
;; fast program: compr(loop((loop((2 + x) mod y, x - 1, x) + 1) div 2, x, x) mod 2, x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div (+ (v3 x) 1) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (mod (+ 2 x) y))))
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (v6 x) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (v4 x) 1))))
