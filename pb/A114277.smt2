;; sequence(s): A114277
;; terms: 1 5 19 67 232 804 2806 9878 35072 125512 452388 1641028 5986993 21954973 80884423 299233543 1111219333 4140813373 15478839553 58028869153
;; small program: loop(loop((loop2(x * y, y - 1, x, 2, 2 * (2 + x)) div loop(x * y, x, 1)) div (2 + x), 1, y) + x, x, 1)
;; fast program: loop(loop(((loop2(x * y, 1 + y, x, 1, 1 + (2 + (2 + x))) div loop(x * y, x, 1)) * 2) div (2 + x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (div (w2 x) (v3 x)) (+ 2 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (* (div (w6 x) (v7 x)) 2) (+ 2 x)))))
(assert (= g5 1))
