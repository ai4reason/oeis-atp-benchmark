;; sequence(s): A104085
;; terms: 1 2 11 26 47 72 99 132 167 206 251 300 351 406 463 526 591 660 735 812
;; small program: loop(compr(loop((((y mod 2) + y) * x) - x, x, 1) mod (1 + x), y) + x, x - 1, 1) + x
;; fast program: loop((compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y) * 2) + x, x - 1, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (- (* (+ (mod y 2) y) x) x))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (v5 x) (+ 1 (+ x x))))))
