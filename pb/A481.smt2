;; sequence(s): A481
;; terms: 1 15 140 1050 6951 42525 246730 1379400 7508501 40075035 210766920 1096190550 5652751651 28958095545 147589284710 749206090500 3791262568401 19137821912055 96416888184100 485000783495250
;; small program: loop(loop((loop2(((2 * ((y * y) + x)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x, x, 1)
;; fast program: loop((loop(((loop(loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div (2 + (2 + 2)), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (v6 x) (+ 2 (+ 2 2))))))
(assert (= g5 1))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (v4 x y) (* (+ x x) 2)) x))))
