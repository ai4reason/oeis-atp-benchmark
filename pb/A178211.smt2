;; sequence(s): A178211
;; terms: 0 1 1 1 1 1 2 1 7 1 4 1 4 1 4 9 13 13 5 1
;; small program: loop2((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + y) + 1) * x, y, 2, 1, x) mod (1 + x)
;; fast program: loop2((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + y) * x) + x, y, 2, 1, x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (w0 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (w3 x) (+ 1 x)))))
