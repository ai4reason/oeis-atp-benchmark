;; sequence(s): A270577
;; terms: 1 4 84 2640 100100 4232592 192203088 9178678080 455053212900 23222793594000 1212760632317520 64534727833692480 3488102039411078544 191031492362224091200 10580671081188491976000
;; small program: loop2(loop(2 * ((x - (x div y)) + x), y, x), y + y, 2, 1, x) div (1 + (x + x))
;; fast program: (((loop2(x * y, 1 + y, x - 1, 1, 2 + ((x + x) + x)) div loop(x * y, x, 1)) * (1 + ((x + x) + x))) * loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x))) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (w0 x) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (div (w2 x) (v3 x)) (+ 1 (+ (+ x x) x))) (w4 x)) (v5 x)))))
