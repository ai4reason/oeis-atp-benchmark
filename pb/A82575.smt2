;; sequence(s): A82575
;; terms: 0 1 3 7 9 10 12 25 27 28 30 34 36 37 39 79 81 82 84 88
;; small program: (compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x) mod 2, 1 + x) - 1) - (x mod 2)
;; fast program: loop((loop((((((loop(((((compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, (x div 2) div 2, x) mod 2, x) div 2) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) + (x mod 2), 1, 1 + x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (+ (v3 x) x) (mod x 2)))))
(assert (= g2 1))
