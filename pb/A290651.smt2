;; sequence(s): A290651
;; terms: 4 6 11 26 73 220 675 2070 6317 19184 58039 175114 527361 1586148 4766603 14316158 42981205 129009112 387158367 1161737202
;; small program: (2 + loop((x - y) + x, x, 1)) + loop((x + x) + x, x, 1)
;; fast program: ((2 + (loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) - loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ 2 (- (w2 x) (v4 x))) x) 2))))
