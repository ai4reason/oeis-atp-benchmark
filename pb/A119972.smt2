...
;; small program: loop(loop((x - 1) mod y, x, 2) - 1, 1 + x, x) * (1 + x)
;; fast program: ((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) + 1) mod 2) * 2) - 1) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (- (* (mod (+ (v2 x) 1) 2) 2) 1) (+ 1 x)))))
