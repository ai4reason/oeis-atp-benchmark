;; sequence(s): A287552
;; terms: 1 8 11 14 20 23 26 29 35 38 41 48 50 53 60 63 68 71 74 77
;; small program: loop(compr(loop2(x + y, (y div 2) div 2, x, 0, x) mod (2 + 2), y), x, 0) + 1
;; fast program: (((((loop2(x - y, y div (2 + 2), 2 + 2, 0, x) mod (2 + 2)) + 1) + x) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (w2 x) (+ 2 2)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (mod (w3 x) (+ 2 2)) 1) x) x) x) x))))
