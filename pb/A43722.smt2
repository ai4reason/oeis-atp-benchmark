;; sequence(s): A43722
;; terms: 1 3 7 10 15 18 20 22 26 31 34 36 38 40 44 46 50 52 54 58
;; small program: compr(((loop2((((y div 2) + y) mod 2) + x, y div 2, x, 0, x) mod (1 + 2)) + 1) mod 2, x) div 1
;; fast program: compr(((((((loop2(((x + y) mod 2) + x, y div 2, x, 1, x) - 1) div 2) - 1) mod (1 + 2)) - (x mod 2)) - 1) mod (1 + x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (mod (+ (div y 2) y) 2) x))))
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ (mod (w1 x) (+ 1 2)) 1) 2))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (- (- (mod (- (div (- (w3 x) 1) 2) 1) (+ 1 2)) (mod x 2)) 1) (+ 1 x)))))
