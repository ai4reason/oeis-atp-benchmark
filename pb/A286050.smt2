;; sequence(s): A286050
;; terms: 1 4 5 7 9 12 13 15 17 20 21 23 25 27 30 31 33 36 37 39
;; small program: (((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), x + x) mod 2) + 1) + x) + x
;; fast program: loop(((((loop2((y - x) div 2, x, 1 + (2 + (2 + 2)), x div 2, x) + x) div 2) + y) mod 2) + x, 2, x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (mod (+ (div (+ (w3 x) x) 2) y) 2) x))))
(assert (= g2 2))
