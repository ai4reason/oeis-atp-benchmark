;; sequence(s): A107819
;; terms: 1 3 4 7 8 11 12 15 20 21 26 29 30 33 38 43 44 49 52 53
;; small program: (loop(compr((1 + loop(2 * x, x, x)) mod (2 + x), y), x, 0) + 1) - x
;; fast program: (((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 1 + x) * 2) - (if x <= 0 then 1 else x)) * 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (- (* (v3 x) 2) (ite (<= x 0) 1 x)) 2) 2))))
