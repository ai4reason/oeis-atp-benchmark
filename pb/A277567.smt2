;; sequence(s): A277567
;; terms: 1 6 7 13 19 25 31 36 37 42 43 49 55 61 67 73 78 79 85 91
;; small program: compr((loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x div (2 + 2), x) mod (2 + (2 + 2))) - 1, 1 + x)
;; fast program: compr((loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x div (2 + (2 * (2 + 2))), x) mod (2 + (2 + 2))) - 1, 1 + x)
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int)) (= (f0 x) (- (mod (v1 x) (+ 2 (+ 2 2))) 1))))
...
(assert (forall ((x Int)) (= (f3 x) (- (mod (v4 x) (+ 2 (+ 2 2))) 1))))
