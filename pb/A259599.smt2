...
;; small program: loop(1 + (((x div 2) + y) * y), x, 1) mod 2
;; fast program: (loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 1 - (x mod (2 + 2)), x div 2), x mod 2, 1 + x) + x) mod 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ 1 (* (+ (div x 2) y) y)))))
