;; sequence(s): A52488
;; terms: 1 3 5 8 11 14 18 21 25 29 33 37 41 45 49 54 58 62 67 71
;; small program: loop(((x * y) + loop(x * y, y, 1)) + x, x, 1) div loop(x * y, x, 1)
;; fast program: ((loop((x * y) + loop(x * y, y - 1, 1), x, 0) * (1 + x)) div loop(x * y, x, 1)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (v3 x) (+ 1 x)) (v5 x)) 1))))
