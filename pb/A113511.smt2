;; sequence(s): A113511
;; terms: 1 2 30 30030 6469693230 614889782588491410 40729680599249024150621323470 2566376117594999414479597815340071648394470 225319534991831177328890236228992001350685163362356544091910
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, loop(x + y, x, 0), 1)
;; fast program: loop((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, y) + 1) * x, ((x * x) + x) div 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g4 x) (div (+ (* x x) x) 2))))
(assert (= h4 1))
