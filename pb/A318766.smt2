;; sequence(s): A318766
;; terms: 1 3 24 576 27648 3317760 557383680 160526499840 57789539942400 30512877089587200 25630816755253248000 24605584085043118080000 33660439028338985533440000 56549537567609495696179200000 104503545424942348046539161600000
;; small program: loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 2, x, y) - x, x, 1)
;; fast program: loop((loop(loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, y) * x) + x, y, 2, 1, x), 1, y) - 1) * x, x, 1)
(set-logic UFNIA)
