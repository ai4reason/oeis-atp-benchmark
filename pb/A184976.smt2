;; sequence(s): A184976
;; terms: 2 7 15 25 38 54 73 94 118 145 174 206 241 279 319 362 408 456 507 561
;; small program: loop(loop(loop(1 + ((x * y) + x), x, 2) div loop(x * y, x, 1), 1, y) + x, x, 2)
;; fast program: loop(loop((((1 + x) * loop(1 + (x * y), x, 1)) + 1) div loop(x * y, x, 1), 1, y) + x, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (* (+ 1 x) (v6 x)) 1) (v7 x)))))
(assert (= g5 1))
