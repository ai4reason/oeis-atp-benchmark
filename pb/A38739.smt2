;; sequence(s): A38739-A38798
;; terms: 1 7 30 103 314 895 2455 6590 17480 46070 121016 317342 831465 2177613 5702054 14929365 39087010 102332805 267912735 701406940
;; small program: loop(loop((loop2(x + y, x, y + y, 0, 1) - y) + x, 2 + y, x), x, 1)
;; fast program: loop(loop(((((loop2(((x - y) + x) + x, x, x, 1 + (2 + 2), 2) - 2) - 2) - (((x * x) + x) div 2)) - x) - x, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (w2 x y) y) x))))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (- (w5 x) 2) 2) (div (+ (* x x) x) 2)) x) x))))
(assert (= g4 1))
