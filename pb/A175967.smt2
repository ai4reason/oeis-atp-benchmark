;; sequence(s): A175967
;; terms: 1 2 6 12 20 29 39 51 65 80 96 114 134 155 177 201 226 252 279 307
;; small program: loop(compr(loop(x * y, x, 2) mod (1 + x), y) + x, x, if x <= 0 then 1 else x)
;; fast program: loop(compr(loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + x, (1 + x) mod 2, x) - x, y) + x, x - 1, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 2) 2) x))))
