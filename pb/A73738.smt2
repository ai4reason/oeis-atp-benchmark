;; sequence(s): A73738
;; terms: 1 2 4 7 11 18 24 35 43 58 72 89 109 130 152 177 205 236 266 303
;; small program: (((loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, y - 2, x div 2, 1, x) * 2) + 1) + x) div 2
...
(assert (forall ((x Int)) (= (small x) (div (+ (+ (* (w0 x) 2) 1) x) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (* (v5 x) (ite (<= x 0) 1 2)) x))))
(assert (= g4 1))
