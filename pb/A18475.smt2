;; sequence(s): A18475
;; terms: 1 5 19 25 95 475
;; small program: loop(compr((loop(((2 * (x + y)) + y) * x, 2, 2) - 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(((2 * (2 + x)) + x) * (x * x), 1, 1 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (- v2 1) (+ 1 x)))))
