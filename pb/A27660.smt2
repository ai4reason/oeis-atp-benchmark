;; sequence(s): A27660
;; terms: 1 4 11 26 56 112 210 372 627 1012 1573 2366 3458 4928 6868 9384 12597 16644 21679 27874
;; small program: loop(loop(loop(loop(x + y, y - 2, x), y, x) + y, 1 + y, x), x, 1)
;; fast program: loop(loop(loop((x * x) + x, 1, 2 + (((x * x) + x) div 2)) div (2 + (2 + 2)), 1, y) + x, x, 1) + (((x * x) + x) div 2)
(set-logic UFNIA)
...
(assert (= g6 1))
(assert (forall ((x Int)) (= (h6 x) (+ 2 (div (+ (* x x) x) 2)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (v4 x) (div (+ (* x x) x) 2)))))
