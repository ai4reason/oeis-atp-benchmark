;; sequence(s): A258085
;; terms: 0 1 2 3 4 5 7 8 12 13 20 21 33 34 54 55 88 89 143 144
;; small program: (loop(loop2(x + y, x, y, 2, 1), x div 2, 1) - 1) + (x mod 2)
;; fast program: (loop(((if x <= 0 then 1 else loop2(x + y, x, x, 2, 1)) - x) - x, 1, x div 2) - 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (- (ite (<= x 0) 1 (w3 x)) x) x))))
(assert (= g2 1))
