;; sequence(s): A280298
;; terms: 73786976294838206464 30903154382632612361920641803529 13552527156068805425093160010874271392822265625 59768263894155949306790119265585619217025149412430681649 539407797827634189900210968137750826278309533633974732577186113975161
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x * x, 2, loop(x * x, 2, x * x)), y, 2, 1, x), 1, y) * x, y, 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * loop(x * x, 2, loop(x * x, 2, x * x)), y, 2, 1, x), 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (v3 x y) 1) (v5 x)))))
...
(assert (forall ((x Int)) (= (g11 x) (mod (+ 1 x) 2))))
