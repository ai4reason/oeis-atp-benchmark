;; sequence(s): A90821
;; terms: 4 24 48 72 90 120 168 210 240 288 360 420 462 528 600 650 702 756 840 960
;; small program: loop2((compr((loop(x * y, x, x) mod (1 + x)) mod 2, y) * x) + x, 1 + y, 2, 1, x)
;; fast program: loop2(loop(compr((if (loop((1 + ((x div 2) mod 2)) * (1 + x), x, 1) mod (1 + x)) <= 0 then 1 else 0) mod (1 + x), x) + 1, 1, y) * x, 1 + y, 2, 1, x)
(set-logic UFNIA)
