;; sequence(s): A28222
;; terms: 1 36 815 14850 238161 3512376 48835795 650202750 8376184421 105168836916 1293777896775 15655641961050 186905779340281 2206620573868656 25809833134389755 299526474742773750
;; small program: loop((loop((loop2((2 * ((2 * (x + x)) + x)) + ((y * y) * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, 1 + x, 1, 2 + (2 * (2 + 2))) - loop((x * x) * x, 1, loop(x + x, x, 2))) div 2, 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (div (- (w8 x) (v9 x)) 2))))
(assert (= g7 1))
