;; sequence(s): A171947
;; terms: 1 3 7 9 11 15 19 23 25 27 31 33 35 39 41 43 47 51 55 57
;; small program: loop(compr(loop(if (x mod 2) <= 0 then ((x div 2) - x) else x, x, x), y), x, 0) + 1
;; fast program: compr(loop(x div loop(1, x mod (2 + 2), 2 + 2), loop(x * x, 2, 2) - 1, x) mod 2, x) + 1
(set-logic UFNIA)
