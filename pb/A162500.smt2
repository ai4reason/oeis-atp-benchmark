;; sequence(s): A162500
;; terms: 1 3 6 9 12 15 17 18 18 17 15 12 9 6 3 1
;; small program: loop(loop(loop2(y, y - x, y div (1 + 2), 1, 0) + x, y, x), x, 1) + x
;; fast program: loop2((((1 - ((1 + x) div (1 + (2 + (2 + 2))))) + x) + x) - y, x, x, 1, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h2 x y) (div y (+ 1 2)))))
(assert (= i2 1))
(assert (= j2 0))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (- (+ (+ (- 1 (div (+ 1 x) (+ 1 (+ 2 (+ 2 2))))) x) x) y))))
