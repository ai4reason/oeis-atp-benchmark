;; sequence(s): A139731
;; terms: 0 3 9 81 81 4617 18225 24057 1056321 4468041 40212369 180512793 1624615137 8091189225 72820703025 420293834937 2372089560705 4422146601609 90579297748689 510533809732953
;; small program: loop((x + x) + x, x + x, 1) mod loop(2 * ((x + x) + x), x, 1)
;; fast program: loop(x * x, 1, loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) mod loop2(x * y, y, x div 2, loop(2 + (2 + 2), x mod 2, 1), loop(x * x, 1, 2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (v2 x) (w5 x)))))
