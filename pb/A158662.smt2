;; sequence(s): A158662
;; terms: 1 3 6 6 11 11 18 18 18 18 29 29 42 42 42 42 59 59 78 78
;; small program: loop((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * (1 + y)) + x, x, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 0, 1, y) + x, x, 1)
(set-logic UFNIA)
