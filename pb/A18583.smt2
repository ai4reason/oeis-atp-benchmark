;; sequence(s): A18583
;; terms: 1 2 4 167 334 668
;; small program: loop(loop(1 + loop((x + x) + x, x, 2), 1 - (x mod (2 + 2)), x) + x, x, 1)
;; fast program: loop(loop(1 + (2 + (2 * ((1 + x) * (x * x)))), 1 - (x mod (2 + 2)), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ 1 (+ 2 (* 2 (* (+ 1 x) (* x x))))))))
