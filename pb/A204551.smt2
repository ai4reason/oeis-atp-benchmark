...
;; small program: 1 + ((loop(y div (2 + x), x + x, 0) div 2) div 2)
;; fast program: ((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x - 1) + 1) div (2 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (div (div (v0 x) 2) 2)))))
