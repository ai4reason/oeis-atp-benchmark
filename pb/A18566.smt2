;; sequence(s): A18566
;; terms: 1 2 3 6 107 214 321 642
;; small program: loop(compr((2 * loop((1 + 2) * (2 + x), 2 + 2, 1)) mod (1 + x), y), x, 0) + 1
;; fast program: compr((2 + (2 * ((1 + (2 + 2)) * loop((x * x) * x, 1, 2 + 2)))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (+ 2 (* 2 (* (+ 1 (+ 2 2)) v4))) (+ 1 x)))))
