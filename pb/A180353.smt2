;; sequence(s): A180353
;; terms: 2 12 135 1792 34375 606528 14000231 318767104 8910671247 290000000000 8844661788941 329895716585472 12417879370282373 477816293498994688 20581012847900390625 977677435906606235648 48807175451293869086443 2400130892593088792100864
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) * loop2((x * y) + x, y, 1 + x, 1, x)
;; fast program: ((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1) * (1 + x)) * loop2(x * y, y, x, 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (+ (v3 x) 1) (+ 1 x)) (w6 x)))))
