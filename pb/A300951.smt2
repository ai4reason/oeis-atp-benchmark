;; sequence(s): A300951
;; terms: 1 1 1 1 2 2 6 6 6 6 30 30 30 30 210 210 210 210 210 210
;; small program: loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, (x div 2) - 1, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, (x - 2) div 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (- (div x 2) 1))))
(assert (= h0 1))
