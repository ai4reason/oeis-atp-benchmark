;; sequence(s): A83215
;; terms: 3 12 37 86 185 328 549 834 1225 1776 2427 3278 4303 5464 6827 8470 10417 12552 15031 17800
;; small program: loop(loop((compr((1 + loop(x + x, x, x)) mod (1 + x), x) + 1) * (1 + (x + x)), 1, y) + x, x, 1) + 2
;; fast program: loop(loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) * (1 + (x + x)), 1, y) + x, x, 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (+ (v2 x) 1) (+ 1 (+ x x))))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ (* (v6 x) 2) 1) (+ 1 (+ x x))))))
(assert (= g5 1))
