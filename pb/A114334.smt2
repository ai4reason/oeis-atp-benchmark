...
;; small program: loop(compr(loop((x div y) * x, 2 + 2, 1 + 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((x * x) * x, 1, loop(x * x, 1, 2 + (2 + 2))) mod (1 + x), x) + 1) div (1 div 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (div x y) x))))
(assert (= g2 (+ 2 2)))
(assert (= h2 (+ 1 2)))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v3 x) 1) (div 1 1)))))
