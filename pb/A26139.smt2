;; sequence(s): A26139-A26175
;; terms: 1 3 7 9 15 19 21 25 27 33 39 43 45 51 55 57 61 63 69 73
;; small program: (loop(compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, x, x) mod (1 + 2)) - 1, y), x, 0) * 2) + 1
;; fast program: ((compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x, 1), x) mod (1 + 2)) - 1, x) * 2) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (mod (v2 x) (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (v3 x) 2) 1) 1))))
