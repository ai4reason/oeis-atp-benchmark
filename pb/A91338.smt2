;; sequence(s): A91338
;; terms: 1 ~1 0 1 ~1 0 ~1 ~1 0 1 1 0 1 1 0 1 ~1 0 ~1 ~1
;; small program: (1 - (loop(x div (1 + (x mod 2)), x, x) mod (2 + 2))) * (((x - 1) mod (1 + 2)) - 1)
;; fast program: ((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) mod 2) * (1 + x)) + (2 + x)) mod (1 + 2)) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- 1 (mod (v0 x) (+ 2 2))) (- (mod (- x 1) (+ 1 2)) 1)))))
...
(assert (forall ((x Int)) (= (fast x) (- (mod (+ (* (mod (v1 x) 2) (+ 1 x)) (+ 2 x)) (+ 1 2)) 1))))
