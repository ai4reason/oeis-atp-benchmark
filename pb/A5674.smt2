;; sequence(s): A5674
;; terms: 0 0 0 0 1 3 10 25 63 144 327 711 1534 3237 6787 14056 28971 59283 120894 245457
;; small program: (((loop2(2 + y, x + x, x, 1, 0) + loop(x + x, x, 1)) div 2) + 1) - loop2(x + y, x, x, 2, 1)
;; fast program: ((((loop(x + x, x div 2, 1 + (2 + (x mod 2))) - 2) + loop(x + x, x, 1)) div 2) + 1) - loop2(x + y, x, x, 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (+ (div (+ (w0 x) (v1 x)) 2) 1) (w2 x)))))
(assert (forall ((x Int)) (= (f3 x) (+ x x))))
(assert (forall ((x Int)) (= (g3 x) (div x 2))))
(assert (forall ((x Int)) (= (h3 x) (+ 1 (+ 2 (mod x 2))))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (div (+ (- (v3 x) 2) (v4 x)) 2) 1) (w5 x)))))
