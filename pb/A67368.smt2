;; sequence(s): A67368
;; terms: 2 6 10 14 16 18 22 26 30 34 38 42 46 48 50 54 58 62 66 70
;; small program: (loop(compr(loop(loop((if (x mod 2) <= 0 then (2 + (2 + 2)) else x) div 2, 1 + 2, x), 2, x), y), x, 0) * 2) + 2
;; fast program: (loop((((loop(loop(x div loop(1, x mod (2 * (2 + 2)), 2 * (2 + 2)), 2, x), 1 - (x mod (2 * (2 + 2))), x) + 1) mod 2) + 1) + x, x, 2) * 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g5 x) (- 1 (mod x (* 2 (+ 2 2)))))))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (+ (+ (mod (+ (v5 x) 1) 2) 1) x))))
