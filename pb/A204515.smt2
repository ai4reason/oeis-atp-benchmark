;; sequence(s): A204515
;; terms: 1 3 40 1050 42336 2328480 163088640 13913499600 1401656256000 162984589447680 21497802046156800 3172717285311974400 518147911684085760000 92790773980160256000000 18083066033253630689280000 3810158522787893903827200000
;; small program: loop2((1 + y) * ((2 + y) * x), 1 + y, x, 1, x) div loop((x * y) + x, x, 1)
;; fast program: ((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1)) * (1 + (x + x))) * loop2(x * y, 1 + y, x - 1, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (div (w2 x) (v3 x)) (+ 1 (+ x x))) (w4 x)))))
