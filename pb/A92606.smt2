;; sequence(s): A92606
;; terms: 0 2 1 0 0 0 2 1 0 2 1 0 2 1 0 0 0 2 1 0
;; small program: (loop(((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), 1 + x) mod 2) - 1) + x, 2, x) - x) + 2
;; fast program: ((loop(((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x - (x mod 2), x), x) + 1) mod 2) + x, 2, x) - x) + 1) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (- (v3 x) x) 1) (+ 1 2)))))
