;; sequence(s): A226804
;; terms: 1 120 10890 914760 74987451 6098153040 494603769780 40080553611120 3247001410058901 263019982119962760 21304965990387308670 1725711626112542281080 139782894999601166334351 11322421333652608793333280 917116312670388314093059560
;; small program: loop(((loop(loop(1 + (loop((x * x) + y, 2, 2) * x), y, 1) + loop((x + x) + x, 2, x), y, 1) + x) + x) + x, x, 1) * loop((x + x) + x, x, 1)
;; fast program: loop(loop(loop2((((y div 2) + y) * y) + x, (y + y) + y, x, 1, 1 + 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + x, x, 1) * loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (* (+ (div y 2) y) y) x))))
...
(assert (forall ((x Int)) (= (f7 x) (* (w8 x) (w9 x)))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (* (v6 x) (w10 x)))))
