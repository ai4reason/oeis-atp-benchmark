;; sequence(s): A263622
;; terms: 0 1 4 14 47 153 486 1516 4669 14255 43268 130818 394491 1187557 3570850 10728920 32219513 96724059 290303232 871171822
;; small program: loop(1 + (((x - y) + x) + x), x, 1) - loop(x + x, x, 1)
;; fast program: ((((((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) - loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2)))) div 2) + 1) + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ 1 (+ (+ (- x y) x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (+ (div (- (* (+ 1 2) (w2 x)) (v4 x)) 2) 1) x) 2))))
