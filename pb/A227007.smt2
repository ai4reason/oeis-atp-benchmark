;; sequence(s): A227007
;; terms: 2 3 4 7 8 15 22 43 44 87 130 259 302 603 904 1807
;; small program: loop(compr(loop((x * x) + x, 1 + 2, 2) mod (1 + x), y), x, 0) + 2
;; fast program: (compr(((loop(2 + ((x + x) + x), 2, 2 + 2) - 1) * loop((x * x) + x, 2, 2)) mod (1 + x), x) + 1) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (- v4 1) v5) (+ 1 x)))))
