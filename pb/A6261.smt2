;; sequence(s): A6261
;; terms: 1 2 4 8 16 32 63 120 219 382 638 1024 1586 2380 3473 4944 6885 9402 12616 16664
;; small program: 1 + loop(loop(loop(loop(x + y, y - 2, x), y, x) + y, y - 1, x), x, x)
;; fast program: loop(loop((((loop(((x * x) * x) - x, 1, x - 2) * x) div (2 * (2 + (2 + 2)))) + ((x * x) - x)) div 2, 1, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (div (* (v6 x) x) (* 2 (+ 2 (+ 2 2)))) (- (* x x) x)) 2))))
(assert (= g5 1))
