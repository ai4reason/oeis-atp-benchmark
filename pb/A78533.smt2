;; sequence(s): A78533
;; terms: 1 4 56 1024 21216 473088 11075328 268435456 6677665280 169514369024 4373549027328 114349209288704 3023068543631360 80675644291153920 2170389180446539776 58798996734949195776 1602737048880933109760 43924199383151211970560
;; small program: (loop2(x * y, 2 + (2 + y), x, 1, 1 + x) * loop2(x * y, y, x, 1, 2 + 2)) div loop((x * y) + x, x, 1)
;; fast program: (loop2(x * y, 2 + (2 + y), x, 1, 1 + x) * loop(x * x, 1, loop(x + x, x, 1))) div loop(x * y, x, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (* (w0 x) (w1 x)) (v2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (w3 x) (v4 x)) (v6 x)))))
