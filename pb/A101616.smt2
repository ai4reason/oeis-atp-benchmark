;; sequence(s): A101616
;; terms: 1 2 4 6 7 8 9 10 12 14 16 18 20 22 23 24 25 26 28 30
;; small program: loop(loop(if (x mod 2) <= 0 then (x div 2) else 2, y, x) + x, 1 + x, 1) - 2
;; fast program: loop(((loop(loop(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x), 1, ((x * x) + x) div 2), 1, x div 2) mod 2) + 1) + x, x, 1)
(set-logic UFNIA)
