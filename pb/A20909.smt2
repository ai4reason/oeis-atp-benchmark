;; sequence(s): A20909
;; terms: 1 1 2 2 3 4 4 5 6 6 7 8 8 9 10 10 11 12 13 13
;; small program: loop(loop(x div 2, x, loop2(x + y, x, y, 1, 0)) + x, x, 1)
;; fast program: (1 + (((loop(y, x div (2 + (2 + 2)), 1) + x) div (1 + 2)) + x)) div (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ 1 (+ (div (+ (v3 x) x) (+ 1 2)) x)) (ite (<= x 0) 1 2)))))
