;; sequence(s): A337895
;; terms: 1 6 21 56 127 258 483 848 1413 2254 3465 5160 7475 10570 14631 19872 26537 34902 45277 58008
;; small program: ((loop(loop(loop(2 + loop(x + y, y - 1, x), y, x), y, x), x, 0) * 2) + 1) + x
;; fast program: ((loop(loop(loop(((((x * x) - x) div (2 + (2 + 2))) + x) + x, 1, ((x * x) + x) div 2), 1, y) + x, x, 0) * 2) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ (div (- (* x x) x) (+ 2 (+ 2 2))) x) x))))
(assert (= g6 1))
