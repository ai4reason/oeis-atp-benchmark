;; sequence(s): A160412
;; terms: 0 3 12 21 48 57 84 111 192 201 228 255 336 363 444 525 768 777 804 831
;; small program: loop2(loop(loop2((((y mod 2) * 2) * x) + x, y div 2, x, 1, x), 1, y) + x, 1 + y, x, 0, x)
;; fast program: ((((loop(loop(loop2((((y mod 2) * 2) * x) + x, y div 2, x, 1, x), 1, y) + x, x - 1, 1) - x) div 2) * 2) + x) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (* (mod y 2) 2) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (* (* (mod y 2) 2) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (div (- (v3 x) x) 2) 2) x) (+ 1 2)))))
