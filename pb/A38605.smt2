;; sequence(s): A38605
;; terms: 2 1 1 1 2 2 2 2 2 2 2 3 3 3 3 3 3 3 3 3
;; small program: compr(loop2(2 + (x + y), x, x, 0, 2) mod (1 + x), 2 + x) div (1 + x)
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(loop((2 * (x + x)) + x, 2, x), 2, 2)) div (1 + x)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (mod (+ (v2 x) 1) v5) (+ 1 x)))))
