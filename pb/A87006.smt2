;; sequence(s): A87006
;; terms: 1 2 3 5 6 7 10 11 13 14 15 21 22 26 30 33 35 39 42 55
;; small program: loop(compr(loop((1 + ((1 + (x * x)) * y)) * (2 + x), 2, 2) mod (1 + x), y), x, x) + 1
;; fast program: loop(compr(loop((1 + ((x * x) * x)) * ((2 + 1) * x), 1, 2 + (2 * (2 + 2))) mod (1 + x), y), x, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 (* (+ 1 (* x x)) y)) (+ 2 x)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* (* x x) x)) (* (+ 2 1) x)))))
(assert (= g5 1))
(assert (= h5 (+ 2 (* 2 (+ 2 2)))))
