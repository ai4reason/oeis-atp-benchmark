;; sequence(s): A4397
;; terms: 3 4 7 10 16 21 30 40 57 84 120 181 274 420 657 1040 1656 2645 4248 6836
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) + loop2(x + y, x, x, 1, 0)
;; fast program: ((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) + loop2(x + y, x, x, 1, 0)) + 2) - (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (+ (+ (* (v3 x) 2) 1) (w7 x)) 2) (ite (<= x 0) 1 2)))))
