;; sequence(s): A2815
;; terms: 0 1 3 6 9 13 17 22 27 32 37 43 49 56 63 70 77 85 93 102
;; small program: loop(loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y - 1) + x, y, x), x, x)
;; fast program: loop(loop(loop(loop(if (((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod (1 + x), 1, y) + x, x - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (ite (<= (- (mod (+ 1 (v8 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g7 1))
