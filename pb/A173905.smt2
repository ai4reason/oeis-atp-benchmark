;; sequence(s): A173905
;; terms: 0 1 2 3 4 6 8 9 10 12 14 15 16 18 20 21 22 24 25 26
;; small program: compr((loop(x * y, x, 2) mod loop(y * y, x, 1)) - x, x)
;; fast program: loop(compr(loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) mod 2) + x, (1 + x) mod 2, x) - x, y) + 1, (x - 1) - 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 0) 2) x))))
...
(assert (forall ((x Int)) (= (g3 x) (- (- x 1) 2))))
