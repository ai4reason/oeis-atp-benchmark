;; sequence(s): A1748
;; terms: 6 9 15 21 33 39 51 57 69 87 93 111 123 129 141 159 177 183 201 213
;; small program: (loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + 2)
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) (+ 1 2)))))
