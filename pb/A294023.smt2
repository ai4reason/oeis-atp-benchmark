;; sequence(s): A294023
;; terms: 0 0 0 0 1 2 4 6 8 10 13 16 19 22 26 30 34 38 42 46
;; small program: loop(loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y - 1) + x, y div 2, x), x, 0)
;; fast program: loop(loop(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 0, 1, y) + x, (x - 1) div 2, 0) mod (1 + x), 1, y) + x, x - 1, 0)
(set-logic UFNIA)
...
(assert (= g2 1))
(assert (forall ((x Int) (y Int)) (= (h2 x y) (- y 1))))
