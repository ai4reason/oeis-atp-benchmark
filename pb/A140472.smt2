;; sequence(s): A140472
;; terms: 0 1 2 2 4 3 4 4 8 5 6 6 8 7 8 8 16 9 10 10
;; small program: (loop(1 + loop((x - 1) mod y, x, 2), x, x) + x) div 2
;; fast program: ((loop2((((y div x) mod 2) + 1) * x, y, 2 + (2 + 2), 1, x - 1) mod (1 + x)) + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (mod (w2 x) (+ 1 x)) x) 2))))
