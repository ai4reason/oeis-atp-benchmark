;; sequence(s): A186300
;; terms: 3 7 15 19 27 31 51 55 75 79 87 91 99 115 135 139 147 159 175 187
;; small program: compr((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x)), 1 + x) + 1
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) * 2) div 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (* (v3 x) 2) 2) 2) 1))))
