;; sequence(s): A24030
;; terms: 1 2 ~119 ~2160 ~16303 ~77882 ~279207 ~821356 ~2090591 ~4763286 ~9940951 ~19310024 ~35300367 ~61154194 ~100630535 ~156510468 ~225388735 ~281198510 ~224799543 268389728
;; small program: loop((x + x) + x, x, 1) - loop2((x * x) * y, x, 2, x, x)
;; fast program: loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) - (loop((x * x) * x, 1, x * x) * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (w2 x) (* (v4 x) x)))))
