;; sequence(s): A131491
;; terms: 4 12 240 10080 79833600 12454041600 711374856192000 243290200817664000 51704033477769953280000 17683523987479403909087232000000 16445677308355845635451125760000000 27527506182452690092631959163161804800000000
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x * y, x, 1), 0, 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x * y, x, 1), x, 2, 0, x)
(set-logic UFNIA)
