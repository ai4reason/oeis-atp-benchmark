;; sequence(s): A73783
;; terms: 2 2 1 1 2 2 1 1 2 2 1 1 2 1 1 1 1 2 2 1
;; small program: ((loop((compr(loop(x * y, x, 2) mod (1 + x), 2 + x) mod 2) + x, 2, x) - x) div 2) + 1
;; fast program: if (((loop((compr((loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 2) - 1, x div 2, x) - 1) mod 2, 1 + x) mod 2) + x, 2, x) - x) * 2) - 2) <= 0 then 1 else 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (ite (<= (- (v6 x) 2) 0) (+ 1 x) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (ite (<= (- (* (- (v3 x) x) 2) 2) 0) 1 2))))
