;; sequence(s): A82578
;; terms: 1 7 67 767 9587 126011 1711595 23796515 336666215 4828084575 69994481871 1023793569567 15086216016367 223704570996367 3335098322412367 49954148031128767 751296616443141667
;; small program: loop(loop2(loop(2 * ((x - (x div y)) + x), y, x), 1 + y, 2, 1, y) + x, x, 2) div 2
;; fast program: loop(loop((loop(x * x, 1, loop(2 * ((x - (x div y)) + x), x, 1)) div (1 + x)) * (1 + (x + x)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (div (v5 x) (+ 1 x)) (+ 1 (+ x x))))))
(assert (= g4 1))
