;; sequence(s): A339584
;; terms: 1 3 2 1 1 1 3 2 1 3 2 1 3 2 1 1 1 3 2 1
;; small program: ((loop(((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), 1 + x) mod 2) - 1) + x, 2, x) - x) + 1) + 2
;; fast program: (((loop(((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x - (x mod 2), x), x) + 1) mod 2) + x, 2, x) - x) + 1) mod (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (ite (<= (mod x 2) 0) 2 x) 2))))
(assert (forall ((x Int)) (= (g5 x) (- x (mod x 2)))))
...
(assert (forall ((x Int)) (= (f3 x) (+ (mod (+ (v4 x) 1) 2) x))))
(assert (= g3 2))
(assert (forall ((x Int)) (= (h3 x) x)))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (- (v3 x) x) 1) (+ 1 2)) 1))))
