;; sequence(s): A65554
;; terms: 2 9 11 13 24 29 31 36 37 40 41 43 49 50 51 67 68 70 72 73
;; small program: compr((loop((x + x) + x, x, 1) mod loop(x + x, x, 1)) mod (1 + 2), 1 + x) - 1
;; fast program: compr(((((((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) mod loop(x + x, x, 2)) + 1) mod (1 + 2)) - 1) mod (1 + x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (mod (v1 x) (v2 x)) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (+ (mod (* (+ 1 2) (w4 x)) (v6 x)) 1) (+ 1 2)) 1) (+ 1 x)))))
