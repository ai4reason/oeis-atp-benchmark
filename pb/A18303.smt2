;; sequence(s): A18303
;; terms: 1 3 7 21 49 147
;; small program: loop(loop2(1 + ((y mod 2) + x), x, x, x, y), x, 1)
;; fast program: loop((2 * ((x + x) + x)) + x, x div 2, loop(1 + 2, x mod 2, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ 1 (+ (mod y 2) x)))))
