;; sequence(s): A94983
;; terms: 1 1 1 1 2 2 2 3 4 5 6 7 8 10 12 15 18 22 26 31
;; small program: loop(x div (1 + (2 + 2)), x, loop(2 * ((x + x) + x), x, 1))
;; fast program: (loop2(x * y, y, x div 2, 1 + (x mod 2), 2 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (w2 x) (w3 x)) (w5 x)))))
