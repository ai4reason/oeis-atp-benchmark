;; sequence(s): A319956
;; terms: 3 1 2 1 2 2 1 2 2 2 2 1 2 2 2 2 2 2 2 2
;; small program: (compr(loop2(x + y, y - x, x, 0, 2) mod (1 + x), 1 + x) + 2) mod (2 + 2)
;; fast program: (((((loop((compr(loop(loop((x - 1) mod y, x, 0), 2, x) mod 2, x) mod 2) + x, 2, x) - x) * 2) + 2) mod (1 + 2)) + 1) - (if x <= 0 then 0 else 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (mod (- x 1) y))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (mod (+ (* (- (v2 x) x) 2) 2) (+ 1 2)) 1) (ite (<= x 0) 0 1)))))
