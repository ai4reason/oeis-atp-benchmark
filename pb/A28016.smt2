;; sequence(s): A28016
;; terms: 1 30 585 9410 135741 1826910 23449345 290918970 3519263781 41763254390 488289628905 5642683638930 64604625484621 734199837187470 8294002882781265 93241186270173290 1044092739977046261
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (2 * (x + x))), y, 1) + x) + x, x, 1)
;; fast program: loop((loop((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + (2 * (2 * (x + x)))) + x, y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
