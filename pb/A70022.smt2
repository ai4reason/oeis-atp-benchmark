;; sequence(s): A70022
;; terms: 11 22 33 44 55 66 88 99 110 132 165 176 198 220 264 275 330 352 396 440
;; small program: (loop(compr(loop((1 + (2 + 2)) * ((x * y) * y), 2 + 2, 2 + 2) mod (1 + x), y), x, 0) + 1) * loop(1 + (x + x), 2, 2)
;; fast program: (loop(compr(loop((2 * ((2 * (x + x)) + x)) * ((x * x) - x), 2, 2 + 2) mod (1 + x), y), x, 0) + 1) * loop(1 + (x + x), 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 (+ 2 2)) (* (* x y) y)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 (+ 2 2)))
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (* (+ (v0 x) 1) v3))))
(assert (forall ((x Int)) (= (f6 x) (* (* 2 (+ (* 2 (+ x x)) x)) (- (* x x) x)))))
(assert (= g6 2))
(assert (= h6 (+ 2 2)))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (mod v6 (+ 1 x)))))
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (* (+ (v4 x) 1) v7))))
