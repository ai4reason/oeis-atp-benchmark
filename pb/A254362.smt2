;; sequence(s): A254362
;; terms: 10 15 27 57 135 345 927 2577 7335 21225 62127 183297 543735 1618905 4832127 14447217 43243335 129533385 388206927 1163834337
;; small program: ((1 + 2) * (2 + loop(x + x, x, 1))) + loop((x + x) + x, x, 1)
;; fast program: ((1 + 2) * (2 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ 1 2) (+ 2 (v0 x))) (v1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ 1 2) (+ 2 (v2 x))) (w5 x)))))
