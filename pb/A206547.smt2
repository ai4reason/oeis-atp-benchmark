;; sequence(s): A206547
;; terms: 1 5 11 13 17 19 23 25 29 31 37 41 43 47 53 55 59 61 65 67
;; small program: loop(compr(loop(loop((x * x) * x, 1, y * y) + x, x, 0) mod (1 + x), y), x, 0) + 1
;; fast program: compr(((((((loop((x * x) * x, 2, x * x) mod loop((x * x) + x, 2, 2)) div 2) - (x mod 2)) + 1) - (x mod 2)) * x) + 1, x)
(set-logic UFNIA)
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f4 x) (+ (* (- (+ (- (div (mod (v5 x) v6) 2) (mod x 2)) 1) (mod x 2)) x) 1))))
