;; sequence(s): A3983
;; terms: 1 1 1 1 2 1 1 2 2 1 1 2 3 2 1 1 2 3 3 2
;; small program: (loop(((loop(loop(y - 1, x - y, x), x, x) + y) div (2 + x)) + x, x, x) - x) + 1
;; fast program: loop(((loop(loop(y, x - y, x), x, 1 + x) + y) div (2 + x)) + x, 1 + x, x) - x
(set-logic UFNIA)
