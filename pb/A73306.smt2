;; sequence(s): A73306
;; terms: 1 1 1 3 10 105 1260 48510 2162160 312161850 52562109600 28836887379300 18539497049673600 38989721014029185400 96410946507417622080000 782067521015701609508820000
;; small program: loop2(loop((1 + loop(2 * ((x - (x div y)) + x), x - 1, 1)) div 2, 1, y) * x, y - 2, x, 1, x)
;; fast program: loop2(loop((1 + loop(2 * ((x - (x div y)) + x), x - 1, 1)) div 2, 1, y) * x, y - 2, x div 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (+ 1 (v5 x)) 2))))
(assert (= g4 1))
