;; sequence(s): A203471
;; terms: 1 5 105 8820 2910600 3745942200 18748440711000 364619674947528000 27558684271884061296000 8100324068034882136733280000 9267305355220395466643896716480000 41308086890359390753018505224037952000000
;; small program: loop(loop(loop2((2 + (2 + y)) * x, 1 + y, x, 1, x) div loop(x * y, x, 1), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(x * y, 1 + y, x, 1, 2 + (2 + x)) div loop(x * y, x, 1), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ 2 (+ 2 x)))))
