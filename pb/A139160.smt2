;; sequence(s): A139160
;; terms: 2 4 61 2521 19958401 3113510401 177843714048001 60822550204416001 12926008369442488320001 4420880996869850977271808000001 4111419327088961408862781440000001
;; small program: (loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x * y, x - 1, 1), y, 2, 1, x) div 2) + 1
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x * y, x - 1, 1), y, 2, 1, x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 1) (v4 x)))))
