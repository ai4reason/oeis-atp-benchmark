;; sequence(s): A130291
;; terms: 2 2 3 4 6 7 9 10 12 15 16 19 21 22 24 27 30 31 34 36
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), loop(y, x, 1)) div 2) + 1
;; fast program: compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), if x <= 0 then 1 else x) + 1
(set-logic UFNIA)
