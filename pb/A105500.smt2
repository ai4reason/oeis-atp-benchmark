;; sequence(s): A105500
;; terms: 1 2 3 2 3 4 3 2 3 4 1 4 3 4 3 2 3 4 1 4
;; small program: (loop(loop(x div (2 - (x mod 2)), x, y) + x, x, 0) mod (2 + 2)) + 1
;; fast program: (((loop2(((1 + y) div (2 + 2)) + x, y div 2, 2 + (2 + 2), 0, x) * 2) + x) mod (2 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (* (w2 x) 2) x) (+ 2 2)) 1))))
