;; sequence(s): A161188
;; terms: 2 3 5 6 8 9 11 12 13 15 16 18 19 20 21 23 24 25 27 28
;; small program: loop((((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 0) div (x * x)) + x, x, 2) + x
;; fast program: loop((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) div (x * x)) + x, x, 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (+ (* (v1 x y) 2) 0) (* x x)) x))))
