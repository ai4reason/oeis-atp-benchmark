;; sequence(s): A94985
;; terms: 1 1 2 4 6 10 16 26 42 68 109 175 281 450 720 1152 1844 2951 4722 7555
;; small program: loop(x div (1 + (2 + 2)), x, loop(((2 * 2) * 2) * x, x, 1))
;; fast program: loop((x * x) * x, 1, loop(x + x, x, 1)) div loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
