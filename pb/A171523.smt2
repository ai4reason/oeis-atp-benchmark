;; sequence(s): A171523
;; terms: 1 8 18 32 45 60 84 112 135 160 198 240 273 308 360 400 442 486 532 600
;; small program: loop(compr(loop(x * y, x, 2) mod (2 + x), y) + 2, x, 1) * (1 + x)
;; fast program: (compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x) mod (1 + x), x) + 1) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 1) (+ 1 x)))))
