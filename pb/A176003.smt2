;; sequence(s): A176003
;; terms: 1 4 7 13 19 31 37 49 55 67 85 91 109 121 127 139 157 175 181 199
;; small program: loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, y, 1 + 2, 1, x)
;; fast program: (((compr((((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) + 1) * (1 + 2)) - 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (- (* (+ (v3 x) 1) (+ 1 2)) 2) 1))))
