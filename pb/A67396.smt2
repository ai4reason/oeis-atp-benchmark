;; sequence(s): A67396
;; terms: 4 13 22 31 36 41 50 59 68 77 86 95 104 109 114 123 132 141 150 159
;; small program: (((compr((loop2((y mod 2) + x, y div (2 - (x mod 2)), x, 1, x) div 2) mod (1 + 2), x) * 2) + 2) * 2) + x
;; fast program: ((loop(((loop((loop(x div loop(1, x mod (2 * (2 + 2)), 2 * (2 + 2)), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (2 + 2)) - 2, x) mod 2) + 1) + x, x, 1) * 2) * 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (div (w1 x) 2) (+ 1 2)))))
(assert (forall ((x Int)) (= (g0 x) x)))
(assert (forall ((x Int)) (= (t0 x) (ite (<= (f0 x) 0) x (t0 (+ x 1))))))
(assert (forall ((x Int)) (= (u0 x) (ite (<= x 0) (t0 0) (t0 (+ (u0 (- x 1)) 1))))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x)))))
(assert (forall ((x Int)) (= (small x) (+ (* (+ (* (v0 x) 2) 2) 2) x))))
(assert (= f5 1))
