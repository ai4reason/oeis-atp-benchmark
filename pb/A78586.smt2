;; sequence(s): A78586
;; terms: 3 21 231 4389 100947 3129357 134562351 6324430497 373141399323 25000473754641 1775033636579511 140227657289781369 11638895555051853627 1198806242170340923581 128272267912226478823167 16290578024852762810542209 2134065721255711928181029379
;; small program: loop(((compr(((1 + loop(x + x, (x div 2) * 2, 2)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, 1 + x, 1)
;; fast program: loop(((compr(((1 + (2 * loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, 1 + x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (mod (+ 1 (v2 x)) (+ 1 (+ x x))) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (* (v1 x y) 2) 1) x))))
...
(assert (forall ((x Int)) (= (f4 x) (- (mod (+ 1 (* 2 (v5 x))) (+ 1 (+ x x))) 2))))
