;; sequence(s): A277637
;; terms: 1 4 34 454 7384 133510 2583958 52468078 1104191608 23891534758 528527606698 11905777228618 272269758961018 6306419621308618 147677930682023818 3491114167267938298 83217317955857060788 1998209467779811473538 48293984598611551487038 1174012306200941229710038
;; small program: loop(loop(loop(x * y, (x + x) + x, 1) div loop(((y * y) + y) * (x * y), x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop(((loop(2 * (((x div y) + x) + x), x, 1) div (1 + x)) * loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x))) div loop(x * y, x, 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (* y y) y) (* x y)))))
(assert (forall ((x Int)) (= (g3 x) x)))
(assert (= h3 1))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v3 x) (u3 (g3 x) h3))))
(assert (forall ((x Int)) (= (f1 x) (div (v2 x) (v3 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (* (div (v6 x) (+ 1 x)) (w7 x)) (v8 x)))))
(assert (= g5 1))
