;; sequence(s): A162681
;; terms: 2 3 6 7 29 72
;; small program: ((loop2(y, x + y, x - 2, 2, 1) * loop(x + x, x, 1)) div 2) + loop2(x + y, x, x, 1, 0)
;; fast program: ((loop2(y, x + y, x - 2, 2, 1) * loop(x + x, x, 1)) div 2) + loop2(x + y, x, x - 1, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (* (w0 x) (v1 x)) 2) (w2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (w3 x) (v4 x)) 2) (w5 x)))))
