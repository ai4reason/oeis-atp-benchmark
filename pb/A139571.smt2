;; sequence(s): A139571
;; terms: 1073741824 205891132094649 931322574615478515625 22539340290692258087863249 17449402268886407318558803753801 2619995643649944960380551432833049
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, (2 + (2 + 2)) * (1 + (2 + 2)), 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(loop((x * x) * x, 1, x * x) * x, 1, x * x), y, 2, 1, x), 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (* (v10 x) x))))
(assert (= g9 1))
