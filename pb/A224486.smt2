;; sequence(s): A224486
;; terms: 1 2 5 6 9 14 18 21 26 29 30 33 41 50 53 54 65 69 74 78
;; small program: compr((1 + loop(x + x, x div 2, 1)) mod (1 + x), 2 + x) div 2
;; fast program: compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), ((x div 2) + x) mod 2, x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g3 x) (mod (+ (div x 2) x) 2))))
