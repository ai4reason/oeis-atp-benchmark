;; sequence(s): A80352
;; terms: 0 0 0 0 1 1 1 2 2 2 2 3 4 4 4 4 5 6 6 6
;; small program: loop(loop((loop(((y + y) div (x * x)) + x, 2 * (x + x), 1) + 1) mod 2, 1, y) + x, x, 0)
;; fast program: loop(loop(loop(loop(((2 * (y + y)) div (x * x)) + x, x + x, 1) mod 2, 1, y) + x, x, 0) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (div (* 2 (+ y y)) (* x x)) x))))
