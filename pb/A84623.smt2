;; sequence(s): A84623
;; terms: 1 1 4 2 16 16 64 16 256 256 1024 512 4096 4096 16384 2048 65536 65536 262144 131072
;; small program: loop2((x - ((x * y) mod 2)) + x, y div 2, x, 1, x)
;; fast program: (((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1) * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- x (mod (* x y) 2)) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (* (v1 x) 2) 1) (v7 x)) (+ 1 x)))))
