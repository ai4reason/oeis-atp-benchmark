;; sequence(s): A113852
;; terms: 128 2187 78125 279936 823543 10000000 19487171 62748517 105413504 170859375 410338673 893871739 1801088541 2494357888 3404825447 8031810176 17249876309 21870000000 27512614111 42618442977
;; small program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) + 1) * (x * x), y, 1 + 2, 1, x)
;; fast program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) + 1) * loop((x * x) * x, 1, x * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 1) (* x x)))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (v6 x)))))
