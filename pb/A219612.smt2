;; sequence(s): A219612
;; terms: 1 4 6 9 11 19 24 29 31 34 41 46 48 59 61 71 72 79 89 94
;; small program: compr(loop2(1 + (x + y), x, x, 0, if 1 <= 0 then 0 else 0) mod (1 + x), x) + 1
;; fast program: (((((compr((((1 + loop2(x + y, x, x, 1 + 2, 1 + 2)) div (1 + 2)) + x) mod (1 + x), x) + 1) + x) - x) + x) - x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (+ (div (+ 1 (w3 x)) (+ 1 2)) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (- (+ (+ (v2 x) 1) x) x) x) x) 1))))
