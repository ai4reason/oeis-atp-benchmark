;; sequence(s): A14686
;; terms: 3 2 6 6 12 12 18 18 24 28 32 36 42 42 48 52 60 60 68 70
;; small program: compr(((loop(x + x, x, 2) - 2) div 1) mod (1 + x), 1 + x) + loop(0, x mod 2, 2)
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) - (if x <= 0 then 1 else 2)) + 2) - ((x + x) mod (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (- (+ (* (v3 x) 2) 2) (ite (<= x 0) 1 2)) 2) (mod (+ x x) (+ 2 2))))))
