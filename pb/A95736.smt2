...
;; small program: loop(compr(loop(loop((2 + x) mod y, x, 0), 1 + 2, x), y), x, 0)
;; fast program: compr(loop((loop(loop(2 + (x mod y), x, 1) - 1, 1 + (2 + 2), x) - 2) - 2, 2, x), compr(0, x))
(set-logic UFNIA)
