;; sequence(s): A285968
;; terms: 2 4 7 10 12 14 17 19 22 24 27 30 32 35 37 40 42 44 47 50
;; small program: loop(1 + ((loop2(x + y, y div 2, x, 1, x) mod 2) + x), x, 2) + x
;; fast program: ((compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) div 2) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (+ 1 (+ (mod (w1 x) 2) x)))))
