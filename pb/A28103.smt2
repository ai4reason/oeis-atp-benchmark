;; sequence(s): A28103
;; terms: 1 33 703 12309 192943 2819949 39304711 529663893 6962879935 89838427965 1142617774519 14370723472677 179151734300527 2217730946235981 27298909572676327 334504592037294261
;; small program: loop(((loop(loop2((2 * ((2 * (x + x)) + x)) + ((y * y) * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* 2 (+ (* 2 (+ x x)) x)) (* (* y y) y)))))
