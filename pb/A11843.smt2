;; sequence(s): A11843
;; terms: 0 0 0 0 0 0 1 3 9 21 42 77 132 214 333 500 728 1031 1428 1938
;; small program: (loop(loop(loop(loop(x + y, (y - 2) - 2, x), y, x), y, x), x, 0) div 2) div (1 + 2)
;; fast program: loop2(x * y, y - 1, 1 + (2 + 2), 1, x) div loop((1 + y) * x, 1 + (2 + 2), 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (+ 1 y) x))))
(assert (= g5 (+ 1 (+ 2 2))))
(assert (= h5 1))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (w4 x) v5))))
