...
;; small program: loop(compr(loop(x * y, loop(2 * (2 + x), 2, 2), 1) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((compr(loop(x * y, x, 2) mod (2 + x), y) * x) + x, ((2 + 2) + 2) * 2, 1) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
