;; sequence(s): A24701
;; terms: 2 6 12 30 42 72 90 132 210 240 342 420 462 552 702 870 930 1122 1260 1332
;; small program: (loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x) div 2) div 2
;; fast program: ((loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, y, 2, 1, x) div 2) div 2) + 0
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (div (w3 x) 2) 2) 0))))
