;; sequence(s): A138955
;; terms: 0 2 1 26 46 206 433 2006 739 19034 53857 76562 435349 1357028 1290286 5539319 1267831 96236090 124187905 993503240
;; small program: loop(x + x, (x + x) + x, 1) mod loop((x + x) + x, x, 1)
;; fast program: loop((x * x) * x, 1, loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (v2 x) (w6 x)))))
