;; sequence(s): A61751
;; terms: 11 17 19 23 26 27 29 31 34 35 39 41 43 44 47 48 49 51 53 54
;; small program: compr(loop(x * y, 1 + x, 1) mod loop(x * x, 2, 1 + x), 1 + x)
;; fast program: compr(loop(x * y, x - 2, 1) mod loop((x * x) * x, 1, 1 + x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (v1 x) (v2 x)))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (v4 x) (v5 x)))))
