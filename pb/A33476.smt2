;; sequence(s): A33476
;; terms: 4 6 9 15 25 35 49 77 121 143 169 221 289 323 361 437 529 667 841 899
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + (y div 2)) + 1) * x, 1 + y, 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + (y div 2)) + 1) * x, 1 + y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (+ 1 (div y 2)))))
...
(assert (forall ((x Int) (y Int)) (= (g4 x y) (+ 1 (div y 2)))))
