;; sequence(s): A71148
;; terms: 3 8 15 26 39 56 75 98 127 158 195 236 279 326 379 438 499 566 637 710
;; small program: loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, 2 + x, x)
;; fast program: (((loop(loop(compr(((1 + ((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) - 1, 1, y) + x, x, 1) + x) * 2) + 1) + x
(set-logic UFNIA)
