;; sequence(s): A60739
;; terms: 1 1 ~2 ~36 7200 17640000 ~560105280000 ~239102222768640000 1408147589778024775680000 116620600756651855983415296000000 ~137839975629646325813680872620851200000000 ~2352568589682795058651211199786427114330521600000000
;; small program: loop(loop((loop2((1 + y) * (x * y), 1 + y, x, 1, x) div loop(x * y, x, 1)) div loop(0 - (x * y), x, 1), 1, y) * x, x - 1, 1)
;; fast program: loop(loop((loop2(((y * y) + y) * (0 - x), 1 + y, x, 1, x) div loop(x * y, x, 1)) div loop(x * y, x, 1), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 y) (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (- 0 (* x y)))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (= h4 1))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) h4))))
(assert (forall ((x Int)) (= (f1 x) (div (div (w2 x) (v3 x)) (v4 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ (* y y) y) (- 0 x)))))
...
(assert (forall ((x Int)) (= (f6 x) (div (div (w7 x) (v8 x)) (v9 x)))))
(assert (= g6 1))
