;; sequence(s): A88236
;; terms: 0 1 2 4 7 10 14 18 23 28 33 39 45 51 57 63 69 76 83 90
;; small program: loop(loop(loop(x div 2, x, y) + x, x, 1) + x, x, 0)
;; fast program: loop(loop((loop(2 + ((x div y) + x), x, 1) + x) div (1 + x), 1, x div 2) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ 2 (+ (div x y) x)))))
(assert (forall ((x Int)) (= (g5 x) x)))
(assert (= h5 1))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) h5))))
(assert (forall ((x Int)) (= (f4 x) (div (+ (v5 x) x) (+ 1 x)))))
(assert (= g4 1))
