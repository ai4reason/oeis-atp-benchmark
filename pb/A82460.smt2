;; sequence(s): A82460
;; terms: 0 1 1 1 2 1 3 1 3 1 4 1 5 1 5 1 6 1 7 1
;; small program: loop(loop(loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, y, 1) - x, x, 0)
;; fast program: (loop((loop(loop((((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, x div 2, 1) * x) - 1, 1 - (x mod 2), x) + 1) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (+ 1 (v3 x)) (+ 1 x)) 0) 1 0))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v4 x) 1) (+ 1 x)))))
