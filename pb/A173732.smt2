;; sequence(s): A173732
;; terms: 0 2 0 5 3 8 2 11 6 14 0 17 9 20 5 23 12 26 3 29
;; small program: loop(x div (1 + (x mod 2)), x, 1 + ((x + x) + x)) div 2
;; fast program: loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + (2 + 2), x mod 2, 1), x), 1, x div 2), x mod 2, x) div 2, (1 + x) mod 2, (x + x) - (x div 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h1 x) (- (+ x x) (div x 2)))))
