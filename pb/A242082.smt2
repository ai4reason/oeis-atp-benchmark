;; sequence(s): A242082
;; terms: 0 1 0 1 2 0 1 0 1 2 0 1 2 0 1 0 1 2 0 1
;; small program: loop(loop(loop((loop(((y * y) div (2 + x)) + y, 1 + x, 0) - x) - 1, 2, x) - x, 2, x), 2, 0 - x) + x
;; fast program: loop((loop(((loop((((((x div (2 + (2 * (2 * (2 + 2))))) - x) div (2 + 2)) + x) mod 2) + x, 2, x) + 1) + x) mod 2, 1, y) * x) + 1, x, 1) - 1
(set-logic UFNIA)
