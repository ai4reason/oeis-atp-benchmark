;; sequence(s): A10434
;; terms: 0 1 2 3 4 6 8 9 12 16 18 19 23 24 25 27 32 35 36 37
;; small program: compr(loop(((loop(x * x, 2, x) * x) * x) mod (1 + (2 * loop(x * x, 1, 2 + (2 + 2)))), 2, x) - 1, x)
;; fast program: compr(loop(loop((x * x) * x, 1, x * x) mod (1 + (2 * loop(x * x, 1, 2 + (2 + 2)))), 2, x) - 1, x)
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f1 x) (mod (* (* (v2 x) x) x) (+ 1 (* 2 v3))))))
(assert (= g1 2))
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (f5 x) (mod (v6 x) (+ 1 (* 2 v7))))))
(assert (= g5 2))
