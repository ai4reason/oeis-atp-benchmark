;; sequence(s): A306192
;; terms: 0 5 14 33 52 85 114 161 232 279 370 451 516 611 742 885 976 1139 1278 1387
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) * x) + x
;; fast program: (((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 1 + x) * (if x <= 0 then 1 else 2)) + 1) * x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (* (v2 x) (ite (<= x 0) 1 2)) 1) x) 1))))
