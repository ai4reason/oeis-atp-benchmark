...
;; small program: loop(loop(x div 2, x, loop2(y, x + y, y, 2, 1)) + x, x, 1)
;; fast program: ((((((if x <= 0 then 1 else x) - 1) div (2 * (2 + (2 + 2)))) + x) + x) div (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (+ (div (- (ite (<= x 0) 1 x) 1) (* 2 (+ 2 (+ 2 2)))) x) x) (+ 1 2)) 1))))
