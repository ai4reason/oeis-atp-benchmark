;; sequence(s): A97957
;; terms: 3 7 13 17 23 37 43 47 53 67 73 83 97 103 107 113 127 137 157 163
;; small program: (compr((1 + loop2(x * y, y, x, 1, 1 + (2 + 2))) mod (1 + (x + x)), 1 + x) * 2) + 1
;; fast program: compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), ((((1 + x) div (1 + (2 + 2))) + x) div 2) mod 2, x), 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g3 x) (mod (div (+ (div (+ 1 x) (+ 1 (+ 2 2))) x) 2) 2))))
