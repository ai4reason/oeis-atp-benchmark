;; sequence(s): A37168
;; terms: 2 4 8 12 20 24 32 36 44 56 60 72 80 84 92 104 116 120 132 140
;; small program: loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) * 2
;; fast program: (compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), if x <= 0 then 1 else x) * 2) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) 2) (ite (<= x 0) 1 2)))))
