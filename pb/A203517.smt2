;; sequence(s): A203517
;; terms: 1 4 96 15360 17203200 138726604800 8203736501452800 3603868630142209228800 11873738053102139590311936000 295578185800614925763054760099840000 55920479534877093093661639943174183976960000
;; small program: loop(loop(2 * (2 * ((x - (x div y)) + x)), y, x), x, 1)
;; fast program: (loop(loop(loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1), 1, y) * x, x - 1, 1) * loop2(x * y, 1 + y, x, 1, 1 + x)) * loop2(x * y, y + y, x, 1, 2)
(set-logic UFNIA)
