;; sequence(s): A317335
;; terms: 1 1 ~2 1 1 ~2 ~2 1 1 1 ~2 ~2 1 ~2 ~2 1 1 1 ~2 1
;; small program: loop(1 - x, loop(x div (1 + (x mod 2)), x, x) div 2, 2) - 1
;; fast program: if (loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) mod 2) <= 0 then 1 else (0 - 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (div (v1 x) 2))))
(assert (= h0 2))
...
(assert (forall ((x Int)) (= (fast x) (ite (<= (mod (v2 x) 2) 0) 1 (- 0 2)))))
