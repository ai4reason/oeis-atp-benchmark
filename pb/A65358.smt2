;; sequence(s): A65358
;; terms: 0 1 0 1 2 1 0 1 2 3 4 3 2 3 4 5 6 5 4 5
;; small program: (loop(loop(loop(loop(if ((2 - loop(x + x, x, 2)) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x - 1, 1) mod 2, 1, y) + x, x, 0) * 2) - x
;; fast program: (loop(loop(loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod 2, 1, y) + x, x - 1, 1) - x, 2, x) * 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (ite (<= (- (mod (+ 1 (v10 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g9 1))
...
(assert (forall ((x Int)) (= (fast x) (- (* (v5 x) 2) x))))
