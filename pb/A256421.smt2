;; sequence(s): A256421
;; terms: 1 3 4 5 6 7 9 10 11 13 14 15 17 19 21 22 23 25 26 27
;; small program: loop((loop(((loop(x * y, x, x) mod (1 + x)) + 1) mod 2, 1, x div 2) + 1) + x, x, 0) + 1
;; fast program: loop((if loop(loop(((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 0) mod (1 + x)) mod 2, 1, x div 2), x mod 2, x) <= 0 then 2 else 1) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (mod (ite (<= (- (v6 x) 2) 0) 1 0) (+ 1 x)) 2))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (f3 x) (+ (ite (<= (v4 x) 0) 2 1) x))))
