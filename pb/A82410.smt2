;; sequence(s): A82410
;; terms: 0 1 1 0 1 1 0 0 1 1 1 0 0 1 0 0 1 1 1 0
;; small program: loop(loop(1 - (x + y), x, x), x, x) mod 2
;; fast program: loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) mod 2, 1, (0 - 1) - x)
(set-logic UFNIA)
...
(assert (= g2 1))
(assert (forall ((x Int)) (= (h2 x) (- (- 0 1) x))))
