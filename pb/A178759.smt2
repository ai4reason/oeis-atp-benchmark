;; sequence(s): A178759
;; terms: 0 0 0 18 144 750 3240 12642 46368 163350 559800 1881066 6229872 20406750 66273480 213759090 685601856 2188698150 6959413080 22053083514
;; small program: loop2((x - 2) + y, 2 * ((y - x) + y), x, 1, 1) * x
;; fast program: (((1 - loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) * (1 + 2)) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) * x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (- 1 (v1 x)) (+ 1 2)) (w4 x)) x))))
