;; sequence(s): A66332
;; terms: 1 1 2 6 24 30 180 210 1680 1890 2100 2310 27720 30030 420420 450450 480480 510510 9189180 9699690
;; small program: loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, x - 2, if x <= 0 then 1 else x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x - 2, if x <= 0 then 1 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (ite (<= x 0) 1 x))))
