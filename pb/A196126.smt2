;; sequence(s): A196126
;; terms: 1 2 4 7 10 14 19 25 32 39 47 56 66 77 89 102 115 129 144 160
;; small program: loop(loop((if ((y * y) - x) <= 0 then 0 else 1) + x, x, y), x, 1) + x
;; fast program: (loop(loop(loop(loop((y div (x * x)) + x, x, 1) - x, 1, y) + x, x, 0) - x, 2, x) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (ite (<= (- (* y y) x) 0) 0 1) x))))
