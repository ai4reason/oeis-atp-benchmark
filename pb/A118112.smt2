;; sequence(s): A118112
;; terms: 1 0 0 0 3 0 0 0 5 0 0 0 0 0 0 0 9 0 0 0
;; small program: (loop2(x * y, 1 + y, x, 1, 2 * (2 + x)) div loop(x * y, x, 1)) mod (2 + x)
;; fast program: (loop(((loop(loop((loop2((((y div 2) * x) * y) + x, y div 2, 1 + (2 + 2), 1, x) mod 2) + x, (1 + x) mod 2, x) - x, 1, x div 2) + x) mod 2) + x, (1 + x) mod 2, x) - x) * (1 + (x div 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (div (w0 x) (v1 x)) (+ 2 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (mod (w5 x) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (v2 x) x) (+ 1 (div x 2))))))
