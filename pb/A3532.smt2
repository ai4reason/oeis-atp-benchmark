;; sequence(s): A3532
;; terms: 1 3 5 7 9 13 15 17 21 35 39 45 51 63 65 85 91 105 117 119
;; small program: loop(compr((loop(x * x, 1 + 2, 2 * (2 + 2)) + x) mod (1 + x), y), x, 0) + 1
;; fast program: ((((compr((loop(x * x, 1 + 2, 2 * (2 + 2)) - 1) mod (1 + x), x) + 1) * (2 + 2)) div 2) + 1) div 2
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (- v4 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (* (+ (v3 x) 1) (+ 2 2)) 2) 1) 2))))
