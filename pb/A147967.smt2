;; sequence(s): A147967
;; terms: 0 2 3 12 5 18 7 24 45 10 55 36 13 42 75 80 17 90 57 20
;; small program: (loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) - x, 1 + y, 2, 0, x) - 1) * (1 + x)
;; fast program: (loop2(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) - x, 1 + y, 2, 0, x) - 1) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- (w0 x) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (w3 x) 1) (+ 1 x)))))
