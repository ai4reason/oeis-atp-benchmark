;; sequence(s): A18645
;; terms: 1 2 4 7 8 14 16 28 49 56 98 112 196 392 784
;; small program: loop(compr(loop((y - x) * (y * y), 2 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop2(compr(loop(x * x, 1, 2 * (2 + (2 * (2 + (2 + 2))))) mod (1 + x), y) + x, y, 2, 2, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (- y x) (* y y)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 1))
