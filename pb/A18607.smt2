;; sequence(s): A18607
;; terms: 1 5 11 13 55 65 143 715
;; small program: loop(compr(loop(1 + (2 * (((2 * y) + x) * x)), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + (2 + x)) * (1 + (x * x)), 1, 2 * (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (* 2 (* (+ (* 2 y) x) x))))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (+ 2 x)) (+ 1 (* x x))))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
