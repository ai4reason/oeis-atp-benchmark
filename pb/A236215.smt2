;; sequence(s): A236215
;; terms: 512 20195 1973320 42326927 2400274618 13004773991 131592650488 454280348267 2255433009730 16762578985599 43202201146270 173163940941347 500545875335308 1003138487272151 2122268960374918 5422032552177051 14085028370831990 25779174463666131
;; small program: loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 1 + (2 * (2 + 2)), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x * x, 2, x * x), y, 2, 1, x - 1), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (v7 x y) 1) (v10 x)))))
