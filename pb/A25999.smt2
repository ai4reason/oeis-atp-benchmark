;; sequence(s): A25999
;; terms: 1 26 445 6370 82901 1019746 12105885 140404290 1603014501 18104952866 202945103725 2262802497410 25134485221301 278430633932386 3078357517755965 33986947913921730 374856803115095301
;; small program: loop((loop((loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
