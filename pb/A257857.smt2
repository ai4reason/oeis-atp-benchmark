;; sequence(s): A257857
;; terms: 2 1 1 0 2 0 1 1 1 1 2 0 2 0 2 1 1 1 1 1
;; small program: (loop(loop(x + y, x, y) div x, x, 1) mod loop(0 - x, x, 2)) + 1
;; fast program: (((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x - 1) + x) mod 2) - (x mod 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (mod (+ (w3 x) x) 2) (mod x 2)) 1))))
