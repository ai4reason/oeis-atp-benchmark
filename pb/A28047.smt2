;; sequence(s): A28047
;; terms: 1 26 441 6214 79157 948090 10903777 121902638 1335052653 14395752514 153368576153 1618401466422 16946521522789 176328040607498 1825046893884369 18806371581048766 193066856641062365
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (x + x)), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * (x + x)), x, 1), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
