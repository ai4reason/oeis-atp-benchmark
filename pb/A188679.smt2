;; sequence(s): A188679
;; terms: 1 10 235 7291 252316 9270325 353892421 13874930821 554792522662 22521121103287 925224047453512 38381686035811912 1605078750713101912 67578873844051699048 2861782692234129345448 121795323921169907086504
;; small program: loop(loop(loop2((x * y) * y, 1 + y, x, 1, 1 + (x + x)) div loop((y * x) * y, x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop2(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * (1 + (x + x)), 1, y) * x, y, 2, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (div (w7 x) (v8 x)) (+ 1 (+ x x))))))
(assert (= g6 1))
