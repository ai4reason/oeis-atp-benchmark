;; sequence(s): A18493
;; terms: 1 2 4 127 254 508
;; small program: loop(compr((2 * (loop(x * x, 2, 2 + 2) - 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(loop(((2 * ((x * x) * x)) - 1) - x, 1 - (x mod (2 + 2)), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* 2 (- v2 2)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (* 2 (* (* x x) x)) 1) x))))
