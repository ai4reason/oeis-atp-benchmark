;; sequence(s): A28815
;; terms: 2 3 4 6 8 12 14 18 20 24 30 32 38 42 44 48 54 60 62 68
;; small program: loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y), x, 2, x, x) + 2
;; fast program: (((compr(loop((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, (1 + x) mod 2, x div 2), x) * 2) + 1) div 2) + 2
(set-logic UFNIA)
