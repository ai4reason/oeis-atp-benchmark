;; sequence(s): A18743
;; terms: 1 3 11 29 33 87 319 957
;; small program: loop(compr((1 + loop((2 + y) * (((x * x) + x) - 1), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 + (2 + x)) * x, 1, 2 + loop((x * x) * x, 1, 1 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 y) (- (+ (* x x) x) 1)))))
(assert (= g2 2))
(assert (= h2 2))
