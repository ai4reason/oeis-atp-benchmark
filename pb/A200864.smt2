;; sequence(s): A200864
;; terms: 1 7 42 230 1211 6237 31732 160300 806421 4046867 20278622 101525970 508028431 2541337897 12710276712 63562145240 317843011241 1589311911327 7946850122002 39735122306110
;; small program: loop(((loop2((2 * (2 * (x + y))) + y, x, y, 1, 0) + x) + x) + x, x, 1)
;; fast program: (loop(((((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), (2 * (2 * (2 + (2 + 2)))) + 1) div (1 + 2)) * 2) + 1) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2, 1, 2 + x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (div (- (+ (* (div (w3 x) (+ 1 2)) 2) 1) (w5 x)) 2))))
(assert (= g2 1))
