;; sequence(s): A2808-A140347
;; terms: 4 6 8 9 10 12 14 15 16 18 20 21 22 24 25 26 27 28 30 32
;; small program: compr(loop(x * y, x, 2) mod (1 + x), 2 + x) + 1
;; fast program: loop(((compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, (x - 1) div 2, x) mod (1 + x), 1 + x) + 1) * 2) div 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (ite (<= (- (v5 x) 2) 0) (+ 1 x) 1) 1))))
...
(assert (forall ((x Int)) (= (f2 x) (div (* (+ (v3 x) 1) 2) 2))))
(assert (= g2 1))
