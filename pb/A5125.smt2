;; sequence(s): A5125
;; terms: 1 2 4 5 7 8 13 14 19 20 22 23 25 29 34 35 37 40 44 47
;; small program: (compr((1 + loop2(x * y, y, x, 1, 2 + 2)) mod (1 + (2 * (x + x))), 1 + x) div 2) + 1
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) div 2) + 1) - x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ 1 (w1 x)) (+ 1 (* 2 (+ x x)))))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (+ 1 (v4 x)) (+ 1 (+ x x))) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (+ (div (v2 x) 2) 1) x) x))))
