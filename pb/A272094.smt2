;; sequence(s): A272094
;; terms: 1 1 6 504 917280 48735086400 94925811409228800 8154182636726616909619200 36091760791026276649159689107865600 9415901310649088228943246038670339934863360000 162992165498634702043940163611264755298214594247272038400000
;; small program: loop(loop(loop2(x * y, y - 1, x, 1, x * x) div loop(x * y, x, 1), 1, y) * x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x, 1, (x * x) + x) div loop(x * y, x, 1)) * (2 + x), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ (* x x) x))))
