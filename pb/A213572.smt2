;; sequence(s): A213572
;; terms: 1 13 82 406 1809 7659 31588 128476 518611 2084809 8361918 33497010 134094757 536608663 2146926472 8588754808 34357247847 137433710421 549744803650 2199000186670
;; small program: ((loop(((((loop(loop2(((y * y) + x) + x, y + y, x, 1, 2), 1, y) + x) + x) + x) - x) + 1, x, 0) * 2) + 1) - loop(x + y, x, x)
;; fast program: (((((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2))) - 1) - 2) - x) * loop(x + x, x, 2)) - 1) - loop(x + y, x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (* y y) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- (+ (+ (+ (v1 x y) x) x) x) x) 1))))
...
(assert (forall ((x Int)) (= (small x) (- (+ (* (v0 x) 2) 1) (v3 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (- (* (- (- (- (v4 x) 1) 2) x) (v7 x)) 1) (v8 x)))))
