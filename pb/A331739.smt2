;; sequence(s): A331739
;; terms: 0 1 0 3 0 3 0 7 0 5 0 9 0 7 0 15 0 9 0 15
;; small program: x - loop(x div (1 + (x mod 2)), x, x)
;; fast program: (loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2) * (0 - 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (v1 x) (- 0 2)) x))))
