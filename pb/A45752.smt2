;; sequence(s): A45752
;; terms: 4 7 9 10 13 14 16 19 22 23 24 25 28 29 30 31 34 36 37 39
;; small program: (compr(loop(loop(x * y, x - (x div (1 + 2)), 1) mod (1 + (x + x)), x mod 2, x), 1 + x) div 2) + 1
;; fast program: (((compr(loop((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0) mod (1 + x), x mod 2, x) mod (1 + x), 1 + x) + 1) div 2) * 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 1 0) (+ 1 x)))))
(assert (forall ((x Int)) (= (g4 x) (mod x 2))))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (mod (v4 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (+ (v3 x) 1) 2) 2) 2))))
