;; sequence(s): A28088
;; terms: 1 32 667 11522 179473 2622284 36714679 498853814 6630652765 86676445856 1118432963011 14283392713226 180889234319977 2275070671007348 28449403226477263 354027885991627358
;; small program: loop(loop(loop2(((2 * (y * y)) - y) + x, y + y, x, 1, 2) * loop((x + x) + x, x, 1), 1, y) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (v1 x y) (- (v4 x) x)))))
...
(assert (forall ((x Int)) (= (f6 x) (* (div (div (v7 x) 2) (+ 1 2)) (w9 x)))))
(assert (= g6 1))
