;; sequence(s): A140155
;; terms: 1 17 44 300 425 1721 2064 6160 6889 16889 18220 38956 41153 79569 82944 148480 153393 258369 265228 425228
;; small program: loop(loop(loop2((x * y) + x, y, 1 + (2 + (x mod 2)), 1, x), 1, y) + x, x, 1)
;; fast program: loop(loop(((1 + loop((2 + x) * x, x mod 2, x)) * (1 + x)) * (1 + x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (* (+ 1 (v5 x)) (+ 1 x)) (+ 1 x)))))
(assert (= g4 1))
