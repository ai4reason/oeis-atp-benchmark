;; sequence(s): A18411
;; terms: 1 3 7 17 21 51 119 357
;; small program: loop(compr((1 + loop(2 - (2 * (x + x)), 2 + 2, x)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((2 * ((x * x) * x)) + x, 1, 2 + (2 + x)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (v4 x) (+ 1 (+ x x))))))
