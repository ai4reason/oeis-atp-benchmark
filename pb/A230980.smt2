;; sequence(s): A230980
;; terms: 0 0 1 2 2 3 3 4 4 4 4 5 5 6 6 6 6 7 7 8
;; small program: loop((loop((x * y) * y, y - 1, 1) mod y) + x, x, 0)
;; fast program: (loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, (x - 1) div 2, 1) - x) mod (if x <= 0 then 1 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (mod (v1 x y) y) x))))
...
(assert (forall ((x Int)) (= (fast x) (mod (- (v2 x) x) (ite (<= x 0) 1 x)))))
