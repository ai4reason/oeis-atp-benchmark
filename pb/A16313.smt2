;; sequence(s): A16313
;; terms: 1 19 257 3047 33825 361767 3782353 38976439 397835009 4034844935 40743914289 410207399511 4121451795553 41350162566823 414451137963665 4151157965737463 41558105760145857 415906740320988231
;; small program: loop((loop2((2 * ((2 * (x + x)) + x)) + y, y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop((loop2(x * y, y, x, 1 + (2 + 2), 1 + (2 + 2)) div 2) * loop(x + x, x - 1, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (div (w4 x) 2) (v5 x)))))
(assert (= g3 1))
(assert (forall ((x Int) (y Int)) (= (h3 x y) y)))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int) (y Int)) (= (v3 x y) (u3 g3 (h3 x y)))))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (v3 x y) x) (* (+ (+ x x) x) 2)))))
