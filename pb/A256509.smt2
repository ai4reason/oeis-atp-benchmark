;; sequence(s): A256509
;; terms: ~1 ~1 ~1 713 620448401733239439359975
;; small program: loop((loop(x * y, x, 1) - (x * y)) + x, 2, x) - 1
;; fast program: (loop(loop2(x * y, 1 + y, x, 1, 1 + x) * loop(x * y, x, 1), 1, loop(x * y, x, 1) div 2) - 1) - loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h2 x) (div (v5 x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (- (v2 x) 1) (v6 x)))))
