;; sequence(s): A4648
;; terms: 0 1 2 3 1 1 3 3 5 9 9 1 2 1 2 5 8 7 10 11
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (1 + x)
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(loop((2 * (x + x)) + x, 2, x), 2, 2)) mod (1 + x)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (mod (mod (+ (v2 x) 1) v5) (+ 1 x)))))
