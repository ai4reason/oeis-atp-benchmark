;; sequence(s): A121566
;; terms: 1 1 2 6 3 15 15 105 105 105 105 1155 1155 15015 15015 15015 15015 255255 255255 4849845
;; small program: loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, x - 1, 1) div (if ((x - 1) - 2) <= 0 then 1 else 2)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x - 1, 1) div (if ((x - 1) - 2) <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (v0 x) (ite (<= (- (- x 1) 2) 0) 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (div (v3 x) (ite (<= (- (- x 1) 2) 0) 1 2)))))
