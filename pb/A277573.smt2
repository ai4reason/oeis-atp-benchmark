;; sequence(s): A277573
;; terms: 1 3 5 6 7 9 11 13 15 17 18 19 21 23 25 27 29 30 31 33
;; small program: loop(compr((loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), 2, x) + 1) mod 2, y), x, 1)
;; fast program: loop(((loop((loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (1 + 2)) - 1, x) mod 2) + 1) + x, x, 1)
(set-logic UFNIA)
(declare-fun f3 () Int)
...
(assert (forall ((x Int)) (= (g5 x) (- (mod x (+ 1 2)) 1))))
