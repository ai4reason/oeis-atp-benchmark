;; sequence(s): A304120
;; terms: 2 15 305 6479
;; small program: (((loop((x * x) + x, 1, loop(x + x, (x + x) + x, 2)) div 2) div (1 + 2)) + 1) div loop2(x * y, y, x, 1, 1 + 2)
;; fast program: (((loop((x * x) + x, 1, loop2(x * y, y, x, 2, 2 * (2 + 2))) div 2) div (1 + 2)) + 1) div loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (div (div (v0 x) 2) (+ 1 2)) 1) (w2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (div (v3 x) 2) (+ 1 2)) 1) (w5 x)))))
