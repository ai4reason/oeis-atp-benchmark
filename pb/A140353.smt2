;; sequence(s): A140353
;; terms: 11 12 14 16 20 22 26 28 32 38 40 46 50 52 56 62 68 70 76 80
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 2) + x, 2, 2, 2, x)
;; fast program: ((((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) + 2) + 2) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 2) 2) 2) 2))))
