;; sequence(s): A216729
;; terms: 1 1 140 168000 812011200 11021058048000 339782903171712000 20692894514095964160000 2254632317437500000000000000 408212019690599470111653888000000 115985264066835726820369478446080000000 49409573278650211500346024173376634880000000 30404587048380414443886075636144408738201600000000
;; small program: (loop(x * y, (x + x) + x, 1) div loop(2 * ((x + x) + x), x, 1)) * loop2(x * y, y, x - 1, 1, 1 + ((x + x) + x))
;; fast program: (loop(x * y, (x + x) + x, 1) div loop2(x * y, y, x, 1, 2 + (2 + 2))) * loop2(x * y, y, x - 1, 1, 1 + ((x + x) + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (v0 x) (v1 x)) (w2 x)))))
...
(assert (= i5 1))
(assert (forall ((x Int)) (= (j5 x) (+ 1 (+ (+ x x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (v3 x) (w4 x)) (w5 x)))))
