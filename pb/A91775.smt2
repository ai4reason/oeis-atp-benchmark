;; sequence(s): A91775
;; terms: 30 354 4890 72354 1108650 17312754 273234810 4338079554 69107159370 1102999460754 17623571298330 281757423024354 4506141560307690 72080471098818354 1153127396812683450 18448597098193370754 295164582378232361610 4722516577573661689554
;; small program: loop(loop((x + x) + x, x, 1) + loop((x * x) + x, 1, loop(x + x, x, 1)), 1, 2 + (x + x)) + 1
;; fast program: loop((1 + loop((x * x) + x, 1, loop(x + x, x, 1))) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))), 1, 2 + (x + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ (+ 1 (v5 x)) (w7 x)))))
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ 2 (+ x x)))))
