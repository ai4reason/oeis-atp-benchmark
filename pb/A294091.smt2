;; sequence(s): A294091
;; terms: 5 11 27 35 59 75 83 107 123 147 179 195 203 219 227 275 299 315 347 363
;; small program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), loop(y - x, x, 1) mod 2, x) - 1, 1 + x) * 2) + 1) + 2
;; fast program: ((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), 1 - (x mod (2 + 2)), x) - 1, 1 + x) * 2) + 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g1 x) (mod (v3 x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v4 x) 2) 1) 2))))
