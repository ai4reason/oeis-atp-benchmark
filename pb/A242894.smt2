;; sequence(s): A242894
;; terms: 5 10 10 20 20 25 35 40 40 45 45
;; small program: ((loop(((compr(loop(x * y, x, 1 + 2) mod (1 + (x + x)), 2 + x) - 1) - 2) - x, 1, 2 + x) - 2) - 2) * (1 + (2 + 2))
;; fast program: (((loop((compr(((loop(if (((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + x)) - 1) mod (1 + x)) <= 0 then 1 else 0, 1, x + x) + 1) div 2) mod 2, x) - 1) - x, 1, 2 + x) - 2) - 2) - 2) * (1 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (- (- (v1 x) 1) 2) x))))
(assert (= g0 1))
(assert (forall ((x Int)) (= (h0 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y))))))
(assert (forall ((x Int)) (= (v0 x) (u0 g0 (h0 x)))))
(assert (forall ((x Int)) (= (small x) (* (- (- (v0 x) 2) 2) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (mod (- (mod (* (+ 1 (mod x 2)) (v6 x)) (+ 1 x)) 1) (+ 1 x)) 0) 1 0))))
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ x x))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (mod (div (+ (v5 x) 1) 2) 2))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (- (- (v3 x) 2) 2) 2) (+ 1 (+ 2 2))))))
