;; sequence(s): A167021
;; terms: 1 1 1 0 1 1 1 0 0 1 1 1 1 0 0 1 1 1 0 0
;; small program: loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x), 1, 1 + x) mod 2
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x), 1, 1 + x) mod 2
(set-logic UFNIA)
