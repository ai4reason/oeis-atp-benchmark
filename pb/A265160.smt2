;; sequence(s): A265160
;; terms: 4 7 13 23 43 77 145 275 535 1053 2079 4133 8233 16427 32815 65589 131131 262205 524355 1048647
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) + loop(x + x, x, 2)
;; fast program: ((((((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) + loop(x + x, x, 2)) + 1) + (x mod 2)) div 2) * 2) + 1) - (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (div (+ (+ (+ (+ (* (v3 x) 2) 1) (v7 x)) 1) (mod x 2)) 2) 2) 1) (ite (<= x 0) 1 2)))))
