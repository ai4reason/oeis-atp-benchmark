;; sequence(s): A18298
;; terms: 1 3 5 9 15 27 45 135
;; small program: compr(loop(1 + (2 * (x * y)), 2 + 2, x) mod (1 + x), x) + 1
;; fast program: (compr((loop(((x * x) * x) + x, 1, 2 + 2) + x) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (+ v3 x) (+ 1 (+ x x))))))
