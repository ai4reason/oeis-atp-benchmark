;; sequence(s): A18453
;; terms: 1 2 7 14 31 62 217 434
;; small program: loop(compr((2 + loop((1 + 2) * (x * x), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + loop((x * x) * x, 1, 2 + (2 + 2))) * (1 + (x mod 2))) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 v4) (+ 1 (mod x 2))) (+ 1 x)))))
