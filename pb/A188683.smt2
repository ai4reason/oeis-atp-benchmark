;; sequence(s): A188683
;; terms: 1 2 43 965 26260 793559 25715833 875686727 30942995146 1125179561729 41860674073996 1586681151506804 61081201435584796 2382392690910289172 93969463115644112428 3742596382979058395348
;; small program: loop(loop((loop2((x * y) * y, 1 + y, x, 1, 1 + (x + x)) div loop((y * x) * y, x, 1)) div (1 + (x + x)), 1, y) - x, x, 1)
;; fast program: loop(loop((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x - 1, x)) * loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x))) div loop(x * y, x, 1)) * (1 + (x + x)), 1, y) - x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (div (* (div (w6 x) (v7 x)) (w8 x)) (v9 x)) (+ 1 (+ x x))))))
(assert (= g5 1))
