;; sequence(s): A121538
;; terms: 1 2 4 6 7 8 10 11 12 14 16 18 19 20 22 24 26 27 28 30
;; small program: compr(1 - loop(loop((y mod 2) - x, x, 1), x + x, x), x) div 1
;; fast program: loop((((((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) - 2) - 1) mod (1 + x)) mod 2) + 1) + x, x, 1 + 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (+ (+ (mod (mod (- (- (v4 x) 2) 1) (+ 1 x)) 2) 1) x))))
