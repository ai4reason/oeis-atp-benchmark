;; sequence(s): A322015
;; terms: 0 0 3 0 0 6 7 0 0 0 11 12 0 14 15 0 0 0 19 0
;; small program: (1 + x) * ((loop(x div (1 + (x mod 2)), x, x) div 2) mod 2)
;; fast program: (loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) mod 2) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ 1 x) (mod (div (v0 x) 2) 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (mod (v1 x) 2) (+ 1 x)))))
