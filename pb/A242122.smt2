;; sequence(s): A242122
;; terms: 2 3 5 7 11 13 17 19 1 7 9 15 19 21 3 9 15 17 1 5
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 + loop(2 * (2 + x), 2, 2))) + 1
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop((loop(1 + y, x, x) + x) + x, 2, 2)) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ 1 y))))
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (fast x) (* (mod (+ (v3 x) 1) v6) 1))))
