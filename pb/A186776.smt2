;; sequence(s): A186776
;; terms: 0 2 3 5 9 11 12 14 27 29 30 32 36 38 39 41 81 83 84 86
;; small program: (compr(loop(if (((x + x) + x) mod 2) <= 0 then (x div (1 + 2)) else 1, x, x) mod 2, x) div 2) + (x mod 2)
;; fast program: ((loop(((((loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + (x mod 2)) + x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ (+ x x) x) 2) 0) (div x (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (small x) (+ (div (v0 x) 2) (mod x 2)))))
(assert (forall ((x Int)) (= (f7 x) (- (* 2 (* x x)) x))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (+ (v2 x) (mod x 2)) x) 1))))
