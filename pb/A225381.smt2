;; sequence(s): A225381
;; terms: 1 2 2 4 3 5 4 8 5 8 6 11 7 11 8 16 9 14 10 18
;; small program: 1 + (x - (loop(x div (1 + (x mod 2)), x, x) div 2))
;; fast program: ((loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2) * (0 - 1)) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (- x (div (v0 x) 2))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v1 x) (- 0 1)) 1) x))))
