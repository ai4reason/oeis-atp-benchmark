;; sequence(s): A50486
;; terms: 1 9 44 156 450 1122 2508 5148 9867 17875 30888 51272 82212 127908 193800 286824 415701 591261 826804 1138500
;; small program: loop(loop(loop(loop(loop((y * y) + x, y, x), y, x), y, x), 1 + y, x), x, 1)
;; fast program: loop(((2 * ((x + x) + x)) div y) + x, x, 1 + (2 + (2 * (2 + x)))) div (1 + (2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div (* 2 (+ (+ x x) x)) y) x))))
(assert (forall ((x Int)) (= (g5 x) x)))
(assert (forall ((x Int)) (= (h5 x) (+ 1 (+ 2 (* 2 (+ 2 x)))))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (fast x) (div (v5 x) (+ 1 (+ 2 (+ 2 2)))))))
