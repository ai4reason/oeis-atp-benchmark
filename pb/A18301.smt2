;; sequence(s): A18301
;; terms: 1 2 4 5 7 10 14 20 28 35 70 140
;; small program: compr(loop((2 + (x + y)) * x, 2, 2) mod (1 + x), x) + 1
;; fast program: compr(((loop(x * x, 1, 2 * (2 + (2 + 2))) - 2) - 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (- (- v3 2) 2) (+ 1 x)))))
