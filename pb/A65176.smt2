;; sequence(s): A65176
;; terms: 0 2 2 4 4 2 2 8 8 2 2 4 4 2 2 16 16 2 2 4
;; small program: loop(1 + loop(if x <= 0 then y else (x - 1), x, 0), x, x)
;; fast program: loop(loop(loop2(1 + (y mod (x + x)), y, x, 1, x) mod (2 + x), x mod 2, x) - 1, 2, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (ite (<= x 0) y (- x 1)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ 1 (mod y (+ x x))))))
