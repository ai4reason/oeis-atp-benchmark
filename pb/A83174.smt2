;; sequence(s): A83174
;; terms: 2 9 15 42 55 117 119 228 207 435 341 666 533 903 705 1272 1003 1647 1273 2130
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y mod 2, 2, 1 + x, x) div 2
;; fast program: (((compr(((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2, x mod 2, 1 + x) + x) mod 2) mod (1 + x), x) + 2) * (1 + x)) * (2 + (x mod 2))) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g0 x y) (mod y 2))))
(assert (= h0 2))
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (+ (v4 x) x) 2) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (+ (v3 x) 2) (+ 1 x)) (+ 2 (mod x 2))) 2))))
