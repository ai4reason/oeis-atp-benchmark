;; sequence(s): A126680
;; terms: 1 10 650 227500 386977500 3006815175000 102547431543375000 14946288147446906250000 9139670148451930618781250000 23146488841058967849982079062500000 240532409681630323860212020187339062500000 10187978717854649915906947316453923964296875000000 1750283547138817933292555967231683354203766362734375000000
;; small program: loop(loop(((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + x) + x) + x, y, 1) * x, x, 1)
;; fast program: loop(loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (+ (+ (w2 x y) x) x) x))))
