;; sequence(s): A267584
;; terms: 1 2 4 2 8 4 4 2 16 8 8 4 8 4 4 2 32 16 16 8
;; small program: loop2((loop(loop(y, x - 1, x) mod 2, 1, y) * x) + x, y div 2, x, 1, x)
;; fast program: loop2((loop((loop(y, x mod 2, 2) + x) mod (1 + x), 1, y) * x) + x, y div 2, 2 + (2 + 2), 1, x) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (w3 x) (ite (<= x 0) 1 2)))))
