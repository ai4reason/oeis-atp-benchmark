;; sequence(s): A293137
;; terms: 0 1 1 2 3 3 3 4 4 5 5 5 5 6 6 6 7 7 7 7
;; small program: loop((y - 1) div (2 + x), 2 * (x + x), 0)
;; fast program: ((loop((loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, (x - 1) div 2) + 1) + x, 2, x) - 2) - (if x <= 0 then 1 else 2)) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (v1 x) 2) (ite (<= x 0) 1 2)) x))))
