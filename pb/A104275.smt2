;; sequence(s): A104275
;; terms: 1 5 8 11 13 14 17 18 20 23 25 26 28 29 32 33 35 38 39 41
;; small program: loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) + 1
;; fast program: (compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) + 1
(set-logic UFNIA)
