;; sequence(s): A18667
;; terms: 1 3 5 11 15 25 33 55 75 165 275 825
;; small program: loop(compr((1 + loop((2 * (2 * (2 + x))) - y, 2 + 2, 1)) mod (1 + x), y), x, 0) + 1
;; fast program: compr((((1 + (2 * (2 * (2 * (2 + 2))))) * loop(x * x, 1, 2 + (2 * (2 + 2)))) mod (1 + x)) + (x mod 2), x) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (+ (mod (* (+ 1 (* 2 (* 2 (* 2 (+ 2 2))))) v4) (+ 1 x)) (mod x 2)))))
