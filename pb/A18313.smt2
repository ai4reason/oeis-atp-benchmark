;; sequence(s): A18313
;; terms: 1 3 5 11 15 33 55 165
;; small program: compr((1 + loop((2 * (x * x)) + y, 2, 2)) mod (1 + x), x) + 1
;; fast program: (compr(((1 + 2) * (loop((x * x) * x, 1, 1 + 2) - x)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (* (+ 1 2) (- v3 x)) (+ 1 (+ x x))))))
