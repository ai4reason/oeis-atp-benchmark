;; sequence(s): A175224
;; terms: 10 11 13 15 19 21 25 27 31 37 39 45 49 51 55 61 67 69 75 79
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 2) + x, 2, 2, 1, x)
;; fast program: (((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) + (if x <= 0 then 2 else 1)) + 2) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (* (v3 x) 2) 2) (ite (<= x 0) 2 1)) 2) 2) 2))))
