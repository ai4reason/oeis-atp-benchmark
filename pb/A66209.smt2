;; sequence(s): A66209
;; terms: 0 0 2 0 0 2 6 0 8 0 10 8 12 4 5 0 16 8 18 0
;; small program: loop(2 * ((2 * (x + x)) + x), x, x) mod (1 + x)
;; fast program: ((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), (2 * (2 * (2 + (2 + 2)))) + 1) * loop(x + x, x, 1)) * x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (* (* (w1 x) (v3 x)) x) (+ 1 x)))))
