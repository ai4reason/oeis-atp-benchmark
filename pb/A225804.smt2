;; sequence(s): A225804
;; terms: 2 3 4 5 6 7 9 10 12 13 15 17 19 21 22 24 26 28 30 32
;; small program: ((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) div (1 + x)) + 2
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) div (1 + x)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (v3 x) 2) (+ 1 x)) 2))))
