;; sequence(s): A71532-A73635
;; terms: 1 0 1 2 3 4 5 6 5 6 5 6 5 6 7 6 7 8 7 8
;; small program: ((loop(loop(loop(x div 2, x, loop(1 + ((x + x) + x), x, 1)) mod 2, 1, y) + x, x, 0) * 2) - x) + 1
;; fast program: ((loop(loop((((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div loop(x + x, x, 2)) mod 2, 1, y) + x, x, 0) * 2) - x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (div (* (+ 1 2) (w6 x)) (v8 x)) 2))))
(assert (= g5 1))
