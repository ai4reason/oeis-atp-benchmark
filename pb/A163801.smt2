;; sequence(s): A163801
;; terms: 0 1 2 2 2 3 4 5 6 6 6 7 8 8 8 9 10 11 12 12
;; small program: loop(loop(compr(loop(x div (if (x mod (2 + 2)) <= 0 then 1 else 2), x, x), (1 + x) div 2) mod 2, 1, y) + x, x, 0)
;; fast program: (compr((loop((((loop((((loop(1 + ((y * y) div (x + y)), x div 2, 1) + x) mod 2) + (x mod 2)) + x, 2, x) + x) mod 2) + (x mod 2)) + x, 2, x) + x) mod 2, 1 + x) - 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ 1 (div (* y y) (+ x y))))))
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ (mod (+ (v7 x) x) 2) (mod x 2)) x))))
(assert (= g6 2))
