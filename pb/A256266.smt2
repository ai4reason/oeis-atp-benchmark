;; sequence(s): A256266
;; terms: 0 6 18 24 48 66 78 84 132 174 210 240 264 282 294 300 396 486 570 648
;; small program: (loop(loop((x - 1) mod y, y, 0) + x, x, x) * (1 + 2)) * 2
;; fast program: (loop(loop(((((1 + loop((x mod y) + y, x div (2 + 2), 0)) * (if (x - 1) <= 0 then 1 else 2)) * (if x <= 0 then 1 else 2)) - x) - 1, 1, y) + x, x, x) * (1 + 2)) * 2
(set-logic UFNIA)
