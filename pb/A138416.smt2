;; sequence(s): A138416
;; terms: 2 9 50 147 605 1014 2312 3249 5819 11774 14415 24642 33620 38829 50807 73034 100949 111630 148137 176435
;; small program: loop2(((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) * x) + 1, y, 2, 1, x) div 2
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) * x) + 1, y, 2, 1, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (* (v1 x y) x) x) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (* (v4 x y) x) x) 1))))
