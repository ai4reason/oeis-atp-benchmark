;; sequence(s): A66885
;; terms: 5 13 25 61 85 145 181 265 421 481 685 841 925 1105 1405 1741 1861 2245 2521 2665
;; small program: (loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x) div 2) + 1
;; fast program: ((loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, y, 2, 2, x) div 2) div 2) + 1
(set-logic UFNIA)
