;; sequence(s): A18606
;; terms: 1 2 3 6 7 14 17 21 34 42 51 102 119 238 357 714
;; small program: loop(compr(loop((y - x) * y, 2 + (2 + 2), x) mod (1 + x), y), x, 0) + 1
;; fast program: compr(loop((2 + loop(x + y, y, x)) * (2 + (x + x)), 2, 1) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ 2 (v5 x y)) (+ 2 (+ x x))))))
(assert (= g4 2))
(assert (= h4 1))
