;; sequence(s): A94988
;; terms: 1 1 1 1 1 2 2 2 3 4 4 5 6 7 8 10 11 13 16 18
;; small program: loop((x div 2) div (1 + 2), x, loop((2 * ((x + x) + x)) + x, x, 1))
;; fast program: (loop2(x * y, y, x div 2, loop(1 + (2 + (2 + 2)), x mod 2, 1), (2 * (2 * (2 * (2 + (2 + 2))))) + 1) div loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div (div x 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (w2 x) (w4 x)) (v6 x)))))
