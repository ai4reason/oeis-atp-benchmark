;; sequence(s): A243348
;; terms: 0 0 0 1 1 1 3 3 4 4 4 5 6 7 7 7 9 11 11 11
;; small program: (loop(loop((if ((y * y) mod x) <= 0 then 1 else 0) + x, x, 1 + x), x, 1) - 1) - x
;; fast program: (((compr(((loop(((loop(((loop((if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0) - x, 2, x) + 1) + x) mod 2, 1, x div 2) + x) mod 2) + x, (1 + x) mod 2, x) + 1) + x) mod 2, x) + 1) div 2) - x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (- (div (+ (v2 x) 1) 2) x) 1))))
