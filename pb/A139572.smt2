;; sequence(s): A139572
;; terms: 68719476736 150094635296999121 14551915228366851806640625 2651730845859653471779023381601 30912680532870672635673352936887453361 12646218552730347184269489080961456410641
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, loop(x * x, 1, 2 + (2 + 2)), 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (loop(((x * x) * x) * x, 2, x) * x), y, 2, 1, x), 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f10 x) (* (* (* x x) x) x))))
(assert (= g10 2))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (v7 x y) 1) (* (v10 x) x)))))
