;; sequence(s): A161627
;; terms: 4 5 6 7 20 21 22 23 28 29 30 31 36 37 38 39 52 53 54 55
;; small program: compr((loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), x, x div 2) + x) mod 2, 1 + x)
;; fast program: ((loop(((compr((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) - 1) mod 2, x) * 2) - 1) - x, 1, x div (2 + 2)) * 2) * 2) + x
(set-logic UFNIA)
