;; sequence(s): A107767
;; terms: 0 1 4 16 52 169 520 1600 4840 14641 44044 132496 397852 1194649 3585040 10758400 32278480 96845281 290545684 871666576
;; small program: loop(((loop(loop2(y, 1 + ((x + x) + x), x, 0, 1), 1, y) + x) + x) + x, x, 0)
;; fast program: (((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) - loop2(x * y, y, x div 2, 2 + (x mod 2), 1 + 2)) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (+ 1 (+ (+ x x) x)))))
...
(assert (forall ((x Int)) (= (i4 x) (+ 2 (mod x 2)))))
(assert (= j4 (+ 1 2)))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (- (div (w3 x) 2) (w4 x)) 2) 1))))
