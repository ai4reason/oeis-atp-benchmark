;; sequence(s): A84468
;; terms: 7 11 13 19 21 25 35 37 41 49 67 69 73 81 97 131 133 137 145 161
;; small program: (compr((if loop(loop((2 + x) mod y, x, 1), 2, x) <= 0 then 1 else x) - 1, 2 + x) * 2) + 1
;; fast program: ((compr((((loop2(((y mod 2) * x) + x, y div 2, 2 * (2 + (2 + 2)), 1, x) - 1) div 2) - 1) mod (1 + x), 1 + x) * 2) - 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (mod (+ 2 x) y))))
...
(assert (forall ((x Int)) (= (f0 x) (- (ite (<= (v1 x) 0) 1 x) 1))))
(assert (forall ((x Int)) (= (g0 x) (+ 2 x))))
(assert (forall ((x Int)) (= (t0 x) (ite (<= (f0 x) 0) x (t0 (+ x 1))))))
(assert (forall ((x Int)) (= (u0 x) (ite (<= x 0) (t0 0) (t0 (+ (u0 (- x 1)) 1))))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x)))))
(assert (forall ((x Int)) (= (small x) (+ (* (v0 x) 2) 1))))
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* (mod y 2) x) x))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (div (- (w4 x) 1) 2) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (* (v3 x) 2) 1) 2))))
