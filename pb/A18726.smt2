;; sequence(s): A18726
;; terms: 1 2 4 8 16 29 32 58 116 232 464 928
;; small program: loop(compr((loop((x * x) + x, 2, 1 + (2 + 2)) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: compr((2 + loop(2 + ((2 * (2 * ((2 * (x + x)) + x))) + x), 2, 2)) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (+ 2 v4) (+ 1 x)))))
