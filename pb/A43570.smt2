;; sequence(s): A43570-A43736-A43742-A43749-A43757-A43766
;; terms: 5 9 11 13 17 19 23 25 27 29 33 35 39 47 49 51 55 57 59 61
;; small program: compr(loop2((((y div 2) + x) mod 2) + x, y div 2, x, x, x) mod (1 + (2 + x)), 1 + x)
;; fast program: (compr((((loop2(((x + y) mod 2) + x, y div 2, x, 1, x) - 1) div 2) - 1) mod (1 + 2), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (mod (+ x y) 2) x))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (- (div (- (w3 x) 1) 2) 1) (+ 1 2)))))
