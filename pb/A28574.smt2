;; sequence(s): A28574
;; terms: 1 46 1356 32856 714672 14543712 283133632 5342645632 98527058688 1785505986048 31916125744128 564249389488128 9885635491508224 171893957900591104 2969895694579974144 51031902826852614144 872728343238158254080
;; small program: loop(loop(loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (2 * (x + x))), y, 1) + (2 * (2 * (x + x))), x, 1) * loop(x + x, x, 1)
;; fast program: loop(loop(loop(loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + (2 * (x + x)), x, 1) * loop(x + x, x, 1), 1, y) + x, x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (w2 x y) (* 2 (* 2 (+ x x)))))))
...
(assert (forall ((x Int)) (= (f7 x) (div (div (v8 x) 2) (+ 1 2)))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (* (v4 x) (v11 x)))))
