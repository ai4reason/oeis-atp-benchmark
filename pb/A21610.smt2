;; sequence(s): A21610
;; terms: 0 0 1 6 5 0 1 6 5 0 1 6 5 0 1 6 5 0 1 6
;; small program: (1 + 2) - loop2(y, 0 - x, x - 1, 1 + 2, 2)
;; fast program: loop(((loop(x + x, x mod (2 + 2), 1) mod loop(1 + (x + x), 2, 2)) - (if x <= 0 then 1 else 2)) + x, 2, x) - x
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f1 x) (+ (- (mod (v2 x) v3) (ite (<= x 0) 1 2)) x))))
(assert (= g1 2))
