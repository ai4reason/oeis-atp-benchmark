;; sequence(s): A112711
;; terms: 1 ~3 29 ~291 3293 ~39715 500957 ~6527779 87188701 ~1187355427 16424527069 ~230141827875 3259720426717 ~46595454638883 671319066305757 ~9738441487391523 142121594825368797 ~2085158937761782563 30737922595101500637 ~455043684091275090723
;; small program: loop(loop(loop((((2 * (x - (2 * (x * y)))) div (1 + y)) * 2) * 2, x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1)) * loop2(x * y, y, x, 1, 0 - (2 + 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (* (div (* 2 (- x (* 2 (* x y)))) (+ 1 y)) 2) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (* (div (w5 x) (v6 x)) (w7 x)))))
(assert (= g4 1))
