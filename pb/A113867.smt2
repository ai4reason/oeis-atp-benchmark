;; sequence(s): A113867
;; terms: 1 17 49 113 1137 3185 7281 72817 203889 466033 4660337 13048945 29826161 298261617 835132529 1908874353 19088743537 53448481905 122167958641 1221679586417
;; small program: loop(loop(loop(loop(1 + (2 * (2 * (x + x))), x mod 2, x) div 2, x, 1) * loop(x + x, x + x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop(loop(loop(1 + (2 * (2 * (x + x))), x mod 2, x) div 2, x, 1) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (+ 1 (* 2 (* 2 (+ x x)))))))
...
(assert (forall ((x Int)) (= (f6 x) (* (v7 x) (v9 x)))))
(assert (= g6 1))
