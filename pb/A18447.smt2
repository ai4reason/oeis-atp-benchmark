;; sequence(s): A18447
;; terms: 1 5 17 25 85 425
;; small program: loop(compr((1 + loop((2 * (x + x)) - 2, 2 + 2, x)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((1 + ((2 + x) * x)) * (1 + (x * x)), 1, 2 + 2) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (* (+ 2 x) x)) (+ 1 (* x x))))))
(assert (= g4 1))
(assert (= h4 (+ 2 2)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod v4 (+ 1 (+ x x))))))
