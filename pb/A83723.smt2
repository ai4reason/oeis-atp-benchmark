;; sequence(s): A83723
;; terms: 2 7 17 31 59 83 125 159 215 299 351 455 545 615 719 863 1019 1115 1291 1439
;; small program: ((loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + x)) + x
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) * ((if x <= 0 then 1 else x) + 1)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ (* (v3 x) 2) 1) (+ (ite (<= x 0) 1 x) 1)) x))))
