;; sequence(s): A332023
;; terms: 0 1 3 4 6 9 10 12 15 19 20 22 25 29 34 35 37 40 44 49
;; small program: loop(loop(x - (if (y - x) <= 0 then y else 0), x, y) + x, x, x)
;; fast program: loop(loop(loop(x - (if (y - x) <= 0 then y else 0), x - (x div 2), x), 1, y) + x, x, x)
(set-logic UFNIA)
