;; sequence(s): A227547
;; terms: 3 5 8 13 20 31 44 61 80 103 132 163 200 241 284 331 384 443 504 571
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 1) + 2) + x
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v3 x) (ite (<= x 0) 1 2)) 2) x))))
