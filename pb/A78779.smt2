;; sequence(s): A78779
;; terms: 1 2 3 4 5 6 7 10 11 12 13 14 15 17 19 20 21 22 23 26
;; small program: loop(loop((if ((y * y) mod (x + x)) <= 0 then 1 else 0) + x, x, 1 + x), x, 1)
;; fast program: (compr(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x, 2) mod (1 + x), x) - 1) + 2
(set-logic UFNIA)
