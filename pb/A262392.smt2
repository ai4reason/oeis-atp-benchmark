;; sequence(s): A262392
;; terms: 2 5 7 13 19 31 43 61 79 103 131 163 199 241 283 331 383 443 503 571
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 2) + (x mod 2)) + x
;; fast program: (((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) + 1) + (x mod 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (v0 x) (mod x 2)) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (* (v3 x) (ite (<= x 0) 1 2)) 1) (mod x 2)) x))))
