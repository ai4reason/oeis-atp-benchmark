;; sequence(s): A91735
;; terms: 2 11 17 31 41 59 67 83 97 109 127 149 157 179 191
;; small program: compr((loop(x + x, x, 2) - 2) mod (1 + x), (((1 + x) + x) + (x mod 2)) + x) + 1
;; fast program: loop(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + x) + 1, 1, (((x mod 2) + x) + x) + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (+ (+ (+ (+ 1 x) x) (mod x 2)) x))))
...
(assert (= g2 1))
(assert (forall ((x Int)) (= (h2 x) (+ (+ (+ (mod x 2) x) x) x))))
