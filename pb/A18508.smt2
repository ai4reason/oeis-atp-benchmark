;; sequence(s): A18508
;; terms: 1 7 11 49 77 539
;; small program: loop(compr(loop(((2 * (x + x)) - 1) + y, 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(((2 * (2 * loop((2 + (x * x)) * x, 1, 1 + (2 + 2)))) + x) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (- (* 2 (+ x x)) 1) y))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (+ (* 2 (* 2 v5)) x) (+ 1 x)))))
