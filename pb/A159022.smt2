;; sequence(s): A159022
;; terms: 29 34 39 45 51 58 65 73 81 90 99 108 118 128 139 150 162 174 187 200
;; small program: loop((loop((y div (x * x)) + x, x, 1) - 1) + x, x, loop(1 + ((2 + y) * x), 2, 2))
;; fast program: loop((loop((y div (x * x)) + x, x, 1) - 1) + x, x, 2 + loop((x * x) * x, 1, 1 + 2))
(set-logic UFNIA)
