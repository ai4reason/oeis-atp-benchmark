;; sequence(s): A133538
;; terms: 2315 80312 901668 20310714 82235688 473087190 1304210412 4298697186 20654701756 44762490420 122444491244 289686151014 466572884988 778441731570 1681334260300 3663362624656 5631394320840 9203454441344 15155831763714 20142518677488
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * (x * x), y, 1 + 2, 1, x), 1, y) + x, 1 + y, 2, 0, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop((x * x) * x, 1, x * x), y, 2, 1, x), 1, y) + x, 1 + y, 2, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (v3 x y) 1) (* x x)))))
