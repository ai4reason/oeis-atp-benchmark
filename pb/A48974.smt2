;; sequence(s): A48974
;; terms: 5 7 9 13 15 19 21 25 31 33 39 43 45 49 55 61 63 69 73 75
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) + 1) + 2
;; fast program: ((((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 1 + x) * 2) - 1) - (2 mod 2)) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (- (* (v2 x) 2) 1) (mod 2 2)) 2) 2))))
