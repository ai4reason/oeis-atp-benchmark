;; sequence(s): A30635
;; terms: 65536 43046721 152587890625 33232930569601 45949729863572161 665416609183179841 48661191875666868481 288441413567621167681 6132610415680998648961 250246473680347348787521 727423121747185263828481 12337511914217166362274241
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, loop(x * x, 2, 2), 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop((x * x) * x, 1, (((x * x) * x) * x) * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (= g8 1))
(assert (forall ((x Int)) (= (h8 x) (* (* (* (* x x) x) x) x))))
