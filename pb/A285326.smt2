;; sequence(s): A285326
;; terms: 0 2 4 4 8 6 8 8 16 10 12 12 16 14 16 16 32 18 20 20
;; small program: loop(1 + loop((x - 1) mod y, x, 0), x, x) + x
;; fast program: (loop2((((y div x) mod 2) + 1) * x, y, 2 + (2 + 2), 1, x - 1) mod (1 + x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (w2 x) (+ 1 x)) x))))
