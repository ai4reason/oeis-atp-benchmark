;; sequence(s): A28092
;; terms: 1 30 583 9360 135373 1838130 23972131 304421700 3797192905 46789978950 571763347519 6946889160120 84072723694597 1014722669092890 12224658793966747 147088107868298220
;; small program: loop(((loop((loop2(((y * y) * y) + loop((2 + y) * x, 2, x), y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (* y y) y) (v3 x)))))
