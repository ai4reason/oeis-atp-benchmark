;; sequence(s): A18598
;; terms: 1 2 4 5 7 10 14 20 25 28 35 50 70 100 140 175 350 700
;; small program: loop(compr(loop((2 * (x * y)) + y, 2 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 * (2 * loop((2 + x) * (x * x), 1, 1 + (2 + 2)))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* 2 (* 2 v5)) (+ 1 x)))))
