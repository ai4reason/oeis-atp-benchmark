;; sequence(s): A28099
;; terms: 1 30 577 9066 126829 1646094 20280169 240528882 2771673157 31228927158 345616983361 3769863028698 40631819886685 433596513609822 4588506840332953 48214238728174914 503555280142266613
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
