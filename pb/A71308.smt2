;; sequence(s): A71308
;; terms: 1 8 720 691200 6739200000
;; small program: loop((loop(((loop2(x * y, 2 + (2 + y), x, 1, 1) * loop(x + x, x, 1)) div loop(x * y, x, 1)) * (x + 1), 1, y) * y) * x, x, 1 + x)
;; fast program: loop(loop(((loop2(x * y, 2 + (2 + y), x, 1, 1) * loop(x + x, x, 1)) div loop(x * y, x - 1, 1)) * (1 + x), 1, y) * x, x, 1) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (div (* (w2 x) (v3 x)) (v4 x)) (+ x 1)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (g7 x y) (+ 2 (+ 2 y)))))
...
(assert (forall ((x Int)) (= (f6 x) (* (div (* (w7 x) (v8 x)) (v9 x)) (+ 1 x)))))
(assert (= g6 1))
