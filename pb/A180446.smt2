;; sequence(s): A180446
;; terms: 0 0 1 2 3 3 4 5 6 7 8 9 9 10 11 12 13 14 15 16
;; small program: loop((loop(1 + ((x + y) div ((x + x) + x)), x + x, 1) - x) - 1, 2, x)
;; fast program: ((loop((loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + 2, x div (1 + 2)) * (0 - 1)) + x, 2, x) + 1) + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ 1 (div (+ x y) (+ (+ x x) x))))))
...
(assert (forall ((x Int)) (= (f2 x) (+ (* (w3 x) (- 0 1)) x))))
(assert (= g2 2))
