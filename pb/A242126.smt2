;; sequence(s): A242126
;; terms: 2 3 5 7 11 13 17 19 23 1 3 9 13 15 19 25 3 5 11 15
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 + loop(1 + (x * x), 2, 2))) + 1
;; fast program: (compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) mod (2 * (2 + (2 * (2 + (2 + 2)))))) + 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (+ 2 v2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (v3 x) (* 2 (+ 2 (* 2 (+ 2 (+ 2 2)))))) 1))))
