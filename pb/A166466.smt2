;; sequence(s): A166466
;; terms: 3 3 9 3 15 9 21 3 27 15 33 9 39 21 45 3 51 27 57 15
;; small program: (1 + 2) * (1 + loop(x div (1 + (x mod 2)), x, x))
;; fast program: ((loop(loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) * 2) + 1) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ 1 2) (+ 1 (v0 x))))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v1 x) 2) 1) (+ 1 2)))))
