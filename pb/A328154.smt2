;; sequence(s): A328154
;; terms: 1 ~1 3 ~5 5 ~3 7 ~13 9 ~5 11 ~15 13 ~7 15 ~29 17 ~9 19 ~25
;; small program: (1 + ((loop(x div (1 + (x mod 2)), x, x) * (1 + 2)) - x)) - x
;; fast program: ((((loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2) * (1 + 2)) + 1) - x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (+ 1 (- (* (v0 x) (+ 1 2)) x)) x))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (+ (* (v1 x) (+ 1 2)) 1) x) 2) 1))))
