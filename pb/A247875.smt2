;; sequence(s): A247875
;; terms: 0 2 4 6 8 9 10 12 14 16 17 18 19 20 22 24 25 26 28 30
;; small program: loop(compr((if loop(x div (if (x mod (2 + 2)) <= 0 then 1 else 2), x, x) <= 0 then (2 + x) else 0) mod 2, y), x, 0)
;; fast program: (compr(loop(((if loop((if (x mod (2 + 2)) <= 0 then (x + x) else x) div 2, x, x) <= 0 then 1 else 0) mod (1 + x)) mod (1 + x), (1 + x) mod 2, x), x) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (ite (<= (v2 x) 0) (+ 2 x) 0) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (div (ite (<= (mod x (+ 2 2)) 0) (+ x x) x) 2))))
(assert (forall ((x Int)) (= (g5 x) x)))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (mod (mod (ite (<= (v5 x) 0) 1 0) (+ 1 x)) (+ 1 x)))))
