;; sequence(s): A129072
;; terms: 37 49 73 97 145 169 217 241 289 361 385 457 505 529 577 649 721 745 817 865
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 2) * x, 2, 2, 2, x) + 1
;; fast program: ((((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) * 2) * 2) * (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 2) 2) (+ 1 2)) 1))))
