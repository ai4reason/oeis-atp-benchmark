;; sequence(s): A45375
;; terms: 2 7 13 19 31 37 43 61 67 73 79 97 103 109 127 139 151 157 163 181
;; small program: (compr(loop((loop(x + x, x, 2) - 2) mod (1 + x), 1 - (x mod (1 + 2)), x) - 1, 1 + x) - 1) + 2
;; fast program: (((((compr(loop((loop((x * x) * x, 1, loop(x + x, x, 1)) - 1) mod (1 + ((x + x) + x)), 1 - (x mod 2), x) - 1, 1 + x) div 2) + 1) * 2) - 1) * (1 + 2)) - (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (* (+ (div (v3 x) 2) 1) 2) 1) (+ 1 2)) (ite (<= x 0) 1 2)))))
