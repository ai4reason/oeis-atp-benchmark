;; sequence(s): A280511
;; terms: 2 2 5 5 5 5 5 13 13 13 13 13 13 13 13 13 13 13 13 13
;; small program: loop(compr(loop(((1 + y) div loop2(x + y, x, x, 1, 0)) + x, x, 1) mod 2, y), x, 2) - x
;; fast program: ((compr(((loop((loop((y div loop2(x + y, x, x, 1, 0)) + x, x, 1) - 1) - x, 2, x) + 1) + x) mod 2, 1 + x) - 1) - x) + 0
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (div y (w7 x)) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (- (v4 x) 1) x) 0))))
