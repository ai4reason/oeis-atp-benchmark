;; sequence(s): A5124
;; terms: 0 1 2 5 7 8 10 13 16 17 20 22 26 28 31 35 38 41 43 47
;; small program: (compr((2 + loop(x * x, 1, loop(x + x, x div 2, 2))) mod (1 + (x + x)), 1 + x) div 2) div 2
;; fast program: compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x), 1 + x) div (2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ 2 (v1 x)) (+ 1 (+ x x))))))
