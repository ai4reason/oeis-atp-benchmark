;; sequence(s): A28100
;; terms: 1 31 618 10094 147203 1997037 25786636 321380188 3901566405 46429426043 544051398254 6298258575882 72214033934407 821632688021449 9290501272470672 104524684322022776 1171184420402768009
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (2 * (x + x))), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (- (* (+ 1 2) (* x x)) x))))
(assert (= g9 1))
