;; sequence(s): A6588
;; terms: 1 12 240 5376 126720 3075072 76038144 1905131520 48199827456 1228623052800 31504481648640 811751838842880 20999667135283200 545086744471535616 14189559697354260480 370298578584748425216 9684502341534993088512 253765034617761850982400
;; small program: loop(loop2((2 + y) * x, 2 + y, x, 1, x) div loop(x * y, x, 1), 1, x + x)
;; fast program: ((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * (1 + (x + x))) * loop(x * x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (div (w3 x) (v4 x)) (+ 1 (+ x x))) (v5 x)))))
