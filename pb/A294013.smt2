;; sequence(s): A294013
;; terms: 0 0 2 6 10 16 22 30 38 46 54 64 74 86 98 110 122 136 150 166
;; small program: loop(loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y - 1) + x, y, x), x, 0) * 2
;; fast program: loop(loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1), 1, y) + x, x - 1, 0) * 2
(set-logic UFNIA)
