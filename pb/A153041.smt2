;; sequence(s): A153041
;; terms: 10 14 17 20 22 23 26 27 29 32 34 35 37 38 41 42 44 47 48 50
;; small program: ((((loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) + 2) + 2) + 2) + 2) + 2
;; fast program: ((compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) + 2) + (2 * (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (v3 x) 2) 2) (* 2 (+ 2 2))))))
