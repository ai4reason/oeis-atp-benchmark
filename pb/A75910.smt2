;; sequence(s): A75910
;; terms: 1 112 7392 376320 16380672 642453504 23410376704 808210923520 26787271999488 860325833342976 26956901684084736 828217683974553600 25047119070415028224 747831252926309859328 22095179333791056396288
;; small program: loop(loop(loop(((((loop(loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1), 1, y) + x) + x) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1) * loop(x + x, x + x, 1)
;; fast program: loop(loop(loop(((((((loop(loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, x, 1), 1, y) + x) + x) + x) + x) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1) * loop(x * x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g7 x) (+ x x))))
(assert (= h7 1))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (forall ((x Int)) (= (v7 x) (u7 (g7 x) h7))))
(assert (forall ((x Int)) (= (small x) (* (v0 x) (v7 x)))))
...
(assert (forall ((x Int)) (= (f14 x) (div (div (v15 x) 2) (+ 1 2)))))
(assert (= g14 1))
...
(assert (forall ((x Int) (y Int)) (= (f12 x y) (+ (+ (v13 x y) (* (+ x x) 2)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f10 x y) (+ (+ (+ (+ (+ (+ (+ (v11 x y) x) x) x) x) x) x) x))))
...
(assert (forall ((x Int)) (= (h17 x) (v18 x))))
(assert (forall ((x Int) (y Int)) (= (u17 x y) (ite (<= x 0) y (f17 (u17 (- x 1) y))))))
(assert (forall ((x Int)) (= (v17 x) (u17 g17 (h17 x)))))
(assert (forall ((x Int)) (= (fast x) (* (v8 x) (v17 x)))))
