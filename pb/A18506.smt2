;; sequence(s): A18506
;; terms: 1 2 3 6 89 178 267 534
;; small program: loop(compr(loop((2 + y) * (y - x), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((loop(((2 + (x * x)) * x) + x, 1, 2 * (2 + 2)) - 2) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 y) (- y x)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (+ (* (+ 2 (* x x)) x) x))))
(assert (= g4 1))
(assert (= h4 (* 2 (+ 2 2))))
