;; sequence(s): A38194
;; terms: 2 3 5 7 2 4 8 1 5 2 4 1 5 7 2 8 5 7 4 8
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (1 + (2 * (2 + 2)))) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod compr((1 - x) mod (2 + 2), 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (+ 1 (* 2 (+ 2 2)))) 1))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (- 1 x) (+ 2 2)))))
(assert (= g5 2))
