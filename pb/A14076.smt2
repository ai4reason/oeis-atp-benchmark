;; sequence(s): A14076
;; terms: 1 9 15 21 25 27 33 35 39 45 49 51 55 57 63 65 69 75 77 81
;; small program: (loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) * 2) + 1
;; fast program: loop(compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) + y, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (mod (- (v6 x) x) 2) (+ 1 x)) 2))))
(assert (= g5 2))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (mod (- (mod (div (+ (v5 x) 1) 2) 2) (mod x 2)) 2))))
