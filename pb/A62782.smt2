;; sequence(s): A62782
;; terms: 1 1 6 13440 81729648000 4963587213865915514880 7973107998754741458076119859200000000 738615790595351500616546025309298211371540807680000000000
;; small program: loop(x * y, x * x, 1) div loop2(x * y, y, x, 1, x)
;; fast program: loop(loop((loop2(((y * y) + y) * x, 2 + y, x, 1, 1 + (x * x)) * (1 + x)) * (1 + x), 1, y) * x, x - 1, 1) div loop2(x * y, y, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (* (w4 x) (+ 1 x)) (+ 1 x)))))
(assert (= g3 1))
