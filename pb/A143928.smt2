;; sequence(s): A143928
;; terms: 18 50 98 242 338 578 722 1058 1682 1922 2738 3362 3698 4418 5618 6962 7442 8978 10082 10658
;; small program: loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, y, 2, 2, x)
;; fast program: loop2(loop((compr(((1 + ((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, y, 2, 1, x) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (mod (+ 1 (* (+ 1 (mod x 2)) (v6 x))) (+ 1 (+ x x))) 2))))
