;; sequence(s): A161989
;; terms: 0 1 2 4 7 8 11 13 14 15 16 19 21 22 23 25 26 27 28 29
;; small program: loop(compr((if loop(loop((2 + x) mod y, x, 1), 2, x) <= 0 then 1 else 0) mod (1 + x), y), x, 0)
;; fast program: compr((((((((loop((loop(loop(2 + (x mod y), x, 1) - 1, 1 + (2 + 2), x) - 1) - x, 2, x) + 2) - x) div 2) div 2) mod 2) * (1 + x)) div 2) - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (mod (+ 2 x) y))))
...
(assert (forall ((x Int)) (= (f1 x) (mod (ite (<= (v2 x) 0) 1 0) (+ 1 x)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ 2 (mod x y)))))
...
(assert (forall ((x Int)) (= (f4 x) (- (div (* (mod (div (div (- (+ (v5 x) 2) x) 2) 2) 2) (+ 1 x)) 2) 1))))
