;; sequence(s): A282029
;; terms: 0 1 2 3 3 4 4 5 6 7 7 8 9 10 10 11 12 13 14 15
;; small program: loop(loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y - 1) + x, x div 2, 1) - x, 2, x)
;; fast program: loop(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else 0, 1, y) + x, (x - 2) div 2, 0) - x, 2, x)
(set-logic UFNIA)
