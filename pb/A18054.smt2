;; sequence(s): A18054
;; terms: 1 21 310 3990 48031 557571 6338620 71164380 792891661 8792412321 97210822930 1072779241170 11824793506891 130242283148271 1433852001421240 15780680237514360 173645640208869721 1910509145600189421 21018450325107861550 231222901641889183950
;; small program: loop(((loop((loop(loop((2 + y) * x, 2, x) - x, y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: ((loop((((loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) + loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2))))) div 2) - loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))) div 2, 1, 2 + x) div 2) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (- (div (+ (w5 x) (w7 x)) 2) (w8 x)) 2))))
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (div (div (div (v4 x) 2) 2) 2))))
