;; sequence(s): A131807
;; terms: 1 2 2 3 4 4 4 5 6 7 8 8 8 9 10 11 12 12 12 13
;; small program: loop(loop(loop(loop(if ((2 - loop(x + x, x, 2)) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x - 1, 1) mod 2, 1, y) + x, x, 1)
;; fast program: loop(loop(loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod 2, 1, y) + x, x - 1, 1) - x, 2, x) + 1
(set-logic UFNIA)
