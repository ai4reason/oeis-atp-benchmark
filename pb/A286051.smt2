;; sequence(s): A286051
;; terms: 2 3 6 8 10 11 14 16 18 19 22 24 26 28 29 32 34 35 38 40
;; small program: ((((compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), x + x) + 1) mod 2) + 1) + x) + x
;; fast program: ((((loop(((((loop2((y - x) div 2, x, 1 + (2 + 2), (2 - x) div (2 + 2), x) + 1) + x) mod 2) + x) - 1, 2, x) + 1) - x) mod 2) + x) + (1 + x)
(set-logic UFNIA)
...
(assert (= h3 (+ 1 (+ 2 2))))
(assert (forall ((x Int)) (= (i3 x) (div (- 2 x) (+ 2 2)))))
...
(assert (forall ((x Int)) (= (f2 x) (- (+ (mod (+ (+ (w3 x) 1) x) 2) x) 1))))
(assert (= g2 2))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 g2 (h2 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (mod (- (+ (v2 x) 1) x) 2) x) (+ 1 x)))))
