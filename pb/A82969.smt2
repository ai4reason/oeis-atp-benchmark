;; sequence(s): A82969
;; terms: 2 10 18 34 58 66 90 106 114 130 154 170 178 202 210 226 250 258 282 298
;; small program: ((compr(loop((loop(loop((2 + x) mod y, x, 1), x, x) + x) mod (1 + x), x mod 2, x) - 1, 1 + x) * 2) - 1) * 2
;; fast program: ((compr(loop(((loop2(x + y, y div 2, x, 1, x) mod 2) + x) mod (1 + x), x mod 2, x) - 1, 1 + x) * 2) - 1) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (v2 x) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ (mod (w6 x) 2) x) (+ 1 x)))))
