;; sequence(s): A28058
;; terms: 1 26 445 6410 84781 1071986 13229485 161139770 1948959661 23484063746 282412377325 3392692006730 40735374508141 488965966028306 5868455675537965 70426728878067290 845152692571634221
;; small program: loop((loop(loop2(((2 * (y * y)) - y) + x, y + y, x, 1, 2) * loop((x + x) + x, x, 1), 1, y) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop((loop((2 * ((x * x) div (1 + 2))) - x, 1, loop(x + x, x, 2)) + 1) * loop((x + x) + x, x, 1), 1, y) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (- (* 2 (div (* x x) (+ 1 2))) x))))
(assert (= g6 1))
