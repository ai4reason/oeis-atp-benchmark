;; sequence(s): A83588
;; terms: 1 5 19 71 273 1081 4367 17835 73165 300317 1230795 5031439 20509577 83366913 337984903 1367085683 5518563909 22239268069 89495156291 359724017367
;; small program: loop(((loop((loop(x + x, x + x, 1) - loop((x + x) + x, x, 1)) div (1 + 2), 1, y) + x) + x) + x, x, 2) - 1
;; fast program: loop(((loop((loop(x * x, 1, loop(x + x, x, 1)) - loop2(x * y, y, x, 1, 1 + 2)) div (1 + 2), 1, y) + x) + x) + x, x, 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (- (v2 x) (v3 x)) (+ 1 2)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (- (v6 x) (w8 x)) (+ 1 2)))))
(assert (= g5 1))
