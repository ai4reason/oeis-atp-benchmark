;; sequence(s): A121365
;; terms: 1 1 1 2 9 43 185 732 2737 9845 34449 118102 398585 1328607 4384393 14348912 46633953 150663529 484275617 1549681962
;; small program: loop((loop2(((x - y) + x) + x, 0, x, y, x) - x) div 2, 2, x)
;; fast program: (((((x + loop2(x * y, y, x - 1, x, 1 + 2)) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2) + 1) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (w1 x y) (u1 (h1 x) (i1 x y) (j1 x)))))
(assert (forall ((x Int) (y Int)) (= (f0 x y) (div (- (w1 x y) x) 2))))
(assert (= g0 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (div (- (+ x (w2 x)) (w3 x)) 2) 1) 2) 1))))
