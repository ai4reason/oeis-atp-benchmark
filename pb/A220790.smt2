;; sequence(s): A220790
;; terms: 1 5 1050 8127000 2273284440000 22906523331216000000 8310241106635054164480000000 108537128570336598656772717772800000000 51032497739317419104816901041614046625792000000000
;; small program: loop(loop((x * x) - x, 1, loop(2 * ((x + x) + x), y, 1)) * x, x, 1) div loop(2 * ((x + x) + x), x, 1)
;; fast program: loop2(((2 * (y * y)) - y) * x, 2 * ((y + y) + y), x, 1, 1 + 2) div loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (- (* 2 (* y y)) y) x))))
(assert (forall ((x Int) (y Int)) (= (g4 x y) (* 2 (+ (+ y y) y)))))
...
(assert (forall ((x Int)) (= (fast x) (div (w4 x) (w5 x)))))
