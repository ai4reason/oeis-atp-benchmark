;; sequence(s): A94918
;; terms: 0 0 1 0 1 4 1 0 4 4 1 4 1 4 13 0 1 4 1 0
;; small program: loop(1 + ((x + x) + x), x, 1) mod (1 + x)
;; fast program: (loop2(loop(loop2(x * y, y, x - (x div 2), 1, 1 + 2), 1, y) * x, 1 + y, 2, 1, x) div 2) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (div (w1 x) 2) (+ 1 x)))))
