;; sequence(s): A319613
;; terms: 6 21 65 133 319 481 731 1007 1403 2059 2449 3293 4141 4601 5311 6943 8201 9211 10921 12283
;; small program: loop2(loop(compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1, 1, y) * x, 1 + (y + y), 2, 1, x)
;; fast program: loop2(loop(compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1, 1, y) * x, 1 + (y + y), 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g4 x y) (+ 1 (+ y y)))))
(assert (= h4 2))
(assert (= i4 1))
