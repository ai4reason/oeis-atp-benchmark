;; sequence(s): A61742
;; terms: 1 4 36 900 44100 5336100 901800900 260620460100 94083986096100 49770428644836900 41856930490307832900 40224510201185827416900 55067354465423397733736100 92568222856376731590410384100 171158644061440576710668800200900
;; small program: loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 2, x, y), x, 1)
;; fast program: loop2(loop(loop(((compr(((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, x - 1, if x <= 0 then 1 else 2), 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
