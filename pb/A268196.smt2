;; sequence(s): A268196
;; terms: 1 3 45 3780 1871100 5618913300 104309506501200 12129109415959536000 8920608231265175901456000 41809329673499408044341517200000 1256161937180234817183361549396758000000 243113461110708695347467432844366521953760000000
;; small program: loop(loop(loop2((x * y) + x, 1 + y, x, 1, x + x) div loop(x * y, x, 1), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1), 1, y) * x, x, 1) * loop2(x * y, 2 + y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (w2 x) (v3 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (w6 x) (v7 x)))))
(assert (= g5 1))
...
(assert (forall ((x Int) (y Int)) (= (g8 x y) (+ 2 y))))
