;; sequence(s): A208976
;; terms: 1 2 4 8 15 29 54 104 195 377 713 1385 2639 5147 9866 19304 37179 72929 140997 277133
;; small program: loop2(loop((x * y) div (y - (y div 2)), y, 1) + x, 1 + y, 2, 0, x) - 1
;; fast program: (loop2(loop((loop(2 * (((x div y) + x) + x), x div 2, 1) * (1 + (x mod 2))) div (1 + x), 1, y) + x, x, 2, 1 + x, x) - x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (* (v4 x) (+ 1 (mod x 2))) (+ 1 x)))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (fast x) (- (- (w2 x) x) 2))))
