;; sequence(s): A112702
;; terms: 1 9 137 2697 60041 1436297 36039305 935717513 24927136393 677493729929 18712061405833 523679956331145 14818155751139977 423231749888535177 12185543261045516937 353292577084597987977 10305591916877658319497
;; small program: loop(loop(((loop(2 * (2 * (y - x)), 2, y) * x) div (1 + y)) * 2, y, 1) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x - 1, x)) * loop((x * x) * x, 1, loop(x + x, x - 1, 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (div (* (v2 x y) x) (+ 1 y)) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (* (div (w5 x) (v6 x)) (v7 x)))))
(assert (= g4 1))
