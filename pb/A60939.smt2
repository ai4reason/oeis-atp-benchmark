;; sequence(s): A60939
;; terms: 3 7 13 21 33 47 65 85 109 139 171 209 251 295 343 397 457 519 587 659
;; small program: loop((compr((1 + loop(x + x, x, x)) mod (1 + x), y) + 2) + x, x, 1) + 2
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, x) * 2) + 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v3 x) 2) 2) 1))))
