;; sequence(s): A18687
;; terms: 1 2 3 6 11 13 22 26 33 39 66 78 143 286 429 858
;; small program: loop(compr(loop((1 + y) * ((2 * (x * x)) - 2), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 * (2 * ((x * x) * x))) - x, 1, 2 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 y) (- (* 2 (* x x)) 2)))))
(assert (= g2 2))
(assert (= h2 2))
