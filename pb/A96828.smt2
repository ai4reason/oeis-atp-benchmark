;; sequence(s): A96828
;; terms: 3 4 5 6 7 8 9 14 15 18 19 20 21 22 23 24 25
;; small program: compr((loop2(y - x, x, x div 2, 2, 0) mod (1 + 2)) - 1, 1 + x)
;; fast program: compr(loop2(x - y, x, (x div 2) mod (2 * (2 + 2)), 1, 1), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (mod (div x 2) (* 2 (+ 2 2))))))
(assert (= i3 1))
(assert (= j3 1))
