;; sequence(s): A42103
;; terms: 1 1 47 48 2255 2303 108193 110496 5191009 5301505 249060239 254361744 11949700463 12204062207 573336561985 585540624192 27508205274817 28093745899009 1319820516629231 1347914262528240
;; small program: loop2((loop(loop((2 + y) * x, 2, x) - 1, y mod 2, 2) * x) + y, x, x, 1, 0) div (1 + (x mod 2))
;; fast program: loop2((loop((2 * (2 * ((x + x) + x))) - 1, y mod 2, 2) * x) + y, x, x - 1, if x <= 0 then 1 else 2, 1) div (1 + (x mod 2))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (mod y 2))))
(assert (= h1 2))
...
(assert (forall ((x Int)) (= (f4 x) (- (* 2 (* 2 (+ (+ x x) x))) 1))))
