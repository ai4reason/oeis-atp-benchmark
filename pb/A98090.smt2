;; sequence(s): A98090
;; terms: 3 4 5 7 8 10 11 13 16 17 20 22 23 25 28 31 32 35 37 38
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) div 2) + 2
;; fast program: (compr((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, 1 + x) + 1) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (v2 x) 1) 1))))
