;; sequence(s): A18648
;; terms: 1 2 5 10 79 158 395 790
;; small program: loop(compr(loop((1 + x) * ((x * x) - y), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(((x - 2) * (x * x)) - x, 1, 2 + (2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 x) (- (* x x) y)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (- (* (- x 2) (* x x)) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (* 2 (+ 2 2)))))
