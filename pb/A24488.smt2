;; sequence(s): A24488
;; terms: 3 18 210 3150 54054 1009008 19953648 411543990 8764362750 191413682460 4266468608220 96706621786320 2223107844022800 51721284534408000 1215794995122150720 28837137540553512390
;; small program: (loop2((2 + y) * x, 1 + y, x + x, 1, x) div loop((1 + y) * (x * y), x, 1)) * (1 + 2)
;; fast program: (((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 2 + x)) div loop(x * y, x, 1)) * (1 + ((x + x) + x))) div (1 + x)) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (w0 x) (v1 x)) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (* (div (* (div (w2 x) (v3 x)) (w4 x)) (v5 x)) (+ 1 (+ (+ x x) x))) (+ 1 x)) (+ 1 2)))))
