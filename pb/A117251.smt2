;; sequence(s): A117251
;; terms: 1 1 ~1 4 ~40 896 ~43008 4325376 ~899678208 383862702080 ~334114095890432 590956713604022272 ~2117988861556815822848 15348576513041885002989568 ~224527747847927003472304537600 6621592717332784844802427579269120
;; small program: loop(loop(loop(x + x, x, loop(x - 2, x, 1)), 1, y) * x, x - 1, 1) div loop(x * y, x, 1)
;; fast program: (loop2(x * y, 1 + y, x - 1, 1, 2 - (x + x)) div loop(x * y, x, 1)) * loop2(x * y, y + y, x - 2, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (v0 x) (v4 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (w5 x) (v6 x)) (w7 x)))))
