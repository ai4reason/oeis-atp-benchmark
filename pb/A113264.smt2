;; sequence(s): A113264
;; terms: 1 ~4 46 ~579 8171 ~123079 1939421 ~31576204 527017546 ~8969076204 155054361296 ~2715355794954 48068823892546 ~858791527669954 15464694800455046 ~280398494896810579 5114753787818033171 ~93796371395287435579 1728250671451392251921
;; small program: loop(loop((loop(2 * ((x - (x div y)) + x), x, 1) div (1 + x)) * loop(x - (2 * ((x + x) + x)), x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1)) * loop2(x * y, y, x, 1, 1 - (2 + (2 + 2))), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- x (* 2 (+ (+ x x) x))))))
(assert (forall ((x Int)) (= (g3 x) x)))
(assert (= h3 1))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 (g3 x) h3))))
(assert (forall ((x Int)) (= (f1 x) (* (div (v2 x) (+ 1 x)) (v3 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (* (div (w6 x) (v7 x)) (w8 x)))))
(assert (= g5 1))
