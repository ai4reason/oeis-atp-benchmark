;; sequence(s): A109492
;; terms: 1 3 7 11 13 21 33 37 39 77 91 111 143 231 259 273 407 429 481 777
;; small program: loop(compr(loop(1 + (2 * ((2 * (x + x)) + x)), 1 + (2 + 2), 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + ((x * x) + x)) * (1 + ((x * x) * x)), 1, 2 + (2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (+ (* x x) x)) (+ 1 (* (* x x) x))))))
(assert (= g5 1))
(assert (= h5 (+ 2 (* 2 (+ 2 2)))))
