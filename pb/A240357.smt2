...
;; small program: loop((x div 2) - x, x mod (1 + loop(x * x, 1, 2 + (2 + 2))), 1) * loop(0 - x, x, 1)
;; fast program: loop((x div 2) - x, x mod (1 + loop(x * x, 1, 2 + (2 + 2))), 1) * loop(0 - 1, x mod 2, 1)
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (g3 x) (mod x (+ 1 v4)))))
(assert (= h3 1))
