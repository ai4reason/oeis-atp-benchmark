;; sequence(s): A24935-A51034
;; terms: 1 1 2 1 2 1 2 2 2 1 2 1 2 2 2 1 2 1 2 2
;; small program: loop((loop((loop((loop(((loop(x * y, x, 1) mod (2 + x)) + 1) mod 2, 1, y) * x) + 1, x, 1) - 1) - x, 2, x) - x) + 2, 2, x)
;; fast program: loop((loop((loop((loop((loop(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0, x mod 2, 1 + x) + x) mod 2, 1, y) * x) + 1, x, 1) - 1) - x, 2, x) + 2) - x, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (mod (v4 x) (+ 2 x)) 1) 2))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (f9 x) (ite (<= (- (mod (+ 1 (v10 x)) (+ 1 x)) 2) 0) 1 0))))
