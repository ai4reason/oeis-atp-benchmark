;; sequence(s): A278481
;; terms: 2 4 4 4 6 4 4 6 6 4 4 6 6 6 4 4 6 6 6 6
;; small program: (loop(loop(y - 1, x - y, 1 + (x div 2)), x, x) * 2) + 2
;; fast program: ((if (loop(loop(y, x - y, x), x, 2 + x) - 2) <= 0 then 1 else 2) * (if x <= 0 then 0 else 2)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (ite (<= (- (v2 x) 2) 0) 1 2) (ite (<= x 0) 0 2)) 2))))
