;; sequence(s): A26490
;; terms: 1 1 2 3 2 1 2 1 2 3 2 3 2 3 2 1 2 1 2 3
;; small program: (((((loop((loop(((loop(loop(if (x mod 2) <= 0 then (x div 2) else 2, y, x) + x, x, 2) mod 2) + 1) + x, 2, x) - x) - 1, 2, x) + 1) + x) div 2) * 2) mod (1 + x)) + 1
;; fast program: ((((((((loop((((loop(loop((loop(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) mod 2, 1, x div 2) + 1) + x, x, 1) mod 2, 1, x div 2) + x) mod 2) + (x mod 2)) + x, 2, x) + 1) + x) mod 2) * 2) + 1) + (x mod 2)) * (if x <= 0 then 1 else 2)) - 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= (mod x 2) 0) (div x 2) 2))))
...
(assert (forall ((x Int)) (= (small x) (+ (mod (* (div (+ (+ (v0 x) 1) x) 2) 2) (+ 1 x)) 1))))
(assert (= f10 1))
(assert (forall ((x Int)) (= (g10 x) (mod x (+ 2 2)))))
(assert (= h10 (+ 2 2)))
(assert (forall ((x Int) (y Int)) (= (u10 x y) (ite (<= x 0) y f10))))
(assert (forall ((x Int)) (= (v10 x) (u10 (g10 x) h10))))
(assert (forall ((x Int)) (= (f9 x) (div x (v10 x)))))
(assert (= g9 2))
...
(assert (forall ((x Int)) (= (f5 x) (mod (v6 x) 2))))
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (div x 2))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (+ (+ (mod (+ (v5 x) x) 2) (mod x 2)) x))))
(assert (= g4 2))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (div (- (* (+ (+ (* (mod (+ (+ (v4 x) 1) x) 2) 2) 1) (mod x 2)) (ite (<= x 0) 1 2)) 1) 2))))
