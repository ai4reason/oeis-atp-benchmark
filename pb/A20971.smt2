;; sequence(s): A20971
;; terms: 1 27 490 7470 103291 1343097 16753780 202932540 2405791381 28065859767 323426323870 3691983687210 41834308968271 471276683342037 5284615302054760 59040661951743480 657669557062017961
;; small program: loop((loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: (loop(((loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) + loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2))))) div 2) - loop2(x * y, y, x, 1, 1 + (2 * (2 + 2))), 1, 2 + x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (div (+ (w5 x) (w6 x)) 2) (w7 x)))))
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (div (div (v4 x) 2) 2))))
