;; sequence(s): A182200
;; terms: 1 6 22 46 118 166 286 358 526 838 958 1366 1678 1846 2206 2806 3478 3718 4486 5038
;; small program: loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 2) * x) - 2, y, 2, 1, x)
;; fast program: (loop2(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y, 2, 1, x) * loop(1, x, 2 + 2)) - (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (- (* (+ (v1 x y) 2) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (w3 x) (v6 x)) (+ 1 2)))))
