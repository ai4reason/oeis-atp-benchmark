;; sequence(s): A20066
;; terms: 1 0 0 2 7 35 198 1302 9843 84216 804738 8494460 98158213 1232430908 16706285646 243169268856 3782633071103 62623591954942 1099391947653437 20399828362013787
;; small program: loop(loop(((x - 1) + x) + x, 2, y) * x, x, 1) div loop2((y * y) * x, y, x, 1, 1 + 2)
;; fast program: loop2(x * y, 1 + ((2 * (2 + 2)) + y), x, 1, 1 + (2 + 2)) div loop(x * x, 1, loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (v0 x) (w2 x)))))
