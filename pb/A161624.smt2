;; sequence(s): A161624
;; terms: 3 5 12 22 56 76 132 162 240 390 441 637 783 855 1023 1311 1634 1738 2107 2366
;; small program: ((loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 1) + x, y, 2, 1, x) - (x * x)) - x) div 2
;; fast program: (((loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 1) + x, y, 2, 1, x) - 1) - (x * x)) - x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (* (v4 x y) x) 1) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (- (- (w3 x) 1) (* x x)) x) 2))))
