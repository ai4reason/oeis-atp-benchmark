...
;; small program: 1 - (loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x) * 2)
;; fast program: ((loop(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 1 + 2, x), 1 - (x mod (2 + 2)), x), 1, 1 + x) mod 2) * 2) - 1
(set-logic UFNIA)
