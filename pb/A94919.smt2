;; sequence(s): A94919
;; terms: 0 1 0 1 1 3 1 5 0 5 1 9 1 5 6 5 1 9 1 5
;; small program: loop(1 + (2 * (x + x)), x, 1) mod (1 + x)
;; fast program: (loop(x * x, 1, 2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) div (1 + 2)) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (div (v1 x) (+ 1 2)) (+ 1 x)))))
