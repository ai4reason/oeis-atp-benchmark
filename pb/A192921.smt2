;; sequence(s): A192921
;; terms: 1 2 2 7 16 44 113 298 778 2039 5336 13972 36577 95762 250706 656359 1718368 4498748 11777873 30834874
;; small program: loop(loop2(y, x + y, (y - 2) + y, 1 + 2, 1) - x, x, 1)
;; fast program: loop2(((((y * y) div (2 + x)) + x) + y) + y, x + y, x - 2, if x <= 0 then 1 else 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h1 x y) (+ (- y 2) y))))
(assert (= i1 (+ 1 2)))
(assert (= j1 1))
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (+ (div (* y y) (+ 2 x)) x) y) y))))
