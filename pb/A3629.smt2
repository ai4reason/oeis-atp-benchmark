;; sequence(s): A3629-A175865-A319041
;; terms: 3 5 11 13 19 29 37 43 53 59 61 67 83 101 107 109 131 139 149 157
;; small program: compr((1 + loop(x + x, x div 2, 1)) mod loop(y, x, 1), 2 + x)
;; fast program: (compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), ((1 + x) div 2) mod 2, x), 1 + x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ 1 (v1 x)) (v2 x)))))
...
(assert (forall ((x Int)) (= (g4 x) (mod (div (+ 1 x) 2) 2))))
