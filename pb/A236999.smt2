;; sequence(s): A236999
;; terms: 1 1 1 13 19 13 17 43 53 1 19 89 103 59 67 151 169 47 13 229
;; small program: loop(1 + loop(x div (1 + (x mod 2)), x, x), 1, loop(x + y, x, x) + x)
;; fast program: loop((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1, 1, ((((x * x) + x) div 2) + x) + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (+ (* (v4 x) 2) 1))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (+ (+ (div (+ (* x x) x) 2) x) x))))
