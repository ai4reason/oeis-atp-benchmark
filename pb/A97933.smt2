;; sequence(s): A97933
;; terms: 11 13 23 37 47 59 61 71 73 83 97 107 109 131 157 167 179 181 191 193
;; small program: compr(((loop2(x - y, x, (x div 2) mod (2 + (2 + 2)), 1, 1) * loop(x * x, 1, loop(x + x, x div 2, 1))) - 1) mod (1 + x), 2 + x) + 1
;; fast program: loop(compr(loop(((loop2(x - y, x, (x div 2) mod (2 + (2 + 2)), 1, 1) * loop(x * x, 1, loop(x + x, x div 2, 1))) - 1) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h1 x) (mod (div x 2) (+ 2 (+ 2 2))))))
(assert (= i1 1))
(assert (= j1 1))
...
(assert (forall ((x Int)) (= (f0 x) (mod (- (* (w1 x) (v2 x)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (h7 x) (mod (div x 2) (+ 2 (+ 2 2))))))
(assert (= i7 1))
(assert (= j7 1))
...
(assert (forall ((x Int)) (= (f6 x) (mod (- (* (w7 x) (v8 x)) 1) (+ 1 x)))))
