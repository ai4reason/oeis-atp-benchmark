;; sequence(s): A60800
;; terms: 7 13 31 57 133 183 307 381 553 871 993 1407 1723 1893 2257 2863 3541 3783 4557 5113
;; small program: loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 1) + 2, y, 2, 1, x)
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 1) + 2, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (* (v4 x y) x) 1) 2))))
