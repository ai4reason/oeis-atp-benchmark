;; sequence(s): A129070
;; terms: 13 17 25 33 49 57 73 81 97 121 129 153 169 177 193 217 241 249 273 289
;; small program: ((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 2) * (2 + 2)) + 1
;; fast program: ((((((compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) * 2) + 2) + 2) - (if x <= 0 then 1 else 2)) * 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (v0 x) 2) (+ 2 2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (- (+ (+ (* (v3 x) 2) 2) 2) (ite (<= x 0) 1 2)) 2) 2) 1))))
