;; sequence(s): A18510
;; terms: 1 2 4 8 16 17 32 34 68 136 272 544
;; small program: loop(compr((2 * loop(2 * ((2 + 2) * (2 + x)), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((loop((y * y) + x, x, x) * x) * x, 1, 2 + 2) mod (1 + x), x) * 2) + 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* 2 (* (+ 2 2) (+ 2 x))))))
(assert (= g2 2))
(assert (= h2 2))
