;; sequence(s): A8883
;; terms: 51 154 77 232 116 58 29 88 44 22 11 34 17 52 26 13 40 20 10 5
;; small program: loop(loop(2 + (2 * ((x + x) + x)), x mod 2, x) div 2, x, loop(1 + ((x * x) * y), 2, 2))
;; fast program: loop(loop(2 + (2 * ((x + x) + x)), x mod 2, x) div 2, x, (1 + 2) * (1 + (2 * (2 * (2 + 2)))))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (* (* x x) y)))))
(assert (= g2 2))
(assert (= h2 2))
