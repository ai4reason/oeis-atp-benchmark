;; sequence(s): A18553
;; terms: 1 2 3 6 103 206 309 618
;; small program: loop(compr(loop((2 * (2 + ((x * x) + x))) + y, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(((1 + (2 * (2 + x))) * (x * x)) + x, 1, 2 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (* (+ 1 (* 2 (+ 2 x))) (* x x)) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
