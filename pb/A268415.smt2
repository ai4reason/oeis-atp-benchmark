;; sequence(s): A268415
;; terms: 1 2 3 4 6 7 8 12 14 15 16 21 24 28 30 31 32 37 41 42
;; small program: compr(loop2((if ((y div 2) mod 2) <= 0 then 1 else y) + x, y div 2, x, 1, x) mod 2, x) + 0
;; fast program: ((compr((((loop2(loop(x + y, y, x), y div 2, x, x, x) div 2) mod 2) - 1) mod 2, x) * 2) div 2) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (- (mod (div (w3 x) 2) 2) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (* (v2 x) 2) 2) 1))))
