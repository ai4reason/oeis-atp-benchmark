;; sequence(s): A107782
;; terms: 0 0 0 0 1 0 0 0 2 1 0 0 1 0 0 0 3 2 1 1
;; small program: loop2(loop(loop(y - ((x div 2) mod 2), x, 2) mod 2, 1, y) + x, y div 2, x, 0, x)
;; fast program: loop2(loop(if ((if x <= 0 then 1 else x) mod (1 + (1 + 2))) <= 0 then 2 else 0, 1, y) + x, y div 2, x, 0, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (mod (ite (<= x 0) 1 x) (+ 1 (+ 1 2))) 0) 2 0))))
(assert (= g4 1))
