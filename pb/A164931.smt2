;; sequence(s): A164931
;; terms: 1 4 9 20 35 66 91 136 171 230 319 372 481 574 645 752 901 1062 1159 1340
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y), x, 0) + 1) * (1 + x)
;; fast program: (((compr((((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) + 1) * 2) div 2) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (div (* (+ (v3 x) 1) 2) 2) (+ 1 x)))))
