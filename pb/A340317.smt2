;; sequence(s): A340317
;; terms: 0 0 0 2 0 0 0 2 3 0 0 6 0 0 0 14 0 12 0 10
;; small program: loop(((loop((loop(x * y, x, x) div (1 + x)) mod 2, 1, y) * x) * y) + x, x, 2) mod (1 + x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x, 1) mod (1 + x)
(set-logic UFNIA)
