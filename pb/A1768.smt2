;; sequence(s): A1768
;; terms: 0 1 3 5 7 10 13 16 19 22 26 30 34 38 42 46 50 54 58 62
;; small program: loop(loop(loop(loop((x - 1) mod y, x, 0), x, y) + x, y, 1) + x, x, 0)
;; fast program: loop(loop((loop(2 + ((x div y) + x), x, 0) + (x div 2)) div (1 + x), 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (v6 x) (div x 2)) (+ 1 x)))))
(assert (= g5 1))
