...
;; small program: (compr(((loop2(((x + y) mod 2) + x, y div 2, x, 1, x) + 1) div 2) mod 2, x) * 2) + 1
;; fast program: (compr(((((loop2(loop(x + y, y, x), y div 2, 2 + x, 1, x) + x) div 2) + 1) + x) mod 2, x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (mod (+ x y) 2) x))))
...
(assert (forall ((x Int)) (= (f0 x) (mod (div (+ (w1 x) 1) 2) 2))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (+ (+ (div (+ (w3 x) x) 2) 1) x) 2))))
