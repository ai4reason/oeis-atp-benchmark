;; sequence(s): A131808
;; terms: 0 0 1 1 1 2 3 3 3 3 3 4 5 5 5 5 5 6 7 7
;; small program: loop(loop(loop(loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x, 1) mod 2, 1, y) + x, x - 1, 0)
;; fast program: loop(loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod 2, 1, y) + x, x - 1, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (ite (<= (- (mod (+ 1 (v9 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g8 1))
