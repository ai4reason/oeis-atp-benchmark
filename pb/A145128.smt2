;; sequence(s): A145128
;; terms: 1 4 13 38 99 231 490 960 1761 3058 5071 8086 12467 18669 27252 38896 54417 74784 101137 134806
;; small program: 1 + loop(loop(loop(loop(loop(x + y, y, x), y, x), y, x), y, x) + y, x, x)
;; fast program: loop(loop((((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 * (2 + (2 + 2))))) * x) div (1 + (2 + 2))) + x, 1, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (div (* (div (v7 x) (* 2 (* 2 (+ 2 (+ 2 2))))) x) (+ 1 (+ 2 2))) x))))
(assert (= g6 1))
