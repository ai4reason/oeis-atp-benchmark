;; sequence(s): A8347
;; terms: 0 2 1 4 3 8 5 12 7 16 13 18 19 22 21 26 27 32 29 38
;; small program: loop((compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x), y) - x) + 1, x, 0)
;; fast program: loop((compr(loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, y) - x) + 1, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g7 x) (- (div x 2) 1))))
(assert (= h7 2))
