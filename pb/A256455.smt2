;; sequence(s): A256455
;; terms: 3 4 5 7 9 11 12 13 15 17 19 21 23 24 25 27 29 31 33 35
;; small program: compr(loop((loop(((y + y) div (x * x)) + x, x, 2) + 1) + x, 2, x) mod 2, 1 + x)
;; fast program: compr(loop(loop((loop(loop((((y - x) + y) div (x * x)) + x, x div 2, 1) + x, 2, x) - 1) - x, 1, x div 2) + x, 2, x) mod 2, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (div (+ (- y x) y) (* x x)) x))))
