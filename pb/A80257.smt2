;; sequence(s): A80257
;; terms: 6 8 10 12 14 15 16 18 20 21 22 24 26 27 28 30 32 33 34 35
;; small program: compr(loop(loop(if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then (1 + x) else 1, 1, y) * x, x div 2, 1) mod (1 + x), 1 + x) + 1
;; fast program: compr(loop(loop(if (loop((1 + (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) mod (1 + x), 2 - (x mod 2), x) - 1) <= 0 then (1 + x) else 1, 1, y) * x, x div 2, 1) mod (1 + x), 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (- (v3 x) 1) 0) (+ 1 x) 1))))
(assert (= g2 1))
