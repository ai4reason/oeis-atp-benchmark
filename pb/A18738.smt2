;; sequence(s): A18738
;; terms: 1 2 3 4 6 12 79 158 237 316 474 948
;; small program: loop(compr(loop(2 * ((1 + 2) * (2 + x)), 1 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 * loop((((2 * (x * x)) + x) * x) + x, 1, 2 + (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* 2 (* (+ 1 2) (+ 2 x))))))
(assert (= g2 (+ 1 2)))
(assert (= h2 2))
