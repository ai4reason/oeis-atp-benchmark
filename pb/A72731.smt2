;; sequence(s): A72731
;; terms: 0 ~1 ~2 ~1 ~2 ~1 ~2 ~1 0 1 0 1 0 1 2 3 2 3 2 3
;; small program: loop(loop((2 + loop(x * y, x, 2)) mod (1 + x), 1, y) + x, x, 0) - x
;; fast program: ((loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 2) - x, 2, x) * 2) - 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (* (v3 x) 2) 2) x))))
