;; sequence(s): A186301
;; terms: 3 11 27 35 51 59 99 107 147 155 171 179 195 227 267 275 291 315 347 371
;; small program: (compr((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x)), 1 + x) * 2) - 1
;; fast program: ((((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x + x) - 1, 1 + x) * 2) * 2) * 2) - 2) div 2
(set-logic UFNIA)
