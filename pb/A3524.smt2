;; sequence(s): A3524
;; terms: 1 3 5 7 9 13 15 21 35 39 45 63 65 91 105 117 195 273 315 455
;; small program: loop(compr((loop(x * x, 2, 2 * (2 + 2)) + x) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr((1 + loop(2 + (x + x), (2 * (2 + 2)) + 2, 2)) mod (1 + (x + x)), x) * 2) + 1) div 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (+ 1 v4) (+ 1 (+ x x))))))
