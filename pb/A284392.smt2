;; sequence(s): A284392
;; terms: 3 4 8 9 11 12 14 15 19 20 24 25 29 30 32 33 35 36 40 41
;; small program: ((compr(loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), x + x, x) mod 2, 1 + x) + x) div 2) + 2
;; fast program: (loop(((compr((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) - 1) mod 2, x) * 2) - 1) - x, 1, x div 2) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (- (* (v3 x) 2) 1) x))))
(assert (= g2 1))
