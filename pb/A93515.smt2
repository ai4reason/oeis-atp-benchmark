;; sequence(s): A93515
;; terms: 2 3 4 5 6 7 8 11 12 13 14 17 18 19 20 23 24 29 30 31
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), x - (x div 2)) - (x mod 2)) + 2
;; fast program: ((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, (1 + x) div 2) * 2) + 1) + ((x - 1) mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v2 x) 2) 1) (mod (- x 1) 2)))))
