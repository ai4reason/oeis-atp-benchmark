;; sequence(s): A246017
;; terms: 1 2 1 2 3 2 3 4 3 4 3 2 3 4 3 4 5 4 5 6
;; small program: ((loop(loop(((1 + (loop2(y - x, y div 2, x + x, 0, x) div 2)) + x) mod 2, 1, y) + x, x, 1) * 2) - 1) - x
;; fast program: ((loop(loop((((((loop2((y - x) div 2, x, 1 + (2 + 2), x div 2, 0) + 1) + x) div 2) + 1) + x) mod 2, 1, y) + x, x, 0) * 2) - x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (+ 1 (div (w2 x) 2)) x) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (div (- y x) 2))))
(assert (forall ((x Int)) (= (g5 x) x)))
(assert (= h5 (+ 1 (+ 2 2))))
(assert (forall ((x Int)) (= (i5 x) (div x 2))))
(assert (= j5 0))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (div (+ (+ (w5 x) 1) x) 2) 1) x) 2))))
(assert (= g4 1))
