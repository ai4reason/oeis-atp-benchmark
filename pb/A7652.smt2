;; sequence(s): A7652
;; terms: 2 3 5 7 1 3 7 9 3 9 1 7 1 3 7 3 9 1 7 1
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 + (2 * (2 + 2)))) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod compr((2 + x) mod (2 + 2), 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ 2 x) (+ 2 2)))))
(assert (= g5 2))
