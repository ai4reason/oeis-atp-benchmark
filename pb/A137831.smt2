;; sequence(s): A137831
;; terms: 0 0 1 2 6 8 14 18 26 42 48 68 84 92 110 140 174 186 224 252
;; small program: loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x) + x, y, 2, 1, x) div loop(2 * (2 + x), 2, 2)
;; fast program: ((((((((loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1, 1, y) * x, y, 2, 1, x) div (1 + (2 + 2))) + 1) * (1 + 2)) div 2) div 2) + 1) div (1 + 2)) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* 2 (+ 2 x)))))
(assert (= g3 2))
(assert (= h3 2))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (div (w0 x) v3))))
...
(assert (forall ((x Int)) (= (f6 x) (- (mod (+ 1 (v7 x)) (+ 1 (+ x x))) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (div (+ (div (div (* (+ (div (w4 x) (+ 1 (+ 2 2))) 1) (+ 1 2)) 2) 2) 1) (+ 1 2)) x) x))))
