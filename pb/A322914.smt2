;; sequence(s): A322914
;; terms: 0 1 15 198 2511 31266 385398 4721004 57590271 700465482 8501284530 103007201364 1246500179910 15068548264212 182007001727244 2196875784339288 26501619841355871 319541469851970522 3851239987536347034 46399926869155488708
;; small program: (loop((((loop((2 * ((2 * (x * y)) - x)) div (1 + y), y, 1) + x) + x) + x) + x, x - 1, 1) * loop2(x * y, y, x, 1, 1 + 2)) div (1 + 2)
;; fast program: (loop((((loop(loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1), 1, y) + x) + x) + x) + x, x - 1, 1) * loop2(x * y, y, x, 1, 1 + 2)) div (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (div (* 2 (- (* 2 (* x y)) x)) (+ 1 y)))))
...
(assert (forall ((x Int)) (= (small x) (div (* (v0 x) (w2 x)) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (div (w5 x) (v6 x)))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (div (* (v3 x) (w7 x)) (+ 1 2)))))
