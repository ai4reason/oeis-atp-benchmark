;; sequence(s): A329952
;; terms: 0 1 2 8 10 16 17 18 24 26 32 33 34 40 42 48 49 50 56 58
;; small program: loop(compr(((1 - x) * ((2 - x) * x)) mod loop(x * x, 2, 2), y), x, 0)
;; fast program: loop(compr(((1 - x) * ((2 - x) * x)) mod (2 * (2 * (2 + 2))), y), x, 0)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* (- 1 x) (* (- 2 x) x)) v2))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (* (- 1 x) (* (- 2 x) x)) (* 2 (* 2 (+ 2 2)))))))
