;; sequence(s): A108582
;; terms: 1 2 2 2 2 2 2 2 2 3 3 3 3 3 3 3 3 3 3 3
;; small program: loop((y div loop(y * y, loop(x + y, x, 0), 0)) + x, x, 1)
;; fast program: (((((((((((((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 1 + 2, (x - 2) div 2) + 2) + x, 2, x) - x) + 1) div 2) * x) div 2) + 1) + x) div 2) + 1) - x) + x) + x) div (2 + x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div y (v1 x)) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (+ (- (+ (div (+ (+ (div (* (div (+ (- (v3 x) x) 1) 2) x) 2) 1) x) 2) 1) x) x) x) (+ 2 x)) 1))))
