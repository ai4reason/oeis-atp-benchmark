;; sequence(s): A236227
;; terms: 524288 1162785755 19074649113880 11417969834487023 61170508418249033314 1523090798793695143991 240595526483945019991144 2219015182144258609115123 76834486109734969171023610 6180095732699726458749873279 27850757952670122653464150750
;; small program: loop(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 1 + (2 + (2 * (2 * (2 + 2)))), 1, x), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), y) * x) + x, y, (2 * (2 + (2 * (2 + 2)))) - 1, 1, x), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (mod (- 2 (v9 x)) (+ 1 x)))))
