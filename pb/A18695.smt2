;; sequence(s): A18695
;; terms: 1 3 9 97 291 873
;; small program: loop(compr(((1 + 2) * loop((x * x) + y, 2, 2 + 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (loop(((loop(1 + (x + x), (x mod (2 + (2 + 2))) - 1, x) + x) + 1) + x, x, 0) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g4 x) (- (mod x (+ 2 (+ 2 2))) 1))))
