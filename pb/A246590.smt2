;; sequence(s): A246590
;; terms: 6 12 14 22 24 28 30 38 44 46 48 54 56 60 62 70 76 78 86 88
;; small program: compr(1 - (loop(loop((y mod 2) - x, x, x), x, x) mod 2), x) * 2
;; fast program: (compr(((((((1 + loop(loop(x div 2, x mod 2, x), 2 + (2 + x), x)) div 2) + 1) + x) mod 2) - (x mod 2)) mod 2, x) * 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (+ (+ (div (+ 1 (v4 x)) 2) 1) x) 2) (mod x 2)) 2))))
