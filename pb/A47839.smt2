;; sequence(s): A47839
;; terms: 2 4 5 6 8 9 10 12 13 14 15 16 18 19 20 21 22 24 25 26
;; small program: loop(2 + ((1 + y) div x), x + x, 2) + x
;; fast program: (((loop((loop(loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x div 2) + x, 2, x) - 1) - x, 1, 2 + (x + x)) div 2) + 1) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (div (v1 x) 2) 1) x) 2))))
