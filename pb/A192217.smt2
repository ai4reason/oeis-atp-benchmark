;; sequence(s): A192217
;; terms: 1 4 36 144 3600 21600 1058400 8467200 76204800 762048000 92207808000 1106493696000 186997434624000 2617964084736000 39269461271040000 628311380336640000 181581988917288960000 3268475800511201280000 1179919763984543662080000
;; small program: loop((((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x) * (1 + y), x, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x, 1) * loop(x * y, x, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (* (* (v1 x y) x) y) x) (+ 1 y)))))
