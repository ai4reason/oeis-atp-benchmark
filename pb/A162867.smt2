;; sequence(s): A162867
;; terms: 4 15 40 85 168 287 464 693 1000 1419 1920 2561 3332 4215 5248 6477 7920 9519 11360 13419
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y) + x, x, 2) + x) * (2 + x)
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) * (2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v3 x) 2) x) (+ 2 x)))))
