;; sequence(s): A55039
;; terms: 14 30 46 56 62 78 94 110 120 126 142 158 174 184 190 206 222 224 238 248
;; small program: compr(((loop(x div loop(1, x mod (2 + 2), 2 + 2), x div 2, x) + 1) mod (2 * (2 + 2))) - 1, 1 + x) * 2
;; fast program: compr(((loop(x div loop(1, x mod (2 + 2), 2 + 2), x div (2 + (2 * (2 * (2 + 2)))), x) + 1) mod (2 * (2 + 2))) mod (1 + (x + x)), 1 + x) * 2
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (+ (v4 x) 1) (* 2 (+ 2 2))) (+ 1 (+ x x))))))
