;; sequence(s): A285980
;; terms: 2 3 5 6 7 8 9 10 12 13 15 16 17 19 20 21 22 23 25 26
;; small program: (((compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 2 + (x + x)) * (1 + 2)) + 1) div (2 + 2)) - x
;; fast program: ((((compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * (1 + 2)) div 2) + 1) div 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (div (+ (* (v0 x) (+ 1 2)) 1) (+ 2 2)) x))))
...
(assert (forall ((x Int)) (= (fast x) (- (div (+ (div (* (v2 x) (+ 1 2)) 2) 1) 2) x))))
