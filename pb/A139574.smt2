;; sequence(s): A139574
;; terms: 4398046511104 109418989131512359209 227373675443232059478759765625 311973482284542371301330321821976049 54763699237492901685126120802225273763666521 61040881526285814362156628321386486455989674569
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x * x, 2, x), y, 1 + 2, 1, x), 1, y) * x, y, 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((((x * x) * x) * x) * x), y, 2, 1, x), 1, y) * (x * x), y, 1 + 2, 1, x)
(set-logic UFNIA)
