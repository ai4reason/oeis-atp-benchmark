;; sequence(s): A69651
;; terms: 1 2 18 1200 735000 4667544000 332086420512000 279394363051195392000 2892376010829659126572800000 379850021025259936655866602240000000 648304836222110631242066578424390188032000000
;; small program: loop(loop((x * y) div (y - (y div 2)), y, x), x + x, 1) div loop(x * y, x, 1)
;; fast program: loop(loop((loop2(((y * y) + y) * x, 1 + y, x, 1, 2 + x) div loop((y * x) * y, x, 1)) div (1 + x), 1, y) * x, x - 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (+ (* y y) y) x))))
...
(assert (forall ((x Int)) (= (f4 x) (div (div (w5 x) (v6 x)) (+ 1 x)))))
(assert (= g4 1))
