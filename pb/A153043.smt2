;; sequence(s): A153043
;; terms: 2 6 9 12 14 15 18 19 21 24 26 27 29 30 33 34 36 39 40 42
;; small program: loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) + 2
;; fast program: (compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 0) 2) 2))))
