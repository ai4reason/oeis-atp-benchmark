;; sequence(s): A350895
;; terms: 1 3 10 51 358 3939 51208 870537 16540204 380424693 11032316098 342001799039 12654066564444 518816729142205 22309119353114816 1048528609596396353 55572016308609006710 3278748962207931395891 200003686694683815149352 13400247008543815615006585
;; small program: loop(((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + 1) + x, x, 1)
;; fast program: loop(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + 1) * x) + 1, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (+ (v4 x y) 1) x) 1))))
