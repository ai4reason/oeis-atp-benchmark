;; sequence(s): A18653
;; terms: 1 2 4 5 8 10 16 20 25 32 40 50 80 100 160 200 400 800
;; small program: loop(compr(loop(2 * ((2 + x) * (2 + x)), 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop(((loop(((x * y) - 1) + x, 2, 2) * (1 + x)) + x) * (2 + x), 1, 2 * (2 + 2)) mod (1 + x), x) + 1) + x) - x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* 2 (* (+ 2 x) (+ 2 x))))))
(assert (= g2 2))
(assert (= h2 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (- (* x y) 1) x))))
(assert (= g5 2))
(assert (= h5 2))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (* (+ (* v5 (+ 1 x)) x) (+ 2 x)))))
(assert (= g4 1))
(assert (= h4 (* 2 (+ 2 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (+ (v3 x) 1) x) x) 1))))
