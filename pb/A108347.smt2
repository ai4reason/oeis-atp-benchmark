;; sequence(s): A108347
;; terms: 1 3 5 7 9 15 21 25 27 35 45 49 63 75 81 105 125 135 147 175
;; small program: loop(compr(loop(x * x, 2, 1 + loop(2 + (2 * (2 * ((x + x) + x))), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((loop(loop((x * x) * x, 1, x * x) * x, 1, 1 + (2 * (2 * (2 + ((x + x) + x))))) * (1 + ((x + x) + x))) * (loop2(x * y, y, 2, 1, x) - 1), 1, 2 * (2 + 2)) mod (1 + x), x) + 1) + x) - x) div 1
(set-logic UFNIA)
...
(assert (= g6 1))
(assert (forall ((x Int)) (= (h6 x) (+ 1 (* 2 (* 2 (+ 2 (+ (+ x x) x))))))))
...
(assert (forall ((x Int)) (= (f5 x) (* (* (v6 x) (+ 1 (+ (+ x x) x))) (- (w8 x) 1)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (+ (v4 x) 1) x) x) 1))))
