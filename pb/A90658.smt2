;; sequence(s): A90658
;; terms: 4 8 12 20 24 32 44 48 60 68 72 80 84 104 108 128 132 140 152 164
;; small program: compr(loop((1 + loop(x + x, x, x)) mod (1 + x), (x div 2) mod 2, x), 1 + x) + 2
;; fast program: (compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 1 + x) * 2) + 2
(set-logic UFNIA)
