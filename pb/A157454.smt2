;; sequence(s): A157454
;; terms: 1 1 1 1 3 1 1 3 3 1 1 3 5 3 1 1 3 5 5 3
;; small program: ((loop(((loop(loop(y - 1, x - y, x), x, x) + y) div (2 + x)) + x, x, x) - x) * 2) + 1
;; fast program: ((loop(((loop(loop(y, x - y, x), x, 1 + x) + y) div (2 + x)) + x, 1 + x, x) - x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (+ (v1 x) y) (+ 2 x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (div (+ (v4 x) y) (+ 2 x)) x))))
