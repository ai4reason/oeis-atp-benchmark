;; sequence(s): A209197
;; terms: 1 4 32 487 11113 335745 12607257 565877928 29553415078 1760584360722 117828762999498 8752769915058447 714626485356930711 63609663369881873031 6130647517448380412727 636052622643842997577302 70679525819378610579659532 8375262433274665594692923984
;; small program: loop(loop(loop2(x * y, y - 1, x, 1, (2 + x) * x) div loop(x * y, x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, y - 1, x - 1, 1, (2 + x) * x) div loop(x * y, x, 1)) * (1 + ((x * x) + x)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (div (w6 x) (v7 x)) (+ 1 (+ (* x x) x))))))
(assert (= g5 1))
