...
;; small program: loop2(y, y + y, x, 0 - 1, 2)
;; fast program: loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) - (if x <= 0 then 2 else 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (v1 x) (ite (<= x 0) 2 0)))))
