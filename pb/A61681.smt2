;; sequence(s): A61681
;; terms: 1 2 4 8 16 17 18 19 20 22 24 26 28 30 33 36 39 42 46 50
;; small program: loop(loop(loop(y, (x div 2) div (1 + (2 + 2)), x), 2, x) + x, x, 1)
;; fast program: loop(loop(loop(y, x div (2 + (2 * (2 + 2))), x), 2, x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (div (div x 2) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (g5 x) (div x (+ 2 (* 2 (+ 2 2)))))))
