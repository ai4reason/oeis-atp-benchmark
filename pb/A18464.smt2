;; sequence(s): A18464
;; terms: 1 5 7 13 35 65 91 455
;; small program: loop(compr(loop(1 - ((2 + y) * x), 2 + 2, x) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((1 + (x * x)) * ((x * x) - x), 1, 2 * (2 + 2)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (* x x)) (- (* x x) x)))))
(assert (= g4 1))
(assert (= h4 (* 2 (+ 2 2))))
