;; sequence(s): A162966
;; terms: 1 4 8 9 12 16 18 20 24 25 27 28 32 36 40 44 45 48 49 50
;; small program: loop(compr((if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0) * x, y), x, 0) + 1
;; fast program: compr(((if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x) <= 0 then 1 else 0) * (if x <= 0 then 1 else 2)) - 1, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (ite (<= (- (v2 x) x) 0) 1 0) x))))
...
(assert (forall ((x Int)) (= (f3 x) (- (* (ite (<= (- (v4 x) x) 0) 1 0) (ite (<= x 0) 1 2)) 1))))
