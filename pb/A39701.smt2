;; sequence(s): A39701
;; terms: 2 0 2 1 2 1 2 1 2 2 1 1 2 1 2 2 2 1 1 2
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (1 + 2)
;; fast program: loop(((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod (2 + y)) - 1, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (- (mod (+ (v3 x) 1) (+ 2 y)) 1))))
(assert (= g2 2))
