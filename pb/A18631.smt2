;; sequence(s): A18631
;; terms: 1 2 4 5 8 10 19 20 38 40 76 95 152 190 380 760
;; small program: loop(compr(loop((x + y) * (2 + (x + x)), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop2(compr((2 * loop((x * x) - x, 1, 2 * (2 + (2 * (2 + 2))))) mod (1 + x), y) + x, y, 2, 2, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ x y) (+ 2 (+ x x))))))
(assert (= g2 2))
(assert (= h2 2))
