;; sequence(s): A62532
;; terms: 1 81 225 441 625 729 1089 1225 1521 2025 2401 2601 3025 3249 3969 4225 4761 5625 5929 6561
;; small program: loop2((compr((loop(x * y, x, 2) - (x mod 2)) mod (1 + x), y) * x) + x, y, 2, 1, x)
;; fast program: loop2((compr(((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + 1) mod 2) + x) mod (1 + x), y) * x) + x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (v2 x) (mod x 2)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (mod (+ (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 x)) 2) 0) 1 x) 1) 2) x) (+ 1 x)))))
