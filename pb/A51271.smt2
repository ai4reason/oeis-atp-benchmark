;; sequence(s): A51271
;; terms: 3 6 16 64 512 5824 92288 1659136 36495872 1021871104 30656104448 1103619690496 44144787464192 1854081073168384 85287729365024768 4434961926979649536 257227791764816003072 15433667505888952582144
;; small program: loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x, x, 1) + loop(x + x, x, 2)
;; fast program: ((loop(compr(((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2, y) * x, x, 2) div 2) + 2) * loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (div (v4 x) 2) 2) (v7 x)))))
