;; sequence(s): A14150
;; terms: 2 9 26 60 122 225 386 624 962 1429 2056 2880 3942 5285 6956 9008 11500 14493 18054 22254
;; small program: loop(loop(loop(compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + x, y, x) + y, 1 + y, x), x, 2)
;; fast program: loop(loop(loop(loop((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 2) + x) * 2) - 1) - x, 1, y) + x, x, 1) - x, 1, y) + x, x, 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g12 x) (mod x 2))))
(assert (forall ((x Int)) (= (f13 x) (* x x))))
(assert (= g13 1))
(assert (forall ((x Int)) (= (f14 x) (+ x x))))
(assert (forall ((x Int)) (= (g14 x) (div x 2))))
(assert (= h14 1))
...
(assert (forall ((x Int)) (= (f11 x) (- (mod (+ 1 (v12 x)) (+ 1 (+ x x))) 2))))
...
(assert (forall ((x Int)) (= (f9 x) (- (- (* (+ (v10 x) x) 2) 1) x))))
(assert (= g9 1))
