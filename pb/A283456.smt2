;; sequence(s): A283456
;; terms: 5 13 40 136 490 1828 6970 26956 105250 413668 1633450 6471676 25703410 102269908 407460730 1625010796 6485595970 25899140548 103467028810 413479908316
;; small program: ((((1 + 2) * loop((x * x) + x, 1, loop(x + x, x, 1))) div 2) + 1) + loop((x + x) + x, x, 1)
;; fast program: (((loop((x * x) + x, 1, loop(x + x, x, 1)) div 2) * (1 + 2)) + 1) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (div (* (+ 1 2) (v0 x)) 2) 1) (v2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (div (v3 x) 2) (+ 1 2)) 1) (w5 x)))))
