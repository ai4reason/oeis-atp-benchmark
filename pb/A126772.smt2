;; sequence(s): A126772
;; terms: 1 1 1 2 4 12 48 240 1680 15120 181440 2903040 60963840 1706987520 63158538240 3094768373760 201159944294400 17299755209318400 1972172093862297600 297797986173206937600
;; small program: loop(loop2(((y * y) + x) div ((x + y) + y), x + y, y, 2, 2) * x, x, 1)
;; fast program: loop(loop(loop2(((y * y) div (x + y)) + x, x, x - 1, (if x <= 0 then 0 else 1) + 2, 1) - 1, 1, y) * x, x - 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (div (+ (* y y) x) (+ (+ x y) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (div (* y y) (+ x y)) x))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (forall ((x Int)) (= (h4 x) (- x 1))))
(assert (forall ((x Int)) (= (i4 x) (+ (ite (<= x 0) 0 1) 2))))
(assert (= j4 1))
