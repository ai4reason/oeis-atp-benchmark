;; sequence(s): A101352
;; terms: 0 2 6 15 33 69 140 280 556 1101 2179 4315 8554 16978 33738 67115 133637 266305 531032 1059500
;; small program: (loop((x + x) + y, x, 0) - 1) + loop2(x + y, x, x, 1, 1)
;; fast program: (((2 * (loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) - 2)) - x) + 1) + loop2(x + y, x, x, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (* 2 (- (v2 x) 2)) x) 1) (w5 x)))))
