;; sequence(s): A30637
;; terms: 262144 387420489 3814697265625 1628413597910449 5559917313492231481 112455406951957393129 14063084452067724991009 104127350297911241532841 3244150909895248285300369 210457284365172120330305161 699053619999045038539170241
;; small program: loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2 + ((2 + 2) * (2 + 2)), 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (loop(x * x, 2, ((x * x) * x) * x) * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (= g7 2))
(assert (forall ((x Int)) (= (h7 x) (* (* (* x x) x) x))))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (forall ((x Int)) (= (v7 x) (u7 g7 (h7 x)))))
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (* (v7 x) x)))))
