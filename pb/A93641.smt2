;; sequence(s): A93641
;; terms: 1 2 3 4 5 6 7 8 10 11 12 13 14 16 17 19 20 22 23 24
;; small program: ((compr(loop2(loop(if ((2 - loop(x + x, x, 2)) mod (1 + x)) <= 0 then (1 + x) else 2, 1, y) * x, y div 2, x, 1, x) mod (1 + x), x) * 2) div 2) + 1
;; fast program: (((compr((((loop(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) - 1, x mod 2, x) mod 2) + 1) + x) mod 2, x) div 2) - x) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (mod (v5 x) 2) 1) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (div (v4 x) 2) x) 1) x))))
