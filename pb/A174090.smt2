;; sequence(s): A174090
;; terms: 1 2 3 4 5 7 8 11 13 16 17 19 23 29 31 32 37 41 43 47
;; small program: compr((loop(x + x, x, 1) mod loop(y + y, x, 1)) - 2, 1 + x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1, x) - 2, y) + 1) * x, 0, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (mod (v1 x) (v2 x)) 2))))
