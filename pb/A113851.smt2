;; sequence(s): A113851
;; terms: 64 729 15625 46656 117649 1000000 1771561 4826809 7529536 11390625 24137569 47045881 85766121 113379904 148035889 308915776 594823321 729000000 887503681 1291467969
;; small program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) * x) + x, y, 2 + (2 + 2), 1, x)
;; fast program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) + 1) * ((((x * x) * x) * x) * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (* (* (* (* x x) x) x) x)))))
