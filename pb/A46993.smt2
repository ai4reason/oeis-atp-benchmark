;; sequence(s): A46993
;; terms: 1 1 2 4 12 36 144 576 2304 9216 46080 230400 1382400 8294400 49766400 298598400 2090188800 14631321600 117050572800 936404582400
;; small program: loop(loop(((loop(x * y, y - 1, 2) mod (1 + y)) div 2) + x, y, 1) * x, x, 1)
;; fast program: loop(loop(loop(loop(if (((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (div (mod (v2 x y) (+ 1 y)) 2) x))))
