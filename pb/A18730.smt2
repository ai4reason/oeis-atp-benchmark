;; sequence(s): A18730
;; terms: 1 5 11 17 55 85 187 935
;; small program: loop(compr((loop(2 * (2 * (2 + x)), 2 + 2, 1) + x) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + ((2 - (x * x)) * x)) * (1 + (x * x)), 1, 2 + 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* (- 2 (* x x)) x)) (+ 1 (* x x))))))
(assert (= g5 1))
(assert (= h5 (+ 2 2)))
