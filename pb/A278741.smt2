;; sequence(s): A278741
;; terms: 3 5 17 65 1025 4097 65537 262145 4194305 268435457 1073741825 68719476737 1099511627777 4398046511105 70368744177665 4503599627370497 288230376151711745 1152921504606846977 73786976294838206465 1180591620717411303425
;; small program: ((loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) div 2) + 1
;; fast program: (((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) + 1) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (div (w0 x) 2) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (div (w4 x) 2) 1) 2) 1))))
