;; sequence(s): A227085
;; terms: 4 12 29 62 122 225 393 655 1048 1618 2421 3524 5006 6959 9489 12717 16780 21832 28045 35610
;; small program: loop(loop((loop(loop(x + y, y, x), y - 1, 2) + x) + y, y, x), 1 + x, 1)
;; fast program: ((((loop(loop((((((loop((x * x) + x, 1, (((x * x) + x) div 2) + x) div 2) div (1 + 2)) + (((x * x) + x) div 2)) + x) + x) + x, 1, y) + x, x, 1) + x) + 2) + x) + x) + 1
(set-logic UFNIA)
...
(assert (= g6 1))
(assert (forall ((x Int)) (= (h6 x) (+ (div (+ (* x x) x) 2) x))))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 g6 (h6 x)))))
(assert (forall ((x Int)) (= (f5 x) (+ (+ (+ (+ (div (div (v6 x) 2) (+ 1 2)) (div (+ (* x x) x) 2)) x) x) x))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (v4 x) x) 2) x) x) 1))))
