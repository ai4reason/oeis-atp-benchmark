;; sequence(s): A86936
;; terms: 1 2 2 2 3 3 4 4 5 6 7 7 8 8 9 10 11 11 12 12
;; small program: loop(loop(((loop(x * y, x - 2, 1) mod x) + 1) mod 2, 1, y) + x, x, 1)
;; fast program: (((loop(loop((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, (x - 1) div 2, 1) * (if (x - 1) <= 0 then 2 else x)) - 1) mod (1 + x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (v2 x) x) 1) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) (ite (<= x 0) 1 2)))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (- (* (v3 x) (ite (<= (- x 1) 0) 2 x)) 1) (+ 1 x)) 1))))
