;; sequence(s): A140592
;; terms: 2 5 7 8 11 12 14 17 19 20 22 25 26 29 31 32 35 36 38 41
;; small program: compr(loop2(loop(1 - x, y, x), y div 2, x, 0, x), x) + 2
;; fast program: ((((loop2((y - x) div 2, x, 2 + 2, x div 2, 0) + x) mod 2) + 2) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (mod (+ (w3 x) x) 2) 2) x) x))))
