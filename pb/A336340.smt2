;; sequence(s): A336340
;; terms: 1 3 5 7 8 9 10 11 12 14 16 17 18 21 24 25 26 27 28 30
;; small program: compr(loop(((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) + x) mod 2) + 1) + x, x + x, 2) mod 2, 1 + x)
;; fast program: compr(loop((if loop(((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2, (1 + x) mod 2, x) <= 0 then 2 else 1) + x, x + x, 2) mod 2, 1 + x)
(set-logic UFNIA)
