;; sequence(s): A60293
;; terms: 0 1 3 6 9 12 15 19 22 26 30 34 38 42 46 50 55 59 63 68
;; small program: (((loop((x * y) + loop(x * y, y - 1, 1), x, 0) * x) - 1) div loop(x * y, x, 1)) + 1
;; fast program: (loop(loop(x * y, y - 1, 1) + (x * y), x, 0) div loop(x * y, x - 1, 1)) + (if (x - 2) <= 0 then 0 else 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (- (* (v0 x) x) 1) (v2 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (v3 x) (v5 x)) (ite (<= (- x 2) 0) 0 1)))))
