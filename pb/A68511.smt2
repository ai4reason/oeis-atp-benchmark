;; sequence(s): A68511
;; terms: 0 0 2 ~2 14 ~2 146 82 ~46 ~302 1286 262 25934 21838 13646 ~2738 444974 379438 9437546 9175402
;; small program: loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, x, 1) - loop(x + x, x, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x, 1) - loop2(x * y, y, x div 2, 1 + (x mod 2), 2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (- (v6 x) 2) 0) (+ 1 x) 1))))
(assert (= g5 1))
