;; sequence(s): A36467
;; terms: 1 1 2 3 4 7 6 11 8 15 14 17 20 21 22 25 28 31 30 37
;; small program: loop((compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + x), y) - x) + 1, x, 1)
;; fast program: loop(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) - x, x - 1, 1)
(set-logic UFNIA)
