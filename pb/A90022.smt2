;; sequence(s): A90022
;; terms: 0 1 25 253 2065 15541 112825 804973 5692705 40071781 281367625 1972955293 13823978545 96820307221 677949854425 4746473419213 33228592555585 232613204977861 1628344491013225 11398619145204733
;; small program: ((loop((2 * ((x + x) + x)) + x, x, 1) - loop((x + x) + x, x, 1)) - loop(x + x, x + x, 1)) + 1
;; fast program: ((loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) - loop(x * x, 1, loop(x + x, x, 1))) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (- (- (v0 x) (v1 x)) (v2 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (- (w3 x) (v4 x)) (w6 x)) 1))))
