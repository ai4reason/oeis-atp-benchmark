...
(assert (forall ((x Int)) (= (small x) (+ (- (+ (+ (+ (+ (div (div (+ (+ (div (+ (div (+ (div (v0 x) (+ 1 2)) x) 2) x) 2) x) 2) 2) 2) x) 2) x) 2) x) 2))))
(assert (forall ((x Int)) (= (f2 x) (+ (ite (<= (- x (* 2 (* 2 (+ 2 2)))) 0) 1 0) x))))
(assert (forall ((x Int)) (= (g2 x) x)))
(assert (= h2 1))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 (g2 x) h2))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (+ (+ (+ (div (div (+ (+ (div (+ (div (+ (div (+ (+ (div (+ (+ (mod (v2 x) 2) (div x 2)) 2) (+ 2 2)) 2) x) 2) x) (+ 1 2)) x) 2) x) 2) 2) 2) 2) x) 2) x) x) 2))))
