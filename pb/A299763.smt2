;; sequence(s): A299763
;; terms: 1 3 4 6 8 12 14 18 20 24 30 32 38 42 44 48 54 60 62 68
;; small program: (compr((loop(x + x, x, 2) - 2) mod (1 + x), x) + 1) + loop(1, x * x, x)
;; fast program: ((compr(loop(loop((((((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) mod 2) + 1) mod (1 + 2)) - 1) mod (1 + 2)) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, (1 + x) mod 2, x) - 1, x) + 1) - 1) + (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (+ (v3 x) 1) 1) (ite (<= x 0) 1 2)))))
