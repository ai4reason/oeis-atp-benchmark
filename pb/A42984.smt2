;; sequence(s): A42984
;; terms: 1 11 48 140 326 657 1197 2024 3231 4927 7238 10308 14300 19397 25803 33744 43469 55251 69388 86204
;; small program: loop(loop(loop(loop(2 + (2 + (2 + (x + y))), y, x) + y, y, x) + y, y, x), x, 1) + x
;; fast program: loop(loop(((loop((loop((x * x) + x, 1, (((x * x) + x) div 2) + x) div (2 + (2 + 2))) + ((x * x) * x), 1, 1 + x) - 1) - x) - x, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ 2 (+ 2 (+ 2 (+ x y)))))))
...
(assert (= g7 1))
(assert (forall ((x Int)) (= (h7 x) (+ (div (+ (* x x) x) 2) x))))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (forall ((x Int)) (= (v7 x) (u7 g7 (h7 x)))))
(assert (forall ((x Int)) (= (f6 x) (+ (div (v7 x) (+ 2 (+ 2 2))) (* (* x x) x)))))
(assert (= g6 1))
