;; sequence(s): A720
;; terms: 0 1 2 2 3 3 4 4 4 4 5 5 6 6 6 6 7 7 8 8
;; small program: loop((loop((x * y) * y, y, 1) mod (1 + y)) + x, x, 0)
;; fast program: loop(loop(if (((1 + loop(x + x, x - 2, 2 + 2)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (v2 x) (+ 1 x)))))
