;; sequence(s): A102699
;; terms: 1 1 2 6 16 42 104 252 592 1370 3112 6996 15536 34244 74832 162616 351136 754938 1615208 3443940
;; small program: loop((loop(loop(x + x, x, 1) - loop((x * y) div (y - (y div 2)), x - 1, 2), 1, y) + x) + x, x - 1, 1)
;; fast program: loop((loop(loop(x + x, x, 1) - loop((x * y) div (y - (y div 2)), x, 1), 1, y) + x) + x, x - 2, 1) * (if (x - 1) <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (div (* x y) (- y (div y 2))))))
...
(assert (forall ((x Int)) (= (fast x) (* (v4 x) (ite (<= (- x 1) 0) 1 2)))))
