...
(assert (forall ((x Int)) (= (small x) (- (div (+ (+ (* (* (div (+ (+ (* (+ (+ (+ (div (+ (+ (div (- (* 2 (+ x x)) (mod (div x 2) 2)) (+ 1 2)) 1) x) (ite (<= x 0) 1 2)) x) x) x) (+ 1 2)) 1) x) (+ 1 2)) (ite (<= x 0) 1 2)) 2) 1) x) (+ 1 2)) x))))
(assert (forall ((x Int) (y Int)) (= (f0 x y) y)))
(assert (forall ((x Int)) (= (g0 x) (- (mod x (+ 2 2)) 2))))
(assert (= h0 2))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (fast x) (- (div (+ (+ (* (* (div (+ (+ (* (+ (+ (+ (div (+ (+ (div (+ 1 (+ (v0 x) x)) (+ 1 2)) x) x) (ite (<= x 0) 1 2)) x) x) x) (+ 1 2)) 1) x) (+ 1 2)) (ite (<= x 0) 1 2)) 2) 1) x) (+ 1 2)) x))))
