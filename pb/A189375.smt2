;; sequence(s): A189375
;; terms: 1 2 3 4 8 12 16 20 30 40 50 60 80 100 120 140 175 210 245 280
;; small program: loop(loop(loop(x + y, y, x), 1 + (y div (2 + 2)), x), x, 1)
;; fast program: loop(loop(loop(((x * x) * x) - x, 1, 2 + (x div (2 + 2))) div (2 + (2 + 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (+ 1 (div y (+ 2 2))))))
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 2 (div x (+ 2 2))))))
