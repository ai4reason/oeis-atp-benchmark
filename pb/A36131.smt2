;; sequence(s): A36131
;; terms: 1 2 4 8 16 32 64 61 55 43 19 38 9 18 36 5 10 20 40 13
;; small program: loop(x + x, x, 1) mod (1 + loop((2 * (x * x)) - x, 2, 2))
;; fast program: loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (2 + loop((x * x) * x, 1, 2 + 2)))
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (mod (v2 x) (+ 1 (+ 2 v5))))))
