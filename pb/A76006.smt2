;; sequence(s): A76006
;; terms: 1 168 17024 1354752 93499392 5881430016 346987429888 19548208103424 1064285732077568 56464495286943744 2936605030892961792 150373246607730671616 7606369972746352328704 381025640076812853706752
;; small program: loop(((loop((loop((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, y, 1) + x) + x) + x, x, 1) * loop(x + x, (x + x) + x, 1)
;; fast program: loop(loop(loop(((((loop(loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1) * loop((x * x) * x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g5 x) (+ (+ x x) x))))
(assert (= h5 1))
...
(assert (forall ((x Int)) (= (fast x) (* (v6 x) (v14 x)))))
