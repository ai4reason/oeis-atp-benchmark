;; sequence(s): A18497
;; terms: 1 2 7 14 37 74 259 518
;; small program: loop(compr(loop(loop(x * x, 2, 2 + x) + x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: compr((((2 * (2 + loop(x * x, 2, 2 + 2))) - x) - x) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (- (- (* 2 (+ 2 v5)) x) x) (+ 1 x)))))
