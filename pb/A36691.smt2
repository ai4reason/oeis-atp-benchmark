;; sequence(s): A36691
;; terms: 1 4 24 192 1728 17280 207360 2903040 43545600 696729600 12541132800 250822656000 5267275776000 115880067072000 2781121609728000 69528040243200000 1807729046323200000 48808684250726400000 1366643159020339200000
;; small program: loop((compr(loop(x * y, x, 2) mod (2 + x), y) + 2) * x, x, 1)
;; fast program: loop((compr(loop(((if (((1 + loop(x + x, (x - 1) div 2, 2)) mod (1 + x)) - 2) <= 0 then 1 else x) mod 2) + x, (1 + x) mod 2, x) - x, y) + 1) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 x) 2) x))))
