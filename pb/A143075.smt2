;; sequence(s): A143075
;; terms: 1 4 11 24 47 86 152 262 444 742 1228 2018 3301 5382 8755 14218 23063 37380 60552 98052
;; small program: loop(loop(loop((loop2(x + y, x, y, 2, 1) - 1) + x, 1 + x, 1) div 2, 1, y) + x, x, 1)
;; fast program: (((loop((loop((loop2(x + y, x, x, 1 + 2, 2) - (((x * x) + x) div 2)) - x, 1, 2 + x) - 2) - 2, 1, 2 + x) * 2) - x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (- (w6 x) (div (+ (* x x) x) 2)) x))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (div (div (- (* (v4 x) 2) x) 2) 2))))
