;; sequence(s): A62548
;; terms: 2 14 38 50 110 122 146 158 326 338 362 374 434 446 470 482 974 986 1010 1022
;; small program: (compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x), x + x) * 2) + 2
;; fast program: ((((loop(((((loop((((((loop(((((loop((((((loop(((((loop2(x - y, y div 2, x, x, x) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 2) * (2 + 2)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (* (+ (v2 x) x) (+ 1 2)) 2) (+ 2 2)) 2))))
