;; sequence(s): A83907
;; terms: 1 2 6 6 30 30 210 210 210 210 2310 2310 30030 30030 30030 30030 510510 510510 9699690 9699690
;; small program: loop(((loop((x * y) * y, y - 1, y) mod (1 + y)) * x) + x, x, 1)
;; fast program: loop((loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then x else 0, 1, y) * x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (mod (v1 x y) (+ 1 y)) x) x))))
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= (- (v4 x) 2) 0) x 0))))
(assert (= g3 1))
