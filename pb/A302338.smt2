;; sequence(s): A302338
;; terms: 4 8 10 16 16 20 22 32 28 32 34 40 40 44 46 64 52 56 58 64
;; small program: (loop(loop((x - 1) mod y, 1 + x, 0), x, x) + x) + ((2 + x) * 2)
;; fast program: (((loop((loop2((((y div x) mod 2) + 1) * x, y, 2 + (2 + 2), 1, x) - 1) + x, x mod 2, x) + 2) + 2) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (v2 x) 2) 2) x) x))))
