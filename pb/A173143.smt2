;; sequence(s): A173143
;; terms: 1 3 6 11 17 24 34 45 58 72 87 104 123 144 166 189 215 244 274 305
;; small program: loop(loop(loop((if ((y * y) mod x) <= 0 then 1 else 0) + x, x, 1 + x), y, 1) + x, x, 1)
;; fast program: loop(compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (ite (<= (mod (* y y) x) 0) 1 0) x))))
