;; sequence(s): A94977
;; terms: 1 3 11 37 123 411 1371 4572 15241 50805 169350 564502 1881676 6272254 20907515 69691719 232305731 774352437 2581174791 8603915972
;; small program: loop(x div (1 + 2), x, loop(2 * ((2 * (x + x)) + x), x, 1))
;; fast program: (loop(x + x, x, 1) * loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))) div loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (v2 x) (w3 x)) (w6 x)))))
