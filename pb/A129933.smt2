;; sequence(s): A129933
;; terms: 1 2 12 840 1663200 174356582400 1561112121913344000 1754317855900734514790400000 341362923889542287855059017400320000000 15163751542692044063740921044974247390216192000000000 195932150519417838363658963697014150610807423633478962380800000000000
;; small program: loop(x * y, loop(x + y, x, 1), 1) div loop(x * y, x, 1)
;; fast program: loop(loop(loop2(x * y, y - 1, x, 1, 1 + (((x * x) + x) div 2)) div x, 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (= i5 1))
(assert (forall ((x Int)) (= (j5 x) (+ 1 (div (+ (* x x) x) 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (div (w5 x) x))))
(assert (= g4 1))
