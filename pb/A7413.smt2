;; sequence(s): A7413
;; terms: 1 2 3 1 3 2 1 2 3 2 1 3 1 2 3 1 3 2 1 3
;; small program: loop2(compr(loop2(x + y, y div 2, x, 1, x) mod 2, y) - x, 1 + y, 2, 0, x)
;; fast program: (((loop((((loop2((y - x) div 2, x, 2 + (2 + 2), 0, x) + 1) mod 2) + 1) + x, 2, x) + x) mod (2 + 2)) mod (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (mod (+ (v3 x) x) (+ 2 2)) (+ 1 2)) 1))))
