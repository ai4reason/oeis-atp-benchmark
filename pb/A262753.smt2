;; sequence(s): A262753
;; terms: 4 9 25 100 361 1369 5476 21609 85849 343396 1371241 5480281 21921124 87665769 350625625 1402502500 5609860201 22439141209 89756564836 359025060969
;; small program: loop(x * x, 1, loop((x - ((x div 2) mod 2)) + x, x, 2))
;; fast program: loop2(loop((loop(((((1 + (2 + (x + x))) div (1 + (2 + (2 + 2)))) + 1) + x) + x, 1, loop(x + x, x, 1)) + 1) div 2, 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (= g0 1))
(assert (forall ((x Int)) (= (f1 x) (+ (- x (mod (div x 2) 2)) x))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (+ (+ (div (+ 1 (+ 2 (+ x x))) (+ 1 (+ 2 (+ 2 2)))) 1) x) x))))
(assert (= g4 1))
