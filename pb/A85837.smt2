;; sequence(s): A85837
;; terms: 3 6 7 9 11 12 13 14 15 17 18 19 21 22 23 24 26 27 28 29
;; small program: compr(if (loop(loop(2 * (2 + x), 2, 2) * x, x, 1) mod (1 + x)) <= 0 then 1 else 0, x) + 1
;; fast program: compr((if loop((if ((1 + x) mod (1 + (2 + 2))) <= 0 then (x div (1 + (2 + 2))) else x) div (1 + (x mod 2)), x, x) <= 0 then 1 else 0) mod (1 + x), 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (ite (<= (mod (+ 1 x) (+ 1 (+ 2 2))) 0) (div x (+ 1 (+ 2 2))) x) (+ 1 (mod x 2))))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (mod (ite (<= (v4 x) 0) 1 0) (+ 1 x)))))
