;; sequence(s): A52294
;; terms: 3 5 6 7 9 10 11 12 13 14 17 18 19 20 21 22 24 25 26 28
;; small program: compr(loop(((loop2((y mod 2) + x, y div 2, x, 0, x) - 1) + x) mod (1 + x), x, x) - 1, 2 + x)
;; fast program: compr(loop(((loop2((y div 2) + x, y div 2, x, 1, x) - x) + 1) mod (1 + x), 1, x + x) mod 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (- (w2 x) 1) x) (+ 1 x)))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div y 2) x))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (- (w5 x) x) 1) (+ 1 x)))))
(assert (= g4 1))
