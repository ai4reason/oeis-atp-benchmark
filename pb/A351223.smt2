;; sequence(s): A351223
;; terms: 1 120 7560 369600 15765750 617512896 22813670880 807723671040 27686621927250 925166131890000 30286238493551040 974802747606105600 30933063577681246800 969808565876506272000 30090926129273230320000 925249170367839629537280 28225069296255264089522250
;; small program: loop2(x * y, 1 + y, (x + x) + x, 1, 2 + 2) div loop(((y * y) * y) * x, x, 1)
;; fast program: (((((loop2((2 + y) * x, 1 + y, 1 + x, 1, x + x) div loop(x * y, x, 1)) * (1 + (x + x))) * loop2(x * y, 1 + y, x, 1, 1 + x)) div loop(x * y, x, 1)) * (1 + x)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (div (* (* (div (w2 x) (v3 x)) (+ 1 (+ x x))) (w4 x)) (v5 x)) (+ 1 x)) 2))))
