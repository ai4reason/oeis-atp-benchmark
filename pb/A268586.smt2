;; sequence(s): A268586
;; terms: 0 0 0 2 9 30 88 240 624 1568 3840 9216 21760 50688 116736 266240 602112 1351680 3014656 6684672
;; small program: loop((loop(loop(x + x, x, 1 + x) div loop(x * x, 2, 2), 1, y) + x) + x, x, 0)
;; fast program: (((((loop(x + x, x - 1, (1 + loop(x + y, x, x)) + 2) div 2) + 1) div 2) + 1) - loop(x + x, x, 1)) div 2
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f1 x) (div (v2 x) v3))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (h4 x) (+ (+ 1 (v5 x)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (div (+ (div (v4 x) 2) 1) 2) 1) (v6 x)) 2))))
