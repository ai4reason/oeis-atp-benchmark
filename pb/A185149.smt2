;; sequence(s): A185149
;; terms: 1 1 3 27 756 68040 20207880 20228087880 69422797604160 828491666608045440 34788365080871828025600 5191328567558179408948185600 2776779354844059467693477099212800 5363460395055494624228658756213491712000
;; small program: loop((loop(2 * ((x - (x div y)) + x), y, 1 + 2) * x) div (2 + y), x, 1) div (1 + x)
;; fast program: loop(loop((loop2(x * y, y - 1, x - 1, 1, 2 + (x + x)) * (if x <= 0 then 1 else (1 + 2))) div loop(x * y, x, 1), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (div (* (v1 x y) x) (+ 2 y)))))
...
(assert (forall ((x Int)) (= (f3 x) (div (* (w4 x) (ite (<= x 0) 1 (+ 1 2))) (v5 x)))))
(assert (= g3 1))
