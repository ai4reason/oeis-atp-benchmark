;; sequence(s): A134181
;; terms: 0 0 0 2 4 8 12 18 28 38 52 68 84 102 124 150 176 206 238 270
;; small program: loop(loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, x) - x, 1, y) + x, 1 + x, 0) * 2
;; fast program: (((loop((((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) + x) * 2) - 1) - x) - x) - x, 1, 1 + x) - 2) - 1) - ((2 + x) * x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (* (+ (v5 x) x) 2) 1) x) x) x))))
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ 1 x))))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 g4 (h4 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (- (- (- (v4 x) 2) 1) (* (+ 2 x) x)) 1))))
