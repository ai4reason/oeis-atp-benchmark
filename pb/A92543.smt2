;; sequence(s): A92543
;; terms: 1 2 1 1 2 3 4 3 2 1 1 2 3 4 5 6 5 4 3 2
;; small program: loop(loop(loop((((y - (x mod 2)) - x) div (x * x)) + x, x, 2) mod 2, 1, y) + x, x + x, 1) - x
;; fast program: loop(((((((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x - 1) + 1) + 2) div 2) + x) mod 2) + y) - x, x + x, 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (- (- y (mod x 2)) x) (* x x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (- (+ (mod (+ (div (+ (+ (w4 x) 1) 2) 2) x) 2) y) x))))
