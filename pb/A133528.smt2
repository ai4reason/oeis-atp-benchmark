;; sequence(s): A133528
;; terms: 134067 1905564 6731644 30853588 77781820 224046148 814042660 1677408772 4196089300 8798157652 14524697380 24416409028 44015043748 81445473148 126644484460 206323651300 312259574092 421413266740
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * (loop(x * x, 2, x) * x), y, 2, 1, x), 1, y) + x, 1 + y, 2 + 2, 0, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((((x * x) * x) * x) * x), y, 2, 1, x), 1, y) + x, 1 + y, 2 + 2, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (v3 x y) 1) (* (v5 x) x)))))
...
(assert (forall ((x Int)) (= (g10 x) (- 1 (mod x 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (* (+ (v9 x y) 1) (* (* (* (* x x) x) x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (w6 x))))
