;; sequence(s): A36584
;; terms: 3 1 2 3 2 1 3 1 2 1 3 2 3 1 2 3 2 1 3 2
;; small program: loop(loop2((loop(loop2(x + y, y div 2, x, 0, x) mod 2, 1, y) + x) * x, 1 + y, 2, 1, x) div 2, 2, x)
;; fast program: ((((loop((((loop2((y - x) div 2, x, 2 + (2 + 2), 0, x) + 1) mod 2) + 1) + x, 2, x) + x) mod (2 + 2)) + 2) mod (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div (w1 x) 2))))
(assert (= g0 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (mod (+ (v4 x) x) (+ 2 2)) 2) (+ 1 2)) 1))))
