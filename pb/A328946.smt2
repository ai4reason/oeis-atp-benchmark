;; sequence(s): A328946
;; terms: 1 1 2 12 72 2160 64800 13608000 2857680000 600112800000 126023688000000 291114719280000000 672475001536800000000 20194424296150104000000000 606438561613387623120000000000 18211350005250030322293600000000000 546886840657658410578476808000000000000
;; small program: loop(loop(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, y - 1, x), x, 1)
;; fast program: loop(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, y, 1) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (ite (<= (- (v7 x) 2) 0) (+ 1 x) 1))))
(assert (= g6 1))
