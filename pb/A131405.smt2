;; sequence(s): A131405
;; terms: 1 2 7 16 37 82 179 384 813 1702 3531 7272 14889 30342 61603 124700 251825 507582 1021535 2053388
;; small program: ((2 * (loop(x + x, x, 2) - loop2(x + y, x, x, 2, 2))) + 1) + x
;; fast program: (1 + (loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2))) - loop2(x + y, x, x, 2 + 2, 2 + 2))) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (* 2 (- (v0 x) (w1 x))) 1) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ 1 (- (v2 x) (w5 x))) x))))
