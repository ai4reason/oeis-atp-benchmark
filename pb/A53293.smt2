;; sequence(s): A53293
;; terms: 1 6 2016 33784128 27811094169600 1122211189922928537600 2218959336124989671614429593600 214992513152176999576908105619651923148800 1020690003311610463765638355505358381593396977336320000 237443634207909205360438080389756681126654524500073656592021585920000
;; small program: loop(loop((x * x) - x, 1, loop((2 * ((x + x) + x)) + x, y, 1)) * x, x, 1) div loop((2 * ((x + x) + x)) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) - 1) * loop2(x * y, y, x - 1, 1, 1 + (2 + (2 + 2))), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (+ (* 2 (+ (+ x x) x)) x))))
(assert (forall ((x Int)) (= (g3 x) x)))
(assert (= h3 1))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 (g3 x) h3))))
(assert (forall ((x Int)) (= (small x) (div (v0 x) (v3 x)))))
...
(assert (forall ((x Int)) (= (f5 x) (* (- (w6 x) 1) (w7 x)))))
(assert (= g5 1))
