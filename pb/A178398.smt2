;; sequence(s): A178398
;; terms: 18 48 180 448 1584 2548 5508 7600 13248 26100 31744 53428 72324 83248 108288 154548 212400 234484 309808 368064
;; small program: loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 1) * (x * x)) + 1, y, 2, 1, x) - 1
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (x * x)) + 1, y, 2, 1, x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (+ (v1 x y) 1) (* x x)) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (+ (v4 x y) 1) (* x x)) 1))))
