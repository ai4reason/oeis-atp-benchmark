;; sequence(s): A3144
;; terms: 1 3 5 7 8 10 12 14 16 18 20 21 23 25 27 29 31 32 34 36
;; small program: loop(compr(compr(loop2(loop(if ((1 + x) mod (2 * (2 + 2))) <= 0 then 1 else 0, 1, y) + x, y div 2, x, 0, x), x) mod 2, y), x, 0) + 1
;; fast program: loop(((compr(loop2(loop(if ((1 + x) mod (2 * (2 + 2))) <= 0 then 1 else 0, 1, y) + x, y div 2, x, 0, x), x) mod 2) + 1) + x, x, 1)
(set-logic UFNIA)
