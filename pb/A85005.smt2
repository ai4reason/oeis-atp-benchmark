;; sequence(s): A85005
;; terms: 1 3 4 4 3 3 4 4 3 1 0 0 1 1 0 0 1 3 4 4
;; small program: loop(loop((loop(loop((loop((y * y) div (x - y), 1 + x, 0) + x) mod 2, 1, y) + x, x, 0) * 2) - x, 1, y) + x, x, 1) + x
;; fast program: loop(loop((loop(loop((((1 + ((x - (x div (1 + (2 + (2 * (2 + 2)))))) + x)) div (1 + (2 + 2))) + 1) mod 2, 1, y) + x, x, 0) * 2) - x, 1, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (mod (+ (div (+ 1 (+ (- x (div x (+ 1 (+ 2 (* 2 (+ 2 2)))))) x)) (+ 1 (+ 2 2))) 1) 2))))
(assert (= g8 1))
