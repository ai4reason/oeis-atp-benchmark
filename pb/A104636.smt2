;; sequence(s): A104636
;; terms: 2 6 8 14 18 20 26 30 36 44 48 50 54 56 68 74 78 86 90 96
;; small program: compr((loop((loop((x - 2) + x, 2, y) * x) div y, x, 2) - 1) mod (1 + (x + x)), 1 + x)
;; fast program: ((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 1)) mod (1 + 2)) - 1) mod (1 + x), 1 + x) div 2) * 2) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (- (v1 x) 1) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (- (mod (+ (v4 x) x) 2) (mod x 1)) (+ 1 2)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (div (v3 x) 2) 2) 1))))
