;; sequence(s): A159465
;; terms: 3 8 15 26 39 54 71 90 111 134 161 190 221 254 289 326 365 406 449 494
;; small program: (loop(loop(((loop(2 + (y div x), x + x, 2) div 2) + x) * 2, 1, y) + x, x, 1) + 2) + x
;; fast program: ((((((((((loop(loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, x - 1) + 1) div 2, 1, y) + x, x, 1) + x) * 2) - 1) - x) + 1) + (x * x)) div 2) + x) * 2) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (+ (div (v2 x) 2) x) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (div (+ (w5 x) 1) 2))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (+ (div (+ (+ (- (- (* (+ (v3 x) x) 2) 1) x) 1) (* x x)) 2) x) 2) 1) x))))
