;; sequence(s): A18760
;; terms: 1 3 7 21 47 141 329 987
;; small program: loop(compr(((loop((1 + (2 + (2 + x))) * (2 + x), 2, 2) - 1) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 * ((2 * ((2 * (x + x)) + x)) * x)) + x, 1, 1 + (2 + (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (* (+ 1 (+ 2 (+ 2 x))) (+ 2 x)))))
(assert (= g2 2))
(assert (= h2 2))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (- (- v2 1) 2) (+ 1 x)))))
