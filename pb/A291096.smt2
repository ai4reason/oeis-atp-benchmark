;; sequence(s): A291096
;; terms: 1 3 36 594 11340 235467 5164236 117704340 2760422652 66179363580 1614629242512 39958835859306 1000667989897524 25310418084553770 645671000841035400 16592979103827051240 429173117580596633820 11163550152596460675012 291848008677713303547312
;; small program: (loop2(x * y, y - 1, x - 1, 1, 2 * (x + x)) div loop(x * y, x, 1)) * loop((x + x) + x, x, 1)
;; fast program: (loop2(x * y, 1 + y, x - 1, 1, 2 + ((x + x) + x)) div loop(x * y, x, 1)) * loop2(x * y, y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (w0 x) (v1 x)) (v2 x)))))
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* x y))))
(assert (forall ((x Int) (y Int)) (= (g3 x y) (+ 1 y))))
(assert (forall ((x Int)) (= (h3 x) (- x 1))))
(assert (= i3 1))
(assert (forall ((x Int)) (= (j3 x) (+ 2 (+ (+ x x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (w3 x) (v4 x)) (w5 x)))))
