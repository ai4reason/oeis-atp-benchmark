;; sequence(s): A73632
;; terms: 1 3 4 5 6 7 8 10 12 14 15 17 18 20 21 22 25 27 30 32
;; small program: compr((loop(x div 2, x, loop((x + x) + x, x, 1)) + 1) mod 2, 1 + x)
;; fast program: (((compr(((loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) div loop(x + x, x, 1)) + 1) mod 2, 1 + x) - 1) - (x mod 2)) + 1) + (x mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (div (w4 x) (v6 x)) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (- (v3 x) 1) (mod x 2)) 1) (mod x 2)))))
