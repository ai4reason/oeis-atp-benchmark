;; sequence(s): A74745
;; terms: 6 21 56 133 276 497 820 1257 1924 2823 3970 5487 7250 9271 11762 14889 18488 22575 27332 32515
;; small program: (loop(loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x) + x, 1 + y, 2, 1, y) + x, x, 2) + 2) + 2
;; fast program: (loop(loop(loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1, 1, y) * x, 1 + y, 2, 1, x), 1, y) + x, x, 2) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (+ (* (v8 x) 2) 1))))
(assert (= g7 1))
