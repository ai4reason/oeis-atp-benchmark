;; sequence(s): A2476-A144921
;; terms: 7 13 19 31 37 43 61 67 73 79 97 103 109 127 139 151 157 163 181 193
;; small program: compr(loop((2 - loop(x + x, x, 2)) mod (1 + x), 1 - (x mod (1 + 2)), x) - 1, 2 + x) + 1
;; fast program: (((compr(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1 + x) * 2) + 1) * (1 + 2)) - 2
(set-logic UFNIA)
