;; sequence(s): A166010
;; terms: 0 5 21 45 117 165 285 357 525 837 957 1365 1677 1845 2205 2805 3477 3717 4485 5037
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) - 2) - 2
;; fast program: loop2(loop((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, x) * 2) + 1, 1, y) * x, y, 2, 1, x) - (if (loop(2 * x, x mod 2, x) - 1) <= 0 then 1 else (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (w3 x) (ite (<= (- (v7 x) 1) 0) 1 (+ 2 2))))))
