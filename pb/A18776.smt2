;; sequence(s): A18776
;; terms: 1 5 7 29 35 145 203 1015
;; small program: loop(compr(loop(1 + ((2 + (y * y)) * (x * x)), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((loop((2 * ((x * x) * x)) - x, 1, 2 * (2 + 2)) + x) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (* (+ 2 (* y y)) (* x x))))))
(assert (= g2 2))
(assert (= h2 2))
