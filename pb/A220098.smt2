;; sequence(s): A220098
;; terms: 1 2 1 2 3 2 3 4 3 2 3 4 5 4 3 4 5 6 5 4
;; small program: ((loop(loop(loop(1 + ((y + y) div (2 + x)), 2 * (x + x), 0) mod 2, 1, y) + x, x, 0) * 2) - x) + 1
;; fast program: ((loop(loop(loop((((loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, (x - 2) div 2) + 2) + x, 2, x) - x) + 1) - x) + x, 1, x + x) mod 2, 1, y) + x, x, 0) * 2) - x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (div (+ y y) (+ 2 x))))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (div (v8 x y) (+ 2 x)))))
(assert (forall ((x Int) (y Int)) (= (g7 x y) y)))
(assert (= h7 (+ 1 2)))
(assert (= i7 (+ 2 2)))
(assert (forall ((x Int)) (= (j7 x) (div (- x 2) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (- (+ (- (v6 x) x) 1) x) x))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (* (v3 x) 2) x) 1))))
