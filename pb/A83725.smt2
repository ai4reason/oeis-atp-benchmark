;; sequence(s): A83725
;; terms: 0 3 12 33 72 145 252 413 624 909 1300 1771 2376 3107 3948 4935 6112 7497 9036 10811
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, x) * x) + x
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * 2) + x) * x
(set-logic UFNIA)
