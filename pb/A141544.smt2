;; sequence(s): A141544
;; terms: 1 3 7 9 13 19 21 27 31 33 37 39 49 51 61 63 67 73 79 81
;; small program: compr(((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x))) - 1, 2 + x) - 2
;; fast program: compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 2 + x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (mod (- (v1 x) x) (+ 1 (+ x x))) 1))))
