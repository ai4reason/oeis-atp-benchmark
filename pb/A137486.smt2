;; sequence(s): A137486
;; terms: 4194304 31381059609 2384185791015625 3909821048582988049 81402749386839761113321 3211838877954855105157369 1174562876521148458974062689 13569980418174090907801371961
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + loop(2 * (2 + x), 2, 2), 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(loop((x * x) * x, 1, x * x) * x, 1, (x * x) * x), y, 2, 1, x)
(set-logic UFNIA)
