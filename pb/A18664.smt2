;; sequence(s): A18664
;; terms: 1 2 4 5 10 20 41 82 164 205 410 820
;; small program: loop(compr(loop(((x * x) * y) + x, 2, 2 + 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((((((((compr(loop((2 * (x * x)) + x, 1, 2 * (2 + (2 * (2 + 2)))) mod (1 + x), x) + 1) + x) - x) + x) - x) * (1 + 2)) + 1) div (1 + 2)) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (+ (* (- (+ (- (+ (+ (v3 x) 1) x) x) x) x) (+ 1 2)) 1) (+ 1 2)) 1))))
