;; sequence(s): A53767
;; terms: 0 4 10 18 27 37 49 63 78 94 112 132 153 175 199 224 250 277 305 335
;; small program: loop(compr(loop(x * y, x, 2) mod (2 + x), y) + x, x, x) + x
;; fast program: loop(compr(if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then x else 0, y) + x, x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) 2) 0) x 0))))
