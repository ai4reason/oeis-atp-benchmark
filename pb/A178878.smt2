;; sequence(s): A178878
;; terms: 1 2 3 4 5 6 7 8 10 12 14 15 16 20 21 24 28 30 35 40
;; small program: loop(compr(loop(((2 + y) * x) + x, 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop2((compr(loop((x - y) * x, 2, 1 + (2 + (2 + 2))) mod (1 + x), y) * x) + x, 0, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (+ 2 y) x) x))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
