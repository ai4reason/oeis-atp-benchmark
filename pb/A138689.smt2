;; sequence(s): A138689
;; terms: 30 35 51 75 147 195 315 387 555 867 987 1395 1707 1875 2235 2835 3507 3747 4515 5067
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) + loop(1 + (x * x), 2, 2)
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * x, y, 2, 1, x) + (2 * (2 * (2 + (2 + 2))))) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (w4 x) (* 2 (* 2 (+ 2 (+ 2 2))))) 2))))
