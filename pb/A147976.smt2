;; sequence(s): A147976
;; terms: ~3 ~5 ~5 25 271 1825 10735 59425 318271 1670785 8656975 44454625 226827871 1151991745 5830280815 29429454625 148249811071 745630312705 3745590106255 18797445635425
;; small program: ((loop((2 * (x + x)) + x, x, 1) - loop((x * x) + x, 1, loop(x + x, x, 1))) - loop((x + x) + x, x, 1)) - 1
;; fast program: ((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) - loop((x * x) + x, 1, loop(x + x, x, 1))) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (- (- (v0 x) (v1 x)) (v3 x)) 1))))
...
(assert (forall ((x Int)) (= (h9 x) (div x 2))))
(assert (= f10 (+ 1 2)))
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (w4 x) (v7 x)) (w9 x)) 1))))
