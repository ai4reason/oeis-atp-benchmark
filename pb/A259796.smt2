;; sequence(s): A259796
;; terms: 3 4 5 7 9 13 19 27 40 59 88 131 196 293 439 658 987 1479 2218 3327
;; small program: loop(x div 2, x, loop(1 + ((x + x) + x), x, 1)) + 2
;; fast program: (((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (+ 1 2) (w2 x)) (* 2 (v4 x))) 2))))
