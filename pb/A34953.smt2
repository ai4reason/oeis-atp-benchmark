;; sequence(s): A34953
;; terms: 3 6 15 28 66 91 153 190 276 435 496 703 861 946 1128 1431 1770 1891 2278 2556
;; small program: loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 1) + 2, y, 2, 1, x) div 2
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 1) + x, y, 2, 1, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (* (v1 x y) x) 1) 2))))
