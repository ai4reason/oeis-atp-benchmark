;; sequence(s): A153766
;; terms: 2 4 5 7 10 11 14 17 20 22 25 26 29 31 34 35 40 46 47 49
;; small program: ((compr((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + (x + x)), 1 + x) div 2) div 2) + 2
;; fast program: (compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + (x + x)), x mod 2, x), 1 + x) div (2 + 2)) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ (v1 x) x) (+ 1 (+ x x))))))
(assert (forall ((x Int)) (= (g0 x) (+ 1 x))))
(assert (forall ((x Int)) (= (t0 x) (ite (<= (f0 x) 0) x (t0 (+ x 1))))))
(assert (forall ((x Int)) (= (u0 x) (ite (<= x 0) (t0 0) (t0 (+ (u0 (- x 1)) 1))))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x)))))
(assert (forall ((x Int)) (= (small x) (+ (div (div (v0 x) 2) 2) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (v5 x) x) (+ 1 (+ x x))))))
