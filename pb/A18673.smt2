;; sequence(s): A18673
;; terms: 1 2 3 6 139 278 417 834
;; small program: loop(compr((2 + loop(((2 * (2 + x)) + x) * x, 1, loop(x * x, 2, 2))) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop((1 + (2 + (2 + x))) * (x * x), 1, 2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 1 (+ 2 (+ 2 x))) (* x x)))))
(assert (= g6 1))
(assert (= h6 (* 2 (+ 2 2))))
