;; sequence(s): A112701
;; terms: 1 8 106 1821 35435 741329 16270997 369570944 8613236374 204812473608 4949266755812 121188396669810 3000342229924222 74979188061284522 1888846103011564082 47915719069874907917 1222954711282739097587
;; small program: loop((loop(2 * ((1 + (2 + (2 + 2))) * ((x - (x div y)) + x)), y, 1) div (1 + y)) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x - 1, x)) * loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* 2 (* (+ 1 (+ 2 (+ 2 2))) (+ (- x (div x y)) x))))))
(assert (forall ((x Int) (y Int)) (= (g1 x y) y)))
(assert (= h1 1))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y) x)))))
(assert (forall ((x Int) (y Int)) (= (v1 x y) (u1 (g1 x y) h1))))
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (v1 x y) (+ 1 y)) x))))
...
(assert (forall ((x Int)) (= (f3 x) (* (div (w4 x) (v5 x)) (w6 x)))))
(assert (= g3 1))
