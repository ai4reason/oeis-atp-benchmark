;; sequence(s): A242120
;; terms: 2 3 5 7 11 13 17 19 3 9 11 17 1 3 7 13 19 1 7 11
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod loop(2 * (2 + x), 2, 2)) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop((2 + ((((x + x) + x) div (1 + (2 + 2))) + x)) + x, 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (+ 2 (+ (div (+ (+ x x) x) (+ 1 (+ 2 2))) x)) x))))
(assert (= g6 2))
(assert (= h6 2))
