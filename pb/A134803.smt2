;; sequence(s): A134803
;; terms: 3 8 119 288 4059 9800
;; small program: loop2(1 + ((y - x) - x), x, (x + x) + 2, 1, x mod 2) - 1
;; fast program: loop2((2 * ((x + x) + x)) - y, x, x, 1 + (2 + loop(0, x mod 2, 2 + 2)), 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h0 x) (+ (+ x x) 2))))
(assert (= i0 1))
...
(assert (forall ((x Int)) (= (i1 x) (+ 1 (+ 2 (v2 x))))))
(assert (= j1 1))
