;; sequence(s): A203474
;; terms: 1 7 252 41580 29729700 89278289100 1104908105901600 55674109640169820800 11329124570678156834592000 9258047307912482983660236480000 30262334718212007877669234596364800000
;; small program: loop(loop(loop2((x * y) + x, y - 1, x, 1, 2 * (2 + x)) div loop(x * y, x, 1), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(x * y, 1 + y, x, 1, 2 + (2 + (2 + x))) div loop(x * y, x, 1), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ 2 (+ 2 (+ 2 x))))))
