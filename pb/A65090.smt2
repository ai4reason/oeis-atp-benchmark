;; sequence(s): A65090
;; terms: 1 2 4 6 8 9 10 12 14 15 16 18 20 21 22 24 25 26 27 28
;; small program: loop(compr(loop(x * y, x, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x) - 1, x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (* (+ (v3 x) 1) 2) 2) 1))))
