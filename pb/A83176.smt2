;; sequence(s): A83176
;; terms: 2 6 10 21 33 52 68 95 115 174 186 259 287 344 376 477 531 610 670 781
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + ((1 + x) div 2))
;; fast program: (compr(loop((((((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) mod 2) + 1) mod (1 + 2)) - 1) mod (1 + 2)) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) * (1 + (x - (x div 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ (v0 x) 1) (+ 1 (div (+ 1 x) 2))))))
(assert (forall ((x Int)) (= (f5 x) (+ x x))))
(assert (forall ((x Int)) (= (g5 x) (div x 2))))
(assert (= h5 1))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) h5))))
(assert (forall ((x Int)) (= (f4 x) (mod (- (mod (- (mod (+ (mod (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 x)) 2) 0) 1 x) 2) 1) (+ 1 2)) 1) (+ 1 2)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 1) (+ 1 (- x (div x 2)))))))
