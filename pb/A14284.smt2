;; sequence(s): A14284
;; terms: 1 3 6 11 18 29 42 59 78 101 130 161 198 239 282 329 382 441 502 569
;; small program: loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 1) + x
;; fast program: (loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) + x
(set-logic UFNIA)
