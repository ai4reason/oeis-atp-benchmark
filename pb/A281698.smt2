;; sequence(s): A281698
;; terms: 5 14 55 269 1465 8369 48865 288449 1713025 10210049 60993025 364899329 2185181185 13094268929 78498422785 470721937409 2823257554945 16935249707009 101594317062145 609497180274689
;; small program: ((((1 + (2 + (2 + loop((x + x) + x, x, 2)))) + loop(x + x, x, 1)) * loop(x + x, x, 1)) div 2) + 1
;; fast program: ((loop((x * x) + x, 1, 2 + loop(x + x, x, 1)) div 2) + loop2(x * y, y, x, 1, 2 + (2 + 2))) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (* (+ (+ 1 (+ 2 (+ 2 (v0 x)))) (v1 x)) (v2 x)) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (div (v3 x) 2) (w5 x)) 2))))
