;; sequence(s): A210469
;; terms: 0 0 0 0 1 1 1 2 2 2 3 3 4 5 5 5 6 7 7 8
;; small program: loop(compr(compr(loop(x * y, x, 2) mod (2 + x), x) mod 2, y), x, 0) - x
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 0 else 1, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 (+ x x))) 2) 0) 0 1))))
(assert (= g5 1))
