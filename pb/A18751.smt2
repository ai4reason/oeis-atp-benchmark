;; sequence(s): A18751
;; terms: 1 2 5 10 97 194 485 970
;; small program: loop(compr((loop(2 * (2 + (x * x)), 2, 1 + 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((((x * x) - 2) * x) - x, 1, 2 + (2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (* (- (* x x) 2) x) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (* 2 (+ 2 2)))))
