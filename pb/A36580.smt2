;; sequence(s): A36580
;; terms: 0 1 2 0 2 1 0 1 2 1 0 2 0 1 2 0 2 1 0 2
;; small program: loop2(compr(loop2(x + y, y div 2, x, 1, x) mod 2, y) - x, 1 + y, 2, 1, x) - 2
;; fast program: ((loop((((loop2((y - x) div 2, x, 2 + (2 + 2), 0, x) + 1) mod 2) + 1) + x, 2, x) + x) mod (2 + 2)) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (mod (+ (v3 x) x) (+ 2 2)) (+ 1 2)))))
