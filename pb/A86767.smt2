;; sequence(s): A86767
;; terms: 1 1 1 1 1 3 1 1 1 5 1 3 1 7 1 1 1 9 1 5
;; small program: 1 + ((x mod 2) * loop(x div (1 + (x mod 2)), x, x))
;; fast program: (loop((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + x, x mod 2, x) - x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (* (mod x 2) (v0 x))))))
