;; sequence(s): A203469
;; terms: 1 3 30 1050 132300 61122600 104886381600 674943865596000 16407885372638760000 1515727634953623371280000 534621388490302221024396480000 722849817707190846398223943885440000 3759035907022704558524683975387453632000000
;; small program: loop(loop(2 * ((x - (x div (1 + y))) + x), y, x), x, 2) div 2
;; fast program: loop(loop(loop2(x * y, 1 + y, x - 1, 1, 2 + x) div loop(x * y, x, 1), 1, y) * x, x, 1) * loop2(x * y, 2 + y, x, 1, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* 2 (+ (- x (div x (+ 1 y))) x)))))
