;; sequence(s): A74938
;; terms: 1 3 9 13 27 31 37 39 81 85 91 93 109 111 117 121 243 247 253 255
;; small program: compr(loop(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x) mod 2, x mod 2, x), 1 + x)
;; fast program: (((((loop(((((loop((((((loop(((((loop((((((loop(((((loop2(x - y, y div 2, x, x, x) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) div 2) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (- (- (+ (* (+ (w8 x) x) (+ 1 2)) 1) x) x))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (- (* (div (- (* (+ (v3 x) x) (+ 1 2)) 1) 2) 2) 1))))
