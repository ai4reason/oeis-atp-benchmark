;; sequence(s): A133549
;; terms: 81 706 3107 17748 46309 129830 260151 539992 1247273 2170794 4044955 6870716 10289517 15169198 23059679 35177040 49022881 69174002 94585683 122983924
;; small program: loop(loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x) + x, y, 2 + 2, 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 2, 1, x), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (v7 x y) 1) (* (* x x) x)))))
