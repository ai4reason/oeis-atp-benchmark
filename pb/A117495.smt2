;; sequence(s): A117495
;; terms: 0 3 10 21 44 65 102 133 184 261 310 407 492 559 658 795 944 1037 1206 1349
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 0) * x) + x
;; fast program: ((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, loop(1 + x, x, 0)) * 2) + 1) * x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (mod (* (* (* (+ 1 (mod x 2)) (v4 x)) (v6 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1))))
(assert (forall ((x Int)) (= (f8 x) (+ 1 x))))
(assert (forall ((x Int)) (= (g8 x) x)))
(assert (= h8 0))
(assert (forall ((x Int) (y Int)) (= (u8 x y) (ite (<= x 0) y (f8 (u8 (- x 1) y))))))
(assert (forall ((x Int)) (= (v8 x) (u8 (g8 x) h8))))
(assert (forall ((x Int)) (= (g3 x) (v8 x))))
