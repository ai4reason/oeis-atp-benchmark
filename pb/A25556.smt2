;; sequence(s): A25556
;; terms: 0 1 2 3 3 4 5 6 6 7 7 8 8 8 9 10 10 11 11 11
;; small program: loop(loop(if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then 1 else 0, 1, y) + x, 1 + x, 0) - 1
;; fast program: loop(loop(if (loop((1 + (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) mod (1 + x), 2, x) - 1) <= 0 then 1 else 0, 1, y) + x, 1 + x, 0) - 1
(set-logic UFNIA)
