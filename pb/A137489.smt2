;; sequence(s): A137489
;; terms: 12288 20480 28672 45056 53248 69632 77824 94208 118784 126976 151552 167936 176128 192512 217088 241664 249856 274432 290816 299008
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) + 1) * loop(x * x, 2, 2 * (2 + 2))
;; fast program: ((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1) * loop(((x * x) * x) * x, 1, 2 * (2 + 2))
(set-logic UFNIA)
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v3 x) 2) 1) v7))))
