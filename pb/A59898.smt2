;; sequence(s): A59898
;; terms: 31 43 109 127 157 223 229 277 283 307 397 433 439 457 499 601 643 691
;; small program: (compr((1 - loop(x + x, x, 1)) mod (1 + ((x + x) + x)), 1 + x) * (1 + 2)) + 1
;; fast program: loop(compr(loop((1 - loop(x + x, (x - 1) div (2 * (1 + 2)), 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g5 x) (div (- x 1) (* 2 (+ 1 2))))))
(assert (= h5 2))
