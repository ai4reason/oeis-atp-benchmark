;; sequence(s): A138410
;; terms: 56 702 15500 117306 1770230 4824612 24132656 47039022 148023722 594798932 887473890 2565675756 4750035320 6321283542 10779111506 22164212252 42180328262 51520147380 90458081406 128099926010
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 1 + 2, 1, x), 1, y) * (x - 1), y, 2, 2, x)
;; fast program: loop2((loop(loop2((compr(loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * (x * x), y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g10 x) (- (div x 2) 1))))
(assert (= h10 2))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ (v8 x y) 1) (* x x)))))
