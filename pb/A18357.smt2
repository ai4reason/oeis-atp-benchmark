;; sequence(s): A18357
;; terms: 1 2 3 4 6 7 9 12 14 18 21 28 36 42 63 84 126 252
;; small program: compr(((loop(x * x, 2, 2 + 2) - 2) - 2) mod (1 + x), x) + 1
;; fast program: 1 + compr(loop((1 + x) * (x * x), 1, 2 + (2 + 2)) mod (1 + x), x)
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (f0 x) (mod (- (- v1 2) 2) (+ 1 x)))))
