;; sequence(s): A1264
;; terms: 1 4 16 64 56 24 96 84 36 44 76 4 16 64 56 24 96 84 36 44
;; small program: loop(x + x, x + x, 1) mod loop(1 + ((2 + x) * x), 2, 2)
;; fast program: loop((x * x) mod loop(1 + ((2 + x) * x), 2, 2), 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (* x x) v3))))
(assert (= g2 1))
