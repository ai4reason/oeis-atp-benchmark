;; sequence(s): A97539
;; terms: 1 9 13 25 33 37 49 57 69 85 93 97 105 109 133 145 153 169 177 189
;; small program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod (2 + 2)), x) - 1, 2 + x) - 1) - 2
;; fast program: ((((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 2)) mod (1 + 2)) - 1) mod (1 + x), 1 + x) - 2) * 2) + 1) - x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (+ (* (- (v3 x) 2) 2) 1) x) x))))
