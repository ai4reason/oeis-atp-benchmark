;; sequence(s): A286679
;; terms: 17 33 81 113 193 241 353 561 641 913 1121 1233 1473 1873 2321 2481 2993 3361 3553 4161
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), 2 + y) * x) + x, y, 2, 1, x) div (1 + 2)) * 2) + 1
;; fast program: ((loop(loop2(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y, 2, 1, x), 1, 2 + x) div (1 + 2)) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (div (w0 x) (+ 1 2)) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (v3 x) (+ 1 2)) 2) 1))))
