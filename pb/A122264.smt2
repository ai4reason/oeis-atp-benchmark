;; sequence(s): A122264
;; terms: 2 7 12 25 30 43 48 61 82 87 108 121 126 139 160 181 186 207 220 225
;; small program: ((((compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + x) - x) - 2) * (2 + 2)) + 2) + x
;; fast program: (((((((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 1 + x) * 2) - 1) * 2) - 1) - x) - x) * 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (* (- (- (v0 x) x) 2) (+ 2 2)) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (- (- (- (* (- (* (v2 x) 2) 1) 2) 1) x) x) 2) x))))
