;; sequence(s): A117590
;; terms: 2 3 5 7 11 16 23 35 52 77 116 173 260 390 584 876 1314 1971 2956 4434
;; small program: loop((1 + x) div 2, x, loop((x + x) + x, x, 2))
;; fast program: ((loop2(x * y, y, x div 2, loop(2 + (2 + 2), x mod 2, 2), 1 + (2 * (2 + 2))) - 1) div loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div (+ 1 x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (- (w2 x) 1) (v4 x)) 1))))
