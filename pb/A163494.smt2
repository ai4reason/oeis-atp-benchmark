;; sequence(s): A163494
;; terms: 1 96 23224320 248314429440000 34014229735617331200000 32036914532626424502681600000000 142357252766974714824047503972761600000000
;; small program: loop(((y - (y div 2)) * x) * y, (x * 2) * 2, 1)
;; fast program: loop((loop2(x * y, 1 + y, x - 1, 1, 2 + x) * loop(x * y, x, 1 + x)) * loop(x * x, 1, loop(x * y, x, 1)), 1, x + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (* (- y (div y 2)) x) y))))
...
(assert (forall ((x Int)) (= (f1 x) (* (* (w2 x) (v3 x)) (v4 x)))))
(assert (= g1 1))
