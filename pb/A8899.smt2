;; sequence(s): A8899
;; terms: 87 260 130 65 194 97 290 145 434 217 650 325 974 487 1460 730 365 1094 547 1640
;; small program: loop(loop((2 * ((x + x) + x)) - 1, x mod 2, x) div 2, x, loop((1 + (x + y)) * x, 2, 2) - 1)
;; fast program: loop(loop((2 * ((x + x) + x)) - 1, x mod 2, x) div 2, x, (1 + 2) * (2 + loop((x * x) * x, 1, 1 + 2)))
(set-logic UFNIA)
