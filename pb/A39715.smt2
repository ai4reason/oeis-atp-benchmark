;; sequence(s): A39715
;; terms: 2 3 5 7 11 13 0 2 6 12 14 3 7 9 13 2 8 10 16 3
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (1 + loop(x * x, 2, 2))
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod compr((1 - x) mod (2 + 2), 2 + 2)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 1) (+ 1 v2)))))
...
(assert (forall ((x Int)) (= (f6 x) (mod (- 1 x) (+ 2 2)))))
(assert (= g6 (+ 2 2)))
