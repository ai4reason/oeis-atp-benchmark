;; sequence(s): A248056
;; terms: 6 10 18 24 30 34 40 46 54 58 66 72 78 86 90 96 102 106 114 120
;; small program: compr(loop(if (x mod 2) <= 0 then ((x div 2) - x) else x, x, x), (1 + x) * 2)
;; fast program: compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (* (+ 1 x) 2))))
