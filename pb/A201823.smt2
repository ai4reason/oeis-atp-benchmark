;; sequence(s): A201823
;; terms: 1 1 55 300830572 1414282077098335379544565517191
;; small program: loop(loop2(x * y, 1 + y, x - 1, 1, (2 + x) div 2) div loop(x * y, x, 1), 1, loop((x + x) + x, x, 1))
;; fast program: loop((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x - 1, 1)) div (if x <= 0 then 1 else x), 1, loop((x + x) + x, x, 1) div 2)
(set-logic UFNIA)
...
(assert (= i1 1))
(assert (forall ((x Int)) (= (j1 x) (div (+ 2 x) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (div (div (w5 x) (v6 x)) (ite (<= x 0) 1 x)))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (h4 x) (div (v7 x) 2))))
