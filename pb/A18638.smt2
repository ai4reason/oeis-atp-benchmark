;; sequence(s): A18638
;; terms: 1 2 3 6 9 18 43 86 129 258 387 774
;; small program: loop(compr(((1 + 2) * (2 + loop(x * x, 2, 2 + 2))) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + 2) * (2 + loop(x * x, 1 + 2, 2))) mod (1 + x), x) + 1) div (1 mod 2)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* (+ 1 2) (+ 2 v2)) (+ 1 x)))))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 2) (+ 2 v4)) (+ 1 x)))))
