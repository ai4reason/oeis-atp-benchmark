;; sequence(s): A25932
;; terms: 1 18 213 2114 19173 165354 1384741 11393778 92748645 750039290 6040740069 48528576642 389243479717 3119026937226 24977625311397 199948094540306 1600220345577189 12804941098049562 102455421613293925
;; small program: loop(((loop((loop2(((2 * (x + x)) + ((y * y) * y)) + x, y + y, y, 1, 2) + x) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + x) + x) + x, y, 1) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (* 2 (+ x x)) (* (* y y) y)) x))))
