;; sequence(s): A62973
;; terms: 6 8 9 10 12 14 16 18 20 21 22 24 25 26 27 28 30 32 33 34
;; small program: loop((((loop(((if (((1 + loop2(x + y, x, x, 1, 2)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + x, (1 + x) mod 2, x) - x) mod 2) + 1) + x, 2 + x, 2)
;; fast program: loop((((loop(((if (((1 + loop2(x + y, x, x, 1, 2)) mod (1 + x)) - 2) <= 0 then 1 else x) mod 2) + x, (1 + x) mod 2, x) - x) mod 2) + 1) + x, 2 + x, 1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (+ (mod (ite (<= (- (mod (+ 1 (w2 x)) (+ 1 x)) 2) 0) 1 2) 2) x))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (mod (ite (<= (- (mod (+ 1 (w5 x)) (+ 1 x)) 2) 0) 1 x) 2) x))))
