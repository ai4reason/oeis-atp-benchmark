;; sequence(s): A236218
;; terms: 4096 535537 244676162 14085963363 3152514340084 26450599462565 609072836692326 2822387755758487 24737012187778808 378551795393247849 1166214579181797610 7749166585021832891 30312656885388018972 70272287682650595373 186463770791599173614
;; small program: loop(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 2 * (2 + (2 + 2)), 1, x), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((((x * x) * x) * x) * x), y, 2, 1, x), 1, y) * x, y, 2, 1, x - 1), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (* (+ (v10 x y) 1) (* (* (* (* x x) x) x) x)))))
