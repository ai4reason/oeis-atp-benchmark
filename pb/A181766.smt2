;; sequence(s): A181766
;; terms: 1 3 5 6 7 9 11 13 14 15 16 17 19 21 23 25 26 27 28 29
;; small program: compr(loop2(x * y, 1 + y, x, 1, x) mod (1 + ((x + x) + x)), 1 + x) - 2
;; fast program: compr((loop(loop(((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2, 1, (x + x) + x), x, x) + 1) mod 2, 1 + x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (mod (ite (<= (- (v5 x) 2) 0) (+ 1 x) 1) (+ 1 x)) 2))))
(assert (= g4 1))
