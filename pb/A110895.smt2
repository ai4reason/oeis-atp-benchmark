;; sequence(s): A110895
;; terms: 1 4 8 14 22 34 48 66 86 110 140 172 210 252 296 344 398 458 520 588
;; small program: loop((compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + 2) + x, x, 1)
;; fast program: (loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) + x) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) x) (ite (<= x 0) 1 2)))))
