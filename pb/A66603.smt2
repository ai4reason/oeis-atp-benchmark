;; sequence(s): A66603
;; terms: 0 1 2 1 0 1 5 1 8 5 5 1 5 11 5 1 5 1 5 5
;; small program: loop((2 * (x + x)) + x, 1 + x, 1) mod (1 + x)
;; fast program: ((1 + (2 + 2)) * loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (* (+ 1 (+ 2 2)) (w1 x)) (+ 1 x)))))
