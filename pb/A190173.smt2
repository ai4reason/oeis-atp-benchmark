;; sequence(s): A190173
;; terms: 0 1 5 17 52 148 408 1101 2937 7777 20504 53912 141520 371113 972573 2547825 6672876 17473996 45754280 119797205
;; small program: loop(loop((loop2(x + y, x, x, 1, 1) - 1) * loop2(x + y, x, x, 1, 0), 1, y) + x, x, 0)
;; fast program: ((loop(x * x, 1, loop2(x + y, x, x, 1, 1)) - (x mod 2)) - loop2(x + y, x, x, 2, 1)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (- (w2 x) 1) (w3 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (- (v4 x) (mod x 2)) (w6 x)) 1))))
