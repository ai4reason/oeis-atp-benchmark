;; sequence(s): A181335
;; terms: 1 4 96 18432 31850496 550376570880 114126085737676800 331312591939905257472000 14427205603578338379772723200000 10051861189298894268003697526046720000000
;; small program: loop(loop((compr(loop(x * y, x, 2) mod (2 + x), y) + 2) * x, y, x), x, 1)
;; fast program: loop(loop((compr(loop(x * y, x, 2) mod (2 + x), y) + 2) * x, y, 1) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ (v2 x y) 2) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (+ (v6 x y) 2) x))))
