;; sequence(s): A14311
;; terms: 7 11 13 14 19 21 22 25 26 28 35 37 38 41 42 44 49 50 52 56
;; small program: compr(loop(loop((2 + x) mod y, x, 1 + 2), 1 + 2, x), 1 + x)
;; fast program: compr(loop(((((loop2(x - y, y div 2, x, x, x) + x) mod (1 + (2 + (2 + 2)))) - 1) mod (2 + 2)) + 1, 2, x) mod 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ (mod (- (mod (+ (w5 x) x) (+ 1 (+ 2 (+ 2 2)))) 1) (+ 2 2)) 1))))
(assert (= g4 2))
