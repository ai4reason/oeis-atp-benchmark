;; sequence(s): A54469
;; terms: 1 7 28 85 218 499 1053 2092 3970 7272 12958 22596 38739 65535 109714 182185 300620 493635 807555 1317360
;; small program: loop(loop(loop(loop((loop2(x + y, x, y, 1, 1) - 2) + x, y, x), y, x), 2 + y, x), x, 1)
;; fast program: loop((((loop(loop(loop(loop((loop(loop(((loop2(x + y, x, x, 1 + 2, 2) - 1) - x) - x, 1, y) + x, x, 1) - x) - x, 1, y) + x, x, 1) - x, 1, y) + x, x, 1) - x) - 1) * 2) + 1, 1, 2 + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int) (z Int)) (= (v12 x y z) (ite (<= x 0) z (g12 (u12 (- x 1) y z))))))
(assert (forall ((x Int)) (= (w12 x) (u12 (h12 x) i12 j12))))
(assert (forall ((x Int)) (= (f11 x) (- (- (- (w12 x) 1) x) x))))
(assert (= g11 1))
...
(assert (forall ((x Int)) (= (f9 x) (- (- (v10 x) x) x))))
(assert (= g9 1))
...
(assert (forall ((x Int)) (= (f5 x) (+ (* (- (- (v6 x) x) 1) 2) 1))))
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 g5 (h5 x)))))
(assert (forall ((x Int)) (= (fast x) (div (v5 x) 2))))
