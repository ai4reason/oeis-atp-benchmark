;; sequence(s): A101601
;; terms: 1 9 81 756 7290 72171 729729 7505784 78298974 826489170 8811646074 94753804536 1026499549140 11192793160815 122744496427425 1352917116177840 14979996753469110 166542316847391870
;; small program: ((loop(2 * ((2 * (x * y)) + x), x, 1) * loop((x + x) + x, x, 1)) * (1 + x)) div loop2(x * y, 1 + y, x, 1, 2 + 2)
;; fast program: loop(((((loop(2 * ((x - (x div y)) + x), x, 2) div (1 + x)) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div (2 + x)) * (if x <= 0 then 1 else x)) div 2, 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (* (* (v0 x) (v1 x)) (+ 1 x)) (w2 x)))))
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* 2 (+ (- x (div x y)) x)))))
...
(assert (forall ((x Int)) (= (f3 x) (div (* (div (* (div (v4 x) (+ 1 x)) (w5 x)) (+ 2 x)) (ite (<= x 0) 1 x)) 2))))
(assert (= g3 1))
