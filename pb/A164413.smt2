;; sequence(s): A164413
;; terms: 1 2 4 8 13 22 36 58 94 152 246 398 644 1042 1686 2728 4414 7142 11556 18698
;; small program: ((((loop2(x + y, x, x + x, 2, 2) - 1) - x) - x) div loop2(x + y, x, x - 1, 2, 1)) + 1
;; fast program: (((((loop2(((x - y) + x) + x, x, x - 1, if x <= 0 then 1 else 2, 1) - 1) * 2) + 0) - x) div loop2(x + y, x, x - 1, 1, 1)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (- (- (- (w0 x) 1) x) x) (w1 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (- (+ (* (- (w2 x) 1) 2) 0) x) (w3 x)) 1))))
