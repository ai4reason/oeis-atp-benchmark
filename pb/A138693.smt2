;; sequence(s): A138693
;; terms: 114 119 135 159 231 279 399 471 639 951 1071 1479 1791 1959 2319 2919 3591 3831 4599 5151
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) + (2 * (2 * loop((x * x) * x, 1, 1 + 2)))) + 2
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * x, y, 2, 1, x) + loop((1 + x) * x, 1, 2 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (+ (+ (w0 x) (* 2 (* 2 v3))) 2))))
...
(assert (forall ((x Int)) (= (f8 x) (* (+ 1 x) x))))
(assert (= g8 1))
(assert (= h8 (+ 2 (* 2 (+ 2 2)))))
