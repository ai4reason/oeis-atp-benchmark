;; sequence(s): A350803
;; terms: 2 4 6 8 10 12 14 15 16 18 20 22 24 26 28 30 32 34 35 36
;; small program: loop(compr(if (loop((if ((y * y) mod (x + y)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0, y), x, 2)
;; fast program: compr((if (loop((if ((y * y) mod (x + y)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0) * x, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (ite (<= (mod (* y y) (+ x y)) 0) 1 0) x))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (ite (<= (mod (* y y) (+ x y)) 0) 1 0) x))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (* (ite (<= (- (v4 x) x) 0) 1 0) x))))
