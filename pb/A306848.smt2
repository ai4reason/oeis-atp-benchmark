;; sequence(s): A306848
;; terms: 1 9 135 2835 70875 1913625 63149625 2210236875 86199238125 3878965715625 190069320065625 9693535323346875 533144442784078125 30389233238692453125 1914521694037624546875 124443910112445595546875 8586629797758746092734375
;; small program: loop((compr(((loop(x * y, x, x) div (1 + x)) + x) mod 2, y) * x) + x, x, 1)
;; fast program: loop(loop(compr(loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - (if x <= 0 then 1 else 2), x) + 1, 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (div (v2 x) (+ 1 x)) x) 2))))
