;; sequence(s): A18765
;; terms: 1 2 3 4 6 12 83 166 249 332 498 996
;; small program: loop(compr(loop(2 * (2 + (x * y)), 2 + 2, 0) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop(((1 + (2 + (2 + 2))) * (x * x)) - x, 1, 2 * (2 + (2 + 2))) mod (1 + x), x) + 1) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (* (+ 1 (+ 2 (+ 2 2))) (* x x)) x))))
(assert (= g4 1))
(assert (= h4 (* 2 (+ 2 (+ 2 2)))))
