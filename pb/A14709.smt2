...
;; small program: 1 + ((loop(x div (1 + (x mod 2)), x, x) div 2) mod 2)
;; fast program: (loop(loop(loop(loop(loop(loop(x, (x mod (2 + 2)) - 2, x) div 2, x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) mod 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (mod (div (v0 x) 2) 2)))))
(assert (forall ((x Int)) (= (f6 x) x)))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (v1 x) 2) 1))))
