;; sequence(s): A94345
;; terms: 0 1 3 4 6 9 11 14 18 19 21 24 26 29 33 36 40 45 47 50
;; small program: loop(loop2((y mod (1 + 2)) + x, y div (1 + 2), x, x, y), x, 1) - 1
;; fast program: loop(loop(((loop2(x - y, y div (1 + 2), 1 + (2 + 2), 0, x) + x) * 2) + x, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (+ (* (+ (w4 x) x) 2) x))))
(assert (= g3 1))
