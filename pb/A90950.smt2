;; sequence(s): A90950
;; terms: 0 11 37 85 163 280 446 672 970 1353 1835 2431 3157 4030 5068 6290 7716 9367 11265 13433
;; small program: loop(loop((loop(x + y, 2 + y, x) + y) + y, y, x) + y, x, x) + x
;; fast program: loop(loop(((((1 + x) * ((2 + x) * x)) div (2 + (2 + 2))) + ((1 + (x + x)) * (2 + x))) + x, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ (+ (div (* (+ 1 x) (* (+ 2 x) x)) (+ 2 (+ 2 2))) (* (+ 1 (+ x x)) (+ 2 x))) x))))
(assert (= g4 1))
