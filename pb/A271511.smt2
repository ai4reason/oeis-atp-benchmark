;; sequence(s): A271511
;; terms: 6 10 21 36 78 105 171 210 300 465 528 741 903 990 1176 1485 1830 1953 2346 2628
;; small program: loop2(((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 2) * x) + 1, y, 2, 1, x) div 2
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 2) * x) + 1, y, 2, 1, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (* (+ (v4 x y) 2) x) 1))))
