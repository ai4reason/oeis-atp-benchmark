;; sequence(s): A18490
;; terms: 1 2 3 4 6 7 8 9 12 14 18 21 24 28 36 42 56 63 72 84
;; small program: compr(loop((((x * x) - y) * x) + x, 2, 2) mod (1 + x), x) + 1
;; fast program: compr(loop(((x * x) * x) - x, 1, 2 * (2 + 2)) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (* (- (* x x) y) x) x))))
(assert (= g1 2))
(assert (= h1 2))
