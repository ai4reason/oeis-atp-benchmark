;; sequence(s): A260400
;; terms: 2 6 8 10 14 18 22 24 26 30 34 38 40 42 46 50 54 56 58 62
;; small program: (compr(loop((if (x mod 2) <= 0 then loop(1 + (x + x), 2 + 2, 0) else x) div 2, x + x, x), x) + 1) * 2
;; fast program: (compr(loop((if (x mod 2) <= 0 then (loop(x * x, 2, 2) - 1) else x) div 2, x + x, x) mod (1 + x), x) * 2) + 2
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (div (ite (<= (mod x 2) 0) v2 x) 2))))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (div (ite (<= (mod x 2) 0) (- v5 1) x) 2))))
