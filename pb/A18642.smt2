;; sequence(s): A18642
;; terms: 1 2 3 4 5 6 10 12 13 15 20 26 30 39 52 60 65 78 130 156
;; small program: compr(loop(2 * ((2 * (x * x)) - y), 2, 2) mod (1 + x), x) + 1
;; fast program: compr(((1 + 2) * loop((1 + ((x * x) * x)) * x, 1, 2 + 2)) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (* (+ 1 2) v3) (+ 1 x)))))
