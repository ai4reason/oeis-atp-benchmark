;; sequence(s): A18513
;; terms: 1 3 9 61 183 549
;; small program: loop(compr(loop(((1 + 2) * (1 + x)) * y, 1 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(loop(compr((loop((1 + x) * (x * x), 1, 2 + (2 * (2 + 2))) - 2) mod (1 + x), y), x, 0) + 1, 1, x + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (* (+ 1 2) (+ 1 x)) y))))
(assert (= g2 (+ 1 2)))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 1 x) (* x x)))))
(assert (= g6 1))
(assert (= h6 (+ 2 (* 2 (+ 2 2)))))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (mod (- v6 2) (+ 1 x)))))
