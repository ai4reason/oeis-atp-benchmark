;; sequence(s): A18572
;; terms: 1 3 7 21 31 93 217 651
;; small program: loop(compr((1 + loop(loop(x + y, x, x) * x, 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((loop(x * x, 2, x) + x) div 2, 1, 2 + (2 + 2)) mod (1 + x), x) div 1) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (+ (v6 x) x) 2))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
