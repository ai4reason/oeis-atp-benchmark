;; sequence(s): A91519
;; terms: 1 6 9 28 25 54 49 120 81 150 121 252 169 294 225 496 289 486 361 700
;; small program: (1 + ((x - loop(x div (1 + (x mod 2)), x, x)) + x)) * (1 + x)
;; fast program: ((((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1) * (1 - 2)) + 2) + x) + x) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ 1 (+ (- x (v0 x)) x)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (+ (* (+ (* (v1 x) 2) 1) (- 1 2)) 2) x) x) (+ 1 x)))))
