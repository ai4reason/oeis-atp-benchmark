;; sequence(s): A120880
;; terms: 1 2 1 2 4 2 1 2 1 2 4 2 4 8 4 2 4 2 1 2
;; small program: loop2((((y mod (1 + 2)) mod 2) * x) + x, y div (1 + 2), x, 1, x)
;; fast program: loop2((loop(((x mod (1 + 2)) mod (1 + 2)) mod 2, 1, y) * x) + x, y div (1 + 2), 1 + (2 + 2), 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (mod (mod y (+ 1 2)) 2) x) x))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (mod (mod x (+ 1 2)) (+ 1 2)) 2))))
(assert (= g2 1))
