;; sequence(s): A78135
;; terms: 1 2 3 5 6 7 10 11 14 15 19 23
;; small program: compr(loop(loop((y - 1) - x, x, 2), 2, x) - 2, x)
;; fast program: compr(x - (2 * (2 * (2 * (x mod (2 + 2))))), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- x (* 2 (* 2 (* 2 (mod x (+ 2 2)))))))))
