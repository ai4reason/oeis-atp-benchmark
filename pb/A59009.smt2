;; sequence(s): A59009
;; terms: 0 2 5 6 8 11 13 14 17 18 20 23 24 27 29 30 32 35 37 38
;; small program: loop(loop(loop((y mod 2) - x, x, 1), x, y) + y, x + x, x)
;; fast program: ((loop2(((if y <= 0 then 0 else 1) + x) + y, y div 2, 2 + (2 + 2), 0, x) mod 2) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (mod (w3 x) 2) x) x))))
