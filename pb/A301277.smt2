;; sequence(s): A301277
;; terms: 2 3 3 4 6 7 8 10 11 13 15 16 18 20 22 24 26 28 30 32
;; small program: (((((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) + 2) - (x div 2)) + x) + x) div (1 + x)
;; fast program: ((((((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) * 2) + 1) + x) div (1 + x)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (+ (- (+ (* (v0 x) 2) 2) (div x 2)) x) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (+ (+ (* (+ (* (v3 x) 2) x) 2) 1) x) (+ 1 x)) 2))))
