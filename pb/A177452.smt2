;; sequence(s): A177452
;; terms: 1 10 66 366 1851 8858 40890 184098 813948 3549758 15317294 65537334 278489619 1176688494 4948173294 20723897214 86494746204 359915608314 1493718226314 6184858989714
;; small program: loop(loop((loop2(x * y, y - 1, x, 1, 2 * (2 + x)) div loop(x * y, x, 1)) * (2 + x), 1, y) + x, x, 2) div 2
;; fast program: loop(loop((loop2(x * y, 1 + y, x, 1, 1 + (2 + (2 + x))) div loop(x * y, x, 1)) * (2 + x), 1, y) + x, x, 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (div (w2 x) (v3 x)) (+ 2 x)))))
(assert (= g1 1))
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ 1 (+ 2 (+ 2 x))))))
...
(assert (forall ((x Int)) (= (f5 x) (* (div (w6 x) (v7 x)) (+ 2 x)))))
(assert (= g5 1))
...
(assert (forall ((x Int)) (= (fast x) (div (v4 x) 2))))
