;; sequence(s): A89607
;; terms: 1 3 1 3 3 1 1 3 3 3 1 1 3 1 1 3 3 3 1 3
;; small program: (loop(x div (1 + (x mod 2)), x, x) - 1) mod loop(2 + 2, x, 2)
;; fast program: (((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) + (if x <= 0 then 0 else 1)) mod 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (mod (+ (v2 x) (ite (<= x 0) 0 1)) 2) 2) 1))))
