...
;; small program: ((compr(loop2(2 + (x + y), x, x, 0, 2) mod (1 + x), 2 + x) div 2) mod (1 + 2)) - 1
;; fast program: (((((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), (1 + x) mod 2, x), x) + 1) + 1) * (if x <= 0 then 1 else 2)) - 1) mod (1 + 2)) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (mod (div (v0 x) 2) (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (- (mod (- (* (+ (+ (v2 x) 1) 1) (ite (<= x 0) 1 2)) 1) (+ 1 2)) 1))))
