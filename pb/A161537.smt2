;; sequence(s): A161537
;; terms: 5 8 11 13 15 18 21 23 25 28 31 33 35 38 40 42 44 46 49 52
;; small program: (compr(loop(x * y, x, 2) mod (1 + x), 2 + x) + 2) + x
;; fast program: ((((((compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, (x - 1) div 2, x), 1 + x) + 2) * 2) + 1) - (x mod 2)) + x) + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (+ (- (+ (* (+ (v2 x) 2) 2) 1) (mod x 2)) x) x) 2))))
