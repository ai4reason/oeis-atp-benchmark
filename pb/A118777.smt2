;; sequence(s): A118777
;; terms: 1 2 1 0 1 0 1 0 1 2 3 2 3 2 3 4 5 4 5 4
;; small program: ((loop(loop(((loop(x * y, x, 1) mod (2 + x)) + 1) mod 2, 1, y - 2) + x, x, 0) * 2) - x) + 1
;; fast program: (((loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 0, 1, y) + x, x - 1, 1) * x) - x) - 1) mod (2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (- (- (* (v3 x) x) x) 1) (+ 2 x)))))
