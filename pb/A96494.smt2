;; sequence(s): A96494
;; terms: 2 2 4 4 6 6 8 8 8 10 10 12 12 12 12 14 14 14 16 16
;; small program: (loop((((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) div (x * x)) + x, x, 2) * 2) - 2
;; fast program: (loop((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) div (x * x)) + x, x, 2) * 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (+ (* (v1 x y) 2) 1) (* x x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (div (+ (* (v4 x y) 2) 1) (* x x)) x))))
