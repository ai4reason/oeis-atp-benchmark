;; sequence(s): A99802
;; terms: 1 2 3 4 4 5 6 6 7 8 8 9 9 9 10 11 11 11 12 12
;; small program: loop(loop(loop(x * y, x, x) mod (1 + x), 1, y + y) + x, x, 1)
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x, 1)
(set-logic UFNIA)
