;; sequence(s): A38872-A141158
;; terms: 5 11 19 29 31 41 59 61 71 79 89 101 109 131 139 149 151 179 181 191
;; small program: compr((loop((loop2(y, x + y, x, 2, 2) - 1) mod (2 + x), (((2 + x) * x) div 2) mod 2, x) mod (1 + x)) mod (1 + x), 1 + x) + 2
;; fast program: compr((loop(loop(((if (((1 + loop2(x + y, x, x, 0, 2)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + x, (1 + x) mod 2, x) - x, x mod 2, 1 + x) + x) mod 2, 1 + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (w2 x) 1) (+ 2 x)))))
(assert (forall ((x Int)) (= (g1 x) (mod (div (* (+ 2 x) x) 2) 2))))
(assert (forall ((x Int)) (= (h1 x) x)))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y))))))
(assert (forall ((x Int)) (= (v1 x) (u1 (g1 x) (h1 x)))))
(assert (forall ((x Int)) (= (f0 x) (mod (mod (v1 x) (+ 1 x)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f5 x) (+ (mod (ite (<= (- (mod (+ 1 (w6 x)) (+ 1 x)) 2) 0) 1 2) 2) x))))
