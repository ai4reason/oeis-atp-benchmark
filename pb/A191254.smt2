;; sequence(s): A191254
;; terms: 0 1 0 2 0 1 0 1 0 1 0 2 0 1 0 2 0 1 0 2
;; small program: (x mod 2) + loop(if (x mod 2) <= 0 then 1 else (x div 2), x, x)
;; fast program: loop(loop((loop(x div (if (x mod (2 + 2)) <= 0 then (2 + 2) else 1), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (2 + 2)) - 2, x) + 1, x mod 2, x) - x
(set-logic UFNIA)
