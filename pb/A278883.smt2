;; sequence(s): A278883
;; terms: 1 5 81 1872 51425 1565109 50979600 1742711616 61765676577 2251396558125 83924761860225 3186277484832000 122829049870699536 4796448751641900752 189381233826675892800 7549371503605704934656 303473219026059360959265 12289574902507266828093525 500960076377670398672062425 20540854991655352005504930000
;; small program: (loop2((x * y) * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop((y * x) * y, x, 1)) * (1 + (2 * (x + x)))
;; fast program: (((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * (1 + (2 * (x + x)))) * loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x))) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (w0 x) (v1 x)) (+ 1 (* 2 (+ x x)))))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (div (w2 x) (v3 x)) (+ 1 (* 2 (+ x x)))) (w4 x)) (v5 x)))))
