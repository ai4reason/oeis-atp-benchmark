;; sequence(s): A71538
;; terms: 0 0 1 1 2 2 2 2 2 2 3 3 3 3 3 3 4 4 4 4
;; small program: loop(loop(((loop((if (loop(x * y, x, 2) mod (1 + x)) <= 0 then 0 else 2) + x, 2, x) - x) div (1 + 2)) mod 2, 1, y) + x, x, 0)
;; fast program: loop(loop((loop((loop(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x, 1 - (x mod 2), x) + 1) + x, if x <= 0 then 1 else 2, x) + 1) mod 2, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (ite (<= (mod (v3 x) (+ 1 x)) 0) 0 2) x))))
(assert (= g2 2))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 g2 (h2 x)))))
(assert (forall ((x Int)) (= (f1 x) (mod (div (- (v2 x) x) (+ 1 2)) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f7 x) (ite (<= (- (mod (+ 1 (v8 x)) (+ 1 x)) 2) 0) 1 x))))
...
(assert (forall ((x Int)) (= (g6 x) (ite (<= x 0) 1 2))))
