;; sequence(s): A317331
;; terms: 4 8 11 16 20 23 27 32 36 40 43 47 52 55 59 64 68 72 75 80
;; small program: loop(((if ((loop(x div (1 + (x mod 2)), x, x) div 2) mod 2) <= 0 then y else 1) + x) + x, 2, x)
;; fast program: (((((((loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) + 1) mod 2) + 1) + x) + x) + x) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (ite (<= (mod (div (v1 x) 2) 2) 0) y 1) x) x))))
(assert (= g0 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (+ (mod (+ (v2 x) 1) 2) 1) x) x) x) x) 2))))
