;; sequence(s): A62747
;; terms: 1 7 89 1447 26713 532391 11165785 242851751 5427716185 123901026215 2876525797465 67710590623655 1612262780199001 38764533106581415 939825790848884825 22950405085586497447
;; small program: loop(loop((loop2((2 + y) * x, 1 + y, x - 1, 1, x + x) div loop(x * y, x, 1)) * loop(x + x, x + x, 2), 1, y) - x, x, 1)
;; fast program: loop(loop(((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * 2) * loop(x * x, 1, loop(x + x, x - 1, 2)), 1, y) - x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (div (w2 x) (v3 x)) (v4 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f6 x) (* (* (div (w7 x) (v8 x)) 2) (v9 x)))))
(assert (= g6 1))
