;; sequence(s): A175222
;; terms: 7 8 10 12 16 18 22 24 28 34 36 42 46 48 52 58 64 66 72 76
;; small program: loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + x, 2, 2, 2, x)
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 2) 2))))
