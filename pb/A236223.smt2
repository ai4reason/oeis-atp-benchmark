;; sequence(s): A236223
;; terms: 32768 14381675 30531959800 4778093469743 4182026262885394 55367919276976151 2917790970786791944 18098918000661590243 284734153465052835850 8913922901063237276799 32379184892907923206750 365825452844723230295243 1920923767836261141183844
;; small program: loop(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) + 1) * (x * x), y, 2 + 2, 1, x), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + 1) * (x * x), y, 2 + 2, 1, x), 1, y) + x, 1 + x, 0)
(set-logic UFNIA)
