;; sequence(s): A246425
;; terms: 0 1 ~2 2 ~1 3 ~4 4 ~2 5 ~10 6 ~3 7 ~9 8 ~4 9 ~15 10
;; small program: (loop(x div (1 + (x mod 2)), x, 1 + ((x + x) + x)) div 2) - x
;; fast program: (((loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + (2 + 2), x mod 2, 1), x), 1, ((x + x) + x) + 1) + 1) div 2) - 1) - x
(set-logic UFNIA)
