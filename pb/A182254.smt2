;; sequence(s): A182254
;; terms: 0 0 0 0 18 30 510 1470 22890 284550 3171630 18486930 45347610 4254260010 2349577230 371965003410 9252750236730 190224835871070 221717289063270 14855058367239090
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, x, 1) mod loop(x * y, x, 1)
;; fast program: loop(((compr(((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, x - 1, 2) mod loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (v0 x) (v3 x)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (v4 x) (v7 x)))))
