;; sequence(s): A18589
;; terms: 1 2 4 5 8 10 17 20 34 40 68 85 136 170 340 680
;; small program: loop(compr(loop(2 * (2 * (2 + x)), 2 + 2, 0) mod (1 + x), y), x, 0) + 1
;; fast program: compr(loop((((((loop(x * x, 2, x) * x) + x) div 2) - x) div (1 + 2)) * x, 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (div (- (div (+ (* (v5 x) x) x) 2) x) (+ 1 2)) x))))
(assert (= g4 1))
(assert (= h4 (+ 2 2)))
