;; sequence(s): A83328
;; terms: 0 1 9 64 423 2716 17199 108004 673983 4183516 25845039 158993044 974443743 5952752716 36262150479 220363399684 1336386968703 8090385360316 48906795729519 295279984663924
;; small program: (((loop2(((2 * (x + x)) + x) + y, 2 * ((y + y) + y), x, 0, 1) - loop((x + x) + x, x, 1)) + 1) + loop2(x * y, y, x, 1, 2 + 2)) div 2
;; fast program: (((loop(x * x, 1, loop(x + x, x, 1)) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) + loop2(x * y, y, x, 1, 2 + (2 + 2))) - loop2(x * y, y, x, 1, 1 + (2 + 2))) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (+ (- (w0 x) (v1 x)) 1) (w2 x)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (+ (- (v3 x) (w5 x)) (w7 x)) (w8 x)) 2))))
