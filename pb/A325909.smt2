;; sequence(s): A325909
;; terms: 1 3 2 4 9 5 7 15 8 10 21 11 13 27 14 16 33 17 19 39
;; small program: loop(loop2(y, 0, x, 1, 1 + y), x, 1) + x
;; fast program: x + loop2(y, 0, x mod (1 + 2), 1, 1 + x)
(set-logic UFNIA)
...
(assert (= i1 1))
(assert (forall ((x Int) (y Int)) (= (j1 x y) (+ 1 y))))
...
(assert (= g2 0))
(assert (forall ((x Int)) (= (h2 x) (mod x (+ 1 2)))))
(assert (= i2 1))
...
(assert (forall ((x Int)) (= (fast x) (+ x (w2 x)))))
