...
;; small program: loop(loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) + x, x, 0) - 1, 2, x) + 1
;; fast program: (loop(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, x) - x, 2, x) + 1) mod (if x <= 0 then 1 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (v4 x) 1) (ite (<= x 0) 1 x)))))
