;; sequence(s): A162735
;; terms: 1 4 6 9 12 15 18 21 26 30 34 39 42 45 50 56 60 64 69 72
;; small program: (loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 1 + 2, 1 + x, if x <= 0 then 0 else 1) - x) div 2
;; fast program: (((loop2(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + x, 1 + y, 2, 1, x) div 2) + 1) * (if x <= 0 then 1 else 2)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (j0 x) (ite (<= x 0) 0 1))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (div (w3 x) 2) 1) (ite (<= x 0) 1 2)) 2))))
