;; sequence(s): A179899
;; terms: 0 12 21 30 36 39 48 51 57 66 72 75 81 84 93 96 102 111 114 120
;; small program: loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) * (1 + 2)
;; fast program: (compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) * (1 + 2)
(set-logic UFNIA)
