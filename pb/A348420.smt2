;; sequence(s): A348420
;; terms: 1 1 3 15 135 1485 22275 467775 10758825 312005925 10296195525 360366843375 14054306891625 576226582556625 29387555710387875 1557540452650557375 98125048516985114625 6378128153604032450625 440090842598678239093125
;; small program: loop(compr(((1 + loop(x + x, x - (x mod 2), 2)) mod (1 + (x + x))) - 2, y) * x, x, 1)
;; fast program: loop(compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 1 + y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (- x (mod x 2)))))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (mod (- (v6 x) x) (+ 1 (+ x x))))))
