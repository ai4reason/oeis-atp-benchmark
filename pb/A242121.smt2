;; sequence(s): A242121
;; terms: 2 3 5 7 11 13 17 19 2 8 10 16 20 1 5 11 17 19 4 8
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (1 + loop(2 * (2 + x), 2, 2))) + 1
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (+ 1 v2)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (v3 x) 1) (+ 1 (* 2 (+ 2 (* 2 (+ 2 2)))))))))
