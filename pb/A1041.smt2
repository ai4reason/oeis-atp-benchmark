;; sequence(s): A1041
;; terms: 12 24 72 360 2520 27720 360360 6126120 116396280 2677114440 77636318760 2406725881560 89048857617720 3651003162326520 156993135980040360 7378677391061896920 391069901726280536760 23073124201850551668840
;; small program: loop2(loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, x), 2, 2, 2, x)
;; fast program: loop((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + 1) * x, x, 2 + 2) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (v4 x) (+ 1 2)))))
