;; sequence(s): A139289
;; terms: 3 15 255 4095 1048575 16777215 4294967295 68719476735 17592186044415 72057594037927935 1152921504606846975 4722366482869645213695 1208925819614629174706175 19342813113834066795298815 4951760157141521099596496895 20282409603651670423947251286015 83076749736557242056487941267521535
;; small program: (((loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), x, 2, 1, x) div 2) div (1 + 2)) div 2) - 1
;; fast program: (((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), x, 2, 1, x) div (1 + 2)) div 2) - 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (div (div (div (w0 x) 2) (+ 1 2)) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (div (div (w4 x) (+ 1 2)) 2) 1) 2))))
