;; sequence(s): A17389
;; terms: 1 15 154 1350 10891 83685 623764 4558380 32875381 234980955 1669192174 11806149810 83252603071 585817587825 4115974729384 28888095527640 202598073849961 1420093671872295 9950191865139394
;; small program: loop(((loop((loop((2 * ((x + x) + x)) + x, y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: (loop(((loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) + loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))) div 2) - loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))), 1, 2 + x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (div (+ (w4 x) (w6 x)) 2) (w7 x)))))
(assert (= g3 1))
