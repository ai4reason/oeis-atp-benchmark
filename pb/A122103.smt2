;; sequence(s): A122103
;; terms: 32 275 3400 20207 181258 552551 1972408 4448507 10884850 31395999 60025150 129369107 245225308 392233751 621578758 1039774251 1754698550 2599294851 3949419958 5753649309
;; small program: loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, 1 + (2 + 2), 1, y) + x, 1 + x, 0)
;; fast program: ((((((((((((((loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (((x * x) * x) * x), y, 2, 1, x), 1, y) + x, x, 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (v7 x y) 1) (* (* (* x x) x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (+ (v4 x) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2) 2))))
