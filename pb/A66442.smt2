;; sequence(s): A66442
;; terms: 0 0 0 0 2 0 5 0 0 4 1 0 12 4 3 0 12 0 12 16
;; small program: loop(loop((2 + y) * x, 2, x), 1 + x, 1) mod (1 + x)
;; fast program: (((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) * loop(x * x, 1, loop(x + x, x, 2))) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (* (* (+ 1 2) (w2 x)) (v4 x)) (+ 1 x)))))
