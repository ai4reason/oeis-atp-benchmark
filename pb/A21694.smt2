;; sequence(s): A21694
;; terms: 1 24 394 5544 71995 891408 10701748 125788848 1456313749 16673208552 189289198462 2135136588312 23963101915663 267883518461856 2985323286760936 33185997429018336 368172943255604137
;; small program: loop(((loop((loop(1 + loop((x + x) + x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop2((((y div 2) + y) * y) + x, (y + y) + y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (v2 x y) x) (v4 x)))))
