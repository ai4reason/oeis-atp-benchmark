;; sequence(s): A4650
;; terms: 0 0 1 2 5 6 1 1 3 7 7 11 13 13 15 2 5 4 7 8
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (2 + x)
;; fast program: ((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (2 + x)) + 0
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (v2 x) 1) (+ 2 x)) 0))))
