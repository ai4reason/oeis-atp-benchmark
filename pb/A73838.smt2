;; sequence(s): A73838
;; terms: 2 6 15 35 35 77 1001 143 2431 46189 46189 96577 96577 7429 215441 6678671 6678671 392863 14535931 765049
;; small program: loop2(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, 1 + y, 2 + x, 1, x)
;; fast program: loop2(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, 1 + y, 2 * (1 + (x div 2)), 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (* 2 (+ 1 (div x 2))))))
(assert (= i3 1))
