;; sequence(s): A18474
;; terms: 1 2 3 6 79 158 237 474
;; small program: loop(compr((loop(((2 * (x + y)) + y) * x, 2, 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop(((1 + (x + x)) * (x * x)) + x, 1, 2 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (* 2 (+ x y)) y) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f5 x) (+ (* (+ 1 (+ x x)) (* x x)) x))))
(assert (= g5 1))
(assert (= h5 (+ 2 (+ 2 2))))
