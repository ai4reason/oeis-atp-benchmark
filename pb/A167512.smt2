;; sequence(s): A167512
;; terms: 0 0 0 0 1 2 6 10 22 34 67 100 188
;; small program: loop(loop(loop2(y, x - y, x, 1, x mod 2), 1, y) + x, x, 0) - x
;; fast program: ((((loop2((x + x) + y, x + y, x - 1, 2, 1) * (if x <= 0 then 1 else 2)) * loop2(x + y, x, x, 1, 2)) + 1) - x) mod loop2(x + y, x, x, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (- (+ (* (* (w3 x) (ite (<= x 0) 1 2)) (w4 x)) 1) x) (w5 x)))))
