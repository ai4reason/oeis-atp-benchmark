;; sequence(s): A19943
;; terms: 1 21 298 3570 38971 401751 3988468 38583300 366449941 3434404281 31873887838 293663563830 2690806228111 24553315831611 223338364450408 2026585451393160 18355202849805481 166009125098571741 1499772036736668178 13537796780062999290
;; small program: loop((loop((loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((((loop(x * x, 1, loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) + loop2(x * y, y, x, 1, 1 + (2 + 2))) div 2) - loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))) div 2, 1, 2 + x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (- (div (+ (v4 x) (w7 x)) 2) (w8 x)) 2))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (div (v3 x) 2))))
