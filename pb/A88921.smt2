;; sequence(s): A88921
;; terms: 1 2 5 13 33 80 185 411 885 1862 3853 7881 15993 32284 64945 130359 261293 523282 1047397 2095781
;; small program: loop(loop((loop(x + x, y, 1) - y) + x, y, x), x, 1)
;; fast program: (((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2))) - 1) - loop(loop(x + y, y, x), x, 2)) - x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (- (v3 x) 1) (v6 x)) x) x))))
