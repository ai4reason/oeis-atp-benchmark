;; sequence(s): A85130
;; terms: 7 14 21 28 35 42 49 56 63 70 84 98 105 112 126 140 147 168 175 189
;; small program: (compr(loop(x * y, 2 + (2 * (2 + 2)), 1) mod (1 + x), x) + 1) * (1 + (2 + (2 + 2)))
;; fast program: (compr(loop((1 + (y + y)) * (x * x), 1 + 2, 2) mod (1 + x), x) + 1) * (1 + (2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ 1 (+ y y)) (* x x)))))
(assert (= g3 (+ 1 2)))
(assert (= h3 2))
