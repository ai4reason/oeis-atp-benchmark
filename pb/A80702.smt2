;; sequence(s): A80702
;; terms: 3 4 6 8 9 10 11 12 14 16 18 20 21 22 23 24 25 26 27 28
;; small program: loop(loop((if x <= 0 then (x div 2) else 2) - x, x, y) + x, 2 + x, x) + 2
;; fast program: (loop(((loop(loop(loop(x div loop(1, 2 + (2 - x), 2), 1 + (2 + 2), 2 + x), 1, y) + x, x, 1) - x) - 1) - x, 1, 2 + x) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (ite (<= x 0) (div x 2) 2) x))))
