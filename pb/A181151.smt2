;; sequence(s): A181151
;; terms: 13 37 151 393 1453 2367 5203 7221 12697 25231 30753 52023 70603 81357 106033 151687 208861 230703 305253 362953
;; small program: loop2((((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) * x) + 1) + x, y, 2, 1, x)
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 2) * (x * x)) - 1, y, 2, 1, x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (* (* (v1 x y) x) x) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (- (* (+ (v4 x y) 2) (* x x)) 1))))
