;; sequence(s): A188682
;; terms: 1 4 49 1057 28282 848101 27357493 928760053 32747441926 1188869998801 44174723634526 1672716549215326 64340599136306926 2507814491482180894 98859670298036582494 3935425516392739090270 158006444406545953115743
;; small program: loop(loop((loop2((x * y) * y, 1 + y, x, 1, 1 + (x + x)) div loop((y * x) * y, x, 1)) div (1 + (x + x)), 1, y) + x, x, 1)
;; fast program: loop(loop((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * (1 + (x + x))) * loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x))) div loop(x * y, x, 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (div (w2 x) (v3 x)) (+ 1 (+ x x))))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (div (* (* (div (w6 x) (v7 x)) (+ 1 (+ x x))) (w8 x)) (v9 x)))))
(assert (= g5 1))
