;; sequence(s): A18542
;; terms: 1 2 7 14 43 86 301 602
;; small program: loop(compr(loop((loop(2 + x, x, x) * x) + x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop((x * x) + x, 1, loop((2 + y) * x, 2, 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= g6 1))
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ 2 y) x))))
(assert (= g7 2))
(assert (= h7 2))
