;; sequence(s): A106588
;; terms: 3 5 16 33 96 133 240 297 448 741 840 1225 1512 1653 1984 2553 3192 3397 4128 4641
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 2, y, 2, 1, x) - ((2 + x) * x)) - 2
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 2, y, 2, 1, x) - ((2 + x) * x)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (- (w0 x) (* (+ 2 x) x)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (- (w3 x) (* (+ 2 x) x)) 2))))
