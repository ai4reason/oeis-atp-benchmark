;; sequence(s): A104406
;; terms: 1 1 2 3 3 3 3 3 4 5 5 6 7 7 7 7 7 7 7 7
;; small program: loop(loop((loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, 2 + (x + x)) + 1) mod 2, 1, y) + x, x, 1)
;; fast program: loop(loop(loop(if (loop2((x * y) + x, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) <= 0 then 1 else 0, 1, y) + x, x, 0) - x, 2, 1 + x)
(set-logic UFNIA)
