;; sequence(s): A134029
;; terms: 3 2 4 1 5 1 4 2 3 3 2 4 1 5 1 4 2 3 3 2
;; small program: loop2(((1 - loop(loop(x - y, x - 2, x) - 1, 2, x)) - x) - y, x, x, 1, 1) + 2
;; fast program: ((loop2((((((2 * loop(y, x div 2, 1)) - 1) + 1) - x) - x) - y, x, x, 1, 1) div 2) + 1) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (- (- (- (+ (- (* 2 (v4 x)) 1) 1) x) x) y))))
