;; sequence(s): A16753
;; terms: 1 12 97 660 4081 23772 133057 724260 3863761 20308332 105558817 544039860 2785713841 14192221692 72020501377 364354427460 1838822866321 9262446387852 46585947584737
;; small program: loop(((loop2(((2 * (x + x)) + (y * y)) + x, y + y, y, 1, 2) + x) + x) + x, x, 1)
;; fast program: loop(((loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) + loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))) div 2) - loop(x * x, 1, loop(x + x, x, 1)), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (div (+ (w3 x) (w5 x)) 2) (v8 x)))))
(assert (= g2 1))
