;; sequence(s): A243883
;; terms: 5 1 13 5 29 5 53 17 85 13 125 37 173 25 229 65 293 41 365 101
;; small program: loop(1 + loop(x div (1 + (x mod 2)), x, x), 1, ((2 + x) * x) + (2 + 2))
;; fast program: loop((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1, 1, ((2 + x) * x) + (2 + 2))
(set-logic UFNIA)
...
(assert (= g0 1))
(assert (forall ((x Int)) (= (h0 x) (+ (* (+ 2 x) x) (+ 2 2)))))
...
(assert (forall ((x Int)) (= (f2 x) (+ (* (v3 x) 2) 1))))
(assert (= g2 1))
(assert (forall ((x Int)) (= (h2 x) (+ (* (+ 2 x) x) (+ 2 2)))))
