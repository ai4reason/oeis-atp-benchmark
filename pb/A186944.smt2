;; sequence(s): A186944
;; terms: 1 2 2 6 6 30 30 210 210 210 210 2310 2310 30030 30030 30030 30030 510510 510510 9699690
;; small program: loop(((loop((loop(x * y, x, x) div (1 + x)) mod 2, 1, y) * x) * y) + x, x - 1, if x <= 0 then 1 else 2)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x - 1, 1) + (if (x - 1) <= 0 then x else 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (div (v2 x) (+ 1 x)) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (v3 x) (ite (<= (- x 1) 0) x 0)))))
