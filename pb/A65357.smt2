...
;; small program: loop(loop(if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then (0 - 1) else 1, 1, y) * x, x - 1, 1)
;; fast program: ((loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else 0, 1, y) + x, x - 1, 1) mod 2) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 1 (v2 x)) (+ 1 x)) 0) (- 0 1) 1))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (fast x) (- (* (mod (v3 x) 2) 2) 1))))
