;; sequence(s): A152919
;; terms: 1 2 4 6 10 12 18 20 28 30 40 42 54 56 70 72 88 90 108 110
;; small program: loop(loop2(y, 2 + x, x, 1, y), x, 1) + x
;; fast program: ((if x <= 0 then 1 else x) + loop((x * x) - x, 1, x div 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (ite (<= x 0) 1 x) (v2 x)) x))))
