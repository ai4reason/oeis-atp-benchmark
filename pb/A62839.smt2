;; sequence(s): A62839
;; terms: 1 3 5 8 10 14 16 20 24 27 33 36 40 44 48 53 57 63 66 70
;; small program: (loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 2, (if x <= 0 then 1 else 2) + x, x) - x) div 2
;; fast program: (loop2(compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + x, x, 2, (if x <= 0 then 1 else 2) + x, x) - x) div 2
(set-logic UFNIA)
...
(assert (= h0 2))
(assert (forall ((x Int)) (= (i0 x) (+ (ite (<= x 0) 1 2) x))))
(assert (forall ((x Int)) (= (j0 x) x)))
(assert (forall ((x Int) (y Int) (z Int)) (= (u0 x y z) (ite (<= x 0) y (f0 (u0 (- x 1) y z) (v0 (- x 1) y z))))))
(assert (forall ((x Int) (y Int) (z Int)) (= (v0 x y z) (ite (<= x 0) z (g0 (u0 (- x 1) y z))))))
(assert (forall ((x Int)) (= (w0 x) (u0 h0 (i0 x) (j0 x)))))
(assert (forall ((x Int)) (= (small x) (div (- (w0 x) x) 2))))
...
(assert (= h3 2))
(assert (forall ((x Int)) (= (i3 x) (+ (ite (<= x 0) 1 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (- (w3 x) x) 2))))
