;; sequence(s): A80422
;; terms: 1 13 105 675 3780 19278 91854 415530 1804275 7577955 30961359 123589557 483611310 1860043500 7046907660 26344593252 97328636181 355781149065 1288173125925 4623863536215
;; small program: loop(((loop(((loop(loop(((loop(loop2(((x + x) + x) + y, (y + y) + y, x, 1, 1), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop(((loop(loop(((loop(loop(((loop(((1 + (2 + x)) * loop((x + x) + x, x, 1)) div (1 + 2), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f11 x) (div (* (+ 1 (+ 2 x)) (v12 x)) (+ 1 2)))))
(assert (= g11 1))
