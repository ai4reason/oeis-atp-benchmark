...
;; small program: loop(loop(loop(y - 1, x - y, x), x, x - 2), 2, x) mod 2
;; fast program: loop((loop(loop(y - 1, x - y, x), x, x) - 1) - 1, 2, x - 2) mod 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (- y 1))))
...
(assert (forall ((x Int)) (= (f3 x) (- (- (v4 x) 1) 1))))
(assert (= g3 2))
