;; sequence(s): A121926
;; terms: 3 5 11 31 131 733 5057 40339 362903 3628829 39916831 479001637 6227020841 87178291243 1307674368047 20922789888053 355687428096059 6402373705728061 121645100408832067 2432902008176640071
;; small program: (compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) + loop((x * y) + x, x, 1)
;; fast program: (((((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, x) * 2) + 1) + loop(x * y, x, 1 + x)) div 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (+ (+ (* (v3 x) 2) 1) (v7 x)) 2) 2) 1))))
