;; sequence(s): A7977
;; terms: 1 4 6 13 20 39 70
;; small program: (loop(x + x, x, 1) - (x div loop(y - x, x, 1 + x))) + x
;; fast program: loop(x + loop(1, 2 - x, 2), x mod 2, x) + loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (- (v0 x) (div x (v1 x))) x))))
(assert (= f3 1))
