;; sequence(s): A147789
;; terms: 3 4 7 10 15 23 34 51 77 115 173 259 389 584 876 1314 1971 2956 4434 6651
;; small program: (((1 + (2 * loop(2 + ((x + x) + x), x, 2))) div loop(x + x, x, 1)) + 1) div 2
;; fast program: ((((2 * ((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))))) - 1) div loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (div (+ 1 (* 2 (v0 x))) (v1 x)) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (- (* 2 (* (+ 1 2) (w2 x))) 1) (v4 x)) 1) 2))))
