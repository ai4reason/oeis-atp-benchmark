;; sequence(s): A26228
;; terms: 2 5 11 14 23 29 32 38 41 50 59 65 68 77 83 86 92 95 104 110
;; small program: (loop(compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, x, x) mod (1 + 2)) - 1, y), x, 0) * (1 + 2)) + 2
;; fast program: (compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x, 1), x) mod (1 + 2)) div 2, x) * (1 + 2)) + 2
(set-logic UFNIA)
