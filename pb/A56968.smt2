;; sequence(s): A56968
;; terms: 0 0 1 0 0 4 1 0 1 0 1 4 1 10 10 0 1 10 1 0
;; small program: loop(2 * ((2 * (x + x)) + x), x, 1) mod (1 + x)
;; fast program: (((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (* (* (+ 1 (mod x 2)) (v1 x)) (w3 x)) (+ 1 x)))))
