;; sequence(s): A105450
;; terms: 0 6 22 60 142 305 607 1134 2008 3396 5520 8668 13206 19591 28385 40270 56064 76738 103434 137484
;; small program: loop(loop(loop(2 + loop(loop(x + y, y, x), y, x), y, 1) + x, y, 1) + x, x, x)
;; fast program: ((((loop(loop((((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 * (2 + (2 + 2))))) * x) div (1 + (2 + 2))) + (x * x), 1, y) + x, x, 1) - 1) + x) + x) + (x * x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (div (* (div (v7 x) (* 2 (* 2 (+ 2 (+ 2 2))))) x) (+ 1 (+ 2 2))) (* x x)))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (- (v5 x) 1) x) x) (* x x)) x))))
