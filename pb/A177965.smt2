;; sequence(s): A177965
;; terms: 1 4 7 10 16 19 22 31 34 37 40 49 52 55 64 70 76 79 82 91
;; small program: (compr(loop((loop(x + x, x, 2) - 2) mod (1 + x), 1 - (x mod (1 + 2)), x) - 1, 1 + x) div 2) + 1
;; fast program: ((compr(loop((loop((x * x) * x, 1, loop(x + x, x, 1)) - 1) mod (1 + ((x + x) + x)), 1 - (x mod 2), x) - 1, 1 + x) div 2) * (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (v3 x) 2) (+ 1 2)) 1))))
