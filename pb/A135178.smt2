;; sequence(s): A135178
;; terms: 12 36 150 392 1452 2366 5202 7220 12696 25230 30752 52022 70602 81356 106032 151686 208860 230702 305252 362952
;; small program: loop2(((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 2) * (x * x)) - 1, y, 2, 1, x) + 1
;; fast program: loop2(((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 2) * (x * x)) - 1, y, 2, 1, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (- (* (+ (v1 x y) 2) (* x x)) 1))))
