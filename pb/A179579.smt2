;; sequence(s): A179579
;; terms: 37 73 142 276 539 1059 2092 4150 8257 16461 32858 65640 131191 262279 524440 1048746 2097341 4194513 8388838 16777468
;; small program: loop((loop(x + x, x, 2) - loop(x - y, x, x)) - 1, 1, 2 + (2 + x))
;; fast program: loop(((2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2)))) - 1) + (((1 + x) * (2 + x)) div 2), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (- (v1 x) (v2 x)) 1))))
(assert (= g0 1))
...
(assert (forall ((x Int)) (= (f3 x) (+ (- (* 2 (v4 x)) 1) (div (* (+ 1 x) (+ 2 x)) 2)))))
(assert (= g3 1))
