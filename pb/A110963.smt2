;; sequence(s): A110963
;; terms: 1 1 1 1 2 1 1 1 3 2 2 1 4 1 1 1 5 3 3 2
;; small program: 1 + loop(loop(x div (1 + (x mod 2)), x, x) div 2, 2, x)
;; fast program: loop(loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), 2, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f11 x) (div x 2))))
