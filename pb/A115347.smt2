;; sequence(s): A115347
;; terms: 2 3 5 13 1597
;; small program: loop(loop2(x + y, x, ((x - 1) div y) + x, 1, 0), x, 2)
;; fast program: loop(loop2(x + y, x, (x div (2 + 2)) + x, 1 + 2, 2) - 2, x, 0) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h1 x y) (+ (div (- x 1) y) x))))
(assert (= i1 1))
(assert (= j1 0))
...
(assert (forall ((x Int)) (= (f2 x) (- (w3 x) 2))))
