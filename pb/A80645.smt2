;; sequence(s): A80645
;; terms: 1 2 4 6 7 8 10 12 13 14 15 16 18 20 22 24 25 26 27 28
;; small program: loop(loop(1 + (loop(y, x - 2, x) div 2), x, x) + x, x, 1)
;; fast program: loop((((loop(loop(x div loop(1, 2 + (2 - x), 2), 1 + (2 + 2), x), (1 + x) mod 2, x) + 1) mod 2) + 1) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (+ 1 (div (v2 x) 2)))))
