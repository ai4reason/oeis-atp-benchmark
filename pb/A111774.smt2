;; sequence(s): A111774
;; terms: 6 9 10 12 14 15 18 20 21 22 24 25 26 27 28 30 33 34 35 36
;; small program: loop(compr(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0, y), 1 + x, 0) + 1
;; fast program: compr(loop((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 + (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x), 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (- (mod (+ 1 (v2 x)) (+ 1 x)) 2) 0) 1 0))))
