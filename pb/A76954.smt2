;; sequence(s): A76954
;; terms: 1 2 18 2250 5402250 870037764750 4199506113235182750 1723219765760312626547490750 29266411525287522788837599332989370750 52713275010243038997421106186697438702252144407250 22176856087751973465466098269669474342964368337745368642450857250
;; small program: loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, y, y, x, y), x, 1)
;; fast program: loop(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) * x) + x, y, x, 1, x), 1, y) * x, x, 1)
(set-logic UFNIA)
