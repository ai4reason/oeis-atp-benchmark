;; sequence(s): A129726
;; terms: 2 5 9 13 19 23 29 33 39 47 51 59 65 69 75 83 91 95 103 109
;; small program: ((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 1) + x) + x
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 x)) x))))
