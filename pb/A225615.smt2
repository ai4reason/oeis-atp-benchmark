;; sequence(s): A225615
;; terms: 1 6 51 506 5351 58481 652256 7376776 84281461 970444596 11242722766 130896288616 1530255133591 17951328648871 211205085558031 2491217772274111 29449438902782636 348806466779875961 4138454609488474736 49176494325141603881
;; small program: loop(loop(loop2((x * y) + x, 1 + y, x, 1, 2 * (x + x)) div loop(x * y, x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + (2 * (x + x))) div loop(x * y, x, 1)) * (1 + (2 * (x + x))), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ 2 (* 2 (+ x x))))))
...
(assert (forall ((x Int)) (= (f5 x) (* (div (w6 x) (v7 x)) (+ 1 (* 2 (+ x x)))))))
(assert (= g5 1))
