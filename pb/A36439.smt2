;; sequence(s): A36439
;; terms: 2 4 7 12 19 30 43 60 79 102 131 162 199 240 283 330 383 442 503 570
;; small program: loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, x, 2) + x
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v3 x) (ite (<= x 0) 1 2)) 1) x))))
