;; sequence(s): A308819
;; terms: 1 1 2 6 24 120 120 840 6720 60480 60480 665280 665280 8648640 8648640 8648640 138378240 2352430080 2352430080 44696171520
;; small program: loop(loop(if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then (1 + x) else 1, 1, y - 1) * x, x, 1)
;; fast program: loop(loop(if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then (1 + x) else 1, 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (- (v2 x) 1) 0) (+ 1 x) 1))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (- (v6 x) 1) 0) (+ 1 x) 1))))
(assert (= g5 1))
