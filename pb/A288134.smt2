;; sequence(s): A288134
;; terms: 3 5 6 8 9 10 11 13 14 15 16 17 18 19 20 22 23 24 25 26
;; small program: ((1 + loop(loop((x + y) div 2, x, y), x, x)) + 2) + x
;; fast program: loop(loop2((if (x * y) <= 0 then y else 1) + x, y div 2, 2 * (2 + 2), 1, x), 1, 2 + (x + x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (ite (<= (* x y) 0) y 1) x))))
