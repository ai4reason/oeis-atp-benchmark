...
;; small program: loop((loop((x * y) * y, y - 1, 1) mod (1 + y)) + x, x, 1) div 2
;; fast program: ((loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x div 2, 1) mod (1 + x)) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (mod (v2 x) (+ 1 x)) 1) 2))))
