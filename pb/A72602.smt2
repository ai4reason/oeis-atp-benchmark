;; sequence(s): A72602
;; terms: 2 4 8 9 10 12 16 17 18 20 24 32 33 34 35 36 37 38 40 41
;; small program: compr((loop2((((y mod 2) * (1 + 2)) * x) + x, y div 2, x, 1, x) div 2) - x, 1 + x)
;; fast program: compr(((loop2((((y mod 2) * (1 + 2)) * x) + x, y div 2, x, 1, x) - x) - x) - 2, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (* (* (mod y 2) (+ 1 2)) x) x))))
...
(assert (forall ((x Int)) (= (f2 x) (- (- (- (w3 x) x) x) 2))))
