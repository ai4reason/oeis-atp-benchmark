;; sequence(s): A285971
;; terms: 2 6 8 11 14 18 21 24 26 30 32 35 38 41 43 47 50 54 56 59
;; small program: (((compr((loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + x, x) + x) mod 2, x) + x) div 2) + 2) + x
;; fast program: (((compr((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + (2 * (2 + 2)), x), 2, x) + 1) mod 2, x) + x) div 2) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (+ (v2 x) x) 2) 2) x))))
