;; sequence(s): A18584
;; terms: 1 2 5 10 67 134 335 670
;; small program: loop(compr((loop(((2 * ((x - 2) + x)) + x) * x, 2, 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + (2 + (x * x))) * (2 + x), 1, 2 * (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (+ 2 (* x x))) (+ 2 x)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
