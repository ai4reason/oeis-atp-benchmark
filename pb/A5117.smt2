;; sequence(s): A5117
;; terms: 1 2 3 5 6 7 10 11 13 14 15 17 19 21 22 23 26 29 30 31
;; small program: loop(loop((if ((y * y) mod x) <= 0 then 1 else 0) + x, x, 1 + x), x, 1)
;; fast program: ((((compr(((loop(((loop(((loop((if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0) - x, 2, x) + 1) + x) mod 2, 1, x div 2) + x) mod 2) + x, (1 + x) mod 2, x) + 1) + x) mod 2, x) div 2) + 1) - x) + x) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (ite (<= (mod (* y y) x) 0) 1 0) x))))
...
(assert (forall ((x Int)) (= (f5 x) (- (ite (<= (- (v6 x) x) 0) 1 0) x))))
(assert (= g5 2))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (- (+ (div (v2 x) 2) 1) x) x) 1))))
