;; sequence(s): A260191
;; terms: 3 5 13 17 21 29 37 45 49 53 61 65 69 77 81 85 93 101 109 113
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x, x) - 1, y), x, 2) + 1
;; fast program: ((((compr(loop(x div loop(1, x mod (2 + 2), 2 + 2), loop(x * x, 1, 2 + 2) - 1, x) mod 2, x) - x) + 1) + (if x <= 0 then 1 else x)) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (+ (- (v3 x) x) 1) (ite (<= x 0) 1 x)) 2) 1))))
