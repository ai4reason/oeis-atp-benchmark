;; sequence(s): A18479
;; terms: 1 3 7 21 23 69 161 483
;; small program: loop(compr(loop(((2 + y) * (x * x)) - 1, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + 2) * (loop((2 + x) * x, 1, 2 * (2 + 2)) - x)) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (- (* (+ 2 y) (* x x)) 1))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 2) (- v4 x)) (+ 1 (+ x x))))))
