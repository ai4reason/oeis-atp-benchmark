;; sequence(s): A94987
;; terms: 1 2 4 10 23 51 113 249 548 1207 2655 5843 12855 28281 62218 136880 301136 662499 1457498 3206497
;; small program: loop(x div (1 + (2 + 2)), x, loop(loop((2 + y) * x, 2, x) - x, x, 1))
;; fast program: (loop2(x * y, y, x - 1, 1, 1 + (2 + (2 * (2 + 2)))) * (if x <= 0 then 1 else (1 + (2 + (2 * (2 + 2)))))) div loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div x (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (w3 x) (ite (<= x 0) 1 (+ 1 (+ 2 (* 2 (+ 2 2)))))) (w4 x)))))
