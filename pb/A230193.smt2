;; sequence(s): A230193
;; terms: 3 6 9 11 12 15 18 21 22 24 27 30 33 36 39 42 44 45 48 51
;; small program: compr(((loop(((x + x) + x) mod (1 + (2 * (2 + (2 + 2)))), x, 1) - 1) * x) mod loop(1 + (x + x), 2, 2), 1 + x)
;; fast program: compr((loop((((x * x) * x) * x) * x, 1, loop(x * x, 1, loop(x * x, 2, x))) mod (1 + (2 * (2 * (2 * (2 + 2)))))) mod 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (+ x x) x) (+ 1 (* 2 (+ 2 (+ 2 2))))))))
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f0 x) (mod (* (- (v1 x) 1) x) v2))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (v4 x) (+ 1 (* 2 (* 2 (* 2 (+ 2 2)))))) 2))))
