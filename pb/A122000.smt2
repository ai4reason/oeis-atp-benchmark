;; sequence(s): A122000
;; terms: 1 7 102943 27368368148803711 533411691585101123706582594658103586126397951 3566766192921360077810945505268211287512797261288920841093043641769808083046939618603793791988232043305924036607
;; small program: loop(loop2(1 - (x * y), y, x, 0, x), 1, loop((2 * x) + 1, x, 1))
;; fast program: loop(loop2(x * y, y, x, 1, x) div (1 + x), 1, loop(x + x, x, 2) - 1) + 1
(set-logic UFNIA)
...
(assert (= g0 1))
(assert (forall ((x Int)) (= (f2 x) (+ (* 2 x) 1))))
...
(assert (forall ((x Int)) (= (f3 x) (div (w4 x) (+ 1 x)))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (h3 x) (- (v5 x) 1))))
