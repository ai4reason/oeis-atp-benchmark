;; sequence(s): A192326
;; terms: 0 0 0 0 2 2 4 4 6 10 10 14 16 16 18 22 26 26 30 32
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (1 + (x + x))
;; fast program: (compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (1 + (x + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 1) (+ 1 (+ x x))))))
