;; sequence(s): A93570
;; terms: 3 15 120 1320 21120 401280 9630720 260029440 8320942080 324516741120 13629703127040 667855453224960 36064194474147840 2055659085026426880 127450863271638466560 8794109565743054192640 668352326996472118640640
;; small program: loop(((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) + y) * x) + x, 1 + x, 1)
;; fast program: loop((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + (1 + y)) * x, 1 + x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (+ (v1 x y) y) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) (+ 1 y)) x))))
