;; sequence(s): A155213
;; terms: 0 0 0 0 1 1 1 2 2 3 3 4 4 4 5 5 6 6 7 7
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) div (1 + 2)) div (1 + 2)
;; fast program: (((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + ((2 + x) * x), 2 + 2, 2)) div (1 + 2)) div (1 + 2)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (div (mod (+ (v2 x) 1) v5) (+ 1 2)) (+ 1 2)))))
