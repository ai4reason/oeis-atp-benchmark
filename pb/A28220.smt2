;; sequence(s): A28220
;; terms: 1 35 770 13630 212331 3041745 41087740 531634460 6658867061 81336796255 974120693910 11484949238490 133713902876191 1540987906351565 17612518552867280 199940810374681720 2257196040059471721 25366166559007515675 283993639507153095850
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop(loop(loop((((loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) + loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2))))) div 2) - loop2(x * y, y, x, 1, 1 + (2 * (2 + 2)))) div 2, 1, 2 + x) div 2, 1, y) + (2 * (2 * (x + x))), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (div (- (div (+ (w8 x) (w9 x)) 2) (w10 x)) 2))))
(assert (= g7 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (v6 x y) (* 2 (* 2 (+ x x)))))))
