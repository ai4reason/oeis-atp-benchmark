;; sequence(s): A76144
;; terms: 1 1 2 3 5 6 7 10 11 13 14 15 17 19 21 22 23 26 29 30
;; small program: loop(loop((if ((y * y) mod x) <= 0 then 1 else 0) + x, x, 1 + x), x - 1, 1)
;; fast program: compr((loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, x - 1, 1) mod (if x <= 0 then 1 else x)) - (x mod 2), if x <= 0 then 1 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (- (mod (v3 x) (ite (<= x 0) 1 x)) (mod x 2)))))
