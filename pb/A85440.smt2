;; sequence(s): A85440
;; terms: 1 244 8020 108020 867395 4951496 22161864 82628040 267156165 770440540 2022773116 4909947484 11150268935 23913084560 48796284560 95322158736 179163294729 325374464580 572984364580 981394464580
;; small program: loop(loop(loop(loop(x * x, 2, x) * x, 1, 1 + loop(x + y, x, x)), 1, y) + x, x, 1)
;; fast program: loop(loop(loop((((x * x) * x) * x) * x, 1, ((1 + x) * (2 + x)) div 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (* (* (* (* x x) x) x) x))))
(assert (= g7 1))
(assert (forall ((x Int)) (= (h7 x) (div (* (+ 1 x) (+ 2 x)) 2))))
