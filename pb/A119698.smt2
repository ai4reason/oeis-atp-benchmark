;; sequence(s): A119698
;; terms: 0 2 48 2160 40320 787500 12773376 198942744 2905943040 40973240880 558886900000 7437264466632 96894798594048 1240510066959800 15645553270401600 194802398523000000 2398428702498447360 29240457953102773740 353391128284307097600
;; small program: loop(2 * ((x - (x div y)) + x), x, x) * loop(x * x, 1, loop2(x + y, x, x, 0, x))
;; fast program: (loop(2 * ((x - (x div y)) + x), x, x) * loop(x * x, 1, loop2(x + y, x, x - 2, 1, 1))) * (x * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) (v4 x)) (* x x)))))
