;; sequence(s): A25988
;; terms: 1 22 315 3740 40121 405042 3935095 37284280 347419941 3201202862 29273641475 266321046420 2414445954961 21837387317482 197190900709455 1778695458106160 16032377546389181 144437380023772902
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x + x, x - 1, 2), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + ((2 + 2) * (x + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) x) (* (+ 2 2) (+ x x))))))
