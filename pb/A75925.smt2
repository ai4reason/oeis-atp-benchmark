;; sequence(s): A75925
;; terms: 1 147 13034 907578 54807627 3016638009 155726334148 7676501248416 365698066506773 16976491006185711 772549060467762942 34614587429584922214 1532054031119984651839 67151990527665760714053
;; small program: loop(((loop((loop((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, y, 1) + x) + x) + x, x, 1) * loop((2 * ((x + x) + x)) + x, x, 1)
;; fast program: loop(loop(loop(((((loop(loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1) * loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f11 x) (div (div (v12 x) 2) (+ 1 2)))))
(assert (= g11 1))
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (+ (+ (+ (v9 x y) x) x) x) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (v6 x) (w14 x)))))
