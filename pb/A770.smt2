;; sequence(s): A770
;; terms: 1 21 266 2646 22827 179487 1323652 9321312 63436373 420693273 2734926558 17505749898 110687251039 693081601779 4306078895384 26585679462804 163305339345225 998969857983405 6090236036084530 37026417000002430
;; small program: loop(loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, y, 1) + (((x + x) + x) * 2), x, 1)
;; fast program: loop(loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, y, 1) + (2 * ((x + x) + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) (* (+ x x) 2)) x))))
