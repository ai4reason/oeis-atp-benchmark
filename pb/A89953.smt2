;; sequence(s): A89953
;; terms: 0 2 4 8 10 12 18 20 22 24 30 32 34 40 44 48 50 52 58 62
;; small program: compr(loop((2 - loop(x + x, x, 2)) mod (1 + x), 1, (x + x) + x), 1 + x) - 2
;; fast program: (compr(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1 + x) - 1) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (- (v3 x) 1) (ite (<= x 0) 1 2)))))
