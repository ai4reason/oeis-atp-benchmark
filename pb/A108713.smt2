;; sequence(s): A108713
;; terms: 1 1 3 128 162000 10319560704 50185433088000000 26294650153960734720000000 1991323677312505284928104038400000000 28163375844474584946472694002483200000000000000000
;; small program: loop2(loop(x * y, y - 1, 1 + y) * x, y, x, 1, x) div (1 + x)
;; fast program: (loop2(x * y, y, x - 2, 1, loop(x * y, x - 1, 1)) * loop2(x * y, y, x - 1, 1, 1 + x)) * loop(x * x, 1, loop(x * y, x - 1, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (w2 x) (w4 x)) (v5 x)))))
