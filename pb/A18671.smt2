;; sequence(s): A18671
;; terms: 1 2 4 8 13 16 26 32 52 64 104 208 416 832
;; small program: loop(compr(loop(((2 * (x + y)) + x) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((((compr(loop((loop(((x * x) * x) - x, 1, 2 + (2 + 2)) - 2) * (x * x), 1, 2) mod (1 + x), x) + 1) + x) - x) + x) - x
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (* (- v5 2) (* x x)))))
(assert (= g4 1))
(assert (= h4 2))
