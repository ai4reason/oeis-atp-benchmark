;; sequence(s): A6308
;; terms: 3 10 21 55 78 136 171 253 406 465 666 820 903 1081 1378 1711 1830 2211 2485 2628
;; small program: loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), 1 + y) * x) + 1, y, 2, 1, x) div 2
;; fast program: (loop2((loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x) - 1, y, 2, 1, x) div 2) + 1
(set-logic UFNIA)
