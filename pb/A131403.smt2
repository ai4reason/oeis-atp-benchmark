;; sequence(s): A131403
;; terms: 1 2 5 10 21 44 93 196 411 856 1771 3642 7451 15178 30809 62358 125921 253800 510777 1026704
;; small program: (1 + (loop(x + x, x, 2) - loop2(x + y, x, x, 2, 2))) + x
;; fast program: (1 + (2 * (loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) - loop2(x + y, x, x, 1, 1)))) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ 1 (- (v0 x) (w1 x))) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ 1 (* 2 (- (v2 x) (w5 x)))) x))))
