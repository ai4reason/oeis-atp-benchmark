;; sequence(s): A78039
;; terms: 1 ~2 4 ~9 19 ~41 88 ~189 406 ~872 1873 ~4023 8641 ~18560 39865 ~85626 183916 ~395033 848491 ~1822473
;; small program: loop2(((((y * y) + y) div (x + y)) + x) + x, x, x, 1, 0) * loop(0 - x, x, 1)
;; fast program: loop2(((((y * y) + y) div (x + y)) + x) + x, x, x - 1, if x <= 0 then 1 else 2, 1) * loop(0 - x, x mod 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (div (+ (* y y) y) (+ x y)) x) x))))
