;; sequence(s): A36579
;; terms: 1 2 0 1 0 2 1 2 0 2 1 0 1 2 0 1 0 2 1 0
;; small program: loop(loop2((loop(loop2(x + y, y div 2, x, 1, x) mod 2, 1, y) + x) * x, 1 + y, 2, 1, x) div 2, 2, x)
;; fast program: (((loop((((loop2((y - x) div 2, x, 2 + (2 + 2), 0, x) + 1) mod 2) + 1) + x, 2, x) + x) mod (2 + 2)) + 1) mod (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (mod (+ (v4 x) x) (+ 2 2)) 1) (+ 1 2)))))
