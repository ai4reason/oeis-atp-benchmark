;; sequence(s): A26225
;; terms: 1 3 4 7 9 10 12 13 16 19 21 22 25 27 28 30 31 34 36 37
;; small program: loop(compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, x, x) - 1) mod (1 + 2), y), x, 1)
;; fast program: compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(1 + (x * x), 2, 2), x) mod (1 + 2)) div 2, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (v2 x) 1) (+ 1 2)))))
