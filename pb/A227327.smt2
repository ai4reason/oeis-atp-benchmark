;; sequence(s): A227327
;; terms: 0 1 4 10 22 41 72 116 180 265 380 526 714 945 1232 1576 1992 2481 3060 3730
;; small program: loop(loop(loop((if (y mod 2) <= 0 then 1 else y) + x, y - 1, x), y, x) + y, x, 0)
;; fast program: loop(loop(loop((if (y mod 2) <= 0 then 1 else y) + x, y, x), y, x) + y, x - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (ite (<= (mod y 2) 0) 1 y) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (ite (<= (mod y 2) 0) 1 y) x))))
