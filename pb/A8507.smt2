;; sequence(s): A8507
;; terms: 0 0 0 1 1 2 2 3 5 5 7 8 8 9 11 13 13 15 16 16
;; small program: ((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 2 + x) div 2) - 1) - x
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 2 + x) div 2) div 1) - (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (div (div (v2 x) 2) 1) (+ 1 x)))))
