;; sequence(s): A99545
;; terms: 1 1 3 1 1 3 3 1 1 1 3 3 1 3 3 1 1 1 3 1
;; small program: 1 + (loop(x div (1 + (x mod 2)), x, x) mod (2 + 2))
;; fast program: ((loop(loop(loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) mod 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 1 (mod (v0 x) (+ 2 2))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (mod (v1 x) 2) 2) 1))))
