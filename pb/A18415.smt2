;; sequence(s): A18415
;; terms: 1 2 3 6 61 122 183 366
;; small program: loop(compr(loop((2 * ((x * x) + x)) + y, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(loop((1 + ((loop(x + y, x, x) * x) + x)) * (2 + x), 1, 2 + 2) mod (1 + x), x) + 1) + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (+ (* (v5 x) x) x)) (+ 2 x)))))
(assert (= g4 1))
(assert (= h4 (+ 2 2)))
