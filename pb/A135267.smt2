;; sequence(s): A135267
;; terms: 0 ~1 ~2 ~3 ~2 ~1 2 5 10 19 28 41 56 71 88 109 134 159 188 219
;; small program: (loop(loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, x) - x, 1, y) + x, x, 0) * 2) - x
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) + 1) - ((1 + x) * (1 + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (v4 x) 2) 1) (* (+ 1 x) (+ 1 x))))))
