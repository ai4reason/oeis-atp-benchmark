;; sequence(s): A6010
;; terms: 1 5 20 52 117 225 400 656 1025 1525 2196 3060 4165 5537 7232 9280 11745 14661 18100 22100
;; small program: loop((((1 + (y * y)) div 2) * y) + x, 1 + x, 0)
;; fast program: loop2(loop(1 + (((x * x) div 2) + x), 1, y) * x, 1 + y, 2, 1, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (div (+ 1 (* y y)) 2) y) x))))
(assert (forall ((x Int)) (= (g0 x) (+ 1 x))))
(assert (= h0 0))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (v0 x))))
(assert (forall ((x Int)) (= (f2 x) (+ 1 (+ (div (* x x) 2) x)))))
(assert (= g2 1))
