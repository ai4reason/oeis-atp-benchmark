;; sequence(s): A85007
;; terms: 1 2 4 5 6 7 8 9 11 12 13 15 16 17 19 20 21 22 23 24
;; small program: loop(loop(if ((x mod (1 + 2)) - 1) <= 0 then 2 else (x div 2), 2 + (2 + 2), y) + x, x, 1)
;; fast program: loop(loop((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 1 + 2, x), 1 - (x mod (2 + 2)), x) + 1) mod 2, 1, y) + x, x - (x div (1 + 2)), 1) + x
(set-logic UFNIA)
