;; sequence(s): A1695
;; terms: 1 3 4 8 65536
;; small program: loop(loop(x + x, x div 2, 1), x - 2, loop2(x + y, x, x, 1, 2))
;; fast program: loop(loop(x + x, x, 1), x - 2, (if x <= 0 then 1 else (2 + 2)) - (x mod 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (- (ite (<= x 0) 1 (+ 2 2)) (mod x 2)))))
