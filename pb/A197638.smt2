;; sequence(s): A197638
;; terms: 4 14 22 26 34 38 46 58 62 74 82 86 94 106 118 122 134 142 146 158
;; small program: ((compr((1 + loop(x + x, x, x)) mod (1 + x), 2 + x) + 2) * (if x <= 0 then 1 else 2)) - 2
;; fast program: (((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 2 + x) * 2) + 2) * (if x <= 0 then 1 else 2)) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (* (+ (v0 x) 2) (ite (<= x 0) 1 2)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (* (v2 x) 2) 2) (ite (<= x 0) 1 2)) 2))))
