;; sequence(s): A87691
;; terms: 9 49 121 361 529 961 1849 2209 3481 4489 5041 6241 6889 10609 11449 16129 17161 19321 22801 26569
;; small program: loop2((compr(loop((1 + loop(x + x, x, x)) mod (1 + x), (x div 2) mod 2, x), 1 + y) * x) + x, y, 2, 1, x)
;; fast program: loop2(((compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 1 + y) * 2) * x) + x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (mod (div x 2) 2))))
