;; sequence(s): A18696
;; terms: 1 2 19 23 38 46 437 874
;; small program: loop(compr((loop((2 + y) * (1 + x), 2 + 2, 1) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((loop(compr(((((1 + (2 + (2 * (2 + 2)))) * loop(x * x, 2, 1 + 2)) - 1) - loop(x * x, 2, 2)) mod (1 + x), y), x, 0) + 1) * 2) div 2) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 y) (+ 1 x)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 1))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y) x)))))
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (- v2 2) (+ 1 x)))))
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f4 x) (mod (- (- (* (+ 1 (+ 2 (* 2 (+ 2 2)))) v5) 1) v6) (+ 1 x)))))
