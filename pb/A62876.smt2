;; sequence(s): A62876
;; terms: 4 12 20 28 44 52 68 76 92 116 124 148 164 172 188 212 236 244 268 284
;; small program: (compr((1 + loop(x + x, x, x)) mod (1 + x), x) + 1) * compr(x mod 2, 2)
;; fast program: (((compr((((if ((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) * 2) + 1) * 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod x 2))))
(assert (= g2 2))
...
(assert (forall ((x Int)) (= (fast x) (* (* (+ (* (v3 x) 2) 1) 2) 2))))
