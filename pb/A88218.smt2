...
;; small program: (1 + loop(2 * ((x - (x div y)) + x), x, 1)) div 2
;; fast program: loop(2 * (((x div y) + x) + x), x - 1, 1) div (if x <= 0 then 1 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (v1 x) (ite (<= x 0) 1 x)))))
