;; sequence(s): A171512
;; terms: 1 3 4 5 7 8 10 11 13 15 17 18 20 21 23 25 27 28 30 31
;; small program: loop((((((loop(x * y, y - 1, 1) + x) mod y) + x) + y) div x) + x, x, 1)
;; fast program: ((((loop(loop((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, (x - 1) div 2, 1) * (if (x - 1) <= 0 then 2 else x)) - 1) mod (1 + x)) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (+ (+ (mod (+ (v1 x y) x) y) x) y) x) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (mod (- (* (v2 x) (ite (<= (- x 1) 0) 2 x)) 1) (+ 1 x)) 1) x))))
