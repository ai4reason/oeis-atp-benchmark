;; sequence(s): A49847
;; terms: 1 1 1 2 1 1 2 1 1 2 1 2 3 1 1 2 3 1 2 1
;; small program: loop((loop((loop(if ((loop(x + x, x, 1) mod (1 + x)) - 1) <= 0 then 1 else 0, 1, x + x) + 1) mod 2, 1, y) * x) + 1, 1 + x, x)
;; fast program: loop((loop((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, 1 + x) + 1) mod 2, 1, y) * x) + 1, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (ite (<= (- (mod (+ 1 (v7 x)) (+ 1 (+ x x))) 2) 0) 1 2))))
(assert (= g6 1))
