;; sequence(s): A67689
;; terms: 2 72 43200 423360000 67212633600000 172153600393420800000 7097063852481244869427200000 4702142622508202833251304734720000000 50019370356486058711268515056654483456000000000
;; small program: loop(loop(loop2(loop(2 * ((x - (x div y)) + x), y, x), 1 + y, 2, 1, x) * (1 + (x + x)), 1, y) * x, x, 2)
;; fast program: loop(loop((loop2((1 + y) * (x * y), 1 + y, x, 1, 2 + x) div loop((y * x) * y, x, 1)) * (2 + x), 1, y) * x, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (w2 x) (+ 1 (+ x x))))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ 1 y) (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (* y x) y))))
