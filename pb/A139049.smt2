;; sequence(s): A139049
;; terms: 8 9 11 13 17 19 23 25 29 35 37 43 47 49 53 59 65 67 73 77
;; small program: (((compr((1 + loop(x + x, x, x)) mod (1 + x), x) + loop(1, x, 2)) + 2) + 2) + 2
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) + (if x <= 0 then 2 else 1)) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (v0 x) (v2 x)) 2) 2) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (* (v3 x) 2) 2) (ite (<= x 0) 2 1)) 2) 2))))
