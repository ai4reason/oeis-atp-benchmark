;; sequence(s): A120307
;; terms: 1 3 60 10500 18522000 359400888000 81408613942656000 224737840779305293440000 7812628980363223707442752000000 3508978524227146242839564498172672000000
;; small program: loop(loop((loop2(loop(2 * ((x - (x div y)) + x), y, x), 1 + y, 2, 1, x) div 2) div (1 + x), 1, y) * x, x, 1)
;; fast program: loop(loop((loop2((x * y) * y, y - 1, x - 1, 1, x + x) div loop(x * x, 1, loop(x * y, x, 1))) * (1 + (x + x)), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (div (w2 x) 2) (+ 1 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (* x y) y))))
(assert (forall ((x Int) (y Int)) (= (g6 x y) (- y 1))))
(assert (forall ((x Int)) (= (h6 x) (- x 1))))
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ x x))))
...
(assert (forall ((x Int)) (= (f5 x) (* (div (w6 x) (v7 x)) (+ 1 (+ x x))))))
(assert (= g5 1))
