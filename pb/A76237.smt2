;; sequence(s): A76237
;; terms: 0 0 0 0 3 3 3 1 8 8 10 9 8 8 8 7 6 6 7 8
;; small program: (loop(compr(loop(x * y, x, 1) mod (1 + x), x), 2, x) + 1) mod (1 + x)
;; fast program: (loop(loop(((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else 0) + 1) + x, 1 + x, 2) - 1, 2, x) + 1) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (v3 x) 1) (+ 1 x)))))
