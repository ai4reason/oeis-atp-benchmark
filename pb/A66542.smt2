;; sequence(s): A66542
;; terms: 3 4 5 7 8 11 13 16 17 19 23 29 31 32 37 41 43 47 53 59
;; small program: compr((loop(loop(2, 1, y) * x, x, 1) mod (1 + x)) - 1, 2 + x) + 1
;; fast program: compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), loop(1, x - 2, 2) mod 2, x) - 2, 2 + x) + 1
(set-logic UFNIA)
(declare-fun f2 () Int)
(declare-fun g2 () Int)
...
(assert (forall ((x Int)) (= (g4 x) (mod (v6 x) 2))))
