;; sequence(s): A26602-A287436
;; terms: 2 4 9 10 15 17 21 23 25 28 33 35 39 41 43 47 49 54 57 59
;; small program: loop(compr(loop2(x + y, y div (1 + 2), x, 2, x) mod (1 + 2), y), x, 1) + 1
;; fast program: ((((loop2(x - y, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) + 1) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (w2 x) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (mod (w3 x) (+ 1 2)) 1) x) x) x))))
