;; sequence(s): A307096
;; terms: 1 5 13 17 29 37 49 61 65 77 101 113 125 133 145 157 193 205 229 241
;; small program: ((loop(compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) div (1 + (x mod 2)), x, x), y), x, 0) * 2) * 2) + 1
;; fast program: ((compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) div (1 + (x mod 2)), 1 + x, x) - 1, x) * 2) * 2) + 1
(set-logic UFNIA)
(declare-fun f4 () Int)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (v5 x) 2) 2) 1))))
