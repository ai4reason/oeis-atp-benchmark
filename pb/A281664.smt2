;; sequence(s): A281664
;; terms: 1 2 6 10 18 22 30 42 46 58 66 70 78 82 102 106 126 130 138 150
;; small program: compr(loop((1 + loop(x + x, x, x)) mod (1 + x), ((x div 2) + x) mod 2, x) - 1, 1 + x)
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x) - 1, x) * 2) + 1) - (if x <= 0 then 1 else 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 2)) 1))))
