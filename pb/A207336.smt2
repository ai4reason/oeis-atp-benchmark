;; sequence(s): A207336
;; terms: 2 4 5 7 8 10 11 14 16 19 20 22 23 26 29 31 34 35 37 40
;; small program: (((compr((1 + loop(x + x, x, x)) mod (1 + x), 2 + x) div (1 + 2)) * (1 + 2)) div 2) + 1
;; fast program: (((((((((compr((((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1, 2 + x) * 2) + 1) div (1 + 2)) * 2) - 2) * (1 + 2)) + 2) div 2) div 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (* (div (v0 x) (+ 1 2)) (+ 1 2)) 2) 1))))
...
(assert (forall ((x Int)) (= (f2 x) (- (mod (* (* (* (+ 1 (mod x 2)) (v3 x)) (v5 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (div (+ (* (- (* (div (+ (* (v2 x) 2) 1) (+ 1 2)) 2) 2) (+ 1 2)) 2) 2) 2) 2))))
