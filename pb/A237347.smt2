;; sequence(s): A237347
;; terms: 3 3 2 3 2 3 2 2 3 2 2 3 2 2 2 3 2 2 2 3
;; small program: (if loop(loop(y - 1, x - (y div 2), x), x, x) <= 0 then 1 else 0) + 2
;; fast program: ((loop(loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, (x - 2) div 2) + 2) + x, 2, x) + 2, 2, x) + x) mod 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (- x (div y 2)))))
...
(assert (forall ((x Int)) (= (small x) (+ (ite (<= (v0 x) 0) 1 0) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (v2 x) x) 2) 2))))
