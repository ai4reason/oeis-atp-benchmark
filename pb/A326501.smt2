;; sequence(s): A326501
;; terms: 1 0 4 ~23 233 ~2892 43764 ~779779 15997437 ~371423052 9628576948 ~275683093663 8640417354593 ~294234689237660 10817772136320356 ~427076118244539019 18019667955465012597 ~809220593930871751580 38537187481365665823844
;; small program: loop(x + loop2(0 - (x * y), y, y, 1, y), x, 1)
;; fast program: loop(loop(loop2(x * y, y, x - 2, 1, x) * (x * x), 1, y) - x, x, 1) div loop(0 - x, x mod 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (w4 x) (* x x)))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (fast x) (div (v2 x) (v5 x)))))
