;; sequence(s): A39713
;; terms: 2 3 5 7 11 13 2 4 8 14 1 7 11 13 2 8 14 1 7 11
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (loop(x * x, 2, 2) - 1)) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + ((x mod (2 + 2)) + x), 2 + 2, 2)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (- v2 1)) 1))))
...
(assert (forall ((x Int)) (= (f6 x) (+ 1 (+ (mod x (+ 2 2)) x)))))
(assert (= g6 (+ 2 2)))
(assert (= h6 2))
