;; sequence(s): A241203
;; terms: 5 6 7 9 12 15 19 23 29 37 46 58 72 90 113 142 177 222 277 346
;; small program: loop((x div 2) div 2, x, loop((2 * (x + x)) + x, 1 + x, 1))
;; fast program: ((1 + (2 + 2)) * loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))) div loop(x * x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ 1 (+ 2 2)) (w2 x)) (v5 x)))))
