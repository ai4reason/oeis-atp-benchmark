;; sequence(s): A56791
;; terms: 1 2 3 4 4 5 5 6 5 6 6 7 6 7 7 8 6 7 7 8
;; small program: loop2(loop(((y mod 2) - 1) + x, y - 2, x), y div 2, x, 1, x) + x
;; fast program: loop2((if y <= 0 then 0 else (1 - y)) + x, y div 2, 2 + (2 + 2), 1, x div 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (mod y 2) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (ite (<= y 0) 0 (- 1 y)) x))))
