;; sequence(s): A340068
;; terms: 0 0 0 1 1 2 3 3 3 4 5 5 6 6 6 6 6 7 8 8
;; small program: loop(loop(if loop(loop((2 + x) mod y, x, 1), 2, x) <= 0 then 1 else 0, 1, y) + x, x, 0)
;; fast program: loop(loop(if (((loop2(x - y, y div 2, 2 + (2 + 2), x, x div 2) - 1) + x) mod (1 + x)) <= 0 then 1 else 0, 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (= h6 (+ 2 (+ 2 2))))
(assert (forall ((x Int)) (= (i6 x) x)))
(assert (forall ((x Int)) (= (j6 x) (div x 2))))
...
(assert (forall ((x Int)) (= (w6 x) (u6 h6 (i6 x) (j6 x)))))
(assert (forall ((x Int)) (= (f5 x) (ite (<= (mod (+ (- (w6 x) 1) x) (+ 1 x)) 0) 1 0))))
(assert (= g5 1))
