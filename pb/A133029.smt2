;; sequence(s): A133029
;; terms: 1 7 13 19 91 133 247 1729
;; small program: loop(compr((1 + loop((2 + (x - y)) * (x * x), 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr((((1 + 2) * loop((1 + x) * (x * x), 1, 2 * (2 + 2))) - x) mod (1 + x), x) + 1) * 2) div 2) * 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (+ 1 v2) (+ 1 x)))))
