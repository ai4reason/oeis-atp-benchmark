;; sequence(s): A14166
;; terms: 0 1 5 16 41 92 189 365 674 1204 2098 3588 6050 10093 16703 27476 44995 73440 119575 194345
;; small program: loop(loop(loop(loop2(x + y, x, y, 1, 1) + x, y, x) - y, y, x), x, 0)
;; fast program: loop((((loop(loop((loop2(x + y, x, x, 1 + 2, 2) - (((x * x) + x) div 2)) - x, 1, y) + x, x, 1) - x) - 1) - x) - x, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (- (- (w7 x) (div (+ (* x x) x) 2)) x))))
(assert (= g6 1))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (v5 x) x) 1) x) x))))
(assert (= g4 1))
