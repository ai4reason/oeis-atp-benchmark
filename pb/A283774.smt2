;; sequence(s): A283774
;; terms: 1 2 8 9 10 16 17 18 24 25 26 32 33 34 40 41 48 49 55 56
;; small program: loop(compr((loop((y * y) div (x - y), x, 0) + x) mod (1 + 2), 1 + y), x, 1)
;; fast program: compr(((((((((((loop((y * y) div (x - y), x, 0) + x) * 2) - (if x <= 0 then 1 else 2)) + 1) - x) - x) - 1) - x) mod (1 + 2)) - 1) mod (1 + x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (v2 x) x) (+ 1 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (div (* y y) (- x y)))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (= h4 0))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) h4))))
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (- (- (- (- (+ (- (* (+ (v4 x) x) 2) (ite (<= x 0) 1 2)) 1) x) x) 1) x) (+ 1 2)) 1) (+ 1 x)))))
