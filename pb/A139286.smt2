;; sequence(s): A139286
;; terms: 8 32 512 8192 2097152 33554432 8589934592 137438953472 35184372088832 144115188075855872 2305843009213693952 9444732965739290427392 2417851639229258349412352 38685626227668133590597632 9903520314283042199192993792
;; small program: (loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), 0, 2, 1, x) div 2) div 2
;; fast program: loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), 0 - 1, 2, 1, x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (w4 x) 2))))
