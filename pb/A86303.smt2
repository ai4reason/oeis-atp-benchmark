;; sequence(s): A86303
;; terms: 2 4 8 14 16 22 26 28 32 38 44 46 52 56 58 64 68 74 82 86
;; small program: (loop(((compr((1 + loop(x + x, x, x)) mod (2 + x), 2 + (1 + x)) div 2) - 2) - 2, 1, 2 + x) - 2) * 2
;; fast program: (loop((loop(compr((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, 2 + x) - 2, 1, 2 + x) - 2) - 2, 1, 2 + x) * 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g1 x) (+ 2 (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f0 x) (- (- (div (v1 x) 2) 2) 2))))
(assert (= g0 1))
...
(assert (forall ((x Int)) (= (f5 x) (- (mod (- (mod (* (* 2 (v6 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1) (+ 1 x)) 1))))
