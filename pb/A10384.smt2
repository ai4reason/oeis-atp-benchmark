;; sequence(s): A10384
;; terms: 0 1 3 4 5 9 11 12 14 15 16 20
;; small program: compr(((loop(x * x, 2, x) * x) mod loop(1 + (x + x), 2, 2)) - 1, x)
;; fast program: compr((((((x * x) * x) * x) * x) mod (1 + (2 + (2 * (2 + 2))))) - 1, x)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f0 x) (- (mod (* (v1 x) x) v2) 1))))
(assert (forall ((x Int)) (= (g0 x) x)))
(assert (forall ((x Int)) (= (t0 x) (ite (<= (f0 x) 0) x (t0 (+ x 1))))))
(assert (forall ((x Int)) (= (u0 x) (ite (<= x 0) (t0 0) (t0 (+ (u0 (- x 1)) 1))))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x)))))
(assert (forall ((x Int)) (= (small x) (v0 x))))
(assert (forall ((x Int)) (= (f3 x) (- (mod (* (* (* (* x x) x) x) x) (+ 1 (+ 2 (* 2 (+ 2 2))))) 1))))
