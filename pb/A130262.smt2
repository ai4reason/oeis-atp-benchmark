;; sequence(s): A130262
;; terms: 0 1 3 5 8 11 14 17 20 24 28 32 36 40 44 48 52 56 60 64
;; small program: loop((loop((y div loop2(x + y, x, x, 1, 0)) + x, y - 1, 1) div 2) + x, x, x)
;; fast program: loop(loop(((loop((y div loop2(x + y, x, x, 2, 1)) + x, x, 0) div 2) + 1) mod (1 + x), 1, y) + x, x - 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div (v1 x y) 2) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div y (w6 x)) x))))
(assert (forall ((x Int)) (= (g5 x) x)))
(assert (= h5 0))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) h5))))
(assert (forall ((x Int)) (= (f4 x) (mod (+ (div (v5 x) 2) 1) (+ 1 x)))))
(assert (= g4 1))
