;; sequence(s): A246440
;; terms: 1 2 3 7 8 9 14 15 16 20 21 22 26 27 28 32 33 34 35 39
;; small program: loop(compr(((((((((2 * (x + x)) - 1) div loop(1 + (x + x), 2, 2)) + 1) + x) div 2) + 1) + x) mod 2, y), x, 2) - 1
;; fast program: loop(compr(((((((((2 * (x + x)) - 1) div (1 + (2 + (2 * (2 + 2))))) + 1) + x) div 2) + 1) + x) mod 2, y), x, 2) - 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (+ (+ (div (+ (+ (div (- (* 2 (+ x x)) 1) v2) 1) x) 2) 1) x) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ (+ (div (+ (+ (div (- (* 2 (+ x x)) 1) (+ 1 (+ 2 (* 2 (+ 2 2))))) 1) x) 2) 1) x) 2))))
