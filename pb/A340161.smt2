;; sequence(s): A340161
;; terms: 1 4 6 7 10 11 13 18 19 21 25 34 35 37 41 49 66 67 69 73
;; small program: compr(loop(loop((2 + x) mod y, x, 1), 2, x - 1), 1 + x)
;; fast program: (compr(loop((((loop2(((y mod 2) * x) + x, y div 2, 2 * (2 + (2 + 2)), 1, x) - 1) div 2) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (- (div (- (w5 x) 1) 2) 1) (+ 1 x)))))
