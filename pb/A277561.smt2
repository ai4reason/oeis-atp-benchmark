;; sequence(s): A277561
;; terms: 1 2 2 2 2 4 2 2 2 4 4 4 2 4 2 2 2 4 4 4
;; small program: loop2(((((((y div 2) * y) + 2) + y) mod 2) * x) + x, y div 2, x, 1, x)
;; fast program: loop2((loop(loop(x - (x div 2), x mod 2, x) mod 2, 1, y) * x) + x, y div 2, 1 + (2 + (2 + 2)), 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (mod (+ (+ (* (div y 2) y) 2) y) 2) x) x))))
...
(assert (forall ((x Int)) (= (f3 x) (- x (div x 2)))))
