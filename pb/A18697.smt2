;; sequence(s): A18697
;; terms: 1 5 7 25 35 125 175 875
;; small program: loop(compr((loop(((2 + y) * x) + 1, 2 + 2, 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 + x) * ((x * x) * x), 1, 1 + (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (+ 2 y) x) 1))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
