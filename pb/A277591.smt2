;; sequence(s): A277591
;; terms: 4 14 24 34 40 44 54 64 74 84 94 104 114 124 134 140 144 154 164 174
;; small program: compr((((loop(x div loop(1, x mod (2 + (2 * (2 + 2))), ((2 + 2) * 2) + 2), x div (2 + 2), x) mod (1 + (2 + 2))) - 1) - 1) mod (1 + x), 1 + x) * 2
;; fast program: compr(((((loop(x div loop(1, x mod (2 + (2 * (2 + 2))), 2 + (2 * (2 + 2))), x div (2 + (2 * (2 + (2 + 2)))), x) mod (1 + (2 + 2))) - 1) mod (1 + 2)) + 1) mod 2, x) * 2
(set-logic UFNIA)
(declare-fun f2 () Int)
...
(assert (forall ((x Int)) (= (f0 x) (mod (- (- (mod (v1 x) (+ 1 (+ 2 2))) 1) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (g4 x) (div x (+ 2 (* 2 (+ 2 (+ 2 2))))))))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (mod (+ (mod (- (mod (v4 x) (+ 1 (+ 2 2))) 1) (+ 1 2)) 1) 2))))
