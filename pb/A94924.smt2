;; sequence(s): A94924
;; terms: 0 0 1 0 1 4 1 0 1 0 1 4 1 10 1 0 1 10 1 0
;; small program: loop(1 + loop((x + x) + x, 2, x), x, 1) mod (1 + x)
;; fast program: (((loop(x * x, 1, loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div (2 * (2 + 2))) * (1 + (2 * (2 + 2)))) + 1) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (* (div (v2 x) (* 2 (+ 2 2))) (+ 1 (* 2 (+ 2 2)))) 1) (+ 1 x)))))
