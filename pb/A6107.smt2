;; sequence(s): A6107
;; terms: 1 341 93093 24208613 6221613541 1594283908581 408235958349285 104514759495347685 26756185103024942565 6849609413493939400165 1753501675591663698472421 448896535558672700374937061 114917519925881846404167134693
;; small program: loop(loop(loop(loop(loop(loop(1 + (loop(x * x, 2, 2) * x), y, 1) + (2 * (x + x)), x, 1) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1)
;; fast program: loop(loop(loop(loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (v4 x) (w7 x)))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (f1 x) (* (v2 x) (w8 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f14 x) (* (div (v15 x) (+ 1 2)) (w17 x)))))
(assert (= g14 1))
...
(assert (forall ((x Int) (y Int)) (= (f13 x y) (+ (v14 x y) x))))
...
(assert (forall ((x Int)) (= (f12 x) (* (v13 x) (w18 x)))))
(assert (= g12 1))
...
(assert (forall ((x Int)) (= (f10 x) (* (v11 x) (w19 x)))))
(assert (= g10 1))
