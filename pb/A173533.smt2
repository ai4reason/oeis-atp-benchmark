;; sequence(s): A173533
;; terms: 4503599627370496 6461081889226673298932241 2220446049250313080847263336181640625 88124787089723195184393736687912818113311201 1420429319844313329730664601483335671261683881745483121 8415003868347247618489696679505181495471801448798649088081
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * ((x * x) * x), y, 1 + 2, 1, x), 1, y) * x, y, 2 + 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * ((x * x) * x), y, 1 + 2, 1, x), 1, y) * x, y, 2 + 2, 1, x)
(set-logic UFNIA)
