;; sequence(s): A94920
;; terms: 0 0 1 0 1 0 1 0 4 6 1 0 1 6 1 0 1 0 1 16
;; small program: loop(1 + ((2 * (x + x)) + x), x, 1) mod (1 + x)
;; fast program: (((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), (2 * (2 * (2 + (2 + 2)))) + 1) div (2 + 2)) * (1 + (2 + 2))) + 1) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (* (div (w1 x) (+ 2 2)) (+ 1 (+ 2 2))) 1) (+ 1 x)))))
