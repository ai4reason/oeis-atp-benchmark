;; sequence(s): A18405
;; terms: 1 2 3 4 6 12 29 58 87 116 174 348
;; small program: compr(loop((1 + (((y * y) + x) + x)) * x, 2, 2) mod (1 + x), x) + 1
;; fast program: compr(((2 * loop((2 + x) * (x * x), 1, 1 + (2 + 2))) - 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ 1 (+ (+ (* y y) x) x)) x))))
(assert (= g1 2))
(assert (= h1 2))
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (f2 x) (mod (- (* 2 v3) 2) (+ 1 x)))))
