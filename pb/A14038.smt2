...
;; small program: loop((x div 2) - x, x mod (2 + loop((x * x) + y, 2, 2)), 1)
;; fast program: loop((x div 2) - x, x mod (2 + loop((x * x) * x, 1, 1 + 2)), 1)
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (g0 x) (mod x (+ 2 v1)))))
(assert (= h0 1))
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (g2 x) (mod x (+ 2 v3)))))
(assert (= h2 1))
