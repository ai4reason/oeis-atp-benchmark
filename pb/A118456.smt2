;; sequence(s): A118456
;; terms: 2 4 12 60 300 2100 14700 161700 1778700 19565700 215222700 2797895100 36372636300 618334817100 10511691890700 178698762141900 3037878956412300 57719700171833700 1096674303264840300 25223508975091326900
;; small program: loop((loop(loop((((loop(x * y, x, x) mod (1 + x)) + 1) mod 2) + x, 1 + x, x), 1, y) * x) + x, x, 2)
;; fast program: (loop(loop((loop(((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) - 1) + x, 1 + (2 + 2), x) + 1) * (if x <= 0 then 1 else 2), 1, y) * x, x, 1) div loop(x + x, x, 1)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (mod (+ (mod (v3 x) (+ 1 x)) 1) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (v4 x) (v9 x)) 2))))
