;; sequence(s): A18419
;; terms: 1 2 3 4 6 12 31 62 93 124 186 372
;; small program: compr(loop((2 - (x * x)) * (2 + x), 2, 2) mod (1 + x), x) + 1
;; fast program: compr(loop((2 + x) * (((x * x) * x) - 2), 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (+ 2 x) (- (* (* x x) x) 2)))))
(assert (= g3 1))
(assert (= h3 (+ 2 2)))
