;; sequence(s): A306556
;; terms: 0 1 2 3 6 7 8 9 18 19 20 21 24 25 26 27 54 55 56 57
;; small program: (loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x div 2), y), x, 0) + 1) div 2
;; fast program: ((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, (x div 2) div 2) * 2) + x) - 2
(set-logic UFNIA)
...
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (div (div x 2) 2))))
