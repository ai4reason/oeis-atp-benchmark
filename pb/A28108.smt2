;; sequence(s): A28108
;; terms: 1 36 835 15870 269041 4235856 63339535 911961090 12757140781 174463937076 2343013658635 31002931521510 405218193255721 5241917197644696 67218151176880135 855512978401565130
;; small program: loop(((loop(loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (* (div (v10 x) (+ 1 2)) (w12 x)))))
(assert (= g9 1))
