;; sequence(s): A18622
;; terms: 1 2 7 14 53 106 371 742
;; small program: loop(compr(loop((x * x) + (x div 2), 1 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + loop((2 + ((x * x) + x)) * (2 + x), 1, 2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 2 (+ (* x x) x)) (+ 2 x)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
