;; sequence(s): A18367
;; terms: 1 3 7 13 21 39 91 273
;; small program: loop(compr(loop(1 + ((x div y) * x), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((1 + loop((x * x) + x, 1, 2 * (2 * (2 + 2)))) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ 1 (* (div x y) x)))))
(assert (= g2 (+ 2 2)))
(assert (= h2 2))
