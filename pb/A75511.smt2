;; sequence(s): A75511
;; terms: 1 42 1064 21168 365232 5743584 84713728 1193127936 16239711488 215394955776 2800564795392 35851775791104 453374980255744 5677724481773568 70550796621971456 871159544637161472
;; small program: loop(loop(((loop((loop2(((2 * (y * y)) - y) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, y, 1) + (((x + x) + x) * 2), x, 1) * loop(x + x, x, 1)
;; fast program: loop(loop((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2)) + x, y, 1) + (((x + x) + x) * 2), x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (v5 x) (v11 x)))))
