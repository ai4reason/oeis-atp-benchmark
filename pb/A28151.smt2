;; sequence(s): A28151
;; terms: 1 32 657 11056 166481 2339904 31427089 409158992 5211577041 65347755616 810110258321 9959365988208 121687051820881 1480042546391168 17940274040148753 216912236044745104 2617684898381621201 31545404154807718560 379747317765849488785
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * (x + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x - 1, 2)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (v3 x y) (* 2 (+ x x))))))
...
(assert (forall ((x Int)) (= (f8 x) (* (div (w9 x) 2) (v10 x)))))
(assert (= g8 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) x) (* (* (+ x x) 2) 2)))))
