;; sequence(s): A18634
;; terms: 1 3 5 9 15 17 45 51 85 153 255 765
;; small program: loop(compr((1 + loop((x - 2) + x, 2 * (2 + 2), x)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(((1 + 2) * (loop(x * x, 2, 2 + 2) - 1)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* (+ 1 2) (- v5 1)) (+ 1 x)))))
