;; sequence(s): A75518
;; terms: 0 0 1 1 2 3 4 4 5 7 7 9 10 10 11 13 14 15 16 17
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) div 2) div 2
;; fast program: ((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop(1 + ((2 + x) * x), 2 + 2, 2)) div loop(1 + x, 2, 2)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f6 x) (+ 1 x))))
(assert (= g6 2))
(assert (= h6 2))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (fast x) (div (mod (+ (v2 x) 1) v5) v6))))
