;; sequence(s): A36557
;; terms: 1 1 2 3 6 10 20 35 70 126 252 463 926 1730 3460 6555 13110 25126 50252 97223
;; small program: ((loop(x + x, x, 1) + loop2(x * y, y, x div 2, 2 + (x mod 2), 1 + 2)) div (2 + (2 + 2))) + 1
;; fast program: (((loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) - 1) + loop2(x * y, y, x div 2, 2 + (x mod 2), 1 + 2)) div (2 + (2 + 2))) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (+ (v0 x) (w1 x)) (+ 2 (+ 2 2))) 1))))
...
(assert (forall ((x Int)) (= (i5 x) (+ 2 (mod x 2)))))
(assert (= j5 (+ 1 2)))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (- (v2 x) 1) (w5 x)) (+ 2 (+ 2 2))) 1))))
