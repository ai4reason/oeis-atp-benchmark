;; sequence(s): A66438
;; terms: 0 1 1 1 2 1 0 1 1 9 7 1 7 7 13 1 7 1 7 1
;; small program: loop((2 * ((x + x) + x)) + x, 1 + x, 1) mod (1 + x)
;; fast program: loop(loop2(x * y, y, x div 2, loop(1 + (2 + (2 + 2)), x mod 2, 1), (2 * (2 * (2 * (2 + (2 + 2))))) + 1) mod x, 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (w2 x) x))))
(assert (= g1 1))
