;; sequence(s): A26140
;; terms: 0 1 3 4 7 9 10 12 13 16 19 21 22 25 27 28 30 31 34 36
;; small program: loop(compr((loop(x div (if (x mod (1 + 2)) <= 0 then (1 + 2) else 1), x, x) mod (1 + 2)) - 1, y), x, 0)
;; fast program: compr((loop(if (x mod (1 + 2)) <= 0 then (x div (1 + 2)) else x, loop(y - x, x, 0), x) mod (1 + 2)) div 2, x) + 0
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (v3 x) 0))))
