;; sequence(s): A110299
;; terms: 2 7 19 45 101 215 447 913 1849 3727 7485 15007 30055 60153 120353 240759 481577 963215 1926497 3853065
;; small program: loop(((compr((1 + loop(x + x, x, x)) mod (1 + x), y) + 1) + x) + x, x, 2)
;; fast program: ((loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x) + x, x, 1) * 2) - 1) + loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (* (v3 x) 2) 1) (v8 x)))))
