;; sequence(s): A31368
;; terms: 2 5 11 17 23 31 41 47 59 67 73 83 97 103 109 127 137 149 157 167
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), compr(loop(1 - x, x, 1), x)) div 1) + 1
;; fast program: compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + (x + x)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g4 x) (+ 1 (+ x x)))))
