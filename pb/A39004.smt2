;; sequence(s): A39004
;; terms: 0 3 6 9 12 15 18 24 27 30 33 36 39 45 48 51 54 57 60 63
;; small program: compr((((loop2(x + y, y div 2, x, 0, x) mod 2) + (x mod (1 + 2))) * 2) - 1, x) div 1
;; fast program: compr(((((((((loop2(x + y, y div 2, 1 + (2 * (2 + 2)), 0, x) mod 2) * loop(1, x + x, x)) mod 2) - 1) mod 2) * (1 + x)) mod (2 + 1)) + 1) mod 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (* (+ (mod (w1 x) 2) (mod x (+ 1 2))) 2) 1))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (+ (mod (* (mod (- (mod (* (mod (w3 x) 2) (v4 x)) 2) 1) 2) (+ 1 x)) (+ 2 1)) 1) 2))))
