;; sequence(s): A28104
;; terms: 1 34 747 13496 218309 3291918 47316319 657096292 8894300217 118046931002 1542749735891 19914792822288 254507220618925 3225793030038886 40605323440744263 508174054158717884
;; small program: loop(((loop((loop2(loop((2 + y) * x, 2, x) + ((y * y) * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (v3 x) (* (* y y) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (w2 x y) x) (v4 x)))))
...
(assert (forall ((x Int)) (= (f8 x) (* (div (v9 x) (+ 1 2)) (w11 x)))))
(assert (= g8 1))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (v8 x y) (* 2 (* 2 (+ x x)))))))
