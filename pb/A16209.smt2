;; sequence(s): A16209
;; terms: 1 9 58 330 1771 9219 47188 239220 1205941 6059229 30384718 152189310 761743711 3811110039 19062724648 95335146600 476740303081 2383895225649 11920057258978 59602029687090
;; small program: loop2(1 + ((2 * (2 * (2 * ((x - y) - y)))) + y), x, x, 1, 0)
;; fast program: (loop(((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) div 2) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2, 1, 2 + x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ 1 (+ (* 2 (* 2 (* 2 (- (- x y) y)))) y)))))
...
(assert (forall ((x Int)) (= (f1 x) (div (- (div (w2 x) 2) (w5 x)) 2))))
(assert (= g1 1))
(assert (forall ((x Int)) (= (h1 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u1 x y) (ite (<= x 0) y (f1 (u1 (- x 1) y))))))
(assert (forall ((x Int)) (= (v1 x) (u1 g1 (h1 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (div (v1 x) 2) 1))))
