;; sequence(s): A30005
;; terms: 0 1 3 10 15 28 36 55 91 105 153 190 210 253 325 406 435 528 595 630
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 0) * x, y, 2, 1, x) div 2) div 2) div 2
;; fast program: ((loop2(loop((compr(((1 + (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) mod (1 + (2 + (x + x)))) - 2, x) * 2) + 1, 1, y) * x, y, 2, 1, x) div 2) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 0) x))))
...
(assert (forall ((x Int)) (= (f5 x) (- (mod (+ 1 (* 2 (v6 x))) (+ 1 (+ 2 (+ x x)))) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (div (w3 x) 2) 2) 2))))
