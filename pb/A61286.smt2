;; sequence(s): A61286
;; terms: 2 4 16 64 1024 4096 65536 262144 4194304 268435456 1073741824 68719476736 1099511627776 4398046511104 70368744177664 4503599627370496 288230376151711744 1152921504606846976
;; small program: (loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) div 2
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), x, 2, 0, x) div 2) div 2
(set-logic UFNIA)
