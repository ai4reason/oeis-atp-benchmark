;; sequence(s): A278617
;; terms: 0 0 1 2 3 3 4 5 5 6 7 7 8 8 8 9 10 10 10 11
;; small program: loop2((loop(x * y, y - 1, 1) mod (1 + y)) + x, 2 + y, x, 1, 0) - 1
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then y else 0, 1, y) + x, x - 1, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (ite (<= (- (mod (+ 1 (v4 x)) (+ 1 (+ x x))) 2) 0) y 0))))
(assert (= g3 1))
