;; sequence(s): A153051
;; terms: 9 13 16 19 21 22 25 26 28 31 33 34 36 37 40 41 43 46 47 49
;; small program: ((((loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y), x, 0) + 1) + 2) + 2) + 2) + 2
;; fast program: ((compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) + 1) + (2 * (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (v2 x) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (v3 x) 2) 1) (* 2 (+ 2 2))))))
