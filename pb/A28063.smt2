;; sequence(s): A28063
;; terms: 1 27 478 7086 95923 1234761 15440896 189849132 2311064965 27967760535 337282318594 4059193972938 48793580278327 586108780927749 7037421345862372 84477944245876104 1013937929024363209
;; small program: loop(((loop((loop((loop(loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop((loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (x + x))) + x, y, 1) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (* (div (v8 x) (+ 1 2)) (w10 x)))))
(assert (= g7 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) (* (+ (+ x x) x) 2)) x))))
