;; sequence(s): A182174
;; terms: 3 7 22 45 116 163 282 353 520 831 950 1357 1668 1835 2194 2793 3464 3703 4470 5021
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 2, y, 2, 1, x) - 2) - x
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 2, y, 2, 1, x) - 2) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (w3 x) 2) x))))
