;; sequence(s): A102770
;; terms: 7 17 38 71 110 161 218 333 449 573 758 881 1010 1245 1563 1799 2043 2378 2591 2883
;; small program: loop2((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x, 1 + y, 2, 1, x) div 2
;; fast program: (loop2(loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x, 1 + y, 2, 1, x) div 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (w3 x) 1) 2))))
