;; sequence(s): A346300
;; terms: 1 3 7 8 9 11 15 16 17 19 23 31 32 33 34 35 36 37 39 40
;; small program: compr(((loop2((((y mod 2) * (1 + 2)) * x) + x, y div 2, x, 1, x) - 1) - x) - x, 1 + x) - 1
;; fast program: compr(((loop2((((y mod 2) * (1 + 2)) * x) + x, y div 2, x, 1, x) - x) - x) - 2, 2 + x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (- (- (w1 x) 1) x) x))))
