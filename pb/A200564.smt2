;; sequence(s): A200564
;; terms: 1 2 12 288 33280 16793600 34360786944 281475110928384 9223372071214514176 1208925819632221360750592 633825300114132715146861084672 1329227995784915909797295207699447808 11150372599265311570918974863776009399828480
;; small program: (loop(loop(x + x, y, x), x, 1) + loop(x * 2, x * x, 1)) div 2
;; fast program: (((1 + loop2(x * y, y + y, x - 1, 1, 2)) * loop(x + x, x, 1)) div 2) * loop2(x * y, y + y, x - 1, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (v0 x) (v2 x)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (* (div (* (+ 1 (w3 x)) (v4 x)) 2) (w5 x)))))
