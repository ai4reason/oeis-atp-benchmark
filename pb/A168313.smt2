;; sequence(s): A168313
;; terms: 1 0 1 0 2 1 0 0 2 1 0 0 2 2 1 0 0 0 2 2
;; small program: loop((loop(1 + (y div ((2 + x) - (x mod 2))), x + x, 1) mod 2) + x, 2, x) - x
;; fast program: loop((compr(((loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 2 + (2 + 2), x div 2) + x, 2, x) + 1) mod (2 + 2)) mod 2, x) mod 2) + x, 2, x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ 1 (div y (- (+ 2 x) (mod x 2)))))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (div (v6 x y) x))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (mod (+ (v4 x) 1) (+ 2 2)) 2))))
