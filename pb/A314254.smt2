...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (mod (ite (<= (mod (div y 2) (+ 2 (+ 2 2))) 0) 1 2) x) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (div (+ (+ (* (+ (+ (div (+ (+ (+ (div (v1 x) 2) (ite (<= x 0) 1 x)) x) x) (ite (<= x 0) 1 2)) x) x) 2) 1) x) (+ 1 2)) x) x) x))))
