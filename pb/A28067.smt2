;; sequence(s): A28067
;; terms: 1 28 513 7844 109073 1435884 18284641 228039988 2806440945 34241833340 415442661569 5021759035332 60553294750417 728980076982796 8766500495230497 105347998786683476 1265376309596698289
;; small program: loop(((loop((loop2(loop((2 + y) * x, 2, x) + ((y * y) * y), y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop((loop(x * x, 1, loop(x + x, x, 2)) div (1 + 2)) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
