;; sequence(s): A109892
;; terms: 2 6 84 20475 234531275 199200973555045 16481425431663122588749 173392935733620216899469862542865 300717095810709134168380432250652303057474577
;; small program: loop(loop2((x * y) + x, 1 + y, x, 1, loop(x * y, x, 1)) div loop(x * y, x, 1), 1, 1 + x)
;; fast program: loop(loop2(x * y, 1 + y, x, 1, 1 + loop(x * y, x - 1, x)) div loop(x * y, x, 1), 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div (w1 x) (v3 x)))))
(assert (= g0 1))
...
(assert (forall ((x Int)) (= (j5 x) (+ 1 (v6 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (div (w5 x) (v7 x)))))
(assert (= g4 1))
