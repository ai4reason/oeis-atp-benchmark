;; sequence(s): A64546
;; terms: 0 0 9 64 1900 5184 695310 15089664 346348629 8189440000 170553548176 4519967588352 202522159449675 8500218417954816 265594041953296875 3543051899458551808 454740628232312103872 5853583711033675481088
;; small program: loop(loop((y * y) * x, x, 1) mod loop2(x * y, y, x, 1, x), 1, 1 + x)
;; fast program: ((loop(x * x, 1, loop(x * y, x, 1)) mod loop2(x * y, y, x - 1, 1, 1 + x)) * (1 + x)) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (v1 x) (w2 x)))))
(assert (= g0 1))
...
(assert (forall ((x Int)) (= (fast x) (* (* (mod (v3 x) (w5 x)) (+ 1 x)) (+ 1 x)))))
