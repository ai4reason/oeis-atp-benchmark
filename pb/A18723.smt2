;; sequence(s): A18723
;; terms: 1 2 3 4 6 7 11 12 14 21 22 28 33 42 44 66 77 84 132 154
;; small program: loop(compr(loop(((x + x) + y) * (2 + x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop(2 * ((1 + 2) * (2 - x)), 2 + 2, 1) mod (1 + x), x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (+ x x) y) (+ 2 x)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (* 2 (* (+ 1 2) (- 2 x))))))
(assert (= g4 (+ 2 2)))
(assert (= h4 1))
