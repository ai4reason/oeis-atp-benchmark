;; sequence(s): A153053
;; terms: 1 4 7 9 10 13 14 16 19 21 22 24 25 28 29 31 34 35 37 39
;; small program: loop(compr(loop(x * y, x, x) mod (1 + (2 * (2 + x))), y), x, 2) - 1
;; fast program: (compr(((loop(((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) + x) mod 2, x mod 2, 1 + x) + x) mod 2) mod (1 + x)) - (x mod 2)) + 1, 2, x) + 1) + x) mod 2, x) div 2) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (v2 x) (+ 1 (* 2 (+ 2 x)))))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 0) x) 2))))
