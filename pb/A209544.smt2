;; sequence(s): A209544
;; terms: 3 17 41 73 89 97 113 137 193 233 241 257 281 313 337 353 401 409 433 449
;; small program: loop2(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod ((1 + x) + x), (1 + x) mod 2, x) - 2, 1 + y) + x, y, 2, 1, x)
;; fast program: (compr(loop(((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else x) mod 2) mod (1 + 2)) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (mod (- 1 (v3 x)) (+ (+ 1 x) x)))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (- (mod (mod (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 (+ x x))) 2) 0) 1 x) 2) (+ 1 2)) 1) (+ 1 x)))))
