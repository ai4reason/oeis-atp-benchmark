;; sequence(s): A3529
;; terms: 1 3 5 11 15 25 31 33 41 55 75 93 123 155 165 205 275 341 451 465
;; small program: loop(compr((1 + loop(x + x, loop(2 * (2 + x), 2, 2), x)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((loop((((x * x) * x) * x) * x, 1, 2 * (2 * (2 + 2))) - 1) mod (1 + x), x) + 1) div (1 mod 2)
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (- v5 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v4 x) 1) (mod 1 2)))))
