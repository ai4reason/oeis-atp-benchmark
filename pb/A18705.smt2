;; sequence(s): A18705
;; terms: 1 3 9 11 27 33 81 99 297 891
;; small program: loop(compr(loop((2 - x) * (x * x), 2, 1 + 2) mod (1 + x), y), x, 0) + 1
;; fast program: ((compr(((1 + (2 * (2 * (2 * (2 + 2))))) * loop((x * x) * x, 1, 1 + 2)) mod (1 + x), x) * 2) + 2) div 2
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 (* 2 (* 2 (* 2 (+ 2 2))))) v4) (+ 1 x)))))
