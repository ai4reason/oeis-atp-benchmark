;; sequence(s): A18625
;; terms: 1 2 4 11 17 22 34 44 68 187 374 748
;; small program: loop(compr((loop(loop(x + y, x, y * x) * x, 2, 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((1 + loop((2 + (x * x)) * x, 1, 1 + (2 * (2 + 2)))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h3 x y) (* y x))))
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (mod (+ 1 v6) (+ 1 x)))))
