;; sequence(s): A97250
;; terms: 1 4 12 60 420 4620 60060 1021020 19399380 446185740 12939386460 401120980260 14841476269620 608500527054420 26165522663340060 1229779565176982820 65178316954380089460 3845520700308425278140 234576762718813941966540 15716643102160534111758180
;; small program: loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) * x) + x, x, if x <= 0 then 1 else 2)
;; fast program: loop(((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, x - 1, if x <= 0 then 1 else (2 + 2))
(set-logic UFNIA)
