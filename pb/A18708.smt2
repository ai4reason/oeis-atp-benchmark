;; sequence(s): A18708
;; terms: 1 2 4 7 8 14 16 28 32 56 64 112 128 224 448 896
;; small program: loop(compr(loop(2 * (((x * x) - x) * x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (((compr(loop((((if y <= 0 then 0 else 2) + x) + y) * x, 2, 2 + 2) mod (1 + x), x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ (+ (ite (<= y 0) 0 2) x) y) x))))
(assert (= g4 2))
(assert (= h4 (+ 2 2)))
