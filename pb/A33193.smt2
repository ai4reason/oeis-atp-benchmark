;; sequence(s): A33193
;; terms: 1 2 6 19 62 207 704 2430 8486 29903 106098 378391 1354700 4863834 17499302 63055947 227465414 821215295 2966571096 10721076118
;; small program: ((if x <= 0 then 1 else loop2((2 * (x + x)) - y, x + y, x - 1, 2, 1)) + loop2(x + y, x, x + x, 1, 0)) div 2
;; fast program: (loop2((2 * (x + x)) - y, x + y, x - 1, if x <= 0 then 1 else 2, 1) + loop2(x + y, x, (x + x) - 1, 1, 1)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (ite (<= x 0) 1 (w0 x)) (w1 x)) 2))))
...
(assert (forall ((x Int)) (= (h3 x) (- (+ x x) 1))))
(assert (= i3 1))
(assert (= j3 1))
