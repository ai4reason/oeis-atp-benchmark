;; sequence(s): A110833
;; terms: 9 16 36 64 144 196 324 400 576 900 1024 1444 1764 1936 2304 2916 3600 3844 4624 5184
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 2) * x, y, 2, 1, x)
;; fast program: loop2((compr(loop(((loop(x * x, 1, loop(x + x, x div 2, 1)) * (1 + (x mod 2))) - 1) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + x) * x, x, 2, 1, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (- (* (v6 x) (+ 1 (mod x 2))) 1) (+ 1 x)))))
