;; sequence(s): A18608
;; terms: 1 2 4 179 358 716
;; small program: loop(loop(loop(loop(x + y, x, y), x, 1), 1 - (x mod (2 + 2)), x) + x, x, 1)
;; fast program: loop(loop(((1 + (2 + (x + x))) * (x * x)) - 1, 1 - (x mod (2 + 2)), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (* (+ 1 (+ 2 (+ x x))) (* x x)) 1))))
