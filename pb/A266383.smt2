;; sequence(s): A266383
;; terms: 1 4 6 12 14 20 24 36 38 44 48 60 64 76 84 108 110 116 120 132
;; small program: loop(loop(loop2(((y mod 2) * x) + x, y div 2, x, 2 + (x mod 2), x) div 2, 1, y) + x, x, 1)
;; fast program: loop(loop(loop2(((y mod 2) * x) + x, y div 2, 2 + (2 + 2), 1, x div 2) * loop(1 + 2, x mod 2, 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (i2 x) (+ 2 (mod x 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (* (mod y 2) x) x))))
