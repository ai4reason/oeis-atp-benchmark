;; sequence(s): A68534
;; terms: 1 1201 3461763601 57522274700204200328401 31764596032183221357775361004244057832177481618001 19372599572885568957165529710172832947035894754619777993470030014145668169574231766337989087612412837201
;; small program: loop(loop(loop(1 + loop2(y * (x * y), y, x, 1, 1 + (2 + (2 + 2))), 1, loop(x + x, x, 1)) div 2, 1, y) * x, x, 1)
;; fast program: loop2(loop(((((1 + 2) * loop(x * x, 1, loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))))) div (1 + 2)) + 1) div 2, 1, y) * x, y + y, x, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ 1 (w3 x)))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f6 x) (div (+ (div (* (+ 1 2) (v7 x)) (+ 1 2)) 1) 2))))
(assert (= g6 1))
