;; sequence(s): A158499
;; terms: 1 1 1 0 ~5 ~24 ~90 ~312 ~1053 ~3536 ~11934 ~40664 ~140114 ~488240 ~1719380 ~6113200 ~21921245 ~79200160 ~288045110 ~1053728920
;; small program: loop((loop((loop((x - 2) + x, 2, y) * x) div y, y, 1) + x) + x, x, 2) div 2
;; fast program: loop((loop(loop(2 * ((x - (x div y)) + x), x, 1) div ((1 - x) - x), 1, y) + x) + x, x, 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (v5 x) (- (- 1 x) x)))))
(assert (= g4 1))
