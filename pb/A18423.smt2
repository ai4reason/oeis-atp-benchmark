;; sequence(s): A18423
;; terms: 1 2 3 6 7 9 14 18 21 27 42 54 63 126 189 378
;; small program: compr(loop(((2 + x) * x) + x, 2, 1 + 2) mod (1 + x), x) + 1
;; fast program: compr(loop((2 + x) * (((x * x) * x) - 1), 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (+ 2 x) (- (* (* x x) x) 1)))))
(assert (= g3 1))
(assert (= h3 (+ 2 2)))
