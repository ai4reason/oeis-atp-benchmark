;; sequence(s): A175078
;; terms: 0 0 1 1 1 1 1 1 1 2 2 1 1 1 1 2 2 1 1 1
;; small program: (((loop((loop(((loop(x * y, x, x) mod (1 + x)) + 1) mod 2, 1, y) * x) + 2, x - 1, 1) - (if x <= 0 then 1 else 2)) + x) + x) div (2 + x)
;; fast program: loop(loop((loop(if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2, 1, x + x) + 1) mod 2, 1, y) + 1, (x - 1) div 2, 1) mod loop2(x + y, x, x - 1, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (v2 x) (+ 1 x)) 1) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (small x) (div (+ (+ (- (v0 x) (ite (<= x 0) 1 2)) x) x) (+ 2 x)))))
(assert (forall ((x Int)) (= (f6 x) (+ x x))))
(assert (forall ((x Int)) (= (g6 x) (div x 2))))
(assert (= h6 1))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 (g6 x) h6))))
(assert (forall ((x Int)) (= (f5 x) (ite (<= (- (mod (+ 1 (v6 x)) (+ 1 x)) 2) 0) 1 2))))
(assert (= g5 1))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (v4 x y) 1))))
(assert (forall ((x Int)) (= (g3 x) (div (- x 1) 2))))
(assert (= h3 1))
...
(assert (forall ((x Int)) (= (fast x) (mod (v3 x) (w7 x)))))
