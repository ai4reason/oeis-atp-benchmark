;; sequence(s): A112051
;; terms: 1 3 11 24 60 84 144 180 264 420 480 684 840 924 1104 1404 1740 1860 2244 2520
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) - loop(1, x - 2, 2)) div 2
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) + 1) * x, y, 2, 1, x) - loop(1, x - 2, 2)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (- (w0 x) (v3 x)) 2))))
