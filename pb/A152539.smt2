;; sequence(s): A152539
;; terms: 6 18 36 58 82 112 144 180 222 268 316 368 422 482 544 610 682 756 834 916
;; small program: ((loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y) + x, 1 + x, 0) - x) * 2) - 2
;; fast program: ((loop(compr((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1 + y) + x, x, 1) + 2) - x) * 2
(set-logic UFNIA)
