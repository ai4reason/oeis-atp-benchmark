;; sequence(s): A18689
;; terms: 1 3 7 21 41 123 287 861
;; small program: loop(compr(loop((2 * (x * x)) - x, 2, (x + x) + x) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((((2 * (2 * loop((x * x) * x, 1, 2 + (2 + 2)))) - 1) - 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (- (- (* 2 (* 2 v5)) 1) 2) (+ 1 x)))))
