;; sequence(s): A87611
;; terms: 0 0 1 2 0 0 2 2 4 8 8 0 1 0 1 4 7 6 9 10
;; small program: compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (1 + x)
;; fast program: ((((compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1) mod (1 + x), (1 + x) mod 2, x), x) div 2) * 2) - 1) - x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (- (- (* (div (v2 x) 2) 2) 1) x) (+ 1 x)))))
