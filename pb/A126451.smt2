;; sequence(s): A126451
;; terms: 1 2 10 165 5985 376992 36288252 4935847320 899749479915 211531737340440 62292206224983306 22453501436688294427 9723205992282927449305 4980663327690172963041600 2978877731799385928100461400 2057145404864429538334152506640
;; small program: loop2(x * y, y - 1, x, 1, loop(loop(x + y, y, x), x, 1)) div loop(x * y, x, 1)
;; fast program: loop(loop2(x * y, y - 1, x, 1, (((1 + x) * ((2 + x) * x)) div (2 + (2 + 2))) + y) div loop(x * y, x, 1), 1, x)
(set-logic UFNIA)
...
(assert (= i5 1))
(assert (forall ((x Int) (y Int)) (= (j5 x y) (+ (div (* (+ 1 x) (* (+ 2 x) x)) (+ 2 (+ 2 2))) y))))
...
(assert (forall ((x Int) (y Int)) (= (w5 x y) (u5 (h5 x) i5 (j5 x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (div (w5 x y) (v6 x)))))
(assert (= g4 1))
