;; sequence(s): A104248
;; terms: 2 1 2 1 2 1 1 2 2 1 2 1 2 2 1 1 2 1 2 1
;; small program: loop(loop(loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), x, 1 + x) + x, x, 1) mod 2, 1, x + x) + 1
;; fast program: (((((loop((((loop(loop2(x + y, y div 2, x, 0, x) mod 2, 1, (x div 2) + x) + x) mod 2) + 1) + x, 2, x) - x) mod (1 + 2)) div 2) + x) mod 2) + 1
(set-logic UFNIA)
...
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (+ (div x 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (mod (+ (div (mod (- (v3 x) x) (+ 1 2)) 2) x) 2) 1))))
