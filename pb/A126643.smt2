;; sequence(s): A126643
;; terms: 6 34 180 886 4116 18334 79260 335446 1398276 5765134 23581740 95900806 388345236 1567647934 6313474620 25382710966 101917608996 408831386734 1638809709900 6565693949926
;; small program: (((loop((loop2((((y * y) + x) + x) + x, y + y, y, 1, 2) + x) + x, x, 1) + loop(x + x, x, 1)) * (1 + 2)) - 1) + loop(x + x, x, 1)
;; fast program: ((loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, x, 1) + loop(x + x, x, 1)) + loop(1 + (2 * (x + x)), x, 1)) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (+ (+ (* y y) x) x) x))))
...
(assert (forall ((x Int)) (= (small x) (+ (- (* (+ (v0 x) (v2 x)) (+ 1 2)) 1) (v3 x)))))
(assert (forall ((x Int)) (= (f6 x) (+ (* x x) x))))
(assert (= g6 1))
(assert (forall ((x Int)) (= (f7 x) (+ 2 (+ x x)))))
...
(assert (forall ((x Int)) (= (f5 x) (div (div (v6 x) 2) (+ 1 2)))))
(assert (= g5 1))
