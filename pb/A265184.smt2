;; sequence(s): A265184
;; terms: 1 ~1 5 ~25 185 ~2125 27905 ~482605 9217085 ~213875785 6255817445 ~194304672685 7226433462125 ~297023830065085 12785737501604945 ~602104045086886465 31987054432103158265 ~1890773295722109480805 115397608063684861502465
;; small program: loop(loop((compr((1 + loop(x * y, x, 1)) mod (1 + x), y) + 1) * (0 - x), y, 1) + x, x, 1)
;; fast program: loop(loop((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), (1 + x) mod 2, x) - 2, y) + 1) * x, y, 1) - x, x, 1) * loop(0 - x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ (v2 x y) 1) (- 0 x)))))
