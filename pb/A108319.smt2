;; sequence(s): A108319
;; terms: 1 2 3 4 6 7 8 9 12 14 16 18 21 24 27 28 32 36 42 48
;; small program: loop(compr(loop((x * x) * x, 2, loop((x * x) + x, 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: compr(loop(2 * (loop((x * x) + x, 2, 2) * x), loop(1 + (x + x), 2, 2) - 1, 1) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v6 (u6 g6 h6)))
(assert (forall ((x Int)) (= (f5 x) (* 2 (* v6 x)))))
