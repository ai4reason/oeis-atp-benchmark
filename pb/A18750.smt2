;; sequence(s): A18750
;; terms: 1 3 17 19 51 57 323 969
;; small program: loop(compr((loop((2 * (2 + x)) + x, 1 + (2 + 2), 2) + x) mod (1 + x), y), x, 0) + 1
;; fast program: (loop(compr((loop(1 + (2 * (2 + ((2 + x) * x))), 2, 2) - 2) mod (1 + x), y), x, 0) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ 1 (* 2 (+ 2 (* (+ 2 x) x)))))))
(assert (= g5 2))
(assert (= h5 2))
