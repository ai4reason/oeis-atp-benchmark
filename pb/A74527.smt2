;; sequence(s): A74527
;; terms: 3 10 38 160 722 3400 16418 80440 397442 1973320 9825698 49007320 244676162 1222305640 6108314978 30531959800 152631002882 763068724360 3815084948258 19074649113880
;; small program: loop2(((x + x) + x) + y, (2 * (y + y)) + y, x, 2, 2) + loop(x + x, x, 1)
;; fast program: (loop(x + x, x, 1) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) + loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (v2 x) (w3 x)) (w5 x)))))
