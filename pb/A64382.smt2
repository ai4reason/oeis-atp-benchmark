;; sequence(s): A64382
;; terms: 1 6 240 100800 605404800 65703372134400 155590841484029952000 9416463528385701835407360000 16689045681854870055279680279347200000 976264092939656812770160178186221444104192000000 2097176933095579995533106263090939623313258344938995712000000
;; small program: loop(loop((loop2(x * y, 1 + y, x, 1, loop(x + y, x, 1)) div loop(x * y, x, 1)) * (2 + x), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(x * y, 1 + y, x, 1, 1 + (((x * x) + x) div 2)) div loop(x * y, x, 1), 1, y) * x, x, 1) * loop((2 + y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (div (w2 x) (v4 x)) (+ 2 x)))))
(assert (= g1 1))
...
(assert (= i7 1))
(assert (forall ((x Int)) (= (j7 x) (+ 1 (div (+ (* x x) x) 2)))))
...
(assert (forall ((x Int)) (= (f6 x) (div (w7 x) (v8 x)))))
(assert (= g6 1))
