;; sequence(s): A139292
;; terms: 1 4 64 1024 262144 4194304 1073741824 17179869184 4398046511104 18014398509481984 288230376151711744 1180591620717411303424 302231454903657293676544 4835703278458516698824704
;; small program: ((((loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop(x + x, x, 1), x, 2, 1, x) div 2) div 2) div 2) div 2) div (1 + 2)
;; fast program: (((((loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop(x + x, x, 1), x, 2, 1, x) div (1 + 2)) div 2) + 1) div 2) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (div (div (div (div (w0 x) 2) 2) 2) 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (div (+ (div (div (w4 x) (+ 1 2)) 2) 1) 2) 2) 2))))
