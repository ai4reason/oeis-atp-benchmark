;; sequence(s): A60620
;; terms: 2 2 3 4 5 6 8 9 11 12 14 16 18 20 21 23 25 27 29 31
;; small program: ((loop(compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) div (1 + x)
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (* (v0 x) 2) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (v3 x) 2) x) (+ 1 x)))))
