;; sequence(s): A158582
;; terms: 4 8 9 10 12 16 17 18 19 20 21 22 24 25 26 28 32 33 34 35
;; small program: compr(if loop(loop((x - 1) mod y, x, 0), 2, x + x) <= 0 then 1 else 0, x) - 1
;; fast program: compr(((((((((((loop(loop(2 + (x mod y), x, 1) - 1, (1 + (2 + 2)) - (x mod 2), x) * 2) + 1) * 2) - 1) * (if x <= 0 then 1 else 2)) div 2) + 1) - (x mod 2)) - x) + 1) - x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g4 x) (- (+ 1 (+ 2 2)) (mod x 2)))))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (- (+ (- (- (+ (div (* (- (* (+ (* (v4 x) 2) 1) 2) 1) (ite (<= x 0) 1 2)) 2) 1) (mod x 2)) x) 1) x))))
