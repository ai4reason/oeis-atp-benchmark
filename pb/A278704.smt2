;; sequence(s): A278704
;; terms: 1 32 1280 81920 9175040 1879048192 721554505728 527765581332480 743093938516131840 2029141848108050677760 10804774512805748248936448 112652543574969605015820304384 2307124092415377510723999833784320 93045959704944111103266494219624120320
;; small program: loop(loop(((x div y) + x) * 2, 2 + y, x) div y, x, 1)
;; fast program: (((loop((x * x) * x, 1, 2 + x) - x) div (2 + (2 + 2))) * loop((x * x) * x, 1, loop(x + x, x, 1))) * loop2(x * y, y + y, x, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ (div x y) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (div (- (v2 x) x) (+ 2 (+ 2 2))) (v3 x)) (w5 x)))))
