;; sequence(s): A57703
;; terms: 0 1 3 7 15 31 62 119 218 381 637 1023 1585 2379 3472 4943 6884 9401 12615 16663
;; small program: loop(loop(loop(loop(x + y, y - 2, x), y, x) + y, y - 1, x), x, x)
;; fast program: loop(loop((loop((x * x) + x, 1, (((x * x) - x) div 2) - x) div (2 + (2 + 2))) + (((x * x) - x) div 2), 1, y) + x, x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (div (v6 x) (+ 2 (+ 2 2))) (div (- (* x x) x) 2)))))
(assert (= g5 1))
