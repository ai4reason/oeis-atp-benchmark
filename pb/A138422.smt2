;; sequence(s): A138422
;; terms: 1 6 50 196 1210 2366 6936 10830 23276 58870 76880 156066 235340 284746 406456 657306 1009490 1153510 1678886 2117220
;; small program: (((loop2((loop(loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) div (1 + 2)) div 2) + 1) div 2
;; fast program: (((loop2((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) div (1 + 2)) div 2) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (div (div (w0 x) (+ 1 2)) 2) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (div (w5 x) (+ 1 2)) 2) 1) 2))))
