;; sequence(s): A80765
;; terms: 5 9 11 13 14 17 19 20 21 23 25 27 29 32 33 34 35 37 38 39
;; small program: loop(compr((if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then (1 + x) else 1) - 1, y), 1 + x, 0)
;; fast program: compr(loop(((if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then 1 else 0) * (1 + y)) + x, 2, x) - x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (ite (<= (- (v2 x) 1) 0) (+ 1 x) 1) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (* (ite (<= (- (v6 x) 1) 0) 1 0) (+ 1 y)) x))))
(assert (= g5 2))
