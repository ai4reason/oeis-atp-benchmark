;; sequence(s): A84879
;; terms: 1 3 18 189 2106 22113 220158 2114829 19853586 183662073 1683014598 15327998469 139038783066 1257874611633 11360039237838 102475402586109 923689049088546 8321664384098793 74945758272961878 674816500839877749
;; small program: (loop2((((x - 1) + x) + x) + y, y + y, x, 0, 1) + 1) * loop((x + x) + x, x, 1)
;; fast program: (loop(((2 + x) * x) + x, 1, loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2) - loop2(x * y, y, x, 1, 2 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (+ (- x 1) x) x) y))))
...
(assert (forall ((x Int)) (= (small x) (* (+ (w0 x) 1) (v1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (div (v2 x) 2) (w5 x)))))
