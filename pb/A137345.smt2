;; sequence(s): A137345
;; terms: 0 1 2 2 3 0 4 4 5 0 6 7 7 0 9 8 9 0 10 0
;; small program: loop(loop2(x * y, y - 1, x, 1, loop(x + y, x, 0)) div loop(x * y, x, 1), 1, 1 + x) mod (1 + x)
;; fast program: (((loop2(x * y, 1 + y, x, 1, 1 + (((x * x) + x) div 2)) div loop(x * y, x, 1)) * (2 + x)) div 2) mod (1 + x)
(set-logic UFNIA)
...
(assert (= i4 1))
(assert (forall ((x Int)) (= (j4 x) (+ 1 (div (+ (* x x) x) 2)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (div (* (div (w4 x) (v5 x)) (+ 2 x)) 2) (+ 1 x)))))
