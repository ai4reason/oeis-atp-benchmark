;; sequence(s): A186293
;; terms: 8 20 36 44 48 56 68 96 116 120 128 140 156 168 176 200 204 216 224 228
;; small program: compr(((1 + loop(0 - (x + x), x, 1)) mod (1 + (2 * (x + x)))) - 2, 1 + x) * 2
;; fast program: compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (2 * (x + x)))) - 2, 1 + x) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (mod (+ 1 (v1 x)) (+ 1 (* 2 (+ x x)))) 2))))
