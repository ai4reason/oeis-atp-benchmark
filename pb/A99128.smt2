;; sequence(s): A99128
;; terms: 1 10 1540 1798940 10981240985 269343686017406 21897427636095471460 5097399860176368033512080 3028721298862926523085514684685 4186904993091626163441378607213473000 12477686558866630120430437118910496237274716
;; small program: loop2(x * y, 1 + y, x, 1, loop((loop((x + x) + x, 2, x) div y) + x, x, 1)) div loop(x * y, x, 1)
;; fast program: loop2(x * y, 1 + y, x, 1, loop((((1 + 2) * ((x + x) + x)) div y) + x, x, 1)) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (div (v2 x) y) x))))
...
(assert (= i4 1))
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (div (* (+ 1 2) (+ (+ x x) x)) y) x))))
