;; sequence(s): A18739
;; terms: 1 2 5 10 19 25 38 50 95 190 475 950
;; small program: loop(compr((loop((2 + ((y * y) + x)) * x, 2, 2 + 2) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 * loop(((2 * (2 * (x * x))) - x) * x, 1, 1 + (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 2 (+ (* y y) x)) x))))
(assert (= g2 2))
(assert (= h2 (+ 2 2)))
...
(assert (forall ((x Int)) (= (f5 x) (* (- (* 2 (* 2 (* x x))) x) x))))
(assert (= g5 1))
(assert (= h5 (+ 1 (+ 2 2))))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* 2 v5) (+ 1 x)))))
