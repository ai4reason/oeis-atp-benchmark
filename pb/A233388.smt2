;; sequence(s): A233388
;; terms: 11 19 35 47 59 67 79 91 107 115 131 143 155 171 179 191 203 211 227 239
;; small program: ((compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 2 + (x + x)) * 2) * 2) - 1
;; fast program: ((compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * 2) * 2) - 1
(set-logic UFNIA)
