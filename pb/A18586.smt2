;; sequence(s): A18586
;; terms: 1 3 5 9 15 25 27 45 75 135 225 675
;; small program: loop(compr((loop(1 + (x * x), 2 + 2, x) - 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((x * x) + x, 1, (2 * (2 + (2 + 2))) - x) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (= g4 1))
(assert (forall ((x Int)) (= (h4 x) (- (* 2 (+ 2 (+ 2 2))) x))))
