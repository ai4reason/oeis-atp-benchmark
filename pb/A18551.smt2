;; sequence(s): A18551
;; terms: 1 3 5 15 41 123 205 615
;; small program: loop(compr((loop((((x + x) + x) + y) * x, 2, 2) + x) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((1 + 2) * ((1 + (2 + 2)) * (1 + x)), 1, 2 * (2 * (2 + (2 * (2 + 2))))) mod (1 + x), x) + 1) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ (+ (+ x x) x) y) x))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 2) (* (+ 1 (+ 2 2)) (+ 1 x))))))
(assert (= g4 1))
