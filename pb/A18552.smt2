;; sequence(s): A18552
;; terms: 1 2 4 7 8 11 14 22 28 44 56 77 88 154 308 616
;; small program: loop(compr(loop((((x + x) + x) + y) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((1 + (2 + x)) * ((x * x) - x), 1, 2 * (2 + 2)) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (+ 2 x)) (- (* x x) x)))))
(assert (= g5 1))
(assert (= h5 (* 2 (+ 2 2))))
