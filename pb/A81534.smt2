;; sequence(s): A81534
;; terms: 1 3 5 7 9 11 13 14 15 17 19 20 21 23 25 27 29 31 32 33
;; small program: loop(compr(((if (loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1) <= 0 then 1 else 0) * (1 + x)) mod 2, y), x, 1)
;; fast program: loop(((((if (loop((1 + (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) mod (1 + x), 2 - (x mod 2), x) - 1) <= 0 then 1 else 0) * (1 + x)) mod 2) + 1) + x, x, 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (* (ite (<= (- (v2 x) 1) 0) 1 0) (+ 1 x)) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (mod (+ 1 (* 2 (v6 x))) (+ 1 x)))))
(assert (forall ((x Int)) (= (g5 x) (- 2 (mod x 2)))))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (+ (+ (mod (* (ite (<= (- (v5 x) 1) 0) 1 0) (+ 1 x)) 2) 1) x))))
