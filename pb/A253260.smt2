;; sequence(s): A253260
;; terms: 16 36 64 81 100 121 144 196 225 256 324 400 441 484 576 625 676 729 784 900
;; small program: loop2((compr(loop(x * y, x, loop(1 + ((x + x) + x), 2, 2)) mod (1 + x), y) * x) + x, y, 2, 2, 2 + x) div 2
;; fast program: loop2((compr(((if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then 1 else 0) * loop(1 + (x + x), 2, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (f5 x) (mod (* (ite (<= (mod (+ 1 (v6 x)) (+ 1 x)) 0) 1 0) v7) (+ 1 x)))))
