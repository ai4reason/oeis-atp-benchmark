;; sequence(s): A1459
;; terms: 1 30 3150 420420 62355150 9816086280 1605660228900 269764879032000 46225898052627150 8042050347997165500 1415997888807961859400 251762943910387780962000 45125969443194371927422500 8143514687130622653091029120 1478138194032735032800001630400
;; small program: (loop(x * y, (2 * (x + x)) + x, 1) div loop((x * y) * y, x + x, 1)) div loop(x * y, x, 1)
;; fast program: ((((loop2(((y * y) + y) * x, 2 + y, x, 1, 1 + ((x + x) + x)) div loop(x * y, x, 1)) div loop2(x * y, 1 + y, x, 1, 1 + x)) * 1) * loop2(x * y, 1 + y, x, 1, 1 + (x + x))) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (div (v0 x) (v1 x)) (v2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (div (div (w3 x) (v4 x)) (w5 x)) 1) (w6 x)) (v7 x)))))
