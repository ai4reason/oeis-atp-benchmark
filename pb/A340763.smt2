...
;; small program: loop(loop((loop(x * y, x, 2) mod (1 + x)) mod (2 + 1), 2, y) + x, x, 0)
;; fast program: loop(loop(loop(if (((((1 + (x mod 2)) * loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + x)) - 1) mod (1 + x)) <= 0 then 1 else 0, 1, (x + x) + x) mod 2, 1, y) + x, x div (1 + 2), x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (mod (v2 x) (+ 1 x)) (+ 2 1)))))
(assert (= g1 2))
