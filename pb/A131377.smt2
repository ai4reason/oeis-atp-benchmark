;; sequence(s): A131377
;; terms: 1 1 0 1 1 0 0 1 1 1 1 0 0 1 1 1 1 0 0 1
;; small program: (loop((loop(x * y, y - 2, 1) + x) mod y, x, 0) + 1) mod 2
;; fast program: (((((loop(loop((((if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1, y) + x, (x - 1) div 2, 1) * (if (x - 1) <= 0 then 2 else x)) - 1) mod (1 + x)) + 1) + x) mod 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (mod (+ (v1 x y) x) y))))
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (+ (mod (- (* (v2 x) (ite (<= (- x 1) 0) 2 x)) 1) (+ 1 x)) 1) x) 2))))
