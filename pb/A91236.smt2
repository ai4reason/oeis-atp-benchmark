;; sequence(s): A91236
;; terms: 15 27 35 39 51 55 63 75 87 91 95 99 111 115 119 123 135 143 147 155
;; small program: (compr(loop(loop(x * y, x - (x div (1 + 2)), 1) mod (1 + (x + x)), x mod 2, x), 1 + x) * 2) + 1
;; fast program: (((((compr((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 2 else 1, x mod 2, x) - 1) mod (1 + x), 1 + x) * 2) + 1) div 2) * 2) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (- x (div x (+ 1 2))))))
(assert (= h2 1))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 2 1))))
(assert (forall ((x Int)) (= (g4 x) (mod x 2))))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (mod (- (v4 x) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (div (+ (* (v3 x) 2) 1) 2) 2) 1) 1))))
