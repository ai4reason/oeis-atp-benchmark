;; sequence(s): A18505
;; terms: 1 2 4 7 14 19 28 38 76 133 266 532
;; small program: loop(compr(loop(2 + (x * y), 1 + (2 + 2), 1) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop(2 * ((2 * ((2 * ((x - y) + x)) + x)) + y), 2, x) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* 2 (+ (* 2 (+ (* 2 (+ (- x y) x)) x)) y)))))
(assert (= g4 2))
