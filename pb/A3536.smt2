;; sequence(s): A3536
;; terms: 1 3 5 15 29 43 87 113 127 129 145 215 339 381 435 565 635 645 1247 1695
;; small program: loop(compr((loop((loop(x * x, 2, loop(x + x, x, x)) * x) * x, 1, 2 + 2) + x) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((loop(loop((x * x) * x, 1, x * x) * x, 1, 2 * (2 * (2 + 2))) + x) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
