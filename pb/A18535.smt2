;; sequence(s): A18535
;; terms: 1 2 5 10 59 118 295 590
;; small program: loop(compr(loop(2 + ((1 + 2) * (x * x)), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 + ((1 + 2) * loop(x * x, 1, 2 + (2 * (2 + (2 + 2)))))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ 2 (* (+ 1 2) (* x x))))))
(assert (= g2 2))
(assert (= h2 2))
