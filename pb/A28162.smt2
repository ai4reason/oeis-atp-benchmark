;; sequence(s): A28162
;; terms: 1 35 783 14287 231959 3494127 49989031 689059679 9238060887 121247325199 1565149220999 19940086389951 251372353802935 3141956721674351 38999543432154087 481325752085244703
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (x + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x + x, x - 1, 2), 1, y) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + x) + (((x * 2) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) x) (* (* (* x 2) 2) 2)))))
