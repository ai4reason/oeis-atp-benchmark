;; sequence(s): A171140
;; terms: 3 7 19 27 47 59 87 139 159 227 279 307 367 467 579 619 747 839 887 1039
;; small program: ((loop2((compr((1 + loop(x + x, x, x)) mod (1 + x), 2 + y) * x) + 2, y, 2, 1, x) div (1 + 2)) div 2) - 1
;; fast program: ((loop(loop2(((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, y, 2, 1, x), 1, 2 + x) div 2) div (1 + 2)) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (div (div (v3 x) 2) (+ 1 2)) 1))))
