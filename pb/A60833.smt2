;; sequence(s): A60833
;; terms: 1 4 7 8 12 13 15 16 20 23 24 25 28 29 31 32 36 39 40 44
;; small program: loop(compr(loop(loop(1 - (x + y), x, x), x, x) mod 2, y), x, 0) + 1
;; fast program: compr(((loop(if (x mod 2) <= 0 then (x div 2) else x, ((x div 2) + x) div 2, x) + 1) mod (2 + 2)) - 1, x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (ite (<= (mod x 2) 0) (div x 2) x))))
(assert (forall ((x Int)) (= (g5 x) (div (+ (div x 2) x) 2))))
