;; sequence(s): A19742
;; terms: 1 25 431 6365 86511 1117605 13957591 170189245 2038704671 24092243285 281680643751 3265150951725 37583315950831 430083097386565 4897580558961911 55540052099419805 627607236896972991
;; small program: loop((loop2((2 * ((2 * (x + x)) + x)) + (y * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(loop((loop2(x * y, y, 1 + x, 1, 1 + (2 + (2 * (2 + 2)))) - loop(x * x, 1, loop(x + x, x, 2))) div (1 + (2 + (2 + 2))), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (* 2 (+ (* 2 (+ x x)) x)) (* y y)))))
...
(assert (forall ((x Int)) (= (f4 x) (div (- (w5 x) (v6 x)) (+ 1 (+ 2 (+ 2 2)))))))
(assert (= g4 1))
