;; sequence(s): A322250
;; terms: 1 1 2 1 4 2 6 1 8 4 10 2 12 6 14 1 16 8 18 4
;; small program: loop(y, loop(x div (1 + (x mod 2)), x, x), 1)
;; fast program: ((((loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) * 2) + 1) * x) + 1) div (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (+ (* (v2 x) 2) 1) x) 1) (+ 1 x)))))
