;; sequence(s): A138420
;; terms: 3 18 150 588 3630 7098 20808 32490 69828 176610 230640 468198 706020 854238 1219368 1971918 3028470 3460530 5036658 6351660
;; small program: ((loop2((loop(loop2((compr((2 + loop(x * y, x, x)) mod (2 + x), y) + 2) * x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) + 1) div 2) div 2
;; fast program: (loop2((loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, y, 2, 1, x), 1, y) * x) - 1, y, 2, 1, x) + 1) div (2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (div (+ (w0 x) 1) 2) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (w5 x) 1) (+ 2 2)))))
