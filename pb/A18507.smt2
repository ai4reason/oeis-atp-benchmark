;; sequence(s): A18507
;; terms: 1 2 4 8 67 134 268 536
;; small program: loop(compr(loop((2 * (((x * x) + y) + 2)) + x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(loop(1 + (2 + ((x * x) - x)), 1 - (x mod (2 * (2 + 2))), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* 2 (+ (+ (* x x) y) 2)) x))))
(assert (= g2 2))
(assert (= h2 2))
