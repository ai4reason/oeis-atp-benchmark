;; sequence(s): A191331
;; terms: 2 6 12 16 18 22 28 32 34 38 42 44 48 54 58 60 64 70 74 80
;; small program: compr(loop((y * y) div (x - y), x + x, 0) mod 2, 1 + x) * 2
;; fast program: loop(compr(((((x div (2 + (2 * (2 * (2 + 2))))) - x) div (2 + 2)) + x) mod 2, y), 1 + x, 0) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (div (- (div x (+ 2 (* 2 (* 2 (+ 2 2))))) x) (+ 2 2)) x) 2))))
