;; sequence(s): A135214
;; terms: 1 17 260 516 3641 4937 21744 25840 84889 94889 255940 276676 647969 686385 1445760 1511296 2931153 3036129 5512228 5672228
;; small program: loop(loop(loop2((x * y) + x, y, 2 + ((1 + 2) - (x mod 2)), 1, x), 1, y) + x, x, 1)
;; fast program: loop(loop(loop2(x * y, y, 2 + (1 - (x mod 2)), 1, 1 + x) * (((2 + x) * x) + 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h2 x) (+ 2 (- (+ 1 2) (mod x 2))))))
(assert (= i2 1))
...
(assert (forall ((x Int)) (= (h5 x) (+ 2 (- 1 (mod x 2))))))
(assert (= i5 1))
...
(assert (forall ((x Int)) (= (f4 x) (* (w5 x) (+ (* (+ 2 x) x) 1)))))
(assert (= g4 1))
