;; sequence(s): A334919
;; terms: 5 8 9 13 16 17 18 21 23 25 28 29 30 33 37 38 40 41 43 44
;; small program: compr(loop(x * y, 2 + x, 1) mod (1 + ((x + x) + x)), 2 + x)
;; fast program: compr(loop(x * y, x, 2 * (2 + x)) mod (1 + ((x + x) + x)), 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (v1 x) (+ 1 (+ (+ x x) x))))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (v3 x) (+ 1 (+ (+ x x) x))))))
