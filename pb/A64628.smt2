;; sequence(s): A64628
;; terms: 1 1 1 2 3 4 5 7 9 13 17 23 31 42 56 74 99 133 177 236
;; small program: loop(x div (1 + 2), x, loop(x + x, x + x, 1))
;; fast program: loop(x * x, 1, loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) div loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (v2 x) (w6 x)))))
