;; sequence(s): A161580
;; terms: 2 5 7 10 14 18 21 23 26 29 31 34 37 39 42 46 50 53 55 58
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 2 + y) + (x div 2), x, 2)
;; fast program: compr(((loop((((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, x, x), x mod 2, x div 2) + x) mod 2) + (x mod 2)) + x, 2, x) + 1) + x) mod 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod x 2) 0) (div x 2) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (v1 x y) (div x 2)))))
