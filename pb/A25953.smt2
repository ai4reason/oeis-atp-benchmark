;; sequence(s): A25953
;; terms: 1 25 420 5990 78431 976275 11762890 138612880 1607750661 18432396125 209480782160 2364808954170 26557583235691 297035583017575 3311510934334230 36823696010925860 408634337848437521
;; small program: loop(((loop((loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + x) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop2(((((y div 2) + y) * y) + x) + x, (y + y) + y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (* (+ (div y 2) y) y) x) x))))
