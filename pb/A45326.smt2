;; sequence(s): A45326
;; terms: 2 3 7 11 19 23 31 43 47 59 67 71 79 83 103 107 127 131 139 151
;; small program: compr(loop((2 - loop(x + x, x, 2)) mod (1 + x), ((x div 2) + x) mod 2, x) div 2, 1 + x) + 1
;; fast program: (((compr(loop(((1 + loop(x * x, 1, loop(x + x, x div 2, 1))) mod (1 + (x + x))) mod (1 + x), x mod 2, x) - 1, x) * 2) - (if x <= 0 then 1 else 2)) + 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g1 x) (mod (+ (div x 2) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (* (v3 x) 2) (ite (<= x 0) 1 2)) 2) 1))))
