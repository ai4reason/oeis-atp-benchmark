;; sequence(s): A1750
;; terms: 10 15 25 35 55 65 85 95 115 145 155 185 205 215 235 265 295 305 335 355
;; small program: (loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + (2 + 2))
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * (1 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ (v0 x) 1) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) (+ 1 (+ 2 2))))))
