;; sequence(s): A138077
;; terms: 7 13 21 31 57 73 91 133 183 273 307 381 553 651 757 871 993
;; small program: loop2(((compr(loop((1 + loop(x + x, x, 2)) mod (1 + x), 2, x) - 1, 1 + y) * x) + 1) + x, y, 2, 1, x)
;; fast program: loop2(((compr(loop((1 + loop(x + x, x, 2)) mod (1 + x), 2 - (x mod 2), x) - 1, 1 + y) * x) + 1) + x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (* (v5 x y) x) 1) x))))
