;; sequence(s): A103904
;; terms: 1 2 24 384 10240 491520 44040192 7516192768 2473901162496 1583296743997440 1981583836043018240 4869940435459321626624 23574053482485268906770432 225305087149939210031640608768
;; small program: loop(loop2(x * y, 2, x div 2, 1, x), 1, (x * x) + x)
;; fast program: (loop(x * x, 1, loop(x + x, x - 1, 1)) * (if x <= 0 then 1 else (((x * x) + x) div 2))) * loop2(x * y, y + y, x - 1, if x <= 0 then 1 else 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v2 x) (ite (<= x 0) 1 (div (+ (* x x) x) 2))) (w4 x)))))
