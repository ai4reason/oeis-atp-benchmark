;; sequence(s): A90090
;; terms: 14 33 65 119 209 299 493 589 851 1189 1333 1739 2173 2537 2867 3551 4189 4453 5293 5893
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, 1 + (2 + y), 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, 2 + (1 + y), 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g3 x y) (+ 2 (+ 1 y)))))
(assert (= h3 2))
(assert (= i3 1))
