;; sequence(s): A4793
;; terms: 1 3 4 6 10 12 13 15 28 30 31 33 37 39 40 42 82 84 85 87
;; small program: (loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x + x), y), x, 0) + 1) + (x mod 2)
;; fast program: ((loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) + (x mod 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (v3 x) x) (mod x 2)) 1))))
