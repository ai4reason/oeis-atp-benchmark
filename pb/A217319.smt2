;; sequence(s): A217319
;; terms: 4 8 12 20 24 28 36 40 44 52 56 60 64 68 72 76 84 88 92 100
;; small program: (loop(((((loop2((y mod 2) + x, y div (2 - (x mod 2)), x, 1, x) div 2) div 2) mod 2) + 1) + x, x, 1) * 2) * 2
;; fast program: ((compr(((((loop((((loop2(x + y, (y div 2) div 2, x, x, x) + x) mod 2) + (x mod 2)) + x, 2, x) + 1) mod 2) + 1) + x) mod 2, x) * 2) + 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (mod y 2) x))))
...
(assert (forall ((x Int)) (= (f0 x) (+ (+ (mod (div (div (w1 x) 2) 2) 2) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (g4 x y) (div (div y 2) 2))))
...
(assert (forall ((x Int)) (= (f3 x) (+ (+ (mod (+ (w4 x) x) 2) (mod x 2)) x))))
(assert (= g3 2))
(assert (forall ((x Int)) (= (h3 x) x)))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (f2 x) (mod (+ (+ (mod (+ (v3 x) 1) 2) 1) x) 2))))
