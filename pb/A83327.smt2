;; sequence(s): A83327
;; terms: 0 1 7 40 217 1156 6097 31900 165697 855076 4387537 22404460 113945377 577590196 2919923377 14729076220 74167952257 372944296516 1873182473617 9399885079180
;; small program: ((loop2(((((y * y) - y) + x) + x) + x, y + y, x, 1, 1) - loop((2 * (x + x)) + x, x, 1)) div 2) * (1 - 2)
;; fast program: ((loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) - loop((x * x) + x, 1, loop(x + x, x, 1))) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (div (- (w0 x) (v1 x)) 2) (- 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (- (w2 x) (v5 x)) (w7 x)) 2))))
