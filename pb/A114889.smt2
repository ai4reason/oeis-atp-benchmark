;; sequence(s): A114889
;; terms: 1 3 4 7 9 10 11 12 13 19 21 22 25 27 28 29 30 31 32 33
;; small program: (loop(((loop(2 * (2 * ((x - (x div y)) + x)), x, 1) mod (1 + 2)) - 1) + x, x, x) + 1) + x
;; fast program: (((loop((loop((loop(if (loop2((x * y) + x, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) <= 0 then 1 else 0, 1, y) * x) + 1, x, 1) - 1) - x, 2, x) + 2) + x) + x) - (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* 2 (* 2 (+ (- x (div x y)) x))))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (* x y) x))))
(assert (forall ((x Int) (y Int)) (= (g5 x y) (div y (+ 1 2)))))
(assert (= h5 (+ 2 2)))
(assert (= i5 1))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (mod (w5 x) (+ 1 2)) 0) 1 0))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (- (+ (+ (+ (v2 x) 2) x) x) (+ 1 x)))))
