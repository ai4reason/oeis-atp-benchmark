;; sequence(s): A120944-A327829
;; terms: 6 10 14 15 21 22 26 30 33 34 35 38 39 42 46 51 55 57 58 62
;; small program: compr(loop(loop(if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then (1 + x) else 1, 1, y) * x, x div 2, 1) mod (1 + x), 1 + x) + 1
;; fast program: compr(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, (x - 1) div 2, 1) mod (1 + x), 1 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (+ 1 (v3 x)) (+ 1 x)) 0) (+ 1 x) 1))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f0 x) (mod (v1 x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (g5 x) (div (- x 1) 2))))
(assert (= h5 1))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) h5))))
(assert (forall ((x Int)) (= (f4 x) (mod (v5 x) (+ 1 x)))))
