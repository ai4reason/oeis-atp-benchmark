;; sequence(s): A161579
;; terms: 0 1 3 4 6 8 9 11 12 13 15 16 17 19 20 22 24 25 27 28
;; small program: loop(loop(if ((x div 2) mod 2) <= 0 then 2 else (x div 2), 1 + x, x) + x, x, 2) - 2
;; fast program: compr(loop((loop((loop(((loop2(x + y, y div 2, x, 0, x) mod 2) + (x mod 2)) + x, 2, x) + x) mod 2, 1, x div 2) + 1) + x, 2, x) mod 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (+ (mod (w6 x) 2) (mod x 2)) x))))
(assert (= g5 2))
