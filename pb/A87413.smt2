;; sequence(s): A87413
;; terms: 1 6 34 199 1200 7388 46148 291305 1853580 11868585 76380825 493606725 3201081873 20821158233 135776966761 887393271310 5811082966885 38119865826420 250447855600320 1647729357535485
;; small program: loop(loop(loop2(x * y, 1 + y, x, 1, x + x) div loop(x * y, x, 2), 1, y) + x, 1 + x, 0)
;; fast program: loop(loop(((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * (1 + ((x + x) + x))) div (1 + x)) * (2 + ((x + x) + x))) div 2, 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (* (div (* (div (w6 x) (v7 x)) (+ 1 (+ (+ x x) x))) (+ 1 x)) (+ 2 (+ (+ x x) x))) 2))))
(assert (= g5 1))
