;; sequence(s): A306546
;; terms: 4 1 10 1 16 3 22 1 28 5 34 3 40 7 46 1 52 9 58 5
;; small program: (((1 + loop(loop(x div 2, x mod 2, x), x, x)) * (1 + 2)) + 1) div (1 + (2 * (x mod 2)))
;; fast program: ((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) + loop(x mod 2, x mod 2, 1 + x)) * 2) - 1) - (x mod 2)) + (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (div (+ (* (+ 1 (v0 x)) (+ 1 2)) 1) (+ 1 (* 2 (mod x 2)))))))
...
(assert (forall ((x Int)) (= (f8 x) (mod x 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (- (* (+ (v2 x) (v8 x)) 2) 1) (mod x 2)) (+ 1 2)))))
