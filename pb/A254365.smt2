;; sequence(s): A254365
;; terms: 15 21 35 69 155 381 995 2709 7595 21741 63155 185349 547835 1627101 4848515 14479989 43308875 129664461 388469075 1164358629
;; small program: 2 + ((2 * (2 + (2 + loop(x + x, x, 2)))) + loop((x + x) + x, x, 1))
;; fast program: ((2 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2)))) + (2 * (2 + 2))) + loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ 2 (+ (* 2 (+ 2 (+ 2 (v0 x)))) (v1 x))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ 2 (v2 x)) (* 2 (+ 2 2))) (w5 x)))))
