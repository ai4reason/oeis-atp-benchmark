;; sequence(s): A15921
;; terms: 1 2 4 6 10 12 14 22 26 30 34 38 46 58 62 74 82 86 94 106
;; small program: (compr((2 * (loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))) - 2)) mod (1 + x), x) + 1) div 1
;; fast program: ((compr(((((loop(x * x, 1, loop(x + x, x div 2, 1)) * (1 + (x mod 2))) - 1) * (1 + (x mod 2))) - 2) mod (1 + x), x) + 1) - x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (* 2 (- (v1 x) 2)) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (- (* (- (* (v5 x) (+ 1 (mod x 2))) 1) (+ 1 (mod x 2))) 2) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (+ (v4 x) 1) x) x))))
