;; sequence(s): A101256
;; terms: 0 0 0 4 4 10 10 18 27 37 37 49 49 63 78 94 94 112 112 132
;; small program: loop(loop(if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then (0 - 1) else x, 1, y) + x, x, x)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (0 - 1) else x, 1, y) + x, x, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 1 (v2 x)) (+ 1 x)) 0) (- 0 1) x))))
(assert (= g1 1))
