;; sequence(s): A75914
;; terms: 1 105 6650 330750 14266875 560896875 20682062500 728227500000 24779833203125 821666548828125 26708267167968750 854772944238281250 27023254648193359375 846046877171630859375 26282219820458984375000 811330550012329101562500
;; small program: loop(((loop((loop((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, y, 1) + x) + x) + x, x, 1) * loop((2 * (x + x)) + x, x, 1)
;; fast program: loop(loop(loop(((((loop(loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, x, 1), 1, y) + x) + x) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1) * loop2(x * y, y, x, 1, 1 + (2 + 2))
(set-logic UFNIA)
