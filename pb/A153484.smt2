;; sequence(s): A153484
;; terms: ~96 115 2997 16679 160923 371165 1419729 2475971 6436215 20511021 28629023 69343829 115856073 147008315 229344879 418195365 714924171 844596173 1350124979 1804229223
;; small program: loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (((x * x) * x) * x), y, 2, 1, x) - loop(2 * (x * x), 2, 2)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (((x * x) * x) * x), y, 2, 1, x) - loop(2 * (x * x), 1, 2 * (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (* (+ (v1 x y) 1) (* (* (* x x) x) x)))))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (small x) (- (w0 x) v4))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (+ (v6 x y) 1) (* (* (* x x) x) x)))))
...
(assert (forall ((x Int)) (= (f9 x) (* 2 (* x x)))))
(assert (= g9 1))
(assert (= h9 (* 2 (+ 2 2))))
(assert (forall ((x Int) (y Int)) (= (u9 x y) (ite (<= x 0) y (f9 (u9 (- x 1) y))))))
(assert (= v9 (u9 g9 h9)))
(assert (forall ((x Int)) (= (fast x) (- (w5 x) v9))))
