;; sequence(s): A90178
;; terms: 2 4 6 9 12 17 20 25 28 33 40 43 50 55 58 63 70 77 80 87
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), loop(y, x, 0)) + 2) + x
;; fast program: ((((compr((((if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) mod (1 + x)) mod 2) mod (1 + x), x) * 2) + 1) div 2) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (div (+ (* (v3 x) 2) 1) 2) x) 2))))
