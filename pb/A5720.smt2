;; sequence(s): A5720
;; terms: 1 10 44 135 336 728 1428 2598 4455 7282 11440 17381 25662 36960 52088 72012 97869 130986 172900 225379
;; small program: loop(loop(loop(loop(loop(x + y, 2 + y, x), y, x), y, x) + y, y, x) + y, x, 1) + x
;; fast program: loop(loop(loop(((((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 * (2 + (2 + 2))))) - 1) - x) - x) - x, 1, y) + x, y, x), 1 + x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (- (- (- (- (div (v8 x) (* 2 (* 2 (+ 2 (+ 2 2))))) 1) x) x) x))))
(assert (= g7 1))
