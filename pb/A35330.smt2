;; sequence(s): A35330
;; terms: 1 15 140 1045 6835 40963 230720 1240740 6437890 32468470 160010280 773624615 3680728375 17274086235 80119845080 367821324040 1673528845710 7554110698850 33858536700040 150802994850570
;; small program: loop(loop((((loop(loop2((2 + (2 + (2 + y))) * x, 1 + y, x, 1, x) div loop(x * y, x, 1), 1, y) + x) + x) + x) + x, y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop(loop((((loop(loop2(x * y, 1 + y, x, 1, 2 + (2 + (2 + x))) div loop(x * y, x, 1), 1, y) + x) + x) + x) + x, y, 1) + (2 * (x + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ 2 (+ 2 (+ 2 y))) x))))
...
(assert (= i8 1))
(assert (forall ((x Int)) (= (j8 x) (+ 2 (+ 2 (+ 2 x))))))
