;; sequence(s): A277801
;; terms: ~1 ~1 ~1 1 5 19 47 109 233 483 993 2011 4055 8149 16337 32715 65477 131011 262077 524217
;; small program: ((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + x) + 1) - loop(x + x, x, 1)) * (0 - 1)
;; fast program: ((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + x) + 1) - loop(x + x, x, 1)) * (1 - 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- (+ (v0 x) 1) (v2 x)) (- 0 1)))))
