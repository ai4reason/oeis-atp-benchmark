;; sequence(s): A18617
;; terms: 1 3 5 7 15 21 35 49 105 147 245 735
;; small program: loop(compr((loop((((x * x) * x) - y) + x, 2, 2) + x) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + 2) * ((1 + (2 + 2)) * loop(x * x, 1, 1 + (2 + (2 + 2))))) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 2) (* (+ 1 (+ 2 2)) v4)) (+ 1 (+ x x))))))
