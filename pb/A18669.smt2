;; sequence(s): A18669
;; terms: 1 2 3 4 6 9 12 18 23 36 46 69 92 138 207 276 414 828
;; small program: loop(compr(loop((1 + y) * ((2 * (x * x)) - x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: compr(loop((2 + (((2 * (x + x)) * x) * x)) * (2 + x), 2, 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 y) (- (* 2 (* x x)) x)))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 2 (* (* (* 2 (+ x x)) x) x)) (+ 2 x)))))
(assert (= g4 2))
(assert (= h4 2))
