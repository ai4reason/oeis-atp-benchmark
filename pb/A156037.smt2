;; sequence(s): A156037
;; terms: 1 1 4 6 10 12 16 18 22 28 30 36 40 42 46 52 58 60 66 70
;; small program: compr((2 - loop(x + x, x, 2)) mod (1 + x), loop((1 mod y) + y, x, 1))
;; fast program: compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), if x <= 0 then 1 else x) * (if (x - 1) <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (mod 1 y) y))))
...
(assert (forall ((x Int)) (= (fast x) (* (v3 x) (ite (<= (- x 1) 0) 1 2)))))
