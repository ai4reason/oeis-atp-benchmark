;; sequence(s): A129071
;; terms: 19 25 37 49 73 85 109 121 145 181 193 229 253 265 289 325 361 373 409 433
;; small program: (((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) + 2) * (1 + 2)) * 2) + 1
;; fast program: (((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) * (1 + 2)) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) (+ 1 2)) 2) 1))))
