;; sequence(s): A18717
;; terms: 1 2 5 7 10 13 14 26 35 65 70 91 130 182 455 910
;; small program: loop(compr(loop((1 + (((x - y) * x) + x)) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((((x * x) - x) * x) + x, 1, 2 + (2 * (2 + 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (+ 1 (+ (* (- x y) x) x)) x))))
(assert (= g2 2))
(assert (= h2 2))
