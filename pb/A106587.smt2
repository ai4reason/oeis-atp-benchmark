;; sequence(s): A106587
;; terms: 5 13 34 65 146 205 338 425 610 941 1082 1513 1850 2045 2434 3065 3770 4045 4850 5441
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + 2, y, 2, 1, x) + ((2 + x) * x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + 2, y, 2, 1, x) + ((2 + x) * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (w3 x) (* (+ 2 x) x)))))
