...
;; small program: loop(loop2(x - y, x, y, 0, 1) - x, x, 1)
;; fast program: (if ((2 + x) mod (1 + 2)) <= 0 then 2 else 1) * loop(0 - 1, x mod 2, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (ite (<= (mod (+ 2 x) (+ 1 2)) 0) 2 1) (v2 x)))))
