;; sequence(s): A240068
;; terms: 24 32 48 64 96 112 144 160 192 240 256 304 336 352 384 432 480 496 544 576
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 2) * (2 * (2 + 2))
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 2 else 1)) * 2) * (2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ (v0 x) 2) (* 2 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 2 1)) 2) (+ 2 2)))))
