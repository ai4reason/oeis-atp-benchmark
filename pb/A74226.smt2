;; sequence(s): A74226
;; terms: 1 4 10 11 13 14 16 23 25 34 35 37 38 40 44 47 49 52 56 58
;; small program: loop(compr(((((loop(x div (1 + (x mod 2)), x, x) div 2) mod 2) * 2) + x) mod (1 + 2), y), x, 0) + 1
;; fast program: compr(((((((loop(x div (2 - (x mod 2)), x, x) div 2) mod 2) * 2) + x) mod (1 + 2)) - 1) mod (1 + x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (* (mod (div (v2 x) 2) 2) 2) x) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (div x (- 2 (mod x 2))))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (forall ((x Int)) (= (h4 x) x)))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) (h4 x)))))
(assert (forall ((x Int)) (= (f3 x) (mod (- (mod (+ (* (mod (div (v4 x) 2) 2) 2) x) (+ 1 2)) 1) (+ 1 x)))))
