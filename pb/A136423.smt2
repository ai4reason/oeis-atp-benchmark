;; sequence(s): A136423
;; terms: 1 1 2 3 4 6 9 13 19 29 43 65 97 146 219 328 493 739 1108 1663
;; small program: (loop(x div 2, x, loop(1 + ((x + x) + x), x, 1)) + 1) div 2
;; fast program: ((((1 + 2) * loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div (2 * loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1))))) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (* (+ 1 2) (w2 x)) (* 2 (v4 x))) 1) 2))))
