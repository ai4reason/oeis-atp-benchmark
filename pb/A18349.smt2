;; sequence(s): A18349
;; terms: 1 2 7 14 17 34 119 238
;; small program: compr(loop((2 * ((x + y) * x)) - 2, 2, 2) mod (1 + x), x) + 1
;; fast program: loop2((((loop2(y, 2 + x, x + y, 2, x) div 2) mod 2) + x) + x, x, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (- (* 2 (* (+ x y) x)) 2))))
(assert (= g1 2))
(assert (= h1 2))
...
(assert (forall ((x Int) (y Int)) (= (h3 x y) (+ x y))))
(assert (= i3 2))
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (mod (div (w3 x y) 2) 2) x) x))))
