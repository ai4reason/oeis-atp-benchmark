;; sequence(s): A94969
;; terms: 1 2 6 15 39 97 244 610 1525 3814 9536 23841 59604 149011 372529 931322 2328306 5820766 14551915 36379788
;; small program: loop(x div 2, x, loop((2 * (x + x)) + x, x, 1))
;; fast program: loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2))) div loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (w2 x) (v5 x)))))
