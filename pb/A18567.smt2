;; sequence(s): A18567
;; terms: 1 2 4 7 14 23 28 46 92 161 322 644
;; small program: loop(compr((2 * loop((2 * (2 + x)) + x, 2 + 2, 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((2 * (2 * (2 * (x * x)))) - (x div 2), 1, 1 + (2 * (2 + 2))) mod (1 + x), x) + 1) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (* 2 (* 2 (* 2 (* x x)))) (div x 2)))))
(assert (= g4 1))
(assert (= h4 (+ 1 (* 2 (+ 2 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 1) 1))))
