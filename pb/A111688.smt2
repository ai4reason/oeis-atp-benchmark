;; sequence(s): A111688
;; terms: 2 4 5 6 7 8 11 12 13 14 17 18 19 20 23 24 29 30 31 32
;; small program: loop(compr((((loop(x * y, x, 2) div (1 + x)) mod (2 + x)) + 1) mod 2, y), x, 0) + 2
;; fast program: (((loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) - 1) - x, 1, x div 2) * 2) + 1) + (if x <= 0 then 1 else 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ (mod (div (v2 x) (+ 1 x)) (+ 2 x)) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 2)) x))))
