...
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (0 - (1 + 2))) + 1
;; fast program: loop(((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (1 + 2)) - 1, 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (mod (v0 x) (- 0 (+ 1 2))) 1))))
