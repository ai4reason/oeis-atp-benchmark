;; sequence(s): A6364
;; terms: 0 1 6 7 10 11 12 13 18 19 20 21 24 25 30 31 34 35 36 37
;; small program: loop(compr(loop(loop((2 + x) mod y, x, 1), x, x), y), x, 0)
;; fast program: ((loop((loop((x * x) mod ((2 * (2 * (2 + 2))) - 1), 2, x div 2) + x) mod 2, 1, x div 2) + x) * 2) - (x mod 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (* x x) (- (* 2 (* 2 (+ 2 2))) 1)))))
(assert (= g5 2))
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (v4 x) x) 2) (mod x 2)))))
