;; sequence(s): A18686
;; terms: 1 2 4 8 107 214 428 856
;; small program: loop(compr(loop(((x + x) + y) * y, 2 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: loop(loop(1 + (2 + ((2 + (2 + x)) * x)), 1 - (y mod (2 + 2)), x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (+ 1 (+ 2 (* (+ 2 (+ 2 x)) x))))))
(assert (forall ((x Int) (y Int)) (= (g4 x y) (- 1 (mod y (+ 2 2))))))
