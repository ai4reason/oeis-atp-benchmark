;; sequence(s): A45331
;; terms: 2 3 7 13 19 31 37 43 61 67 73 79 97 103 109 127 139 151 157 163
;; small program: loop(compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod (2 + (2 + 2))), x) - 2, 1 + y) + 1, x, 2)
;; fast program: (((compr(loop(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), ((1 + x) mod (1 + 2)) mod 2, x) - 2, (1 + x) mod 2, x) - 1, 1 + x) + 1) * 2) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g2 x) (- 1 (mod x (+ 2 (+ 2 2)))))))
...
(assert (forall ((x Int)) (= (g6 x) (mod (mod (+ 1 x) (+ 1 2)) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (* (+ (v4 x) 1) 2) 2) 1))))
