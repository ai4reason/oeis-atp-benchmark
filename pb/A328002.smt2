;; sequence(s): A328002
;; terms: 1 3 8 22 50 130 280 700 1470 3570 7392 17556 36036 84084 171600 394680 802230 1823250 3695120 8314020
;; small program: loop((loop(loop(2 * ((x - (x div y)) + x), x div 2, if (x mod 2) <= 0 then 1 else x), 1, y) + x) + x, x, 1)
;; fast program: loop((loop(loop2(x * y, y - 1, x - (x div 2), 1, x) div loop(x * y, x div 2, 1), 1, y) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h2 x) (ite (<= (mod x 2) 0) 1 x))))
...
(assert (forall ((x Int)) (= (h5 x) (- x (div x 2)))))
(assert (= i5 1))
