;; sequence(s): A18467
;; terms: 1 2 4 5 10 20 23 46 92 115 230 460
;; small program: loop(compr(loop(2 * (2 - (x + x)), 2 + 2, x) mod (1 + x), y), x, 0) + 1
;; fast program: compr((2 * (2 + loop((2 + (x * x)) * x, 1, 2 + (2 + 2)))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* 2 (+ 2 v4)) (+ 1 x)))))
