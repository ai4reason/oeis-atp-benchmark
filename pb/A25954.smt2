;; sequence(s): A25954
;; terms: 1 26 457 6854 94789 1251002 16035889 201651758 2502830077 30781099778 376097622121 4573691815862 55428986530165 670050005786954 8084719590584353 97413710820261566 1172538211571391853
;; small program: loop(loop(loop2(((((y div 2) + y) * y) + x) + x, (y + y) + y, x, 1, 1 + 2), 1, y) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop2((2 * (x + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (* (+ (div y 2) y) y) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (* 2 (+ x x)) y))))
(assert (forall ((x Int) (y Int)) (= (g6 x y) (+ 1 (+ (+ y y) y)))))
