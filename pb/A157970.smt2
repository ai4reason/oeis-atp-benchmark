;; sequence(s): A157970
;; terms: 5 9 17 23 29 33 39 45 53 57 65 71 77 85 89 95 101 105 113 119
;; small program: compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + (x - 1), x), (2 + x) + x)
;; fast program: (compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g1 x) (+ x (- x 1)))))
