;; sequence(s): A257200
;; terms: 1 6 22 63 154 336 672 1254 2211 3718 6006 9373 14196 20944 30192 42636 59109 80598 108262 143451
;; small program: loop(loop(loop(loop(loop(x + y, y, x) - y, y, x) + y, y, x), 1 + y, x), x, 1)
;; fast program: loop(loop(loop(loop((((x * x) - x) div (2 + (2 + 2))) + x, 1, ((1 + x) * (2 + x)) div 2), 1, y) + x, y, 1) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (+ (div (- (* x x) x) (+ 2 (+ 2 2))) x))))
(assert (= g8 1))
(assert (forall ((x Int)) (= (h8 x) (div (* (+ 1 x) (+ 2 x)) 2))))
