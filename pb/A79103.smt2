;; sequence(s): A79103
;; terms: 1 4 625 7529536 9682651996416 1605976966052654874624 38858631894268190306056236008241 149521802722388792654037601564900000000000000 97507780747332761768395977098452480878573838263199129993216
;; small program: loop2(loop((((1 + 2) * (x * y)) div (2 + y)) + x, y, x), y, x + x, 1, x)
;; fast program: loop2(loop((loop2((1 + y) * (x * y), 1 + y, x, 1, 2 + x) div loop((2 + y) * x, x, 1)) div loop(x * y, x, 1), 1, y) * x, y, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ 1 y) (* x y)))))
...
(assert (forall ((x Int)) (= (f3 x) (div (div (w4 x) (v5 x)) (v6 x)))))
(assert (= g3 1))
