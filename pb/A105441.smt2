;; sequence(s): A105441
;; terms: 9 15 18 21 25 27 30 33 35 36 39 42 45 49 50 51 54 55 57 60
;; small program: compr((loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then 1 else 0, 1, loop(loop(x div 2, x mod 2, x), x, x)) mod 2) mod (1 + x), 1 + x) + 1
;; fast program: compr(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) mod (1 + x), 1 + x) + 1
(set-logic UFNIA)
