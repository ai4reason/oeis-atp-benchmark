;; sequence(s): A287925
;; terms: 32 97 641 2417 14657 28577 83537 130337 279857 707297 923537 1874177 2825777 3418817 4879697 7890497 12117377 13845857 20151137 25411697
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, y, 2 + 2, 1, x) + loop(x * x, 2, 2)
;; fast program: ((((((((loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * (((x * x) * x) * x), 0, 2, 1, x) div 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (+ (v5 x y) 1) (* (* (* x x) x) x)))))
(assert (= g4 0))
(assert (= h4 2))
(assert (= i4 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (+ (+ (+ (div (w4 x) 2) 2) 2) 2) 2) 2) 2) 2) 2))))
