;; sequence(s): A85157
;; terms: 1 1 2 3 4 5 6 14 24 36 50 66 168 312 504 750 1056 2856 5616 9576
;; small program: loop2(loop(if x <= 0 then 1 else x, 1, y) * x, ((y - 1) - 2) - 2, x, 1, x)
;; fast program: loop2(loop(if x <= 0 then 1 else x, 1, y) * x, ((y - 2) - 2) - 1, x div 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g0 x y) (- (- (- y 1) 2) 2))))
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= x 0) 1 x))))
(assert (= g3 1))
...
(assert (forall ((x Int) (y Int)) (= (g2 x y) (- (- (- y 2) 2) 1))))
