;; sequence(s): A336339
;; terms: 6 12 18 24 26 28 32 34 36 42 48 50 52 60 68 70 74 76 78 84
;; small program: (compr(loop(loop(if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, x, 0) mod 2, 1 + x) * 2) + 2
;; fast program: ((compr((loop(loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, y) + x, x div 2, 1), x mod 2, x) - 1) mod 2, 1 + x) div 2) + 1) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (- (mod (+ 1 (v3 x)) (+ 1 (+ x x))) 2) 0) 1 0))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (f7 x) (ite (<= (- (mod (+ 1 (v8 x)) (+ 1 (+ x x))) 2) 0) 1 2))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (div (v4 x) 2) 1) 2))))
