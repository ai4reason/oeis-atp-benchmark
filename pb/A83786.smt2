;; sequence(s): A83786
;; terms: 4 6 8 9 0 2 4 5 6 8 0 1 2 4 5 6 7 8 0 2
;; small program: (compr(loop(x * y, x, 2) mod (1 + x), 2 + x) + 1) mod (2 + (2 * (2 + 2)))
;; fast program: (compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x), 1 + x) + 1) mod (2 + (2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 1) (+ 2 (* 2 (+ 2 2)))))))
