;; sequence(s): A18418
;; terms: 1 2 5 10 37 74 185 370
;; small program: loop(compr(loop(loop((x - 1) + x, 2, x) * x, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 + ((x * x) + x)) * (2 + x), 1, 2 * (2 + 2)) mod (1 + x), x) + 1, 1, (x div 2) + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (+ 2 (+ (* x x) x)) (+ 2 x)))))
(assert (= g6 1))
(assert (= h6 (* 2 (+ 2 2))))
