;; sequence(s): A28007
;; terms: 1 28 511 7722 105105 1340976 16393987 194632774 2263422469 25933837644 293957202903 3305777399106 36960914527993 411484609521832 4566596614746859 50561336628369918 558851419944076077
;; small program: loop((loop((loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
