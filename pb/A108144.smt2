;; sequence(s): A108144
;; terms: 5 7 9 11 13 15 17 21 23 25 27 29 33 35 39 41 45 47 49 53
;; small program: ((compr(loop2(loop(if ((2 - loop(x + x, x, 2)) mod (1 + x)) <= 0 then (1 + x) else 2, 1, y) * x, y div 2, x, 1, x) mod (1 + x), 1 + x) * 2) + 1) + 2
;; fast program: (compr((loop(loop((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 0) + x) mod 2) + (x mod 2), 1, loop(loop(x div 2, x mod 2, x), x, x)) mod 2, x mod 2, x) + 1) mod 2, 1 + x) + 1) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (- 2 (v3 x)) (+ 1 x)) 0) (+ 1 x) 2))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (small x) (+ (+ (* (v0 x) 2) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (v4 x) 1) 1))))
