;; sequence(s): A53643
;; terms: 0 0 0 0 0 1 1 4 10 21 42 77 132 215 334 501 728 1032 1428 1938
;; small program: loop(loop(loop(loop(loop(x + y, y - 2, x), y, x), y - 2, x), y, x), x, x) div (1 + x)
;; fast program: ((loop(loop(loop(loop(x + y, y - 2, x), y, x), y - 2, x), x, 2) div (1 + 2)) + 1) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g8 x y) (- y 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (v5 x) (+ 1 2)) 1) 2))))
