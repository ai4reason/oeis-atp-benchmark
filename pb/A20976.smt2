;; sequence(s): A20976
;; terms: 1 27 487 7335 99631 1265607 15341887 179688375 2050086511 22913907687 251930201887 2733012078615 29322230800591 311701053949767 3287717299503487 34448718207176055 358912563957741871
;; small program: loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop(((loop((x * x) * x, 1, loop(x + x, x - 1, 2)) + loop2(x * y, y, x, 1, 2 + (2 * (2 + 2)))) div 2) - loop2(x * y, y, x, 1, 1 + (2 * (2 + 2))), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (div (+ (v4 x) (w6 x)) 2) (w7 x)))))
(assert (= g3 1))
