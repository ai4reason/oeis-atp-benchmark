;; sequence(s): A18404
;; terms: 1 3 5 15 23 69 115 345
;; small program: loop(compr(loop(((0 - x) * (x * x)) + y, 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((2 + loop((x * x) * x, 1, 1 + (2 + (2 + 2)))) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (- 0 x) (* x x)) y))))
(assert (= g2 2))
(assert (= h2 2))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (+ 2 v4) (+ 1 (+ x x))))))
