;; sequence(s): A220657
;; terms: 0 1 3 6 9 12 15 19 23 27 31 35 39 43 47 51 55 59 63 67
;; small program: loop(loop(loop(x div y, x, y) + x, y - 1, 1) + x, x, 0)
;; fast program: (loop(loop((y div loop(x * y, x, 1)) + x, y, 1) + x, x - 1, 1) * (if x <= 0 then 1 else 2)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (div x y))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (div y (v5 x)) x))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (v3 x) (ite (<= x 0) 1 2)) 2))))
