;; sequence(s): A44432
;; terms: 1 2 5 11 22 45 90 181 363 726 1453 2907 5814 11629 23258 46517 93035 186070 372141 744282
;; small program: loop((loop(compr(loop(x div (if (x mod (2 + 2)) <= 0 then 1 else 2), x, x), 1 + x) mod 2, 1, y) + x) + x, x, 1)
;; fast program: loop((loop((loop(1 + (((((2 * (x + x)) div (1 + (2 * (2 * (2 + 2))))) + x) mod 2) + x), 2, x) + x) mod 2, 1, y) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ 1 (+ (mod (+ (div (* 2 (+ x x)) (+ 1 (* 2 (* 2 (+ 2 2))))) x) 2) x)))))
(assert (= g6 2))
