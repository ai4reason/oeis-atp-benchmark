;; sequence(s): A10426
;; terms: 0 1 4 9 10 14 16 25 26 29 30 35 36 39 40 49 51 55 56 61
;; small program: compr(((loop((x * x) * x, 1, x * x) * x) - x) mod (1 + ((2 * loop(x * x, 2, 2)) * 2)), x)
;; fast program: compr(((loop((x * x) * x, 1, x * x) * x) - x) mod (1 + loop(x * x, 1, 2 * (2 + 2))), x)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f0 x) (mod (- (* (v1 x) x) x) (+ 1 (* (* 2 v2) 2))))))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f3 x) (mod (- (* (v4 x) x) x) (+ 1 v5)))))
