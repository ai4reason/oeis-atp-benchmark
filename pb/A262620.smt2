;; sequence(s): A262620
;; terms: 1 5 17 21 49 69 81 85 145 197 241 277 305 325 337 341 465 581 689 789
;; small program: ((loop(loop((x - 1) mod y, y, 1) + x, (1 + x) + x, 0) + x) * 2) + 1
;; fast program: ((((((loop(loop(((((1 + loop((x mod y) + y, x div (2 + 2), 0)) * (if (x - 1) <= 0 then 1 else 2)) * (if x <= 0 then 1 else 2)) - x) - 1, 1, y) + x, x, 1) * 2) - 1) + x) * 2) - 1) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (* (+ (- (* (v2 x) 2) 1) x) 2) 1) 2) 1))))
