;; sequence(s): A278998
;; terms: 0 3 4 5 8 10 13 15 18 19 20 23 24 25 28 29 30 33 35 38
;; small program: loop(compr((((loop(if ((1 + x) mod (1 + (2 + 2))) <= 0 then (x div (1 + (2 + 2))) else x, x, x) mod (1 + (2 + 2))) + 1) div 2) mod 2, y), x, x)
;; fast program: loop(compr((((loop(if ((1 + x) mod (1 + (2 + 2))) <= 0 then (x div (1 + (2 + 2))) else x, 2, x) + 1) mod (1 + (2 + 2))) div 2) mod 2, y), x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (div (+ (mod (v2 x) (+ 1 (+ 2 2))) 1) 2) 2))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (div (mod (+ (v5 x) 1) (+ 1 (+ 2 2))) 2) 2))))
