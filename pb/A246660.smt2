;; sequence(s): A246660
;; terms: 1 1 1 2 1 1 2 6 1 1 1 2 2 2 6 24 1 1 1 2
;; small program: loop2((loop(((((loop2((y mod 2) + x, y div (2 - (x mod 2)), x, 1, x) div 2) + 1) * (2 - (x mod 2))) - 2) mod (1 + x), 1, y) * x) + x, y div 2, x, 1, x)
;; fast program: loop2((loop((((((loop(((loop2((y mod 2) + x, y div (2 - (x mod 2)), 2 * (2 + (2 + 2)), 1, x) div 2) + 1) + x, x mod 2, x) - x) + 1) - (x mod 2)) + x) - (x mod 2)) mod (1 + x), 1, y) * x) + x, y div 2, x, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g2 x y) (div y (- 2 (mod x 2))))))
...
(assert (forall ((x Int)) (= (f1 x) (mod (- (* (+ (div (w2 x) 2) 1) (- 2 (mod x 2))) 2) (+ 1 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (mod y 2) x))))
(assert (forall ((x Int) (y Int)) (= (g6 x y) (div y (- 2 (mod x 2))))))
(assert (= h6 (* 2 (+ 2 (+ 2 2)))))
(assert (= i6 1))
