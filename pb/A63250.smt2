;; sequence(s): A63250
;; terms: 0 0 1 0 2 2 1 0 3 3 3 3 2 2 1 0 4 4 4 4
;; small program: loop2(loop(if (loop(x + x, x, 2) mod (1 + x)) <= 0 then 1 else 2, 1, y) + x, y div 2, x, 0, x) - x
;; fast program: loop2(loop(if (loop((x * x) * x, 1, 2 + 2) mod (1 + x)) <= 0 then 1 else 2, 1, y) + x, y div 2, x, 0, x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (v2 x) (+ 1 x)) 0) 1 2))))
(assert (= g1 1))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (ite (<= (mod v5 (+ 1 x)) 0) 1 2))))
(assert (= g4 1))
