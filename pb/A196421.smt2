;; sequence(s): A196421
;; terms: 2 9 30 70 165 273 476 684 1035 1595 2046 2886 3731 4515 5640 7208 9027 10431 12730 14910
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + loop(x + y, x, x))
;; fast program: (((((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) - (if x <= 0 then 1 else 2)) + 2) * (1 + x)) + 1) + x) * (2 + x)) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (+ (v0 x) 1) (+ 1 (v3 x))))))
...
(assert (forall ((x Int)) (= (f4 x) (mod (- (mod (- (mod (* (* (* (* 2 (v5 x)) (+ 1 (mod x 2))) (v7 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1) (+ 1 x)) 1) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (+ (+ (* (+ (- (* (v4 x) 2) (ite (<= x 0) 1 2)) 2) (+ 1 x)) 1) x) (+ 2 x)) 2))))
