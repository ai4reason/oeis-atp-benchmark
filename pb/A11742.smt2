...
;; small program: (((loop(x div (1 + (x mod 2)), x, x) div 2) div loop(2 + (x + x), 2, 2)) + 1) div 2
;; fast program: (((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) div 2) div (1 + (2 + (2 + 2)))) + 1) div 2
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (small x) (div (+ (div (div (v0 x) 2) v1) 1) 2))))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (div (div (v2 x) 2) (+ 1 (+ 2 (+ 2 2)))) 1) 2))))
