;; sequence(s): A284389
;; terms: 1 2 4 5 9 10 12 13 15 16 20 21 25 26 30 31 33 34 36 37
;; small program: loop((loop(loop(if (x mod 2) <= 0 then (x div 2) else 2, y, x) + x, x + x, 2) - 1) - x, 1, x div 2) + x
;; fast program: (loop((compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 1 + 2, x), 1 - (x mod (2 + 2)), x) mod 2, x) - 1) - x, 1, x div 2) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (v3 x) x) 2))))
