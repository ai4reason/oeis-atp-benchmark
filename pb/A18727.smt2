;; sequence(s): A18727
;; terms: 1 2 3 5 6 10 15 30 31 62 93 155 186 310 465 930
;; small program: loop(compr(loop((x * x) + x, 2, 1 + (2 + 2)) mod (1 + x), y), x, 0) + 1
;; fast program: (compr((((2 * (2 * (2 * (2 + 2)))) - 1) * loop((x * x) + x, 1, 1 + (2 + 2))) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (- (* 2 (* 2 (* 2 (+ 2 2)))) 1) v4) (+ 1 x)))))
