;; sequence(s): A378
;; terms: 0 1 2 3 4 5 6 8 9 10 11 12 13 14 16 17 18 19 20 21
;; small program: loop(compr(if ((loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) + 1) mod (2 * (2 + 2))) <= 0 then 1 else 0, y), x, x)
;; fast program: loop(((if ((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) + 1) mod (2 * (2 + 2))) <= 0 then 1 else 0) + 1) + x, x, 1) - 1
(set-logic UFNIA)
(declare-fun f3 () Int)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ (v2 x) 1) (* 2 (+ 2 2))) 0) 1 0))))
...
(assert (forall ((x Int)) (= (f4 x) (+ (+ (ite (<= (mod (+ (v5 x) 1) (* 2 (+ 2 2))) 0) 1 0) 1) x))))
