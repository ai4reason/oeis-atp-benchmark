;; sequence(s): A55247
;; terms: 1 2 7 8 19 20 25 26 55 56 61 62 73 74 79 80 163 164 169 170
;; small program: (compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, (x div 2) div 2, x) mod 2, x) - (x mod 2)) + 1
;; fast program: (((loop((((loop((((((loop(((((loop((((((loop(((((loop2(x - y, y div 2, x, x, x) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x, 1, x div 2) + x) * 2) - 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (- (v0 x) (mod x 2)) 1))))
...
(assert (forall ((x Int)) (= (f2 x) (- (+ (* (+ (v3 x) x) (+ 1 2)) 1) x))))
(assert (= g2 1))
(assert (forall ((x Int)) (= (h2 x) (div x 2))))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 g2 (h2 x)))))
(assert (forall ((x Int)) (= (fast x) (- (- (* (+ (v2 x) x) 2) 1) x))))
