;; sequence(s): A94984
;; terms: 1 1 1 2 3 5 7 10 14 20 28 40 56 79 111 155 217 304 426 597
;; small program: loop(x div (1 + (2 + 2)), x, loop((2 * ((x + x) + x)) + x, x, 1))
;; fast program: loop2(x * y, y, x div 2, loop(1 + (2 + (2 + 2)), x mod 2, 1), (2 * (2 * (2 * (2 + (2 + 2))))) + 1) div loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (w2 x) (w4 x)))))
