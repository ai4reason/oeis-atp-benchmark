;; sequence(s): A112997
;; terms: 1 2 4 7 13 20 30 41 55 74 94 119 147 176 208 245 287 330 378 429
;; small program: loop((compr((loop(x + x, x, 1) - 1) mod (1 + x), y) - y) + x, x, 1)
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) + 1) - (((x * x) + x) div 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (+ (* (v3 x) 2) 1) (div (+ (* x x) x) 2)))))
