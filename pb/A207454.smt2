;; sequence(s): A207454
;; terms: 6 36 90 330 1008 3354 10710 34884 112530 364722 1179360 3817938 12352782 39978180 129366090 418647162 1354755024 4384104618 14187219750 45910873476
;; small program: ((2 * ((x mod 2) + loop(x + x, x, 2))) - 1) * loop2(x + y, x, x, 2, 2)
;; fast program: (loop(2 + (x + x), x mod 2, loop(x * x, 1, loop(x + x, x div 2, 2))) - 1) * loop2(x + y, x, x, 2, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (- (* 2 (+ (mod x 2) (v0 x))) 1) (w1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (* (- (v2 x) 1) (w5 x)))))
