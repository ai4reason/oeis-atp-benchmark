;; sequence(s): A267217
;; terms: 10 27 85 175 451 637 1105 1387 2047 3277 3751 5365 6601 7267 8695 11077 13747 14701 17755 19951
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) + 1) * loop((x - 1) + x, 2, x), y, 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop((x - 1) + x, 2, x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (+ (- x 1) x))))
(assert (= g8 2))
