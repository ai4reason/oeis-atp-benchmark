;; sequence(s): A267654
;; terms: 2 4 2 4 6 4 6 4 6 8 6 8 6 8 6 8 10 8 10 8
;; small program: loop(((if (y div (x * x)) <= 0 then (0 - 2) else 0) + 1) + x, x, 1) * 2
;; fast program: (((((loop((loop(loop2(loop(x + y, x, y) div x, y, 1 + 2, 1 + 2, x div 2) + x, 2, x) - 1) - x, 1, x div (2 + 2)) + x) div 2) * 2) - x) * 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (ite (<= (div y (* x x)) 0) (- 0 2) 0) 1) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (- (* (div (+ (v1 x) x) 2) 2) x) 2) 2))))
