;; sequence(s): A38347
;; terms: 3 10 21 40 63 94 137 184 243 310 381 460 543 646 753 880 1011 1150 1301 1464
;; small program: loop(((compr(((1 + (2 * loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) * 2) + 1) + x, 1 + x, 0)
;; fast program: ((loop(compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x) - 1, 1 + y) + x, x, 1) * 2) + 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (mod (+ 1 (* 2 (v2 x))) (+ 1 (+ x x))) 2))))
...
(assert (forall ((x Int)) (= (f6 x) (mod (- (v7 x) x) (+ 1 (+ x x))))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (* (v4 x) 2) 1) x))))
