;; sequence(s): A262869
;; terms: 0 1 1 2 2 3 3 3 3 4 4 5 5 6 6 6 6 6 6 7
;; small program: loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0, 1, y div 2) + x, x, 1) div 2
;; fast program: loop(loop(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x) <= 0 then 1 else 0, 1, y) + x, (x - 1) div 2, if x <= 0 then 0 else 1)
(set-logic UFNIA)
