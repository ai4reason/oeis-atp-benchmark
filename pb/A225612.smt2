;; sequence(s): A225612
;; terms: 1 5 33 253 2073 17577 152173 1336213 11854513 105997793 953658321 8622997453 78291531921 713305091521 6518037055321 59712126248041 548239063327621 5043390644753269 46475480410336709 428936432074181109
;; small program: loop(loop(loop2(x * y, y - 1, x, 1, 2 * (x + x)) div loop(x * y, x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, 1 + y, x - 1, 1, 2 + ((x + x) + x)) div loop(x * y, x, 1)) * (1 + ((x + x) + x)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (= i6 1))
(assert (forall ((x Int)) (= (j6 x) (+ 2 (+ (+ x x) x)))))
