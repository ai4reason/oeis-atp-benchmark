;; sequence(s): A11974
;; terms: 2 5 8 12 18 24 30 36 42 52 60 68 78 84 90 100 112 120 128 138
;; small program: (loop2(compr((2 - loop(x + x, x, 2)) mod (1 + x), y) + x, 1 + x, 2, x, x) - x) + (if x <= 0 then 1 else 2)
;; fast program: (loop2(compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, y) + x, 1 + x, 2, x, x) - x) + (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (w3 x) x) (ite (<= x 0) 1 2)))))
