;; sequence(s): A330349
;; terms: 2 13 101 1147 14999 255223 4849781 111546307 3234846359 100280244553 3710369066381 152125131761557 6541380665830919 307444891294237513 16294579238595005981 961380175077106286767
;; small program: loop((compr((1 + loop(x + x, x, x)) mod (1 + x), y) * x) + x, 1 + x, 1) - loop(x + x, x, 1)
;; fast program: loop(((compr(((1 + loop(x + x, x - 1, 2)) mod (1 + (x + x))) - 2, y) * 2) + 1) * x, 1 + x, 1) - loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (v4 x) (v7 x)))))
