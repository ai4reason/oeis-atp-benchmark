;; sequence(s): A167878
;; terms: 0 2 2 6 8 8 8 8 8 18 20 20 24 26 26 26 26 26 26 26
;; small program: loop((((loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x) + 1) mod 2) - 1) + x, x, x + x)
;; fast program: loop((loop((loop(if (loop2((x * y) + x, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) <= 0 then 1 else 0, 1, y) * x) + 1, x, 1) - 1) - x, 2, x) * 2
(set-logic UFNIA)
