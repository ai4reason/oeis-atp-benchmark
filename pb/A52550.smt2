;; sequence(s): A52550
;; terms: 1 1 4 11 35 108 337 1049 3268 10179 31707 98764 307641 958273 2984932 9297787 28961747 90213164 281005665 875306665
;; small program: loop(loop(loop2((((((y * y) - x) div ((x + y) + y)) + x) + x) + x, x + y, x, 1, 0), 1, y) - x, x, 1)
;; fast program: loop(loop(loop2((((((y - 2) * y) div ((x + y) + y)) + x) + x) + x, x + y, x - 1, if x <= 0 then 1 else 2, 1), 1, y) - x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (+ (div (- (* y y) x) (+ (+ x y) y)) x) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (+ (div (* (- y 2) y) (+ (+ x y) y)) x) x) x))))
