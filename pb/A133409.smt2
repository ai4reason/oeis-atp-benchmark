;; sequence(s): A133409
;; terms: 0 0 0 0 1 4 13 39 117 351 1054 3163 9490 28470 85410 256230 768691 2306074 6918223 20754669
;; small program: loop((x + x) + x, x, 1) div loop(2 * (2 * (2 + x)), 2, 1)
;; fast program: loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2))) div loop((x * x) - x, 1, 2 * (2 + 2))
(set-logic UFNIA)
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (fast x) (div (w2 x) v4))))
