;; sequence(s): A153218
;; terms: 0 1 2 4 5 6 9 10 11 12 15 16 17 20 22 24 25 26 29 31
;; small program: (compr((1 - loop((x * x) * x, 1, loop(x + x, x, 1))) mod (1 + ((x + x) + x)), 1 + x) div 2) - 1
;; fast program: compr(loop(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1, 1 + x), x)
(set-logic UFNIA)
