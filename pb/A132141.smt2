;; sequence(s): A132141
;; terms: 1 3 4 5 9 10 11 12 13 14 15 16 17 27 28 29 30 31 32 33
;; small program: loop(compr((1 + loop(loop(y, x div (1 + 2), x), x, x)) mod 2, y), x, 1)
;; fast program: (((loop(loop((1 + 2) * (1 + x), y - x, x), x, 0) + 1) + 2) div (1 + 2)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (+ 1 (v2 x)) 2))))
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 2) (+ 1 x)))))
(assert (forall ((x Int) (y Int)) (= (g5 x y) (- y x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (+ (+ (v4 x) 1) 2) (+ 1 2)) x))))
