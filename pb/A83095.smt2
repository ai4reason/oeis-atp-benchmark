;; sequence(s): A83095-A83097
;; terms: 0 2 5 6 14 15 18 20 41 42 45 47 54 56 59 60 122 123 126 128
;; small program: loop(compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, 2 * (x + x)), y), x, 0)
;; fast program: (((loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 2) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) div 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod x 2) 0) (div x (+ 1 2)) 1))))
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (- (* (+ (v4 x) x) (+ 1 2)) 2) 2) x) x))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (div x 2))))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (div (* (+ (v3 x) x) (+ 1 2)) 2) 2))))
