;; sequence(s): A16280
;; terms: 1 16 195 2210 24521 270396 2976415 32746870 360234741 3962640176 43589217035 479481914730 5274302648161 58017333896356 638190687176055 7020097601917790 77221073750104781 849431811638310936
;; small program: loop(((loop2((loop((2 + y) * x, 2, x) - x) + y, y + y, y, 1, 2) + x) + x) + x, x, 1)
;; fast program: (loop((loop2((x + x) + y, (2 * ((2 * (y + y)) + y)) + y, x, 1, 1) - loop2(x * y, y, x div 2, loop(1 + 2, x mod 2, 1), 1 + (2 * (2 + 2)))) div 2, 1, 2 + x) div 2) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ x x) y))))
(assert (forall ((x Int) (y Int)) (= (g4 x y) (+ (* 2 (+ (* 2 (+ y y)) y)) y))))
...
(assert (forall ((x Int)) (= (f3 x) (div (- (w4 x) (w5 x)) 2))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (div (div (v3 x) 2) 2))))
