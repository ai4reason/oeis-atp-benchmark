;; sequence(s): A28160
;; terms: 1 35 785 14395 235401 3578595 51754945 722167115 9811679801 130610749555 1711051941105 22130751379035 283285847668201 3595414758854915 45309603386047265 567596497380674155
;; small program: loop((loop(loop(((loop(1 + (x + x), y, 1) + x) + x) + x, x, 1) * loop(x + x, x + x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
;; fast program: loop((loop(((loop2((2 * (x + y)) + x, y + y, x, 1, 2 + 2) div 2) + 1) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (v1 x y) x) (* (+ (* (+ x x) 2) x) 2)))))
...
(assert (forall ((x Int)) (= (f6 x) (* (+ (div (w7 x) 2) 1) (v8 x)))))
(assert (= g6 1))
