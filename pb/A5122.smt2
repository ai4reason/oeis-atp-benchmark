;; sequence(s): A5122
;; terms: 1 3 4 6 9 10 13 16 19 21 24 25 28 30 33 34 39 45 46 48
;; small program: compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1, (((x * 2) * 2) * 2) - 2), 1 + x)
;; fast program: (compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) + x) mod (1 + (x + x)), x mod 2, x), 1 + x) + 1) div (2 + 2)
(set-logic UFNIA)
...
(assert (= g1 1))
(assert (forall ((x Int)) (= (h1 x) (- (* (* (* x 2) 2) 2) 2))))
