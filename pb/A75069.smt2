;; sequence(s): A75069
;; terms: 6 60 15120 8648640 14079294028800 32382376266240000 415017197290314178560000 2149789081963827444940800000 106425394494182556214892101632000000 132923807095642467606593507268303331000320000000
;; small program: loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) + 1) * loop2(x * y, 1 + y, x - 1, 1, 1 + x), y, 2, 1, x)
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) + 1) * loop2(x * y, 1 + y, x - 1, 1, 1 + x), y, 2, 1, x)
(set-logic UFNIA)
