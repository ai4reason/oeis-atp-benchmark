;; sequence(s): A308833
;; terms: 1 7 8 13 14 19 20 23 24 25 26 31 32 33 34 37 38 43 44 47
;; small program: loop(compr((loop(x * y, x, 2) div (1 + x)) mod (2 + x), y), x, 1)
;; fast program: loop(compr(loop(((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1, (x mod 2) + x), y), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (div (v2 x) (+ 1 x)) (+ 2 x)))))
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ (mod x 2) x))))
