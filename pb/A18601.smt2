;; sequence(s): A18601
;; terms: 1 3 5 15 47 141 235 705
;; small program: loop(compr(loop(1 - ((y * y) * x), 2 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (loop(compr((1 + loop(2 + (2 + (x * x)), 2, 2 + x)) mod (1 + (x + x)), y), x, 0) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (+ 1 (v5 x)) (+ 1 (+ x x))))))
