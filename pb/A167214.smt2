;; sequence(s): A167214
;; terms: 2 10 30 68 140 246 406 616 900 1290 1760 2364 3094 3934 4920 6096 7480 9018 10792 12780
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y) + x, x, 2) + x) * (1 + x)
;; fast program: ((loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 1) * 2) + x) * (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v3 x) 2) x) (+ 1 x)))))
