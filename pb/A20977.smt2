;; sequence(s): A20977
;; terms: 1 28 525 8240 116921 1555428 19795525 244020280 2937425841 34724726828 404763120125 4666104381120 53319258206761 604990586218228 6825603208406325 76653180504610760 857610367436699681 9565794278591213628 106430473594100486125
;; small program: loop((loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop((loop2(x * y, y, 1 + x, 1, 1 + (2 + (2 * (2 + 2)))) - loop((x * x) * x, 1, loop(x + x, x, 2))) div (1 + 2), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (div (- (w6 x) (v7 x)) (+ 1 2)))))
(assert (= g5 1))
