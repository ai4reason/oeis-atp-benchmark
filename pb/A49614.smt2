;; sequence(s): A49614
;; terms: 1 1 1 1 4 4 24 24 192 1728 17280 17280 207360 207360 2903040 43545600 696729600 696729600 12541132800 12541132800
;; small program: loop((loop(if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then 0 else x, 1, y) * x) + x, x - 1, 1)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else (1 + x), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 1 (v2 x)) (+ 1 x)) 0) 0 x))))
(assert (= g1 1))
