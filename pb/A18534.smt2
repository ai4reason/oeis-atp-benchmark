;; sequence(s): A18534
;; terms: 1 2 3 4 6 7 12 14 21 28 42 49 84 98 147 196 294 588
;; small program: compr((2 * (2 + loop(2 * (2 + (x * x)), 2, 2))) mod (1 + x), x) + 1
;; fast program: compr((2 * loop((((2 + x) * x) * x) + x, 1, 2 + (2 + 2))) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (f0 x) (mod (* 2 (+ 2 v1)) (+ 1 x)))))
