;; sequence(s): A73425
;; terms: 0 2 3 4 4 4 5 6 6 6 7 8 8 8 9 9 9 9 9 10
;; small program: loop(compr(loop(x * y, x, 2) mod (2 + x), y) + 1, x, 0) - x
;; fast program: compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x) mod (1 + x), x) - x
(set-logic UFNIA)
