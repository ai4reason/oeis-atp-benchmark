;; sequence(s): A25986
;; terms: 1 20 257 2704 25389 221676 1841449 14758568 115171397 880623172 6627177921 49248518592 362296167325 2643446894108 19158543110873 138087153497176 990728497913973 7081081591668084
;; small program: loop((loop((loop2((2 * ((x + x) + x)) + y, y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x + x, x - 1, 2), 1, y) + (2 * (x + x))) + x, y, 1) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (v4 x y) (* (+ (+ x x) x) 2)) x))))
