;; sequence(s): A223906
;; terms: 17 51 161 531 1817 6411 23201 85731 322217 1227771 4729841 18379731 71908217 282817131 1116854081 4424238531 17567551817 69882262491 278365739921 1109974078131
;; small program: loop((x + x) + x, 2 + x, 1) + loop((2 + x) * x, 1, loop(x + x, x, 2))
;; fast program: ((1 + 2) * loop2(x * y, y, x div 2, loop(1 + (2 * (2 + 2)), x mod 2, 1 + 2), 1 + (2 * (2 + 2)))) + loop((2 + x) * x, 1, loop(x + x, x, 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ 1 2) (w3 x)) (v5 x)))))
