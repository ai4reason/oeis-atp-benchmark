;; sequence(s): A229702
;; terms: 1 10 70 440 2675 16106 96720 580440 3482805 20897050 125382586 752295880 4513775735 27082654970 162495930500 974975583816 5849853503865 35099121024330 210594726147310 1263568356885400
;; small program: loop(loop(loop(x + y, y, x), 1 + y, 2 * ((x + x) + x)), x, 1)
;; fast program: loop(loop((loop((x * x) * x, 1, 2 + x) - x) div (2 + (2 + 2)), 1, y) + (2 * ((x + x) + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (div (- (v5 x) x) (+ 2 (+ 2 2))))))
(assert (= g4 1))
