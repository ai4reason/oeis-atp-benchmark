;; sequence(s): A94995
;; terms: 1 1 1 2 3 4 6 9 12 17 24 33 45 62 86 118 163 224 308 424
;; small program: loop(((x div 2) div 2) div 2, x, loop(loop((2 + y) * x, 2, x) - x, x, 1))
;; fast program: ((loop2(x * y, y, x - 1, 1, 1 + (2 + (2 * (2 + 2)))) * (if x <= 0 then 1 else (1 + (2 + (2 * (2 + 2)))))) div loop(x * x, 1, loop(x + x, x, 1))) div loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (div (* (w3 x) (ite (<= x 0) 1 (+ 1 (+ 2 (* 2 (+ 2 2)))))) (v4 x)) (v6 x)))))
