;; sequence(s): A5381
;; terms: 9 10 15 16 21 22 25 26 27 28 33 34 35 36 39 40 45 46 49 50
;; small program: compr((loop(x * y, x, 1) mod ((1 + x) * (2 + x))) - 1, 2 + x) + 2
;; fast program: (loop((((compr(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, x) * 2) + 1) - x) - x, 1, x div 2) + x) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (- (mod (v1 x) (* (+ 1 x) (+ 2 x))) 1))))
...
(assert (forall ((x Int)) (= (f2 x) (- (- (+ (* (v3 x) 2) 1) x) x))))
(assert (= g2 1))
