;; sequence(s): A152117
;; terms: 8 21 43 83 133 197 271 359 497 631 785 977 1135 1307 1553 1851 2101 2371 2693 2953
;; small program: loop2((compr(loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * (1 + y)) + x, 1 + y, 2, 1, x) + (2 + (x + x))
;; fast program: loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2, 1 + y) * (1 + y)) + x, 1 + y, 2, 1, x) + (2 + (x + x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (w4 x) (+ 2 (+ x x))))))
