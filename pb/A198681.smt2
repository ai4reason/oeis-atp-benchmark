;; sequence(s): A198681
;; terms: 3 9 24 27 42 48 60 66 72 81 96 102 114 120 126 138 144 159 168 174
;; small program: loop(compr(loop2(x + y, y div (1 + 2), 1 + x, 2, x) mod (1 + 2), x), 2, x)
;; fast program: ((((loop2(x - y, y div (1 + 2), 2 + 2, 1, x) mod (1 + 2)) + x) + x) + x) * (1 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (+ (mod (w3 x) (+ 1 2)) x) x) x) (+ 1 2)))))
