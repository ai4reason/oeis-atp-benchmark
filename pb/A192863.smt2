;; sequence(s): A192863
;; terms: 3 5 7 9 11 13 15 17 21 23 25 27 29 31 33 35 39 41 43 45
;; small program: ((compr(loop(loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then (1 + x) else 2, 1, y) * x, 1 + x, 1) mod (1 + x), x) * 2) + 1) + 2
;; fast program: ((((compr(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 2, 1, y) * x, x, 1) mod (1 + x), x) * 2) + 2) div 2) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (ite (<= (mod (+ 1 (v3 x)) (+ 1 x)) 0) (+ 1 x) 2))))
(assert (= g2 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (div (+ (* (v4 x) 2) 2) 2) 2) 1))))
