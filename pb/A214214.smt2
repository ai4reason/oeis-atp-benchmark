;; sequence(s): A214214
;; terms: 1 3 5 9 11 15 19 21 23 27 31 35 39 41 43 45 47 51 55 59
;; small program: loop(loop(((if ((loop(x * y, x, 1) div (1 + x)) mod 2) <= 0 then 1 else x) + x) div 2, x, y) + x, x, 1) + x
;; fast program: ((loop(loop((loop(x div loop(1, 1 + (2 - x), 2), 1 + (2 + 2), x) + 1) mod 2, 1, y) + x, x - 1, 1) + x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (+ (ite (<= (mod (div (v2 x) (+ 1 x)) 2) 0) 1 x) x) 2))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (v3 x) x) 2) 1))))
