;; sequence(s): A83726
;; terms: 0 3 8 18 32 60 84 126 160 216 300 352 456 546 616 720 864 1020 1116 1292
;; small program: (loop(compr((2 - loop(x + x, x, 2)) mod (1 + x), y), x, 0) + 2) * x
;; fast program: (compr(loop(loop((((((((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else x) mod 2) + 1) mod (1 + 2)) - 1) mod (1 + 2)) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, (1 + x) mod 2, x) - 1, x) + 2) * (if (1 + x) <= 0 then 0 else x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (v3 x) 2) (ite (<= (+ 1 x) 0) 0 x)))))
