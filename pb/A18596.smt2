;; sequence(s): A18596
;; terms: 1 3 7 9 11 21 33 63 77 99 231 693
;; small program: loop(compr(loop((2 * ((2 + y) * x)) + x, 1 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(((1 + 2) * loop((x * x) + x, 1, 1 + (2 * (2 + (2 * (2 + 2)))))) mod (1 + (x + x)), x) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* 2 (* (+ 2 y) x)) x))))
(assert (= g2 (+ 1 2)))
(assert (= h2 1))
...
(assert (= g4 1))
(assert (= h4 (+ 1 (* 2 (+ 2 (* 2 (+ 2 2)))))))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y))))))
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (f3 x) (mod (* (+ 1 2) v4) (+ 1 (+ x x))))))
