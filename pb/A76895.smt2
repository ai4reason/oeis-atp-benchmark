;; sequence(s): A76895
;; terms: 1 1 2 3 3 4 4 5 6 7 7 8 9 10 10 11 11 12 12 13
;; small program: loop(loop(loop((y mod 2) - x, x, 1), y, y) + x, x + x, 1) - x
;; fast program: loop(loop((loop(((loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) - 1) - 2) mod (1 + x), x mod 2, 1 + x) + x) mod 2, 1, y) + x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (mod (- (- (v6 x) 1) 2) (+ 1 x)))))
