;; sequence(s): A152241
;; terms: 216 3375 42875 456533 2924207 10793861 33698267 83453453 296740963 726572699 1509003523 3491055413 5479701947 8254655261 15456856771 30576209383 46617130799 68267486503 107646386093 139233463487
;; small program: loop2(loop(loop2((compr((1 + loop(x * y, x, 1)) mod (1 + x), 1 + y) * x) + x, 1 + y, 2, 1, x), 1, y) * (x * x), y, 2, 1, x)
;; fast program: loop2(loop(loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, 1 + y, 2, 1, x), 1, y) * (x * x), y, 2, 1, x)
(set-logic UFNIA)
