;; sequence(s): A24892
;; terms: 2 4 6 10 12 14 20 22 24 26 32 34 36 42 46 50 52 54 60 64
;; small program: compr((1 - loop((x * x) * x, 1, loop(x + x, x, 1))) mod (1 + ((x + x) + x)), 1 + x)
;; fast program: (compr(((loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 2, 1, (x + x) + x) + 1) mod 2) mod (1 + x), 1 + x) * 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (- 1 (v1 x)) (+ 1 (+ (+ x x) x))))))
...
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (mod (+ 1 (v5 x)) (+ 1 (+ x x))) 2) 0) 1 2))))
(assert (= g4 1))
