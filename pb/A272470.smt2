;; sequence(s): A272470
;; terms: 14 21 35 49 77 91 119 133 161 203 217 259 287 301 329 371 413 427 469 497
;; small program: loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, 1 + 2, 2, 1, x)
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * (1 + (2 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) (+ 1 (+ 2 (+ 2 2)))))))
