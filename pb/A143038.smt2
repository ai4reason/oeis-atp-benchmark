;; sequence(s): A143038
;; terms: 1 1 2 2 3 4 4 6 7 8 8 12 14 15 16 16 24 28 30 31
;; small program: (loop(1 + ((loop((x mod y) + y, x, 0) + x) div 2), x, 1) + 1) div 2
;; fast program: (loop((loop(((((1 + loop((x mod y) + y, x div (2 + 2), 0)) * (if (x - 1) <= 0 then 1 else 2)) * (if x <= 0 then 1 else 2)) - 1) - x, 1, x div 2) + 1) + x, x, 1) + 1) div 2
(set-logic UFNIA)
