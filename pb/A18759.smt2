;; sequence(s): A18759
;; terms: 1 2 17 29 34 58 493 986
;; small program: loop(compr(loop(2 + (2 * (2 * (x * y))), 1 + 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((((2 * (2 + (2 + 2))) + x) * x) - 2, 2, 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (* (+ (* 2 (+ 2 (+ 2 2))) x) x) 2))))
(assert (= g5 2))
(assert (= h5 2))
