;; sequence(s): A78916
;; terms: 4 7 11 15 21 25 31 35 41 49 53 61 67 71 77 85 93 97 105 111
;; small program: (((compr((1 + loop(x + x, x, x)) mod (1 + x), x) + 1) + loop(y, x, 1)) + 2) + x
;; fast program: ((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + 2) + (if x <= 0 then 1 else x)) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (v0 x) 1) (v2 x)) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (* (v3 x) 2) 1) 2) (ite (<= x 0) 1 x)) x))))
