;; sequence(s): A118113
;; terms: 1 3 5 9 11 17 19 21 33 35 37 41 43 65 67 69 73 75 81 83
;; small program: (loop(compr(loop2((((y div 2) * y) mod 2) + x, y div 2, x, 0, x), y), x, 0) * 2) + 1
;; fast program: (compr(loop(((loop2((((y div 2) * x) * y) + x, y div 2, 1 + (2 * (2 + 2)), 1, x) mod 2) + x) mod (1 + x), x mod 2, x), 1 + x) div 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (mod (* (div y 2) y) 2) x))))
