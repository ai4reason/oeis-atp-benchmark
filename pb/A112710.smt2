;; sequence(s): A112710
;; terms: 1 ~2 16 ~119 1015 ~9191 87037 ~851186 8531044 ~87167702 904619302 ~9509144240 101036961052 ~1083385595648 11708378016712 ~127402051267703 1394629704432367 ~15347719608268403 169699299111055897 ~1884322608673443833
;; small program: loop(loop(((2 * ((2 * (x * y)) - x)) div (1 + y)) * (1 + 2), y, 1) - x, x, 1) * loop(0 - x, x, 1)
;; fast program: loop(loop((loop(2 * ((x - (x div y)) + x), x, 1) div (1 + x)) * loop2(x * y, y, x, 1, 0 - (1 + 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (div (* 2 (- (* 2 (* x y)) x)) (+ 1 y)) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f4 x) (* (div (v5 x) (+ 1 x)) (w6 x)))))
(assert (= g4 1))
