;; sequence(s): A113850
;; terms: 32 243 3125 7776 16807 100000 161051 371293 537824 759375 1419857 2476099 4084101 5153632 6436343 11881376 20511149 24300000 28629151 39135393
;; small program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) * x) + x, y, 1 + (2 + 2), 1, x)
;; fast program: loop2((compr(loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x, 1 + y) + 1) * (((x * x) * x) * x), y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ (v4 x y) 1) (* (* (* x x) x) x)))))
