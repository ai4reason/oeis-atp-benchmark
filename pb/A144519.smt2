;; sequence(s): A144519
;; terms: 6 15 28 66 91 153 190 276 435 496 703 861 946 1128 1431 1770 1891 2278 2556 2701
;; small program: loop2(((compr((1 + loop(x + x, x, x)) mod (2 + x), y) + 2) * x) + 1, y, 2, 1, x) div 2
;; fast program: (loop2((loop((compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, 1 + x) * 2) + 1, 1, y) * x) + 1, y, 2, 1, x) div 2) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (* (+ (v1 x y) 2) x) 1))))
...
(assert (forall ((x Int)) (= (fast x) (div (div (w3 x) 2) 1))))
