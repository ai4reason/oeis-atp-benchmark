;; sequence(s): A3525
;; terms: 1 3 43 127 129 381 5461 16383
;; small program: loop(compr((1 + loop(x + x, loop(x * x, 2, 2) - 2, x)) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((loop(loop((x * x) * x, 1, x * x) * x, 1, 2 + 2) + x) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
