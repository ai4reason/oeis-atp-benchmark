;; sequence(s): A286047
;; terms: 2 4 6 7 10 12 13 16 18 20 22 23 26 28 30 31 34 36 38 39
;; small program: (((compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 2 + (x + x)) mod 2) + 1) + x) + x
;; fast program: (((((loop((((loop2((y - x) div 2, x, 1 + (2 + (2 + 2)), x div 2, x) + x) div 2) mod 2) + x, 2, x) + 1) + x) mod 2) + 1) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (mod (+ (+ (v2 x) 1) x) 2) 1) x) x))))
