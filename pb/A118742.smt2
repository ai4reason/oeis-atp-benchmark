;; sequence(s): A118742
;; terms: 0 5 7 8 9 11 13 14 15 17 19 20 21 23 24 25 26 27 29 31
;; small program: loop(compr(loop(x * y, x, 1) mod (1 + x), y), x, 0)
;; fast program: compr(loop((if (loop((1 - loop(x + x, x div 2, 1)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 1) - 1, x div 2, x), 1 + x) * (if x <= 0 then 0 else 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (v3 x) (ite (<= x 0) 0 1)))))
