;; sequence(s): A183868
;; terms: 3 5 7 8 9 11 12 14 15 16 17 19 20 21 23 24 25 26 27 29
;; small program: loop(loop(((y div x) + x) div 2, x, 1) + x, 2, 1 + x)
;; fast program: loop(((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2, (x - 1) div 2) + 1) + 2) + x, 2, x) - 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f2 x) (+ (+ (+ (w3 x) 1) 2) x))))
(assert (= g2 2))
