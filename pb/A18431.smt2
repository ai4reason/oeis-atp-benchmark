;; sequence(s): A18431
;; terms: 1 2 3 4 6 9 11 12 18 22 33 36 44 66 99 132 198 396
;; small program: compr(loop(((x + y) * x) + x, 2 + 2, 1) mod (1 + x), x) + 1
;; fast program: compr(loop((2 + ((x * x) * x)) * (2 + x), 1, 2 + 2) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (* (+ 2 (* (* x x) x)) (+ 2 x)))))
(assert (= g3 1))
(assert (= h3 (+ 2 2)))
