;; sequence(s): A129768
;; terms: 1 1 1 1 2 2 3 3 4 6 6 8 9 9 10 12 14 14 16 17
;; small program: ((compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) div 2) - x) + 1
;; fast program: (((((compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod loop(1 + ((2 + x) * x), 2 + 2, 2)) + 1) - x) - x) div 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (- (div (v0 x) 2) x) 1))))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (fast x) (div (- (- (+ (mod (+ (v2 x) 1) v5) 1) x) x) 2))))
