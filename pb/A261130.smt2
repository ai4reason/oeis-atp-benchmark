;; sequence(s): A261130
;; terms: 1 2 3 5 35 7 77 143 143 2431 46189 4199 96577 7429 7429 215441 6678671 392863 392863 765049
;; small program: loop2(((loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) * x) * y) + x, 1 + y, x, 1, x)
;; fast program: loop2(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then (1 + x) else 1, 1, y) * x, 1 + y, x - (1 - (x mod 2)), 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (- x (- 1 (mod x 2))))))
(assert (= i3 1))
