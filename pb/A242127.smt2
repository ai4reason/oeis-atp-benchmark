;; sequence(s): A242127
;; terms: 2 3 5 7 11 13 17 19 23 0 2 8 12 14 18 24 1 3 9 13
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (2 + loop((x * x) + y, 2, 2))
;; fast program: (compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (2 + loop((x * x) * x, 1, 1 + 2))
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 1) (+ 2 v2)))))
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (mod (+ (v3 x) 1) (+ 2 v7)))))
