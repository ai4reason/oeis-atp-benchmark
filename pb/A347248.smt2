;; sequence(s): A347248
;; terms: 2 3 4 5 6 7 8 10 11 12 13 14 15 16 17 19 20 21 22 23
;; small program: compr(loop(loop(if ((1 + loop(x + x, x, x)) mod (1 + x)) <= 0 then (1 + x) else 2, 1, y) * x, x, 1) mod (1 + x), 1 + x) + 1
;; fast program: compr(loop(loop(if (loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then (1 + x) else 2, 1, y) * x, x, 1) mod (1 + x), 1 + x) + 1
(set-logic UFNIA)
