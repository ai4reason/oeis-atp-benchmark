;; sequence(s): A291681
;; terms: 1 8 0 25 ~7 56 ~24 105 ~55 176 ~104 273 ~175 400 ~272 561 ~399 760 ~560 1001
;; small program: loop(loop((if (x mod 2) <= 0 then (x div 2) else x) * x, 1, 2 + y) - x, x, 1)
;; fast program: loop(loop(loop(x div 2, 1 - (x mod 2), x) * x, 1, 2 + y) - x, x, 1)
(set-logic UFNIA)
