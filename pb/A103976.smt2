;; sequence(s): A103976
;; terms: 0 1 4 9 18 29 44 61 82 109 138 173 212 253 298 349 406 465 530 599
;; small program: loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y) + x, x, 0) - x
;; fast program: (loop(compr(((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2, y) + x, x, 0) * 2) - x
(set-logic UFNIA)
