;; sequence(s): A213258
;; terms: 4 12 20 28 32 36 44 52 60 68 76 84 92 96 100 108 116 124 132 140
;; small program: ((loop(compr(loop(loop((if (x mod 2) <= 0 then (2 + (2 + 2)) else x) div 2, 1 + 2, x), 2, x), y), x, 0) * 2) + 2) * 2
;; fast program: (loop(((loop((loop(x div loop(1, x mod (2 * (2 + 2)), 2 * (2 + 2)), x mod (2 + 2), 1 + x) mod 2) + x, (x mod (2 + 2)) - 2, x) mod 2) + 1) + x, x, 1) * 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (div (ite (<= (mod x 2) 0) (+ 2 (+ 2 2)) x) 2))))
(assert (= g3 (+ 1 2)))
