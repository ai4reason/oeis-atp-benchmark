;; sequence(s): A63960
;; terms: 0 0 0 2 2 5 5 5 5 10 10 10 10 17 17 17 17 17 17 17
;; small program: loop(loop(if ((1 + loop(x * y, x, 1)) mod (1 + x)) <= 0 then (1 + x) else 0, 1, y) + x, (x - 1) div 2, 0)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then (1 + x) else 0, 1, y) + x, (x - 1) div 2, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (ite (<= (mod (+ 1 (v2 x)) (+ 1 x)) 0) (+ 1 x) 0))))
(assert (= g1 1))
