;; sequence(s): A28060
;; terms: 1 24 370 4680 53011 560784 5670820 55595760 533237221 5034115944 46976235670 434592270840 3994583040631 36537064953504 332949559310920 3025434129325920 27431505237783241 248303640811921464
;; small program: loop(((loop((loop((loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop(((loop(loop((((loop2(x * y, y, x, 1, 1 + (2 + 2)) + loop2(x * y, y, x, 1, 1 + (2 * (2 + 2)))) div 2) - loop2(x * y, y, x, 1, 1 + (2 + (2 + 2)))) div 2, 1, 2 + x) div 2, 1, y) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (v3 x y) (* 2 (+ (+ x x) x))) x))))
