;; sequence(s): A18563
;; terms: 1 2 11 22 29 58 319 638
;; small program: loop(compr(loop(1 + loop((x + x) + x, 2, x), 1 + 2, x) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr(loop((2 * (((2 + (2 + (2 - x))) * x) + x)) + x, 2, 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (* 2 (+ (* (+ 2 (+ 2 (- 2 x))) x) x)) x))))
(assert (= g6 2))
(assert (= h6 2))
