;; sequence(s): A126684
;; terms: 0 1 2 4 5 8 10 16 17 20 21 32 34 40 42 64 65 68 69 80
;; small program: loop(compr(loop2((y mod 2) + x, y div (2 + (x + x)), x, 0, x) - 1, y), x, 0)
;; fast program: compr((loop2((y mod 2) + x, y div (2 + (if x <= 0 then 0 else 2)), x, 0, x) - 1) mod (1 + x), x) * 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g2 x y) (div y (+ 2 (+ x x))))))
...
(assert (forall ((x Int) (y Int)) (= (g4 x y) (div y (+ 2 (ite (<= x 0) 0 2))))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (- (w4 x) 1) (+ 1 x)))))
