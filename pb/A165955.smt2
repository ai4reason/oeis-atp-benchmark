;; sequence(s): A165955
;; terms: 1 13 21 29 35 39 47 51 57 65 71 75 81 85 93 97 103 111 115 121
;; small program: ((loop(compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y) + 1, x, 0) + x) * 2) + 1
;; fast program: ((((compr(((((loop((((loop(((if (((1 + loop(x + x, x div 2, 1)) mod (1 + x)) - 2) <= 0 then 1 else 2) mod 2) + 2, (1 + x) mod 2, x) - x) mod 2) mod (1 + x)) + 2, 2, x) + 1) div 2) mod 2) - (x mod 2)) mod 2, x) div 2) + (if x <= 0 then 1 else 2)) + x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (+ (div (v3 x) 2) (ite (<= x 0) 1 2)) x) 2) 1))))
