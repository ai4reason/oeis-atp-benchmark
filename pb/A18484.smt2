;; sequence(s): A18484
;; terms: 1 2 3 4 6 12 41 82 123 164 246 492
;; small program: loop(compr((2 * (2 * loop(2 + (x * x), 2, 1 + 2))) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((2 * (2 * (loop((x * x) * x, 1, 1 + (2 + 2)) - 2))) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* 2 (* 2 v2)) (+ 1 x)))))
...
(assert (= v5 (u5 g5 h5)))
(assert (forall ((x Int)) (= (f4 x) (mod (* 2 (* 2 (- v5 2))) (+ 1 x)))))
