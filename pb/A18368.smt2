;; sequence(s): A18368
;; terms: 1 5 11 25 55 275
;; small program: loop(compr(loop(1 + (2 + ((2 + x) * y)), 2 + 2, 1) mod (1 + x), y), x, 0) + 1
;; fast program: ((((((((loop((2 * (x * y)) - x, x, 1) div loop(x + x, x div 2, 1)) + (x * x)) div 2) * 2) + 1) - (x mod 2)) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (- (+ (* (div (+ (div (v3 x) (v4 x)) (* x x)) 2) 2) 1) (mod x 2)) x) x) x))))
