;; sequence(s): A52254
;; terms: 1 17 108 444 1410 3762 8844 18876 37323 69355 122408 206856 336804 531012 813960 1217064 1780053 2552517 3595636 4984100
;; small program: loop(loop(loop(loop(loop((((2 * ((y - 2) + y)) + y) * y) + x, y, x), y, x), y, x), y, x), 1 + x, 0)
;; fast program: loop(loop(loop((((2 * (x + x)) + x) div y) + x, x, 1 + (2 + ((2 * (x + x)) + x))) div (1 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* (+ (* 2 (+ (- y 2) y)) y) y) x))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (div (+ (* 2 (+ x x)) x) y) x))))
(assert (forall ((x Int)) (= (g7 x) x)))
(assert (forall ((x Int)) (= (h7 x) (+ 1 (+ 2 (+ (* 2 (+ x x)) x))))))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v7 x) (u7 (g7 x) (h7 x)))))
(assert (forall ((x Int)) (= (f6 x) (div (v7 x) (+ 1 2)))))
(assert (= g6 1))
