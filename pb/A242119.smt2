;; sequence(s): A242119
;; terms: 2 3 5 7 11 13 17 1 5 11 13 1 5 7 11 17 5 7 13 17
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod (2 + loop(x * x, 2, 2))) + 1
;; fast program: (compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod compr((x - 2) mod (2 + 2), 2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (mod (- x 2) (+ 2 2)))))
(assert (= g6 (+ 2 2)))
