;; sequence(s): A242125
;; terms: 2 3 5 7 11 13 17 19 23 2 4 10 14 16 20 26 5 7 13 17
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) mod loop((x * x) + y, 2, 2)) + 1
;; fast program: ((compr(loop(((loop2(x + y, x, x, 1, 2) mod loop((x * x) * x, 1, 1 + x)) - 1) mod (1 + x), (1 + x) mod 2, x) - 1, 1 + x) + 1) mod loop((x * x) * x, 1, 1 + 2)) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (mod (- (mod (w5 x) (v6 x)) 1) (+ 1 x)))))
...
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (div (mod (+ (v3 x) 1) v7) 1))))
