;; sequence(s): A145129
;; terms: 1 5 18 56 155 386 876 1836 3597 6655 11726 19812 32279 50948 78200 117096 171513 246297 347434 482240
;; small program: loop(loop((loop((((2 * (x + x)) + x) div y) + x, y - 1, 1) + x) + y, y, x) + y, x, 1) + x
;; fast program: loop(loop(loop((((loop((2 + x) * (((x * x) * x) - x), 1, 2 + x) div (2 * (2 * (2 + (2 + 2))))) * x) div (1 + (2 + 2))) + x, 1, y) + x, y, x) + y, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (+ (div (* (div (v6 x) (* 2 (* 2 (+ 2 (+ 2 2))))) x) (+ 1 (+ 2 2))) x))))
(assert (= g5 1))
