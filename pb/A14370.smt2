;; sequence(s): A14370
;; terms: 1 2 4 5 7 10 11 13 16 20 21 23 26 30 35 36 38 41 45 50
;; small program: loop(loop(loop(y, (x - y) - 1, x), x, y) + x, x, 1)
;; fast program: loop(loop(loop(x - (if (y - x) <= 0 then y else 0), x - (x div 2), x), 1, y) + x, x, 1) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g2 x y) (- (- x y) 1))))
