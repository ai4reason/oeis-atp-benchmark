;; sequence(s): A28163
;; terms: 1 36 829 15576 260425 4039212 59479093 843439392 11625297409 156744987828 2076870835117 27134173366248 350447396932153 4483154549898684 56894676264296101 717171756670960944
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + x) + (((x + x) * 2) * 2), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (* (div (w10 x) 2) (v11 x)))))
(assert (= g9 1))
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (v9 x y) x) (* (* (+ x x) 2) 2)))))
