;; sequence(s): A10068
;; terms: 1 2 4 8 11 17 24 28 36 37 39 43 46 52 59 68 76 82 89 98
;; small program: loop((loop(loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x div (2 + 2), x) - 1, x, x) + x) + x, x, 1)
;; fast program: loop((loop(loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x div (2 + (2 + 2)), x) - 1, x, x) + x) + x, x, 1)
(set-logic UFNIA)
(declare-fun f3 () Int)
...
(assert (forall ((x Int)) (= (g6 x) (div x (+ 2 (+ 2 2))))))
