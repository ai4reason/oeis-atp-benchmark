;; sequence(s): A18577
;; terms: 1 2 7 14 47 94 329 658
;; small program: loop(compr(loop(2 + (2 * (2 + (2 + (x * x)))), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: loop(compr((loop((1 + (2 * (x * x))) * ((x * x) + x), 1, 2 + 2) - 2) mod (1 + x), y), x, 0) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (+ 1 (* 2 (* x x))) (+ (* x x) x)))))
(assert (= g5 1))
(assert (= h5 (+ 2 2)))
