;; sequence(s): A127847
;; terms: 0 1 20 1344 348160 357564416 1465657589760 24017731997138944 1574098141758535761920 412645105639632468417970176 432690992231222540584116394393600 1814838857553600260569028862094736359424 30447948962887853884238295543591323273401466880
;; small program: loop(x + x, (x * x) - x, loop(1 + (2 * (x + x)), x, 0))
;; fast program: ((loop(x * x, 1, loop(x + x, x, 1)) div (1 + 2)) * loop2(x * y, y + y, x - 1, 1, 2)) * loop2(x * y, y + y, x - 1, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (div (v2 x) (+ 1 2)) (w4 x)) (w5 x)))))
