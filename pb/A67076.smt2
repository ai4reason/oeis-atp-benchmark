;; sequence(s): A67076
;; terms: 0 1 2 4 5 7 8 10 13 14 17 19 20 22 25 28 29 32 34 35
;; small program: compr((loop(loop(2, 2, 2) * x, x, 2) - 1) mod (2 + x), x) div 2
;; fast program: (compr((((((2 * loop(x * x, 2, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, 1 + x) - 1) + 0
(set-logic UFNIA)
(declare-fun f2 () Int)
(declare-fun g2 () Int)
(declare-fun h2 () Int)
...
(assert (forall ((x Int)) (= (fast x) (+ (- (v3 x) 1) 0))))
