;; sequence(s): A151342
;; terms: 1 1 4 25 196 1764 17424 184041 2044900 23639044
;; small program: loop(x * x, 1, loop(2 * ((x - (x div y)) + x), x, 1) div (1 + x))
;; fast program: loop(x * x, 1, loop2(x * y, y - 1, x - 1, 1, x + x)) div loop(x * x, 1, loop(x * y, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h0 x) (div (v1 x) (+ 1 x)))))
