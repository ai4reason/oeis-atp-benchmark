;; sequence(s): A86023
;; terms: 1 26 251 1476 6376 22252 66352 175252 420277 931502 1933503 3796728 7109128 12773528 22137128 37160504 60634529 96454754 149963979 228375004
;; small program: loop(loop(x * x, 1, loop(((2 * (x + x)) div y) + x, y, 1)) + x, x, 1)
;; fast program: loop(loop((loop(loop(loop(x * x, 1, ((x * x) - x) div 2), 1, ((x * x) + x) div 2), 1, 2 + x) div (1 + 2)) div (1 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (= g1 1))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (div (* 2 (+ x x)) y) x))))
