;; sequence(s): A80801
;; terms: 2 1 3 2 2 1 4 3 3 2 3 2 2 1 5 4 4 3 4 3
;; small program: loop((loop2(((if y <= 0 then 0 else 1) + x) + y, y div 2, x, 1, x) - x) - x, 1, 2 + x)
;; fast program: loop(loop2((if y <= 0 then 0 else (1 + y)) + x, y div 2, 2 + (2 + 2), 1, x div 2) - x, 1, 2 + x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (ite (<= y 0) 0 (+ 1 y)) x))))
