;; sequence(s): A18688
;; terms: 1 2 4 5 10 20 43 86 172 215 430 860
;; small program: loop(compr(loop(loop((2 + y) * (2 + x), 2, 1 + x), 2, 2) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((x + y) * (2 * (2 + (2 * (2 + 2)))), 1 + 2, x) mod (1 + x), x) + 1) div 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* (+ 2 y) (+ 2 x)))))
(assert (= g3 2))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (+ x y) (* 2 (+ 2 (* 2 (+ 2 2))))))))
(assert (= g5 (+ 1 2)))
...
(assert (forall ((x Int)) (= (fast x) (div (+ (v4 x) 1) 1))))
