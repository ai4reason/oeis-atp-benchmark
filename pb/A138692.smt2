;; sequence(s): A138692
;; terms: 90 95 111 135 207 255 375 447 615 927 1047 1455 1767 1935 2295 2895 3567 3807 4575 5127
;; small program: (loop2((compr((2 - loop(x + x, x, 2)) mod (1 + x), 1 + y) * x) + x, y, 2, 1, x) + (2 * loop((x * x) + x, 2, 2))) + 2
;; fast program: (loop2((compr(loop((1 - loop(x + x, (x - 1) div 2, 2)) mod (1 + x), 1 - (x mod 2), x) - 2, 1 + y) * x) + x, y, 2, 1, x) + (2 * loop((x * x) + x, 2, 2))) + 2
(set-logic UFNIA)
...
(assert (= v3 (u3 g3 h3)))
(assert (forall ((x Int)) (= (small x) (+ (+ (w0 x) (* 2 v3)) 2))))
...
(assert (= v8 (u8 g8 h8)))
(assert (forall ((x Int)) (= (fast x) (+ (+ (w4 x) (* 2 v8)) 2))))
