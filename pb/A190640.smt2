;; sequence(s): A190640
;; terms: 2 8 20 26 56 62 74 80 164 170 182 188 218 224 236 242 488 494 506 512
;; small program: compr(loop(if (x mod 2) <= 0 then (x div (1 + 2)) else 1, x, x), x + x) + 2
;; fast program: (((loop((((((loop(((((loop((((((loop(((((loop((2 * (x * x)) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) + 1) - x) - x, 1, x div 2) + x) * (1 + 2)) - 1) - 2) - x) - x, 1, x div 2) + x) * (1 + 2)) * 2) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (* (+ (v2 x) x) (+ 1 2)) 2) 2))))
