;; sequence(s): A89819
;; terms: 2 2 2 4 4 8 8 16 32 64 64 128 128 256 512 1024 1024 2048 2048 4096
;; small program: loop(loop(((loop(x * y, x, 2) mod (1 + x)) - x) - 1, 2, y) * x, x, 2)
;; fast program: loop(loop(if (loop((1 - loop(x + x, (x div 2) - 1, 2)) mod (1 + x), 1 - (x mod 2), x) - 2) <= 0 then 1 else 2, 1, y) * x, x, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (- (mod (v2 x) (+ 1 x)) x) 1))))
(assert (= g1 2))
