;; sequence(s): A65310
;; terms: 3 2 2 1 1 2 2 1 1 2 2 1 1 2 1 1 1 1 2 2
;; small program: loop2(compr((loop(x * y, x, x) mod (1 + x)) mod 2, y) - x, 1 + y, 2, 0, x)
;; fast program: (((((loop((compr(loop(x * y, x, 2 + (2 + 2)) mod (1 + x), 2 + x) mod 2) + x, 2, x) + 2) - x) div 2) * 2) + 1) div (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (+ (* (div (- (+ (v3 x) 2) x) 2) 2) 1) (ite (<= x 0) 1 2)))))
