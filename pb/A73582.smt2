;; sequence(s): A73582
;; terms: 6 8 10 14 22 26 34 38 46 58 62 74 82 86 94 106 118 122 134 142
;; small program: (compr(loop2(2 + (x + y), x, x, 0, 2) mod (1 + x), 2 + x) * 2) + 2
;; fast program: (((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) - (if (x - 1) <= 0 then 1 else 2)) + 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (- (+ (+ (* (v2 x) 2) 1) (ite (<= x 0) 1 0)) (ite (<= (- x 1) 0) 1 2)) 2) 2))))
