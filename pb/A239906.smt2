;; sequence(s): A239906
;; terms: 0 0 1 2 3 3 5 5 7 7 8 9 11 11 12 12 15 15 16 17
;; small program: loop2(loop(((loop((if (x mod 2) <= 0 then 2 else x) div 2, 2, x) mod (1 + x)) + 1) mod 2, 1, y) + x, y div 2, x, 0, x)
;; fast program: loop2(loop((loop(1 - ((((1 + x) mod (2 * (2 + 2))) mod (1 + 2)) mod 2), x mod 2, x) + 1) mod 2, 1, y) + x, y div 2, x, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- 1 (mod (mod (mod (+ 1 x) (* 2 (+ 2 2))) (+ 1 2)) 2)))))
