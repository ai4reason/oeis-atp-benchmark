;; sequence(s): A28089
;; terms: 1 27 466 6558 82075 952329 10485532 111125916 1144265749 11522055831 113982102598 1111567913274 10714164992623 102277469691333 968483992074064 9108558269363832 85172800746064297
;; small program: loop(((loop((loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (- (* (+ 1 2) (* x x)) x))))
(assert (= g8 1))
