;; sequence(s): A259749
;; terms: 1 2 5 7 10 11 13 17 19 23 25 26 29 31 34 35 37 41 43 47
;; small program: loop(compr(loop((y * y) + x, x div 2, 0) mod (1 + x), y), x, 0) + 1
;; fast program: (compr(loop((((y div 2) * y) * y) + x, x, 0) mod (1 + x), x) div 1) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* (* (div y 2) y) y) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (v3 x) 1) 1))))
