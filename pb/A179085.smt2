;; sequence(s): A179085
;; terms: 1 3 5 7 9 21 23 25 27 29 41 43 45 47 49 61 63 65 67 69
;; small program: compr((loop2(x + y, (y div 2) div (1 + (2 + 2)), x, 0, x) + x) mod 2, x + x) + 1
;; fast program: compr(loop((loop2(x + y, y div (2 + (2 * (2 + 2))), x, 0, x) + x) mod 2, x mod 2, x), 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (div (div y 2) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ (w1 x) x) 2))))
...
(assert (forall ((x Int) (y Int)) (= (g4 x y) (div y (+ 2 (* 2 (+ 2 2)))))))
...
(assert (forall ((x Int)) (= (f3 x) (mod (+ (w4 x) x) 2))))
