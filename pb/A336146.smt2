;; sequence(s): A336146
;; terms: 1 2 3 2 4 5 6 2 7 8 9 5 10 11 12 2 13 14 15 8
;; small program: (((((1 + loop(x div (1 + (x mod 2)), x, x)) * (1 + (x mod 2))) * (1 + 2)) div 2) div 2) + 1
;; fast program: (((((loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2) * 2) + 1) * (1 + (x mod 2))) * (1 + 2)) div (2 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (div (div (* (* (+ 1 (v0 x)) (+ 1 (mod x 2))) (+ 1 2)) 2) 2) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (div (* (* (+ (* (v1 x) 2) 1) (+ 1 (mod x 2))) (+ 1 2)) (+ 2 2)) 1))))
