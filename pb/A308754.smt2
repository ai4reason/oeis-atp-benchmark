;; sequence(s): A308754
;; terms: 0 1 2 2 3 4 4 5 6 6 7 7 7 8 9 9 9 10 10 11
;; small program: loop(loop((loop(x * y, x, x) mod (1 + x)) mod 2, 1, y) - x, 2 + (x + x), 0)
;; fast program: loop(loop(if (((1 + loop(x + x, x mod 2, loop(x * x, 1, loop(x + x, x div 2, 1)))) mod (1 + (x + x))) - 2) <= 0 then 1 else 0, 1, y) + x, 1 + x, 0) - 1
(set-logic UFNIA)
