;; sequence(s): A28005
;; terms: 1 26 437 6034 74469 855042 9338629 98346578 1007581157 10104420898 99624776901 968871159282 9317203571365 88769381856194 839190346712453 7881486986566546 73610099911105893 684230318175538530
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + (((x + x) * 2) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (- (* (* x x) x) x))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (f6 x) (div (v7 x) (+ 2 (+ 2 2))))))
(assert (= g6 1))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) (* (* (+ x x) 2) 2)) x))))
