;; sequence(s): A96197
;; terms: 0 0 0 0 2 2 4 4 6 0 10 2 3 2 3 6 9 8 11 12
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 2) mod (1 + x)
;; fast program: (((loop2(loop(compr(loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1, 1, y) * x, 0, 2, 2, x) div 2) div 2) - x) mod (1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 2) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (fast x) (mod (- (div (div (w2 x) 2) 2) x) (+ 1 x)))))
