;; sequence(s): A3160
;; terms: 1 1 1 2 3 4 4 4 5 5 5 6 6 6 7 8 9 9 9 10
;; small program: (compr(loop((if (x mod 2) <= 0 then 2 else x) div 2, x + x, x), 1 + x) - 1) - x
;; fast program: loop((loop((loop(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x), 1 - (x mod (2 + 2)), x) mod 2, 1, x div 2) + 1) + x, x, 1) - 1) - x, 1, 2 + x)
(set-logic UFNIA)
