;; sequence(s): A340764
;; terms: 0 1 1 1 2 2 2 2 2 2 3 3 3 3 3 3 4 4 4 4
;; small program: loop(loop(((loop(x * y, x, 1) mod (1 + x)) mod (1 + 2)) mod 2, 1, y) + x, x, 0)
;; fast program: loop(loop(if (loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), (1 + x) mod 2, x) - 1) mod (1 + x), ((1 + x) mod (1 + 2)) - 1, x) - 1) <= 0 then 1 else 0, 1, y) + x, x, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (mod (mod (mod (v2 x) (+ 1 x)) (+ 1 2)) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (g5 x) (- (mod (+ 1 x) (+ 1 2)) 1))))
(assert (forall ((x Int)) (= (h5 x) x)))
(assert (forall ((x Int) (y Int)) (= (u5 x y) (ite (<= x 0) y (f5 (u5 (- x 1) y))))))
(assert (forall ((x Int)) (= (v5 x) (u5 (g5 x) (h5 x)))))
(assert (forall ((x Int)) (= (f4 x) (ite (<= (- (v5 x) 1) 0) 1 0))))
(assert (= g4 1))
