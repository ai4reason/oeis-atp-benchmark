;; sequence(s): A186296
;; terms: 2 6 10 22 30 34 42 54 66 70 82 90 106 114 126 142 154 166 174 190
;; small program: compr((2 + loop(x * x, 1, loop(x + x, x div 2, 2))) mod (1 + (x + x)), 1 + x) + 1
;; fast program: (compr(loop((loop(x * x, 1, loop(x + x, x div 2, 1)) - x) mod (1 + (x + x)), x mod 2, x), 1 + x) + 1) + 0
(set-logic UFNIA)
