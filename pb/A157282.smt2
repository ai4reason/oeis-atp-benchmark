;; sequence(s): A157282
;; terms: 1 2 2 3 4 5 6 7 7 8 9 10 11 12 12 13 14 14 15 16
;; small program: loop(loop((loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x, 2 + (x + x)) + 1) mod 2, 1, y) + x, x, 1)
;; fast program: loop(loop((loop(loop(x div loop(1, x mod (2 + (2 + 2)), 2 + (2 + 2)), x mod (1 + 2), 1 + x), 1, 1 + (x + x)) + 1) mod 2, 1, y) + x, x, 1)
(set-logic UFNIA)
(declare-fun f3 () Int)
