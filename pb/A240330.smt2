...
;; small program: loop((x div 2) - x, x mod (1 + loop((x * x) + x, 2, 2)), 1)
;; fast program: loop(loop2(y, x + y, x, 1, 0) - 1, x mod (1 + loop((x * x) + x, 2, 2)), 1)
(set-logic UFNIA)
...
(assert (= v1 (u1 g1 h1)))
(assert (forall ((x Int)) (= (g0 x) (mod x (+ 1 v1)))))
(assert (= h0 1))
...
(assert (= v4 (u4 g4 h4)))
(assert (forall ((x Int)) (= (g2 x) (mod x (+ 1 v4)))))
(assert (= h2 1))
