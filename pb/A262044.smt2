;; sequence(s): A262044
;; terms: 9 24 45 70 97 130 165 204 249 298 349 404 461 524 589 658 733 810 891 976
;; small program: loop((compr(loop(x * y, 2 + x, 1) mod (1 + (x + x)), 1 + y) * 2) + x, 1 + x, 1) + x
;; fast program: ((((loop(compr((((if (((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2) <= 0 then 1 else 2) + x) + x) mod (if x <= 0 then 1 else 2), 1 + y) + x, x, 1) + 2) * 2) + 1) + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (* (+ (v3 x) 2) 2) 1) x) 2))))
