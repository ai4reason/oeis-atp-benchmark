;; sequence(s): A33286
;; terms: 2 6 15 28 55 78 119 152 207 290 341 444 533 602 705 848 1003 1098 1273 1420
;; small program: (loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y), x, 1) + 1) * (1 + x)
;; fast program: ((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) * ((if x <= 0 then 1 else x) + 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v3 x) 2) 1) (+ (ite (<= x 0) 1 x) 1)))))
