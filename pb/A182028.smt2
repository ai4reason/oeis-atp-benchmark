;; sequence(s): A182028
;; terms: 0 1 2 4 9 18 37 74 148 297 594 1188 2377 4754 9509 19018 38036 76073 152146 304293
;; small program: loop((loop((compr(loop(x div (if (x mod (2 + 2)) <= 0 then 1 else 2), x, x), 1 + x) + 1) mod 2, 1, y) + x) + x, x, 0)
;; fast program: loop((loop((loop(((((2 * (2 + (x + x))) div (1 + (2 * (2 * (2 + 2))))) + x) mod 2) + x, 2, x) + x) mod 2, 1, y) + x) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (+ (mod (+ (div (* 2 (+ 2 (+ x x))) (+ 1 (* 2 (* 2 (+ 2 2))))) x) 2) x))))
(assert (= g6 2))
