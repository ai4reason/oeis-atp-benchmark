;; sequence(s): A55669
;; terms: 24 96 144 192 288 336 432 480 576 720 768 912 1008 1056 1152 1296 1440 1488 1632 1728
;; small program: (((loop(compr((1 + loop(x + x, x, x)) mod (1 + x), y) + 2, x, 1) * 2) * 2) * 2) * (1 + 2)
;; fast program: (((((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 2) * (1 + 2)) * (if x <= 0 then 1 else 2)) * 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (* (* (* (v0 x) 2) 2) 2) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (* (* (+ (* (v3 x) 2) 2) (+ 1 2)) (ite (<= x 0) 1 2)) 2) 2))))
