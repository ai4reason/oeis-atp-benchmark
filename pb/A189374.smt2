;; sequence(s): A189374
;; terms: 1 2 3 7 11 15 25 35 45 65 85 105 140 175 210 266 322 378 462 546
;; small program: loop(loop(loop(x + y, y, x), 1 + (y div (1 + 2)), x), x, 1)
;; fast program: loop(loop(loop(((x * x) * x) - x, 1, 2 + (x div (1 + 2))) div (2 + (2 + 2)), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (+ 1 (div y (+ 1 2))))))
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (+ 2 (div x (+ 1 2))))))
