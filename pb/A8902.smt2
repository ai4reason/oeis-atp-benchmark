;; sequence(s): A8902
;; terms: 99 296 148 74 37 110 55 164 82 41 122 61 182 91 272 136 68 34 17 50
;; small program: loop(loop((2 * ((x + x) + x)) - 1, x mod 2, x) div 2, x, loop(1 + ((2 + x) * x), 2, 2) - 1)
;; fast program: loop(loop((2 * ((x + x) + x)) - 1, x mod 2, x) div 2, x, loop(x * x, 1, 2 + (2 * (2 + 2))) - 1)
(set-logic UFNIA)
