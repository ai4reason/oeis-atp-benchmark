;; sequence(s): A76010
;; terms: 1 90 5265 255150 11160261 458810730 18124795305 697117731750 26323112938221 981154011007170 36233774365169745 1329174591745823550 48521083977375207381 1764912230785563088410 64027726517340144702585
;; small program: loop(((loop((loop(1 + (2 * (x + x)), y, 1) + x) + x, y, 1) + x) + x) + x, x, 1) * loop((x + x) + x, x + x, 1)
;; fast program: loop(((loop((loop((x * x) + x, 1, loop(2 + (x + x), x, 2)) div 2) div (1 + 2), 1, y) + x) + x) + x, x, 1) * loop2(x * y, y, x, 1, 1 + (2 * (2 + 2)))
(set-logic UFNIA)
