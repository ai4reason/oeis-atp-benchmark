;; sequence(s): A203465
;; terms: 1 2 24 5376 72253440 192663508746240 345230911480770991226880 1436598918224589625071929521581588480 48781096034575545526663437061892218092260229434572800
;; small program: loop(loop(loop2(((x * y) - x) * y, y + y, x, 1, 2) div loop(x * y, x, 1), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(((y - 1) + y) * (x * y), y + y, x, 1, 2) div loop((1 + y) * x, x, 1), 1, y) * x, x - 1, 1) * loop2(x * y, y + y, x, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* (+ (- y 1) y) (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (+ 1 y) x))))
