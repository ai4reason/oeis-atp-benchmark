;; sequence(s): A264828
;; terms: 1 8 9 12 15 16 18 20 21 24 25 27 28 30 32 33 35 36 39 40
;; small program: loop(compr(loop(((y mod 2) + y) * x, x, 1) mod (2 + x), y) + 2, x, 1)
;; fast program: compr(loop(((y * y) * y) * (x + x), x div 2, 1) mod (1 + x), x) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (* (* (* y y) y) (+ x x)))))
