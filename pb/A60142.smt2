;; sequence(s): A60142
;; terms: 0 1 3 4 7 9 12 15 16 19 25 28 31 33 36 39 48 51 57 60
;; small program: loop(compr(loop(loop(x div (if (x mod (2 + 2)) <= 0 then (2 + 2) else 1), 2, x) div (1 + (x mod 2)), x, x), y), x, 0)
;; fast program: compr(loop(loop(x div loop(1, x mod (2 + 2), 2 + 2), 2, x) div (1 + (x mod 2)), 1 + (x div 2), x) div 2, x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g5 x) (+ 1 (div x 2)))))
