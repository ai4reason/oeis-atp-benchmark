;; sequence(s): A91785
;; terms: 3 5 9 12 15 17 20 23 27 29 33 36 39 43 45 48 51 53 57 60
;; small program: compr(loop(if (x mod 2) <= 0 then (x div 2) else 2, x + x, x) - 1, 2 + (x + x))
;; fast program: compr(((loop((((loop((loop(loop2(x + y, y div 2, 2 + (2 * (2 + 2)), 0, x) mod 2, 1, x div 2) + x) mod 2, 1, x div 2) + x) mod 2) + x) - 1, 2, x) + 1) + x) mod (if x <= 0 then 1 else 2), 1 + x) * 1
(set-logic UFNIA)
