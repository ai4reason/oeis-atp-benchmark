;; sequence(s): A163637
;; terms: 1 3 5 7 20 24 45 51 80 144 156 240 297 315 380 504 644 672 832 935
;; small program: loop2((compr(((1 + loop(x + x, x, 1)) mod (1 + (x + x))) - 2, y) * x) + x, y, 2, 1, x) - (x * x)
;; fast program: loop2((compr((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1, y) * x) + x, y, 2, 1, x) - (x * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (mod (- (mod (* (* (* (* 2 (v5 x)) (+ 1 (mod x 2))) (v7 x)) (+ 1 (mod x 2))) (+ 1 (+ x x))) 1) (+ 1 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (- (w3 x) (* x x)))))
