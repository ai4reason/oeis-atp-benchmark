;; sequence(s): A62320
;; terms: 16 64 81 144 256 324 400 576 625 729 784 1024 1296 1600 1936 2025 2304 2401 2500 2704
;; small program: loop2((compr(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x, x) - x) <= 0 then 1 else 0, y) * x) + x, y, 2, 1, x)
;; fast program: loop2(loop(compr(if (loop((if ((y * y) mod (1 + x)) <= 0 then 1 else 0) + x, x - (x div 2), x) - x) <= 0 then 1 else 0, x) + 1, 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
