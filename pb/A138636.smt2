;; sequence(s): A138636
;; terms: 12 18 30 42 66 78 102 114 138 174 186 222 246 258 282 318 354 366 402 426
;; small program: ((loop(compr((1 + loop(2 * x, x, x)) mod (1 + x), y), x, 1) * 2) + 2) * (1 + 2)
;; fast program: (((compr(((((((((2 * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) * loop(x * x, 1, loop(x + x, x div 2, 1))) * (1 + (x mod 2))) mod (1 + (x + x))) - 1) mod (1 + x)) - 1) mod (1 + x), x) * 2) + 1) + (if x <= 0 then 1 else 0)) * (2 * (1 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (+ (+ (* (v3 x) 2) 1) (ite (<= x 0) 1 0)) (* 2 (+ 1 2))))))
