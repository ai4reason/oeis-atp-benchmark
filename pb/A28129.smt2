;; sequence(s): A28129
;; terms: 1 32 665 11440 177441 2582592 36039145 488323280 6475959281 84497107552 1088651482425 13885593262320 175667802947521 2207466992246912 27583620276442505 343037771674508560
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + (y * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop2(x * y, y, x, 1 + 2, 1 + 2) div 2) * loop(x * x, 1, loop(x + x, x - 1, 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) x) (* (+ (* (* x 2) 2) x) 2)))))
