;; sequence(s): A28069
;; terms: 1 28 510 7700 104951 1344168 16531180 197743000 2319239901 26813094308 306681461450 3479254697100 39224238812851 440041610610448 4917647821351320 54788913249014000 608931512071655801
;; small program: loop(((loop((loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop((loop(loop(((1 + 2) * (x * x)) - x, 1, loop2(x * y, y, x, 1, 1 + 2)) div 2, 1, y) + (2 * (x + x))) + x, y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
