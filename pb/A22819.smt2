;; sequence(s): A22819
;; terms: 0 0 1 2 4 6 8 11 13 16 19 22 25 28 31 34 38 41 44 48
;; small program: ((loop((x * y) + loop(x * y, y - 1, 1), x, 0) * x) div loop(x * y, x, 1)) - x
;; fast program: (loop((x * y) + loop(x * y, y - 1, 1), x, 0) div loop(x * y, x - 1, 1)) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (div (* (v0 x) x) (v2 x)) x))))
...
(assert (forall ((x Int)) (= (fast x) (- (div (v3 x) (v5 x)) x))))
