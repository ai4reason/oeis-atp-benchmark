;; sequence(s): A460
;; terms: 1 11 66 302 1191 4293 14608 47840 152637 478271 1479726 4537314 13824739 41932745 126781020 382439924 1151775897 3464764515 10414216090 31284590870
;; small program: loop(((loop((loop(loop(loop(2 * (x * y), y, 1) + (x * y), y, 1) + (x * y), x, 1) * (1 + x)) div loop(x * y, x, 1), 1, y) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(loop(((((loop2(x * y, y, x div 2, loop(1 + (2 * (2 + 2)), x mod 2, 1 + 2), 1 + (2 * (2 + 2))) - loop(x + x, x, 2)) * 2) + 1) - 2) - x, 1, y) + x, x, 1), 1, y) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (div (* (v2 x) (+ 1 x)) (v5 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (w10 x) (u10 (h10 x) (i10 x) j10))))
...
(assert (forall ((x Int)) (= (f9 x) (- (- (+ (* (- (w10 x) (v12 x)) 2) 1) 2) x))))
(assert (= g9 1))
