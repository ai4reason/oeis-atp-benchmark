;; sequence(s): A138649
;; terms: 0 1 11 91 46 1526 15406 45561 117116 312071 1872426 21000181 223657336 853662766 5121976596 12421312701 74527876206 141991475986 2377827762166 18081663838621
;; small program: loop(2 * ((x + x) + x), x, 1) mod loop((2 * (x + x)) + x, x, 1)
;; fast program: loop2(x * y, y, x div 2, loop(2 + (2 + 2), x mod 2, 1), loop(x * x, 1, 2 + (2 + 2))) mod loop2(x * y, y, x div 2, loop(1 + (2 + 2), x mod 2, 1), loop(x * x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (mod (w2 x) (w5 x)))))
