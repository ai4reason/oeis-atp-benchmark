;; sequence(s): A25997
;; terms: 1 24 375 4850 56481 616164 6434155 65146950 644889861 6275802104 60272841135 572861926650 5399415089641 50546032304844 470534835243315 4359828735795950 40238834684119821 370153792218396384
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(((x * x) * x) - x, 1, loop(x + x, x, 2)) div (2 + (2 + 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (- (* (* x x) x) x))))
(assert (= g8 1))
...
(assert (forall ((x Int)) (= (f7 x) (div (v8 x) (+ 2 (+ 2 2))))))
(assert (= g7 1))
