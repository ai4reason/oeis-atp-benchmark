;; sequence(s): A286065
;; terms: 3 5 9 10 15 17 22 23 27 29 33 34 39 40 44 46 51 53 57 58
;; small program: (((((compr((loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + x, x) + x) mod 2, x) + 1) + x) div 2) - (x mod 2)) + 2) + x
;; fast program: (((((compr((loop(loop((if (x mod 2) <= 0 then 2 else x) div 2, 1 + (2 * (2 + 2)), x), 2, x) + 1) mod 2, x) + 1) + x) div 2) - (x mod 2)) + 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (+ (- (div (+ (+ (v0 x) 1) x) 2) (mod x 2)) 2) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (- (div (+ (+ (v2 x) 1) x) 2) (mod x 2)) 2) x))))
