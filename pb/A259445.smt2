;; sequence(s): A259445
;; terms: 1 2 3 2 5 6 7 2 9 10 11 6 13 14 15 2 17 18 19 10
;; small program: loop(x div (if (x mod (2 + 2)) <= 0 then 2 else 1), x, 1 + x)
;; fast program: ((loop(loop(loop(loop(loop(1 + loop(loop(x div 2, x mod 2, x), loop(2 + 2, x mod 2, 0), x), 1, x div 2), x mod 2, x) div 2, x mod 2, x div 2), x mod 2, x div 2), x mod 2, x div 2) * 2) + 1) * (1 + (x mod 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (div x (ite (<= (mod x (+ 2 2)) 0) 2 1)))))
...
(assert (forall ((x Int)) (= (fast x) (* (+ (* (v1 x) 2) 1) (+ 1 (mod x 2))))))
