;; sequence(s): A268554
;; terms: 1 36 6300 1552320 445945500 139815211536 46384755633216 16009450307136000 5689533506261190300 2067982222137781950000 765185639177176836418800 287266309673587605560908800 109149488451384203661831720000
;; small program: ((loop2(x * y, 1 + y, x + x, 1, 1 + (x + x)) div loop((y * y) * x, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + (x + x))) div loop(x * y, x, 1)
;; fast program: (((((loop2(x * y, y - 1, x, 1, 1 + ((x + x) + x)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + (x + x))) div loop(x * y, x, 1)) * (1 + (x + x))) * loop2(x * y, 1 + y, x - 1, 1, 2 + ((x + x) + x))) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (= i8 1))
(assert (forall ((x Int)) (= (j8 x) (+ 2 (+ (+ x x) x)))))
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (div (* (div (w4 x) (v5 x)) (w6 x)) (v7 x)) (+ 1 (+ x x))) (w8 x)) (v9 x)))))
