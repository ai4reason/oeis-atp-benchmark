;; sequence(s): A39703
;; terms: 2 3 0 2 1 3 2 4 3 4 1 2 1 3 2 3 4 1 2 1
;; small program: (compr(loop2(1 + (x + y), x, x, 0, 1) mod (1 + x), 1 + x) + 1) mod (1 + (2 + 2))
;; fast program: (compr(loop((loop(loop2(x + y, x, x, 1, 2) mod (1 + x), 1 - (x mod 2), x) - 1) mod (1 + x), 1 - (x mod 2), x) - 1, 1 + x) + 1) mod (1 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (mod (+ (v0 x) 1) (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (fast x) (mod (+ (v2 x) 1) (+ 1 (+ 2 2))))))
