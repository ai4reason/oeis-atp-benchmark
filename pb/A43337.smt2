;; sequence(s): A43337
;; terms: 1 4 6 7 9 13 16 18 19 24 26 27 28 30 31 33 36 38 39 41
;; small program: compr((loop2((((y div 2) + y) mod 2) + x, y div (2 + 2), 2 + 2, 1, x) + x) mod (1 + 2), x)
;; fast program: compr(loop2((((y div 2) + y) mod 2) + x, y div (2 + 2), 1 + 2, 1 + x, x) mod (1 + 2), x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f0 x) (mod (+ (w1 x) x) (+ 1 2)))))
...
(assert (forall ((x Int)) (= (f2 x) (mod (w3 x) (+ 1 2)))))
