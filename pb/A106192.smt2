;; sequence(s): A106192
;; terms: 1 ~2 ~1 ~6 ~11 ~34 ~95 ~298 ~953 ~3158 ~10677 ~36750 ~128249 ~452774 ~1614049 ~5801654 ~21003739 ~76516994 ~280293319 ~1031794394
;; small program: loop(loop(loop((loop((x - 2) + x, 2, y) * x) div y, y, 1) - x, y, 1) + x, x, 1)
;; fast program: loop(loop(loop(2 * ((x - (((x + x) + x) div y)) + x), y, 1) - x, y, 1) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (div (* (v3 x y) x) y))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (* 2 (+ (- x (div (+ (+ x x) x) y)) x)))))
