;; sequence(s): A97895
;; terms: 0 0 2 3 11 20 51 99 222 441 935 1872 3863 7751 15774 31653 63939 128232 257963 517011
;; small program: (loop(x + x, x, 1) - loop2(y, x + x, x, 0, 1)) - loop2(x + y, x, x, 1, 0)
;; fast program: ((loop(x + x, x - (x div 2), 1) - (x mod 2)) * loop(x + x, x div 2, 1)) - loop2(x + y, x, x - 1, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (- (- (v0 x) (w1 x)) (w2 x)))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (v3 x) (mod x 2)) (v4 x)) (w5 x)))))
