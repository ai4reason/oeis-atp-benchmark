;; sequence(s): A8977
;; terms: 1 24 2520 369600 63063000 11732745024 2308743493056 472518347558400 99561092450391000 21452752266265320000 4705360871073570227520 1047071828879079131681280 235809301462142612780721600
;; small program: loop2(loop(2 * ((x - (x div y)) + x), y, x) * x, y + y, 2, 1, x)
;; fast program: (((((loop2(x * y, 1 + y, x - 1, 1, 2 + (x + x)) div loop(x * y, x, 1)) * loop2(x * y, 1 + y, x, 1, 1 + x)) div loop(x * y, x, 1)) * (1 + (x + x))) * loop2(x * y, 1 + y, x, 1, 1 + ((x + x) + x))) div loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (div (* (* (div (* (div (w2 x) (v3 x)) (w4 x)) (v5 x)) (+ 1 (+ x x))) (w6 x)) (v7 x)))))
