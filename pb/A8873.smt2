;; sequence(s): A8873
;; terms: 97 292 146 73 220 110 55 166 83 250 125 376 188 94 47 142 71 214 107 322
;; small program: loop(loop(2 + (2 * ((x + x) + x)), x mod 2, x) div 2, x, loop((2 * (x * x)) - 1, 2, 2))
;; fast program: loop(loop(2 + (2 * ((x + x) + x)), x mod 2, x) div 2, x, 1 + loop((2 + x) * (x * x), 1, 2 + 2))
(set-logic UFNIA)
