;; sequence(s): A229025
;; terms: 1 26 417 5334 59829 616602 5996089 55931678 505925277 4470884418 38804443041 332070466182 2809908472645 23562316644074 196128590350473 1622684846427246 13358308842842733 109510501632625170 894623468678425585 7286920685445869270
;; small program: loop((loop((loop2(((((y - 1) + y) * (y * y)) + x) + (2 * ((x + x) + x)), y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(((loop(((x - 1) + x) * ((x * x) - x), 1, loop(x + x, x, 2)) div (1 + 2)) + 1) div 2, 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (* (+ (- y 1) y) (* y y)) x) (* 2 (+ (+ x x) x))))))
...
(assert (forall ((x Int)) (= (f7 x) (* (+ (- x 1) x) (- (* x x) x)))))
(assert (= g7 1))
...
(assert (forall ((x Int)) (= (f6 x) (div (+ (div (v7 x) (+ 1 2)) 1) 2))))
(assert (= g6 1))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (v4 x y) x) (* (+ (+ x x) x) 2)))))
