;; sequence(s): A95034
;; terms: 1 7 9 10 12 16 26 33 34
;; small program: compr(loop(((x * x) * x) mod (loop(2 + (x * x), 2, 2) - 1), 2, x) - 1, 1 + x)
;; fast program: compr(((loop((x * x) * x, 1, (x * x) * x) * x) - x) mod (1 + loop(x * x, 1, 2 + (2 + 2))), 1 + x)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (f1 x) (mod (* (* x x) x) (- v2 1)))))
(assert (= g1 2))
