;; sequence(s): A119681
;; terms: 3 7 9 15 19 21 27 31 37 45 49 51 55 57 69 75 79 87 91 97
;; small program: compr((loop((loop((x - 2) + x, 2, y) * x) div y, x, 2) - 1) mod (1 + (x + x)), 1 + x) + 1
;; fast program: ((compr((((((loop(((if (loop(((loop((((2 * (y + y)) - 2) * x) div (1 + y), x, 1) mod (1 + (x + x))) - 2) mod (1 + x), (1 + x) mod 2, x) - 2) <= 0 then 1 else 2) mod (1 + x)) mod 2, 1 - (x mod 2), x) + x) mod 2) - (x mod 2)) mod (1 + 2)) + 1) mod 2, x) * 2) div 2) + 1
(set-logic UFNIA)
