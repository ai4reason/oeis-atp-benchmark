;; sequence(s): A182835
;; terms: 3 4 6 7 8 9 11 12 13 14 16 17 18 19 20 21 23 24 25 26
;; small program: loop((y div ((x * x) div 2)) + x, 2 + x, 2) + x
;; fast program: loop(loop((loop2(loop(x + y, x, y) div (2 + x), y, 1 + 2, 2 + 2, (x - 1) div 2) + 2) + x, 2, x) - x, 1, x div 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (div y (div (* x x) 2)) x))))
